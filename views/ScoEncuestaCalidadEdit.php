...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fsco_encuesta_calidadedit" id="fsco_encuesta_calidadedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { sco_encuesta_calidad: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fsco_encuesta_calidadedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fsco_encuesta_calidadedit")
        .setPageId("edit")
...
<span id="el_sco_encuesta_calidad_activo">
    <select
        ...
        data-select2-id="fsco_encuesta_calidadedit_x_activo"
        <?php } ?>
        data-table="sco_encuesta_calidad"
        data-field="x_activo"
...
<?php if (!$Page->activo->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_encuesta_calidadedit", function() {
    var options = { name: "x_activo", selectId: "fsco_encuesta_calidadedit_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_encuesta_calidadedit.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fsco_encuesta_calidadedit" };
    } else {
        options.ajax = { id: "x_activo", form: "fsco_encuesta_calidadedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="sco_encuesta_calidad" data-field="x_Nencuesta_calidad" data-hidden="1" name="x_Nencuesta_calidad" id="x_Nencuesta_calidad" value="<?= HtmlEncode($Page->Nencuesta_calidad->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_encuesta_calidadedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_encuesta_calidadedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
