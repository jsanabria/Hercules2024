...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fsco_tablaedit" id="fsco_tablaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<span id="el_sco_tabla_tabla">
    <select
        ...
        data-select2-id="fsco_tablaedit_x_tabla"
        <?php } ?>
...
<?php if (!$Page->tabla->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_tablaedit", function() {
    var options = { name: "x_tabla", selectId: "fsco_tablaedit_x_tabla" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_tablaedit.lists.tabla?.lookupOptions.length) {
        options.data = { id: "x_tabla", form: "fsco_tablaedit" };
    } else {
        options.ajax = { id: "x_tabla", form: "fsco_tablaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="sco_tabla" data-field="x_Ntabla" data-hidden="1" name="x_Ntabla" id="x_Ntabla" value="<?= HtmlEncode($Page->Ntabla->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_tablaedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_tablaedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
