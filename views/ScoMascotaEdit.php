...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fsco_mascotaedit" id="fsco_mascotaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { sco_mascota: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fsco_mascotaedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fsco_mascotaedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["Nmascota", [fields.Nmascota.visible && fields.Nmascota.required ? ew.Validators.required(fields.Nmascota.caption) : null], fields.Nmascota.isInvalid],
...
<?php if ($Page->Nmascota->Visible) { // Nmascota ?>
    <div id="r_Nmascota"<?= $Page->Nmascota->rowAttributes() ?>>
        <label id="elh_sco_mascota_Nmascota" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Nmascota->caption() ?><?= $Page->Nmascota->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->Nmascota->cellAttributes() ?>>
<span id="el_sco_mascota_Nmascota">
<span<?= $Page->Nmascota->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Nmascota->getDisplayValue($Page->Nmascota->EditValue))) ?>"></span>
<input type="hidden" data-table="sco_mascota" data-field="x_Nmascota" data-hidden="1" name="x_Nmascota" id="x_Nmascota" value="<?= HtmlEncode($Page->Nmascota->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_sco_mascota_tipo">
    <select
        ...
        data-select2-id="fsco_mascotaedit_x_tipo"
        <?php } ?>
...
<?php if (!$Page->tipo->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_mascotaedit", function() {
    var options = { name: "x_tipo", selectId: "fsco_mascotaedit_x_tipo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_mascotaedit.lists.tipo?.lookupOptions.length) {
        options.data = { id: "x_tipo", form: "fsco_mascotaedit" };
    } else {
        options.ajax = { id: "x_tipo", form: "fsco_mascotaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_mascota_procedencia">
    <select
        ...
        data-select2-id="fsco_mascotaedit_x_procedencia"
        <?php } ?>
        data-table="sco_mascota"
...
<script>
loadjs.ready("fsco_mascotaedit", function() {
    var options = { name: "x_procedencia", selectId: "fsco_mascotaedit_x_procedencia" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_mascotaedit.lists.procedencia?.lookupOptions.length) {
        options.data = { id: "x_procedencia", form: "fsco_mascotaedit" };
    } else {
        options.ajax = { id: "x_procedencia", form: "fsco_mascotaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_sco_mascota_tarifa">
    <select
        ...
        data-select2-id="fsco_mascotaedit_x_tarifa"
        <?php } ?>
        data-table="sco_mascota"
        data-field="x_tarifa"
...
<script>
loadjs.ready("fsco_mascotaedit", function() {
    var options = { name: "x_tarifa", selectId: "fsco_mascotaedit_x_tarifa" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_mascotaedit.lists.tarifa?.lookupOptions.length) {
        options.data = { id: "x_tarifa", form: "fsco_mascotaedit" };
    } else {
        options.ajax = { id: "x_tarifa", form: "fsco_mascotaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["fsco_mascotaedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fsco_mascotaedit", "x_fecha_cremacion", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fsco_mascotaedit", "timepicker"], () => ew.createTimePicker("fsco_mascotaedit", "x_hora_cremacion", Object.assign({"step":15}, { timeFormat: "<?= DateFormat(4) ?>" })));
</script>
<?php } ?>
</span>
</div></div>
...
<?php
    if (in_array("sco_nota_mascota", explode(",", $Page->getCurrentDetailTable())) && $sco_nota_mascota->DetailEdit) {
?>
...
<?php } ?>
<?php
    if (in_array("sco_mascota_estatus", explode(",", $Page->getCurrentDetailTable())) && $sco_mascota_estatus->DetailEdit) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_mascotaedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_mascotaedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
loadjs.ready("load", function () {
    ...
    // document.write("page loaded");
    	...
    	$(document).ready(function(){
    		$('#r_tipo_otro').hide();
    		if($('#x_tipo').val() == "Otro") $('#r_tipo_otro').show();
