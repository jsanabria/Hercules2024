...
$Page->showMessage();
?>
<form name="fsco_user_notaadd" id="fsco_user_notaadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<script>
loadjs.ready("fsco_user_notaadd", function() {
    fsco_user_notaadd.createAutoSuggest(Object.assign({"id":"x_user","forceSelect":false}, { lookupAllDisplayFields: <?= $Page->user->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.sco_user_nota.fields.user.autoSuggestOptions));
});
</script>
...
<span id="el_sco_user_nota_tipo">
    <select
        ...
        data-select2-id="fsco_user_notaadd_x_tipo"
        <?php } ?>
...
<script>
loadjs.ready("fsco_user_notaadd", function() {
    var options = { name: "x_tipo", selectId: "fsco_user_notaadd_x_tipo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_user_notaadd.lists.tipo?.lookupOptions.length) {
        options.data = { id: "x_tipo", form: "fsco_user_notaadd" };
    } else {
        options.ajax = { id: "x_tipo", form: "fsco_user_notaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_user_notaadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_user_notaadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
