...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fsco_bloque_horarioedit" id="fsco_bloque_horarioedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { sco_bloque_horario: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fsco_bloque_horarioedit;
...
<span id="el_sco_bloque_horario_servicio_tipo">
    <select
        ...
        data-select2-id="fsco_bloque_horarioedit_x_servicio_tipo"
        <?php } ?>
...
<?php if (!$Page->servicio_tipo->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_bloque_horarioedit", function() {
    var options = { name: "x_servicio_tipo", selectId: "fsco_bloque_horarioedit_x_servicio_tipo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_bloque_horarioedit.lists.servicio_tipo?.lookupOptions.length) {
        options.data = { id: "x_servicio_tipo", form: "fsco_bloque_horarioedit" };
    } else {
        options.ajax = { id: "x_servicio_tipo", form: "fsco_bloque_horarioedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_bloque_horario_hora">
    <select
        ...
        data-select2-id="fsco_bloque_horarioedit_x_hora"
        <?php } ?>
        data-table="sco_bloque_horario"
        data-field="x_hora"
...
<script>
loadjs.ready("fsco_bloque_horarioedit", function() {
    var options = { name: "x_hora", selectId: "fsco_bloque_horarioedit_x_hora" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_bloque_horarioedit.lists.hora?.lookupOptions.length) {
        options.data = { id: "x_hora", form: "fsco_bloque_horarioedit" };
    } else {
        options.ajax = { id: "x_hora", form: "fsco_bloque_horarioedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_sco_bloque_horario_bloque">
    <select
        ...
        data-select2-id="fsco_bloque_horarioedit_x_bloque"
        <?php } ?>
        data-table="sco_bloque_horario"
        data-field="x_bloque"
...
<script>
loadjs.ready("fsco_bloque_horarioedit", function() {
    var options = { name: "x_bloque", selectId: "fsco_bloque_horarioedit_x_bloque" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_bloque_horarioedit.lists.bloque?.lookupOptions.length) {
        options.data = { id: "x_bloque", form: "fsco_bloque_horarioedit" };
    } else {
        options.ajax = { id: "x_bloque", form: "fsco_bloque_horarioedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php } ?>
    ...
    <input type="hidden" data-table="sco_bloque_horario" data-field="x_Nbloque_horario" data-hidden="1" name="x_Nbloque_horario" id="x_Nbloque_horario" value="<?= HtmlEncode($Page->Nbloque_horario->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_bloque_horarioedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_bloque_horarioedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
