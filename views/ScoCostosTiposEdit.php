...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fsco_costos_tiposedit" id="fsco_costos_tiposedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { sco_costos_tipos: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fsco_costos_tiposedit;
...
<input type="hidden" data-table="sco_costos_tipos" data-field="x_Ncostos_tipos" data-hidden="1" data-old name="o_Ncostos_tipos" id="o_Ncostos_tipos" value="<?= HtmlEncode($Page->Ncostos_tipos->OldValue ?? $Page->Ncostos_tipos->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_costos_tiposedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_costos_tiposedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
