...
// Page object
$ScoNotificacionesAdd = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { sco_notificaciones: currentTable } });
var currentPageID = ew.PAGE_ID = "add";
var currentForm;
var fsco_notificacionesadd;
...
$Page->showMessage();
?>
<form name="fsco_notificacionesadd" id="fsco_notificacionesadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<?php if (Config("CHECK_TOKEN")) { ?>
<input type="hidden" name="<?= $TokenNameKey ?>" value="<?= $TokenName ?>"><!-- CSRF token name -->
<input type="hidden" name="<?= $TokenValueKey ?>" value="<?= $TokenValue ?>"><!-- CSRF token value -->
<?php } ?>
<input type="hidden" name="t" value="sco_notificaciones">
...
<?php if ($Page->notificar->Visible) { // notificar ?>
    <div id="r_notificar"<?= $Page->notificar->rowAttributes() ?>>
        <label id="elh_sco_notificaciones_notificar" for="x_notificar" class="<?= $Page->LeftColumnClass ?>"><?= $Page->notificar->caption() ?><?= $Page->notificar->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->notificar->cellAttributes() ?>>
<span id="el_sco_notificaciones_notificar">
    <select
        id="x_notificar"
        name="x_notificar"
        class="form-select ew-select<?= $Page->notificar->isInvalidClass() ?>"
        <?php if (!$Page->notificar->IsNativeSelect) { ?>
        data-select2-id="fsco_notificacionesadd_x_notificar"
        <?php } ?>
        data-table="sco_notificaciones"
...
<?php if (!$Page->notificar->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_notificacionesadd", function() {
    var options = { name: "x_notificar", selectId: "fsco_notificacionesadd_x_notificar" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_notificacionesadd.lists.notificar?.lookupOptions.length) {
        options.data = { id: "x_notificar", form: "fsco_notificacionesadd" };
    } else {
        options.ajax = { id: "x_notificar", form: "fsco_notificacionesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.sco_notificaciones.fields.notificar.selectOptions);
...
<?php if ($Page->asunto->Visible) { // asunto ?>
    <div id="r_asunto"<?= $Page->asunto->rowAttributes() ?>>
        <label id="elh_sco_notificaciones_asunto" for="x_asunto" class="<?= $Page->LeftColumnClass ?>"><?= $Page->asunto->caption() ?><?= $Page->asunto->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->asunto->cellAttributes() ?>>
<span id="el_sco_notificaciones_asunto">
<input type="<?= $Page->asunto->getInputTextType() ?>" name="x_asunto" id="x_asunto" data-table="sco_notificaciones" data-field="x_asunto" value="<?= $Page->asunto->EditValue ?>" size="30" maxlength="255" placeholder="<?= HtmlEncode($Page->asunto->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->asunto->formatPattern()) ?>"<?= $Page->asunto->editAttributes() ?> aria-describedby="x_asunto_help">
...
<?php if ($Page->notificacion->Visible) { // notificacion ?>
    <div id="r_notificacion"<?= $Page->notificacion->rowAttributes() ?>>
        <label id="elh_sco_notificaciones_notificacion" class="<?= $Page->LeftColumnClass ?>"><?= $Page->notificacion->caption() ?><?= $Page->notificacion->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->notificacion->cellAttributes() ?>>
<span id="el_sco_notificaciones_notificacion">
<?php $Page->notificacion->EditAttrs->appendClass("editor"); ?>
<textarea data-table="sco_notificaciones" data-field="x_notificacion" name="x_notificacion" id="x_notificacion" cols="35" rows="4" placeholder="<?= HtmlEncode($Page->notificacion->getPlaceHolder()) ?>"<?= $Page->notificacion->editAttributes() ?> aria-describedby="x_notificacion_help"><?= $Page->notificacion->EditValue ?></textarea>
<?= $Page->notificacion->getCustomMessage() ?>
<div class="invalid-feedback"><?= $Page->notificacion->getErrorMessage() ?></div>
<script>
loadjs.ready(["fsco_notificacionesadd", "editor"], function() {
    ew.createEditor("fsco_notificacionesadd", "x_notificacion", 35, 4, <?= $Page->notificacion->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<?php if ($Page->notificados->Visible) { // notificados ?>
    <div id="r_notificados"<?= $Page->notificados->rowAttributes() ?>>
        <label id="elh_sco_notificaciones_notificados" for="x_notificados" class="<?= $Page->LeftColumnClass ?>"><?= $Page->notificados->caption() ?><?= $Page->notificados->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->notificados->cellAttributes() ?>>
<span id="el_sco_notificaciones_notificados">
<textarea data-table="sco_notificaciones" data-field="x_notificados" name="x_notificados" id="x_notificados" cols="35" rows="4" placeholder="<?= HtmlEncode($Page->notificados->getPlaceHolder()) ?>"<?= $Page->notificados->editAttributes() ?> aria-describedby="x_notificados_help"><?= $Page->notificados->EditValue ?></textarea>
...
<?php if ($Page->fecha->Visible) { // fecha ?>
    <div id="r_fecha"<?= $Page->fecha->rowAttributes() ?>>
        <label id="elh_sco_notificaciones_fecha" for="x_fecha" class="<?= $Page->LeftColumnClass ?>"><?= $Page->fecha->caption() ?><?= $Page->fecha->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->fecha->cellAttributes() ?>>
<span id="el_sco_notificaciones_fecha">
<input type="<?= $Page->fecha->getInputTextType() ?>" name="x_fecha" id="x_fecha" data-table="sco_notificaciones" data-field="x_fecha" value="<?= $Page->fecha->EditValue ?>" placeholder="<?= HtmlEncode($Page->fecha->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->fecha->formatPattern()) ?>"<?= $Page->fecha->editAttributes() ?> aria-describedby="x_fecha_help">
...
<?php if ($Page->enviado->Visible) { // enviado ?>
    <div id="r_enviado"<?= $Page->enviado->rowAttributes() ?>>
        <label id="elh_sco_notificaciones_enviado" for="x_enviado" class="<?= $Page->LeftColumnClass ?>"><?= $Page->enviado->caption() ?><?= $Page->enviado->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->enviado->cellAttributes() ?>>
<span id="el_sco_notificaciones_enviado">
    <select
        id="x_enviado"
        name="x_enviado"
        class="form-select ew-select<?= $Page->enviado->isInvalidClass() ?>"
        <?php if (!$Page->enviado->IsNativeSelect) { ?>
        data-select2-id="fsco_notificacionesadd_x_enviado"
        <?php } ?>
        data-table="sco_notificaciones"
...
<script>
loadjs.ready("fsco_notificacionesadd", function() {
    var options = { name: "x_enviado", selectId: "fsco_notificacionesadd_x_enviado" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_notificacionesadd.lists.enviado?.lookupOptions.length) {
        options.data = { id: "x_enviado", form: "fsco_notificacionesadd" };
    } else {
        options.ajax = { id: "x_enviado", form: "fsco_notificacionesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.sco_notificaciones.fields.enviado.selectOptions);
    ew.createSelect(options);
});
</script>
<?php } ?>
...
<?php if ($Page->adjunto->Visible) { // adjunto ?>
    <div id="r_adjunto"<?= $Page->adjunto->rowAttributes() ?>>
        <label id="elh_sco_notificaciones_adjunto" class="<?= $Page->LeftColumnClass ?>"><?= $Page->adjunto->caption() ?><?= $Page->adjunto->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->adjunto->cellAttributes() ?>>
<span id="el_sco_notificaciones_adjunto">
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_notificacionesadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_notificacionesadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
loadjs.ready("head", function() {
    ew.addEventHandlers("sco_notificaciones");
});
</script>
<script>
