...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fsco_articuloedit" id="fsco_articuloedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { sco_articulo: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fsco_articuloedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fsco_articuloedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["Narticulo", [fields.Narticulo.visible && fields.Narticulo.required ? ew.Validators.required(fields.Narticulo.caption) : null], fields.Narticulo.isInvalid],
...
<?php if ($Page->Narticulo->Visible) { // Narticulo ?>
    <div id="r_Narticulo"<?= $Page->Narticulo->rowAttributes() ?>>
        <label id="elh_sco_articulo_Narticulo" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Narticulo->caption() ?><?= $Page->Narticulo->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->Narticulo->cellAttributes() ?>>
<span id="el_sco_articulo_Narticulo">
<span<?= $Page->Narticulo->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Narticulo->getDisplayValue($Page->Narticulo->EditValue))) ?>"></span>
<input type="hidden" data-table="sco_articulo" data-field="x_Narticulo" data-hidden="1" name="x_Narticulo" id="x_Narticulo" value="<?= HtmlEncode($Page->Narticulo->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_sco_articulo_tipo_insumo">
    <select
        ...
        data-select2-id="fsco_articuloedit_x_tipo_insumo"
        <?php } ?>
...
<?php if (!$Page->tipo_insumo->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_articuloedit", function() {
    var options = { name: "x_tipo_insumo", selectId: "fsco_articuloedit_x_tipo_insumo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_articuloedit.lists.tipo_insumo?.lookupOptions.length) {
        options.data = { id: "x_tipo_insumo", form: "fsco_articuloedit" };
    } else {
        options.ajax = { id: "x_tipo_insumo", form: "fsco_articuloedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_articuloedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_articuloedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
