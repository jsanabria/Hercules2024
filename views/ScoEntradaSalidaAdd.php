...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        ...
        .setFields([
            ...
            ["registro", [fields.registro.visible && fields.registro.required ? ew.Validators.required(fields.registro.caption) : null, ew.Validators.datetime(fields.registro.clientFormatPattern)], fields.registro.isInvalid]
        ])
...
$Page->showMessage();
?>
<form name="fsco_entrada_salidaadd" id="fsco_entrada_salidaadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_sco_entrada_salida_tipo_doc">
    <select
        ...
        data-select2-id="fsco_entrada_salidaadd_x_tipo_doc"
        <?php } ?>
...
<?php if (!$Page->tipo_doc->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_entrada_salidaadd", function() {
    var options = { name: "x_tipo_doc", selectId: "fsco_entrada_salidaadd_x_tipo_doc" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_entrada_salidaadd.lists.tipo_doc?.lookupOptions.length) {
        options.data = { id: "x_tipo_doc", form: "fsco_entrada_salidaadd" };
    } else {
        options.ajax = { id: "x_tipo_doc", form: "fsco_entrada_salidaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_entrada_salida_proveedor">
    <select
        ...
        data-select2-id="fsco_entrada_salidaadd_x_proveedor"
        data-table="sco_entrada_salida"
...
<script>
loadjs.ready("fsco_entrada_salidaadd", function() {
    var options = { name: "x_proveedor", selectId: "fsco_entrada_salidaadd_x_proveedor" };
    if (fsco_entrada_salidaadd.lists.proveedor?.lookupOptions.length) {
        options.data = { id: "x_proveedor", form: "fsco_entrada_salidaadd" };
    } else {
        options.ajax = { id: "x_proveedor", form: "fsco_entrada_salidaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_entrada_salida_clasificacion">
    <select
        ...
        data-select2-id="fsco_entrada_salidaadd_x_clasificacion"
        <?php } ?>
        data-table="sco_entrada_salida"
...
<script>
loadjs.ready("fsco_entrada_salidaadd", function() {
    var options = { name: "x_clasificacion", selectId: "fsco_entrada_salidaadd_x_clasificacion" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_entrada_salidaadd.lists.clasificacion?.lookupOptions.length) {
        options.data = { id: "x_clasificacion", form: "fsco_entrada_salidaadd" };
    } else {
        options.ajax = { id: "x_clasificacion", form: "fsco_entrada_salidaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["fsco_entrada_salidaadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fsco_entrada_salidaadd", "x_fecha", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<input type="<?= $Page->registro->getInputTextType() ?>" name="x_registro" id="x_registro" data-table="sco_entrada_salida" data-field="x_registro" value="<?= $Page->registro->EditValue ?>" placeholder="<?= HtmlEncode($Page->registro->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->registro->formatPattern()) ?>"<?= $Page->registro->editAttributes() ?> aria-describedby="x_registro_help">
<?= $Page->registro->getCustomMessage() ?>
<div class="invalid-feedback"><?= $Page->registro->getErrorMessage() ?></div>
...
loadjs.ready(["fsco_entrada_salidaadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fsco_entrada_salidaadd", "x_registro", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php
    if (in_array("sco_entrada_salida_detalle", explode(",", $Page->getCurrentDetailTable())) && $sco_entrada_salida_detalle->DetailAdd) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_entrada_salidaadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_entrada_salidaadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
