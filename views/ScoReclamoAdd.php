...
$Page->showMessage();
?>
<form name="fsco_reclamoadd" id="fsco_reclamoadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_sco_reclamo_tipo">
    <select
        ...
        data-select2-id="fsco_reclamoadd_x_tipo"
...
<script>
loadjs.ready("fsco_reclamoadd", function() {
    var options = { name: "x_tipo", selectId: "fsco_reclamoadd_x_tipo" };
    if (fsco_reclamoadd.lists.tipo?.lookupOptions.length) {
        options.data = { id: "x_tipo", form: "fsco_reclamoadd" };
    } else {
        options.ajax = { id: "x_tipo", form: "fsco_reclamoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_reclamo_parentesco">
    <select
        ...
        data-select2-id="fsco_reclamoadd_x_parentesco"
        <?php } ?>
        data-table="sco_reclamo"
...
<script>
loadjs.ready("fsco_reclamoadd", function() {
    var options = { name: "x_parentesco", selectId: "fsco_reclamoadd_x_parentesco" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_reclamoadd.lists.parentesco?.lookupOptions.length) {
        options.data = { id: "x_parentesco", form: "fsco_reclamoadd" };
    } else {
        options.ajax = { id: "x_parentesco", form: "fsco_reclamoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php
    if (in_array("sco_reclamo_nota", explode(",", $Page->getCurrentDetailTable())) && $sco_reclamo_nota->DetailAdd) {
?>
...
<?php } ?>
<?php
    if (in_array("sco_reclamo_adjunto", explode(",", $Page->getCurrentDetailTable())) && $sco_reclamo_adjunto->DetailAdd) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_reclamoadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_reclamoadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
