...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fsco_embalajeedit" id="fsco_embalajeedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { sco_embalaje: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fsco_embalajeedit;
...
<script>
loadjs.ready(["fsco_embalajeedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fsco_embalajeedit", "x_fecha_servicio", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->anulado->Visible) { // anulado ?>
    <div id="r_anulado"<?= $Page->anulado->rowAttributes() ?>>
        <label id="elh_sco_embalaje_anulado" for="x_anulado" class="<?= $Page->LeftColumnClass ?>"><?= $Page->anulado->caption() ?><?= $Page->anulado->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<span id="el_sco_embalaje_anulado">
    <select
        ...
        data-select2-id="fsco_embalajeedit_x_anulado"
        <?php } ?>
...
<?php if (!$Page->anulado->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_embalajeedit", function() {
    var options = { name: "x_anulado", selectId: "fsco_embalajeedit_x_anulado" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_embalajeedit.lists.anulado?.lookupOptions.length) {
        options.data = { id: "x_anulado", form: "fsco_embalajeedit" };
    } else {
        options.ajax = { id: "x_anulado", form: "fsco_embalajeedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="sco_embalaje" data-field="x_Nembalaje" data-hidden="1" name="x_Nembalaje" id="x_Nembalaje" value="<?= HtmlEncode($Page->Nembalaje->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_embalajeedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_embalajeedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
