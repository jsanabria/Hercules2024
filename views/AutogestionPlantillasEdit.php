...
// Page object
$AutogestionPlantillasEdit = &$Page;
?>
...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fautogestion_plantillasedit" id="fautogestion_plantillasedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { autogestion_plantillas: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fautogestion_plantillasedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fautogestion_plantillasedit")
        .setPageId("edit")
...
<span id="el_autogestion_plantillas_servicio_tipo">
    <select
        ...
        data-select2-id="fautogestion_plantillasedit_x_servicio_tipo"
        <?php } ?>
...
<?php if (!$Page->servicio_tipo->IsNativeSelect) { ?>
<script>
loadjs.ready("fautogestion_plantillasedit", function() {
    var options = { name: "x_servicio_tipo", selectId: "fautogestion_plantillasedit_x_servicio_tipo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fautogestion_plantillasedit.lists.servicio_tipo?.lookupOptions.length) {
        options.data = { id: "x_servicio_tipo", form: "fautogestion_plantillasedit" };
    } else {
        options.ajax = { id: "x_servicio_tipo", form: "fautogestion_plantillasedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_autogestion_plantillas_nivel">
    <select
        ...
        data-select2-id="fautogestion_plantillasedit_x_nivel"
        <?php } ?>
        data-table="autogestion_plantillas"
        data-field="x_nivel"
...
<script>
loadjs.ready("fautogestion_plantillasedit", function() {
    var options = { name: "x_nivel", selectId: "fautogestion_plantillasedit_x_nivel" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fautogestion_plantillasedit.lists.nivel?.lookupOptions.length) {
        options.data = { id: "x_nivel", form: "fautogestion_plantillasedit" };
    } else {
        options.ajax = { id: "x_nivel", form: "fautogestion_plantillasedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["fautogestion_plantillasedit", "editor"], function() {
    ew.createEditor("fautogestion_plantillasedit", "x_descripcion", 35, 6, <?= $Page->descripcion->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<input type="hidden" name="fa_x_imagen" id= "fa_x_imagen" value="<?= (Post("fa_x_imagen") == "0") ? "0" : "1" ?>">
...
<?php } ?>
    ...
    <input type="hidden" data-table="autogestion_plantillas" data-field="x_Nplantilla" data-hidden="1" name="x_Nplantilla" id="x_Nplantilla" value="<?= HtmlEncode($Page->Nplantilla->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fautogestion_plantillasedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fautogestion_plantillasedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
