...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fsco_costos_tarifaedit" id="fsco_costos_tarifaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { sco_costos_tarifa: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fsco_costos_tarifaedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fsco_costos_tarifaedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["Ncostos_tarifa", [fields.Ncostos_tarifa.visible && fields.Ncostos_tarifa.required ? ew.Validators.required(fields.Ncostos_tarifa.caption) : null], fields.Ncostos_tarifa.isInvalid],
...
<?php if ($Page->Ncostos_tarifa->Visible) { // Ncostos_tarifa ?>
    <div id="r_Ncostos_tarifa"<?= $Page->Ncostos_tarifa->rowAttributes() ?>>
        <label id="elh_sco_costos_tarifa_Ncostos_tarifa" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Ncostos_tarifa->caption() ?><?= $Page->Ncostos_tarifa->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->Ncostos_tarifa->cellAttributes() ?>>
<span id="el_sco_costos_tarifa_Ncostos_tarifa">
<span<?= $Page->Ncostos_tarifa->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Ncostos_tarifa->getDisplayValue($Page->Ncostos_tarifa->EditValue))) ?>"></span>
<input type="hidden" data-table="sco_costos_tarifa" data-field="x_Ncostos_tarifa" data-hidden="1" name="x_Ncostos_tarifa" id="x_Ncostos_tarifa" value="<?= HtmlEncode($Page->Ncostos_tarifa->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_sco_costos_tarifa_localidad">
    <select
        ...
        data-select2-id="fsco_costos_tarifaedit_x_localidad"
        <?php } ?>
...
<?php if (!$Page->localidad->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_costos_tarifaedit", function() {
    var options = { name: "x_localidad", selectId: "fsco_costos_tarifaedit_x_localidad" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_costos_tarifaedit.lists.localidad?.lookupOptions.length) {
        options.data = { id: "x_localidad", form: "fsco_costos_tarifaedit" };
    } else {
        options.ajax = { id: "x_localidad", form: "fsco_costos_tarifaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_costos_tarifa_tipo_servicio">
    <select
        ...
        data-select2-id="fsco_costos_tarifaedit_x_tipo_servicio"
        <?php } ?>
        data-table="sco_costos_tarifa"
...
<script>
loadjs.ready("fsco_costos_tarifaedit", function() {
    var options = { name: "x_tipo_servicio", selectId: "fsco_costos_tarifaedit_x_tipo_servicio" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_costos_tarifaedit.lists.tipo_servicio?.lookupOptions.length) {
        options.data = { id: "x_tipo_servicio", form: "fsco_costos_tarifaedit" };
    } else {
        options.ajax = { id: "x_tipo_servicio", form: "fsco_costos_tarifaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_sco_costos_tarifa_horas">
    <select
        ...
        data-select2-id="fsco_costos_tarifaedit_x_horas"
        <?php } ?>
        data-table="sco_costos_tarifa"
        data-field="x_horas"
...
<script>
loadjs.ready("fsco_costos_tarifaedit", function() {
    var options = { name: "x_horas", selectId: "fsco_costos_tarifaedit_x_horas" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_costos_tarifaedit.lists.horas?.lookupOptions.length) {
        options.data = { id: "x_horas", form: "fsco_costos_tarifaedit" };
    } else {
        options.ajax = { id: "x_horas", form: "fsco_costos_tarifaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_sco_costos_tarifa_activo">
    <select
        ...
        data-select2-id="fsco_costos_tarifaedit_x_activo"
        <?php } ?>
        data-table="sco_costos_tarifa"
        data-field="x_activo"
...
<script>
loadjs.ready("fsco_costos_tarifaedit", function() {
    var options = { name: "x_activo", selectId: "fsco_costos_tarifaedit_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_costos_tarifaedit.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fsco_costos_tarifaedit" };
    } else {
        options.ajax = { id: "x_activo", form: "fsco_costos_tarifaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php
    if (in_array("sco_costos_tarifa_detalle", explode(",", $Page->getCurrentDetailTable())) && $sco_costos_tarifa_detalle->DetailEdit) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_costos_tarifaedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_costos_tarifaedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
