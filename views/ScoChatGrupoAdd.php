...
$Page->showMessage();
?>
<form name="fsco_chat_grupoadd" id="fsco_chat_grupoadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_sco_chat_grupo_grupo">
    <select
        ...
        data-select2-id="fsco_chat_grupoadd_x_grupo"
        <?php } ?>
...
<?php if (!$Page->grupo->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_chat_grupoadd", function() {
    var options = { name: "x_grupo", selectId: "fsco_chat_grupoadd_x_grupo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_chat_grupoadd.lists.grupo?.lookupOptions.length) {
        options.data = { id: "x_grupo", form: "fsco_chat_grupoadd" };
    } else {
        options.ajax = { id: "x_grupo", form: "fsco_chat_grupoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_chat_grupo_ve_todo">
    <select
        ...
        data-select2-id="fsco_chat_grupoadd_x_ve_todo"
        <?php } ?>
        data-table="sco_chat_grupo"
...
<script>
loadjs.ready("fsco_chat_grupoadd", function() {
    var options = { name: "x_ve_todo", selectId: "fsco_chat_grupoadd_x_ve_todo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_chat_grupoadd.lists.ve_todo?.lookupOptions.length) {
        options.data = { id: "x_ve_todo", form: "fsco_chat_grupoadd" };
    } else {
        options.ajax = { id: "x_ve_todo", form: "fsco_chat_grupoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_chat_grupoadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_chat_grupoadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
