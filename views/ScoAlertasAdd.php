...
$Page->showMessage();
?>
<form name="fsco_alertasadd" id="fsco_alertasadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_sco_alertas_activo">
    <select
        ...
        data-select2-id="fsco_alertasadd_x_activo"
        <?php } ?>
...
<?php if (!$Page->activo->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_alertasadd", function() {
    var options = { name: "x_activo", selectId: "fsco_alertasadd_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_alertasadd.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fsco_alertasadd" };
    } else {
        options.ajax = { id: "x_activo", form: "fsco_alertasadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_alertasadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_alertasadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
