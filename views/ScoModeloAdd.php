...
$Page->showMessage();
?>
<form name="fsco_modeloadd" id="fsco_modeloadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_sco_modelo_tipo">
    <select
        ...
        data-select2-id="fsco_modeloadd_x_tipo"
        <?php } ?>
...
<?php if (!$Page->tipo->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_modeloadd", function() {
    var options = { name: "x_tipo", selectId: "fsco_modeloadd_x_tipo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_modeloadd.lists.tipo?.lookupOptions.length) {
        options.data = { id: "x_tipo", form: "fsco_modeloadd" };
    } else {
        options.ajax = { id: "x_tipo", form: "fsco_modeloadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_modelo_marca">
    <select
        ...
        data-select2-id="fsco_modeloadd_x_marca"
        data-table="sco_modelo"
        data-field="x_marca"
...
<script>
loadjs.ready("fsco_modeloadd", function() {
    var options = { name: "x_marca", selectId: "fsco_modeloadd_x_marca" };
    if (fsco_modeloadd.lists.marca?.lookupOptions.length) {
        options.data = { id: "x_marca", form: "fsco_modeloadd" };
    } else {
        options.ajax = { id: "x_marca", form: "fsco_modeloadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_modelo_activo">
    <select
        ...
        data-select2-id="fsco_modeloadd_x_activo"
        <?php } ?>
        data-table="sco_modelo"
        data-field="x_activo"
...
<script>
loadjs.ready("fsco_modeloadd", function() {
    var options = { name: "x_activo", selectId: "fsco_modeloadd_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_modeloadd.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fsco_modeloadd" };
    } else {
        options.ajax = { id: "x_activo", form: "fsco_modeloadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_modeloadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_modeloadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
