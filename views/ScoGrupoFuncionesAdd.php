...
$Page->showMessage();
?>
<form name="fsco_grupo_funcionesadd" id="fsco_grupo_funcionesadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_sco_grupo_funciones_funcion">
    <select
        ...
        data-select2-id="fsco_grupo_funcionesadd_x_funcion"
        <?php } ?>
...
<?php if (!$Page->funcion->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_grupo_funcionesadd", function() {
    var options = { name: "x_funcion", selectId: "fsco_grupo_funcionesadd_x_funcion" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_grupo_funcionesadd.lists.funcion?.lookupOptions.length) {
        options.data = { id: "x_funcion", form: "fsco_grupo_funcionesadd" };
    } else {
        options.ajax = { id: "x_funcion", form: "fsco_grupo_funcionesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_grupo_funcionesadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_grupo_funcionesadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
