...
$Page->showMessage();
?>
<form name="fsco_orden_salidaadd" id="fsco_orden_salidaadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_sco_orden_salida_grupo">
    <select
        ...
        data-select2-id="fsco_orden_salidaadd_x_grupo"
...
<script>
loadjs.ready("fsco_orden_salidaadd", function() {
    var options = { name: "x_grupo", selectId: "fsco_orden_salidaadd_x_grupo" };
    if (fsco_orden_salidaadd.lists.grupo?.lookupOptions.length) {
        options.data = { id: "x_grupo", form: "fsco_orden_salidaadd" };
    } else {
        options.ajax = { id: "x_grupo", form: "fsco_orden_salidaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_orden_salida_conductor">
    <select
        ...
        data-select2-id="fsco_orden_salidaadd_x_conductor"
        data-table="sco_orden_salida"
...
<script>
loadjs.ready("fsco_orden_salidaadd", function() {
    var options = { name: "x_conductor", selectId: "fsco_orden_salidaadd_x_conductor" };
    if (fsco_orden_salidaadd.lists.conductor?.lookupOptions.length) {
        options.data = { id: "x_conductor", form: "fsco_orden_salidaadd" };
    } else {
        options.ajax = { id: "x_conductor", form: "fsco_orden_salidaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_orden_salida_placa">
    <select
        ...
        data-select2-id="fsco_orden_salidaadd_x_placa"
        data-table="sco_orden_salida"
        data-field="x_placa"
...
<script>
loadjs.ready("fsco_orden_salidaadd", function() {
    var options = { name: "x_placa", selectId: "fsco_orden_salidaadd_x_placa" };
    if (fsco_orden_salidaadd.lists.placa?.lookupOptions.length) {
        options.data = { id: "x_placa", form: "fsco_orden_salidaadd" };
    } else {
        options.ajax = { id: "x_placa", form: "fsco_orden_salidaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_orden_salida_motivo">
    <select
        ...
        data-select2-id="fsco_orden_salidaadd_x_motivo"
        <?php } ?>
        data-table="sco_orden_salida"
        data-field="x_motivo"
...
<script>
loadjs.ready("fsco_orden_salidaadd", function() {
    var options = { name: "x_motivo", selectId: "fsco_orden_salidaadd_x_motivo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_orden_salidaadd.lists.motivo?.lookupOptions.length) {
        options.data = { id: "x_motivo", form: "fsco_orden_salidaadd" };
    } else {
        options.ajax = { id: "x_motivo", form: "fsco_orden_salidaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_orden_salidaadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_orden_salidaadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
