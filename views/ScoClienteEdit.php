...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fsco_clienteedit" id="fsco_clienteedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { sco_cliente: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fsco_clienteedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fsco_clienteedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["Ncliente", [fields.Ncliente.visible && fields.Ncliente.required ? ew.Validators.required(fields.Ncliente.caption) : null, ew.Validators.integer], fields.Ncliente.isInvalid],
...
<?php if ($Page->Ncliente->Visible) { // Ncliente ?>
    <div id="r_Ncliente"<?= $Page->Ncliente->rowAttributes() ?>>
        <label id="elh_sco_cliente_Ncliente" for="x_Ncliente" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Ncliente->caption() ?><?= $Page->Ncliente->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->Ncliente->cellAttributes() ?>>
<span id="el_sco_cliente_Ncliente">
<span<?= $Page->Ncliente->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Ncliente->getDisplayValue($Page->Ncliente->EditValue))) ?>"></span>
<input type="hidden" data-table="sco_cliente" data-field="x_Ncliente" data-hidden="1" name="x_Ncliente" id="x_Ncliente" value="<?= HtmlEncode($Page->Ncliente->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_sco_cliente_activo">
    <select
        ...
        data-select2-id="fsco_clienteedit_x_activo"
        <?php } ?>
...
<?php if (!$Page->activo->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_clienteedit", function() {
    var options = { name: "x_activo", selectId: "fsco_clienteedit_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_clienteedit.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fsco_clienteedit" };
    } else {
        options.ajax = { id: "x_activo", form: "fsco_clienteedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_clienteedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_clienteedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
