...
$Page->showMessage();
?>
<form name="fsco_flotaadd" id="fsco_flotaadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_sco_flota_tipo_flota">
    <select
        ...
        data-select2-id="fsco_flotaadd_x_tipo_flota"
...
<script>
loadjs.ready("fsco_flotaadd", function() {
    var options = { name: "x_tipo_flota", selectId: "fsco_flotaadd_x_tipo_flota" };
    if (fsco_flotaadd.lists.tipo_flota?.lookupOptions.length) {
        options.data = { id: "x_tipo_flota", form: "fsco_flotaadd" };
    } else {
        options.ajax = { id: "x_tipo_flota", form: "fsco_flotaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_flota_marca">
    <select
        ...
        data-select2-id="fsco_flotaadd_x_marca"
        data-table="sco_flota"
        data-field="x_marca"
...
<script>
loadjs.ready("fsco_flotaadd", function() {
    var options = { name: "x_marca", selectId: "fsco_flotaadd_x_marca" };
    if (fsco_flotaadd.lists.marca?.lookupOptions.length) {
        options.data = { id: "x_marca", form: "fsco_flotaadd" };
    } else {
        options.ajax = { id: "x_marca", form: "fsco_flotaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_flota_modelo">
    <select
        ...
        data-select2-id="fsco_flotaadd_x_modelo"
        data-table="sco_flota"
        data-field="x_modelo"
...
<script>
loadjs.ready("fsco_flotaadd", function() {
    var options = { name: "x_modelo", selectId: "fsco_flotaadd_x_modelo" };
    if (fsco_flotaadd.lists.modelo?.lookupOptions.length) {
        options.data = { id: "x_modelo", form: "fsco_flotaadd" };
    } else {
        options.ajax = { id: "x_modelo", form: "fsco_flotaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_flota_tipo">
    <select
        ...
        data-select2-id="fsco_flotaadd_x_tipo"
        <?php } ?>
        data-table="sco_flota"
        data-field="x_tipo"
...
<script>
loadjs.ready("fsco_flotaadd", function() {
    var options = { name: "x_tipo", selectId: "fsco_flotaadd_x_tipo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_flotaadd.lists.tipo?.lookupOptions.length) {
        options.data = { id: "x_tipo", form: "fsco_flotaadd" };
    } else {
        options.ajax = { id: "x_tipo", form: "fsco_flotaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_flota_activo">
    <select
        ...
        data-select2-id="fsco_flotaadd_x_activo"
        <?php } ?>
        data-table="sco_flota"
        data-field="x_activo"
...
<script>
loadjs.ready("fsco_flotaadd", function() {
    var options = { name: "x_activo", selectId: "fsco_flotaadd_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_flotaadd.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fsco_flotaadd" };
    } else {
        options.ajax = { id: "x_activo", form: "fsco_flotaadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_flotaadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_flotaadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
