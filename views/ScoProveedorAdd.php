...
$Page->showMessage();
?>
<form name="fsco_proveedoradd" id="fsco_proveedoradd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_sco_proveedor_estado">
    <select
        ...
        data-select2-id="fsco_proveedoradd_x_estado"
        <?php } ?>
...
<?php if (!$Page->estado->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_proveedoradd", function() {
    var options = { name: "x_estado", selectId: "fsco_proveedoradd_x_estado" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_proveedoradd.lists.estado?.lookupOptions.length) {
        options.data = { id: "x_estado", form: "fsco_proveedoradd" };
    } else {
        options.ajax = { id: "x_estado", form: "fsco_proveedoradd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_proveedor_tipo_proveedor">
    <select
        ...
        data-select2-id="fsco_proveedoradd_x_tipo_proveedor"
        <?php } ?>
        data-table="sco_proveedor"
...
<script>
loadjs.ready("fsco_proveedoradd", function() {
    var options = { name: "x_tipo_proveedor", selectId: "fsco_proveedoradd_x_tipo_proveedor" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_proveedoradd.lists.tipo_proveedor?.lookupOptions.length) {
        options.data = { id: "x_tipo_proveedor", form: "fsco_proveedoradd" };
    } else {
        options.ajax = { id: "x_tipo_proveedor", form: "fsco_proveedoradd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_sco_proveedor_activo">
    <select
        ...
        data-select2-id="fsco_proveedoradd_x_activo"
        <?php } ?>
        data-table="sco_proveedor"
        data-field="x_activo"
...
<script>
loadjs.ready("fsco_proveedoradd", function() {
    var options = { name: "x_activo", selectId: "fsco_proveedoradd_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_proveedoradd.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fsco_proveedoradd" };
    } else {
        options.ajax = { id: "x_activo", form: "fsco_proveedoradd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php
    if (in_array("sco_ofrece_servicio", explode(",", $Page->getCurrentDetailTable())) && $sco_ofrece_servicio->DetailAdd) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_proveedoradd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_proveedoradd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
