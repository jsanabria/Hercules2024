...
// Page object
$ScoFlotaIncidenciaDetalleAdd = &$Page;
?>
<script>
...
var currentForm;
var fsco_flota_incidencia_detalleadd;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fsco_flota_incidencia_detalleadd")
        .setPageId("add")
...
$Page->showMessage();
?>
<form name="fsco_flota_incidencia_detalleadd" id="fsco_flota_incidencia_detalleadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_sco_flota_incidencia_detalle_tipo">
    <select
        ...
        data-select2-id="fsco_flota_incidencia_detalleadd_x_tipo"
        <?php } ?>
...
<?php if (!$Page->tipo->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_flota_incidencia_detalleadd", function() {
    var options = { name: "x_tipo", selectId: "fsco_flota_incidencia_detalleadd_x_tipo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_flota_incidencia_detalleadd.lists.tipo?.lookupOptions.length) {
        options.data = { id: "x_tipo", form: "fsco_flota_incidencia_detalleadd" };
    } else {
        options.ajax = { id: "x_tipo", form: "fsco_flota_incidencia_detalleadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_flota_incidencia_detalle_proveedor">
    <select
        ...
        data-select2-id="fsco_flota_incidencia_detalleadd_x_proveedor"
        <?php } ?>
        data-table="sco_flota_incidencia_detalle"
...
<script>
loadjs.ready("fsco_flota_incidencia_detalleadd", function() {
    var options = { name: "x_proveedor", selectId: "fsco_flota_incidencia_detalleadd_x_proveedor" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_flota_incidencia_detalleadd.lists.proveedor?.lookupOptions.length) {
        options.data = { id: "x_proveedor", form: "fsco_flota_incidencia_detalleadd" };
    } else {
        options.ajax = { id: "x_proveedor", form: "fsco_flota_incidencia_detalleadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_flota_incidencia_detalleadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_flota_incidencia_detalleadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
