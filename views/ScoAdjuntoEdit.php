...
?>
<main class="edit">
<form name="fsco_adjuntoedit" id="fsco_adjuntoedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { sco_adjunto: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fsco_adjuntoedit;
...
<input type="hidden" data-table="sco_adjunto" data-field="x_tipo" data-hidden="1" name="x_tipo" id="x_tipo" value="<?= HtmlEncode($Page->tipo->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="sco_adjunto" data-field="x_archivo" data-hidden="1" name="x_archivo" id="x_archivo" value="<?= HtmlEncode($Page->archivo->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_sco_adjunto_activo">
    <select
        ...
        data-select2-id="fsco_adjuntoedit_x_activo"
        <?php } ?>
...
<?php if (!$Page->activo->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_adjuntoedit", function() {
    var options = { name: "x_activo", selectId: "fsco_adjuntoedit_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_adjuntoedit.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fsco_adjuntoedit" };
    } else {
        options.ajax = { id: "x_activo", form: "fsco_adjuntoedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="sco_adjunto" data-field="x_Nadjunto" data-hidden="1" name="x_Nadjunto" id="x_Nadjunto" value="<?= HtmlEncode($Page->Nadjunto->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_adjuntoedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_adjuntoedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
