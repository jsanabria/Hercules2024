...
$Page->showMessage();
?>
<form name="fview_velacionadd" id="fview_velacionadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_view_velacion_tipo">
    <select
        ...
        data-select2-id="fview_velacionadd_x_tipo"
        <?php } ?>
...
<?php if (!$Page->tipo->IsNativeSelect) { ?>
<script>
loadjs.ready("fview_velacionadd", function() {
    var options = { name: "x_tipo", selectId: "fview_velacionadd_x_tipo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fview_velacionadd.lists.tipo?.lookupOptions.length) {
        options.data = { id: "x_tipo", form: "fview_velacionadd" };
    } else {
        options.ajax = { id: "x_tipo", form: "fview_velacionadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.view_velacion.fields.tipo.selectOptions);
...
<span id="el_view_velacion_activo">
    <select
        ...
        data-select2-id="fview_velacionadd_x_activo"
        <?php } ?>
        data-table="view_velacion"
        data-field="x_activo"
...
<script>
loadjs.ready("fview_velacionadd", function() {
    var options = { name: "x_activo", selectId: "fview_velacionadd_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fview_velacionadd.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fview_velacionadd" };
    } else {
        options.ajax = { id: "x_activo", form: "fview_velacionadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fview_velacionadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fview_velacionadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
