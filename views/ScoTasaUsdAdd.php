...
$Page->showMessage();
?>
<form name="fsco_tasa_usdadd" id="fsco_tasa_usdadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_sco_tasa_usd_moneda">
    <select
        ...
        data-select2-id="fsco_tasa_usdadd_x_moneda"
        <?php } ?>
        data-table="sco_tasa_usd"
        data-field="x_moneda"
...
<?php if (!$Page->moneda->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_tasa_usdadd", function() {
    var options = { name: "x_moneda", selectId: "fsco_tasa_usdadd_x_moneda" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_tasa_usdadd.lists.moneda?.lookupOptions.length) {
        options.data = { id: "x_moneda", form: "fsco_tasa_usdadd" };
    } else {
        options.ajax = { id: "x_moneda", form: "fsco_tasa_usdadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.sco_tasa_usd.fields.moneda.selectOptions);
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_tasa_usdadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_tasa_usdadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
loadjs.ready("load", function () {
    ...
    $(document).ready(function() {
        $("#x_tasa").on("keypress", function(e) {
            // Permitir: backspace, delete, tab, escape, enter y el punto (.)
            // El código del punto es 46
            if ($.inArray(e.keyCode, [46, 8, 9, 27, 13]) !== -1 ||
                // Permitir: Ctrl+A, Ctrl+C, Ctrl+V, Ctrl+X, Home, End
                (e.ctrlKey === true || e.metaKey === true)) {
                // Si es un punto, verificar que no exista ya uno en el campo
            ...
            // Bloquear si no es un número (teclas entre 48 y 57)
            if ((e.shiftKey || (e.which < 48 || e.which > 57))) {
                e.preventDefault();
            }
        });

        // Filtro de limpieza para cuando pegan texto con el mouse
        $("#x_tasa").on("input", function() {
            let self = $(this);
            // Elimina cualquier cosa que no sea número o punto
            let validVal = self.val().replace(/[^0-9.]/g, '');

            // Evita que haya más de un punto si pegaron algo como "10.50.2"
            let parts = validVal.split('.');
            if (parts.length > 2) {
                validVal = parts[0] + '.' + parts.slice(1).join('');
            }
            if (self.val() !== validVal) {
                self.val(validVal);
            }
        });
    });
