...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fsco_grama_pagosedit" id="fsco_grama_pagosedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { sco_grama_pagos: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fsco_grama_pagosedit;
...
<span id="el_sco_grama_pagos_tipo">
    <select
        ...
        data-select2-id="fsco_grama_pagosedit_x_tipo"
        <?php } ?>
...
<?php if (!$Page->tipo->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_grama_pagosedit", function() {
    var options = { name: "x_tipo", selectId: "fsco_grama_pagosedit_x_tipo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_grama_pagosedit.lists.tipo?.lookupOptions.length) {
        options.data = { id: "x_tipo", form: "fsco_grama_pagosedit" };
    } else {
        options.ajax = { id: "x_tipo", form: "fsco_grama_pagosedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fsco_grama_pagosedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fsco_grama_pagosedit", "x_fecha", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_sco_grama_pagos_moneda">
    <select
        ...
        data-select2-id="fsco_grama_pagosedit_x_moneda"
        <?php } ?>
        data-table="sco_grama_pagos"
        data-field="x_moneda"
...
<script>
loadjs.ready("fsco_grama_pagosedit", function() {
    var options = { name: "x_moneda", selectId: "fsco_grama_pagosedit_x_moneda" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_grama_pagosedit.lists.moneda?.lookupOptions.length) {
        options.data = { id: "x_moneda", form: "fsco_grama_pagosedit" };
    } else {
        options.ajax = { id: "x_moneda", form: "fsco_grama_pagosedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_sco_grama_pagos_cta_destino">
    <select
        ...
        data-select2-id="fsco_grama_pagosedit_x_cta_destino"
        <?php } ?>
        data-table="sco_grama_pagos"
...
<script>
loadjs.ready("fsco_grama_pagosedit", function() {
    var options = { name: "x_cta_destino", selectId: "fsco_grama_pagosedit_x_cta_destino" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_grama_pagosedit.lists.cta_destino?.lookupOptions.length) {
        options.data = { id: "x_cta_destino", form: "fsco_grama_pagosedit" };
    } else {
        options.ajax = { id: "x_cta_destino", form: "fsco_grama_pagosedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php } ?>
    ...
    <input type="hidden" data-table="sco_grama_pagos" data-field="x_Ngrama_pagos" data-hidden="1" name="x_Ngrama_pagos" id="x_Ngrama_pagos" value="<?= HtmlEncode($Page->Ngrama_pagos->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_grama_pagosedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_grama_pagosedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
