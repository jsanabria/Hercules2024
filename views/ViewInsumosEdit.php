...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fview_insumosedit" id="fview_insumosedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { view_insumos: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fview_insumosedit;
...
<span id="el_view_insumos_tipo">
    <select
        ...
        data-select2-id="fview_insumosedit_x_tipo"
        <?php } ?>
...
<?php if (!$Page->tipo->IsNativeSelect) { ?>
<script>
loadjs.ready("fview_insumosedit", function() {
    var options = { name: "x_tipo", selectId: "fview_insumosedit_x_tipo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fview_insumosedit.lists.tipo?.lookupOptions.length) {
        options.data = { id: "x_tipo", form: "fview_insumosedit" };
    } else {
        options.ajax = { id: "x_tipo", form: "fview_insumosedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php if ($Page->Nservicio->Visible) { // Nservicio ?>
    <div id="r_Nservicio"<?= $Page->Nservicio->rowAttributes() ?>>
        <label id="elh_view_insumos_Nservicio" for="x_Nservicio" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Nservicio->caption() ?><?= $Page->Nservicio->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<?php if (!EmptyString($Page->Nservicio->EditValue) && $Page->Nservicio->linkAttributes() != "") { ?>
<a<?= $Page->Nservicio->linkAttributes() ?>><input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Nservicio->getDisplayValue($Page->Nservicio->EditValue))) ?>"></a>
<?php } else { ?>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Nservicio->getDisplayValue($Page->Nservicio->EditValue))) ?>">
<?php } ?>
</span>
<input type="hidden" data-table="view_insumos" data-field="x_Nservicio" data-hidden="1" name="x_Nservicio" id="x_Nservicio" value="<?= HtmlEncode($Page->Nservicio->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_view_insumos_articulo_inventario">
    <select
        ...
        data-select2-id="fview_insumosedit_x_articulo_inventario"
        <?php } ?>
        data-table="view_insumos"
...
<script>
loadjs.ready("fview_insumosedit", function() {
    var options = { name: "x_articulo_inventario", selectId: "fview_insumosedit_x_articulo_inventario" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fview_insumosedit.lists.articulo_inventario?.lookupOptions.length) {
        options.data = { id: "x_articulo_inventario", form: "fview_insumosedit" };
    } else {
        options.ajax = { id: "x_articulo_inventario", form: "fview_insumosedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_view_insumos_activo">
    <select
        ...
        data-select2-id="fview_insumosedit_x_activo"
        <?php } ?>
        data-table="view_insumos"
        data-field="x_activo"
...
<script>
loadjs.ready("fview_insumosedit", function() {
    var options = { name: "x_activo", selectId: "fview_insumosedit_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fview_insumosedit.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fview_insumosedit" };
    } else {
        options.ajax = { id: "x_activo", form: "fview_insumosedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fview_insumosedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fview_insumosedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
