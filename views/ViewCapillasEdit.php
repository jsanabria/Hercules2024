...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fview_capillasedit" id="fview_capillasedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { view_capillas: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fview_capillasedit;
...
<input type="hidden" data-table="view_capillas" data-field="x_tipo" data-hidden="1" name="x_tipo" id="x_tipo" value="<?= HtmlEncode($Page->tipo->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
<?php if ($Page->Nservicio->Visible) { // Nservicio ?>
    <div id="r_Nservicio"<?= $Page->Nservicio->rowAttributes() ?>>
        <label id="elh_view_capillas_Nservicio" for="x_Nservicio" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Nservicio->caption() ?><?= $Page->Nservicio->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="view_capillas" data-field="x_Nservicio" data-hidden="1" name="x_Nservicio" id="x_Nservicio" value="<?= HtmlEncode($Page->Nservicio->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_view_capillas_activo">
    <select
        ...
        data-select2-id="fview_capillasedit_x_activo"
        <?php } ?>
...
<?php if (!$Page->activo->IsNativeSelect) { ?>
<script>
loadjs.ready("fview_capillasedit", function() {
    var options = { name: "x_activo", selectId: "fview_capillasedit_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fview_capillasedit.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fview_capillasedit" };
    } else {
        options.ajax = { id: "x_activo", form: "fview_capillasedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fview_capillasedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fview_capillasedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
