...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fsco_seguroedit" id="fsco_seguroedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<span id="el_sco_seguro_tipo_contratacion">
    <select
        ...
        data-select2-id="fsco_seguroedit_x_tipo_contratacion"
        <?php } ?>
...
<?php if (!$Page->tipo_contratacion->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_seguroedit", function() {
    var options = { name: "x_tipo_contratacion", selectId: "fsco_seguroedit_x_tipo_contratacion" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_seguroedit.lists.tipo_contratacion?.lookupOptions.length) {
        options.data = { id: "x_tipo_contratacion", form: "fsco_seguroedit" };
    } else {
        options.ajax = { id: "x_tipo_contratacion", form: "fsco_seguroedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_seguro_activo">
    <select
        ...
        data-select2-id="fsco_seguroedit_x_activo"
        <?php } ?>
        data-table="sco_seguro"
        data-field="x_activo"
...
<script>
loadjs.ready("fsco_seguroedit", function() {
    var options = { name: "x_activo", selectId: "fsco_seguroedit_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_seguroedit.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fsco_seguroedit" };
    } else {
        options.ajax = { id: "x_activo", form: "fsco_seguroedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php } ?>
    ...
    <input type="hidden" data-table="sco_seguro" data-field="x_Nseguro" data-hidden="1" name="x_Nseguro" id="x_Nseguro" value="<?= HtmlEncode($Page->Nseguro->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_seguroedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_seguroedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
