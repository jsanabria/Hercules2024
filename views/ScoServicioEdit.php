...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fsco_servicioedit" id="fsco_servicioedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { sco_servicio: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fsco_servicioedit;
...
<input type="hidden" data-table="sco_servicio" data-field="x_tipo" data-hidden="1" name="x_tipo" id="x_tipo" value="<?= HtmlEncode($Page->tipo->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
<?php if ($Page->Nservicio->Visible) { // Nservicio ?>
    <div id="r_Nservicio"<?= $Page->Nservicio->rowAttributes() ?>>
        <label id="elh_sco_servicio_Nservicio" for="x_Nservicio" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Nservicio->caption() ?><?= $Page->Nservicio->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="sco_servicio" data-field="x_Nservicio" data-hidden="1" name="x_Nservicio" id="x_Nservicio" value="<?= HtmlEncode($Page->Nservicio->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_sco_servicio_articulo_inventario">
    <select
        ...
        data-select2-id="fsco_servicioedit_x_articulo_inventario"
        <?php } ?>
...
<?php if (!$Page->articulo_inventario->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_servicioedit", function() {
    var options = { name: "x_articulo_inventario", selectId: "fsco_servicioedit_x_articulo_inventario" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_servicioedit.lists.articulo_inventario?.lookupOptions.length) {
        options.data = { id: "x_articulo_inventario", form: "fsco_servicioedit" };
    } else {
        options.ajax = { id: "x_articulo_inventario", form: "fsco_servicioedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_servicio_activo">
    <select
        ...
        data-select2-id="fsco_servicioedit_x_activo"
        <?php } ?>
        data-table="sco_servicio"
        data-field="x_activo"
...
<script>
loadjs.ready("fsco_servicioedit", function() {
    var options = { name: "x_activo", selectId: "fsco_servicioedit_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_servicioedit.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fsco_servicioedit" };
    } else {
        options.ajax = { id: "x_activo", form: "fsco_servicioedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_sco_servicio_venta">
    <select
        ...
        data-select2-id="fsco_servicioedit_x_venta"
        <?php } ?>
        data-table="sco_servicio"
        data-field="x_venta"
...
<script>
loadjs.ready("fsco_servicioedit", function() {
    var options = { name: "x_venta", selectId: "fsco_servicioedit_x_venta" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_servicioedit.lists.venta?.lookupOptions.length) {
        options.data = { id: "x_venta", form: "fsco_servicioedit" };
    } else {
        options.ajax = { id: "x_venta", form: "fsco_servicioedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_servicioedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_servicioedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
