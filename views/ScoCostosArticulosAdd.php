...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fsco_costos_articulosadd")
        .setPageId("add")
        ...
        .setFields([
            ...
            ["variacion", [fields.variacion.visible && fields.variacion.required ? ew.Validators.required(fields.variacion.caption) : null, ew.Validators.float], fields.variacion.isInvalid],
...
$Page->showMessage();
?>
<form name="fsco_costos_articulosadd" id="fsco_costos_articulosadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_sco_costos_articulos_tipo">
    <select
        ...
        data-select2-id="fsco_costos_articulosadd_x_tipo"
        <?php } ?>
...
<?php if (!$Page->tipo->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_costos_articulosadd", function() {
    var options = { name: "x_tipo", selectId: "fsco_costos_articulosadd_x_tipo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_costos_articulosadd.lists.tipo?.lookupOptions.length) {
        options.data = { id: "x_tipo", form: "fsco_costos_articulosadd" };
    } else {
        options.ajax = { id: "x_tipo", form: "fsco_costos_articulosadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<input type="<?= $Page->Ncostos_articulo->getInputTextType() ?>" name="x_Ncostos_articulo" id="x_Ncostos_articulo" data-table="sco_costos_articulos" data-field="x_Ncostos_articulo" value="<?= $Page->Ncostos_articulo->EditValue ?>" size="30" maxlength="10" placeholder="<?= HtmlEncode($Page->Ncostos_articulo->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->Ncostos_articulo->formatPattern()) ?>"<?= $Page->Ncostos_articulo->editAttributes() ?> aria-describedby="x_Ncostos_articulo_help">
<?= $Page->Ncostos_articulo->getCustomMessage() ?>
...
<input type="<?= $Page->variacion->getInputTextType() ?>" name="x_variacion" id="x_variacion" data-table="sco_costos_articulos" data-field="x_variacion" value="<?= $Page->variacion->EditValue ?>" size="30" placeholder="<?= HtmlEncode($Page->variacion->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->variacion->formatPattern()) ?>"<?= $Page->variacion->editAttributes() ?> aria-describedby="x_variacion_help">
<?= $Page->variacion->getCustomMessage() ?>
...
<span id="el_sco_costos_articulos_tipo_hercules">
    <select
        ...
        data-select2-id="fsco_costos_articulosadd_x_tipo_hercules"
        <?php } ?>
        data-table="sco_costos_articulos"
...
<script>
loadjs.ready("fsco_costos_articulosadd", function() {
    var options = { name: "x_tipo_hercules", selectId: "fsco_costos_articulosadd_x_tipo_hercules" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_costos_articulosadd.lists.tipo_hercules?.lookupOptions.length) {
        options.data = { id: "x_tipo_hercules", form: "fsco_costos_articulosadd" };
    } else {
        options.ajax = { id: "x_tipo_hercules", form: "fsco_costos_articulosadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_sco_costos_articulos_articulo_hercules">
    <select
        ...
        data-select2-id="fsco_costos_articulosadd_x_articulo_hercules"
        <?php } ?>
        data-table="sco_costos_articulos"
...
<script>
loadjs.ready("fsco_costos_articulosadd", function() {
    var options = { name: "x_articulo_hercules", selectId: "fsco_costos_articulosadd_x_articulo_hercules" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_costos_articulosadd.lists.articulo_hercules?.lookupOptions.length) {
        options.data = { id: "x_articulo_hercules", form: "fsco_costos_articulosadd" };
    } else {
        options.ajax = { id: "x_articulo_hercules", form: "fsco_costos_articulosadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_costos_articulosadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_costos_articulosadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
