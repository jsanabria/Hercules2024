...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fsco_lapidas_registroadd")
        .setPageId("add")
...
$Page->showMessage();
?>
<form name="fsco_lapidas_registroadd" id="fsco_lapidas_registroadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_sco_lapidas_registro_parentesco">
    <select
        ...
        data-select2-id="fsco_lapidas_registroadd_x_parentesco"
        <?php } ?>
...
<?php if (!$Page->parentesco->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_lapidas_registroadd", function() {
    var options = { name: "x_parentesco", selectId: "fsco_lapidas_registroadd_x_parentesco" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_lapidas_registroadd.lists.parentesco?.lookupOptions.length) {
        options.data = { id: "x_parentesco", form: "fsco_lapidas_registroadd" };
    } else {
        options.ajax = { id: "x_parentesco", form: "fsco_lapidas_registroadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_lapidas_registro_tipo">
    <select
        ...
        data-select2-id="fsco_lapidas_registroadd_x_tipo"
        <?php } ?>
        data-table="sco_lapidas_registro"
        data-field="x_tipo"
...
<script>
loadjs.ready("fsco_lapidas_registroadd", function() {
    var options = { name: "x_tipo", selectId: "fsco_lapidas_registroadd_x_tipo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_lapidas_registroadd.lists.tipo?.lookupOptions.length) {
        options.data = { id: "x_tipo", form: "fsco_lapidas_registroadd" };
    } else {
        options.ajax = { id: "x_tipo", form: "fsco_lapidas_registroadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_sco_lapidas_registro_maqueteo">
    <select
        ...
        data-select2-id="fsco_lapidas_registroadd_x_maqueteo"
        <?php } ?>
        data-table="sco_lapidas_registro"
...
<script>
loadjs.ready("fsco_lapidas_registroadd", function() {
    var options = { name: "x_maqueteo", selectId: "fsco_lapidas_registroadd_x_maqueteo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_lapidas_registroadd.lists.maqueteo?.lookupOptions.length) {
        options.data = { id: "x_maqueteo", form: "fsco_lapidas_registroadd" };
    } else {
        options.ajax = { id: "x_maqueteo", form: "fsco_lapidas_registroadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php
    if (in_array("sco_lapidas_notas", explode(",", $Page->getCurrentDetailTable())) && $sco_lapidas_notas->DetailAdd) {
?>
...
<?php } ?>
<?php
    if (in_array("sco_lapidas_adjunto", explode(",", $Page->getCurrentDetailTable())) && $sco_lapidas_adjunto->DetailAdd) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_lapidas_registroadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_lapidas_registroadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
loadjs.ready("load", function () {
    // Startup script
    // Startup Script de PHPMaker 2024
    // Aplicar máscaras a teléfonos
    // $("#x_telefono1, #x_telefono2").mask("(9999) 999-99-99");

    // Evento Change para la Cédula del Difunto
    // Evento Change para la Cédula del Difunto con Loader dinámico
    $("#x_ci_difunto").on("change", function() {
        let $el = $(this);
        let valor = $el.val().trim().toUpperCase();

        // 1. Insertar el loader solo si no existe ya en el DOM
        ...
        // 2. Limpieza y Validaciones de Formato previo al AJAX
        // Quitar espacios, puntos, comas y guiones para validar el contenido puramente
        let limpia = valor.replace(/[\s\.,-]/g, "");
        ...
        // 3. Formatear visualmente (Letra-Números) y preparar para envío
        let ciFormateada = primeraLetra + "-" + numeros;
        $el.val(ciFormateada);

        // 4. Iniciar proceso de búsqueda (AJAX)
        ...
        $.getJSON("dashboard/buscar_parcela_ubicacion_lapida.php", { ci_difunto: ciFormateada }, function(data) {
            ...
            // Lógica de Alerta de Diseño Existente o Llenado de campos
            if (data.alerta && data.alerta !== "") {
                ew.confirm(data.alerta, function(result) {
                    if (result) {
        ...
        .fail(function() {
            console.error("Error al consultar la ubicación.");
            ew.alert("Hubo un error al conectar con el servidor.");
        })
        .always(function() {
            // Ocultar loader y habilitar campo siempre al finalizar
            $loader.addClass("d-none");
    ...
    // Evento para Maqueteo (Visualización de diseños)
