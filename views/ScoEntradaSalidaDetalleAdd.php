...
// Page object
$ScoEntradaSalidaDetalleAdd = &$Page;
?>
<script>
...
var currentForm;
var fsco_entrada_salida_detalleadd;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fsco_entrada_salida_detalleadd")
        .setPageId("add")
...
$Page->showMessage();
?>
<form name="fsco_entrada_salida_detalleadd" id="fsco_entrada_salida_detalleadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_sco_entrada_salida_detalle_articulo">
    <select
        ...
        data-select2-id="fsco_entrada_salida_detalleadd_x_articulo"
...
<script>
loadjs.ready("fsco_entrada_salida_detalleadd", function() {
    var options = { name: "x_articulo", selectId: "fsco_entrada_salida_detalleadd_x_articulo" };
    if (fsco_entrada_salida_detalleadd.lists.articulo?.lookupOptions.length) {
        options.data = { id: "x_articulo", form: "fsco_entrada_salida_detalleadd" };
    } else {
        options.ajax = { id: "x_articulo", form: "fsco_entrada_salida_detalleadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <?php if (strval($Page->entrada_salida->getSessionValue()) != "") { ?>
    <input type="hidden" name="x_entrada_salida" id="x_entrada_salida" value="<?= HtmlEncode(strval($Page->entrada_salida->getSessionValue())) ?>">
    <?php } ?>
    <?php if (strval($Page->tipo_doc->getSessionValue()) != "") { ?>
    <input type="hidden" name="x_tipo_doc" id="x_tipo_doc" value="<?= HtmlEncode(strval($Page->tipo_doc->getSessionValue())) ?>">
    <?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_entrada_salida_detalleadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_entrada_salida_detalleadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
