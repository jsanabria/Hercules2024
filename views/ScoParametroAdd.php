...
$Page->showMessage();
?>
<form name="fsco_parametroadd" id="fsco_parametroadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_sco_parametro_codigo">
    <select
        ...
        data-select2-id="fsco_parametroadd_x_codigo"
        <?php } ?>
...
<?php if (!$Page->codigo->IsNativeSelect) { ?>
<script>
loadjs.ready("fsco_parametroadd", function() {
    var options = { name: "x_codigo", selectId: "fsco_parametroadd_x_codigo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_parametroadd.lists.codigo?.lookupOptions.length) {
        options.data = { id: "x_codigo", form: "fsco_parametroadd" };
    } else {
        options.ajax = { id: "x_codigo", form: "fsco_parametroadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_parametro_descripcion">
    <select
        ...
        data-select2-id="fsco_parametroadd_x_descripcion"
        <?php } ?>
        data-table="sco_parametro"
...
<script>
loadjs.ready("fsco_parametroadd", function() {
    var options = { name: "x_descripcion", selectId: "fsco_parametroadd_x_descripcion" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_parametroadd.lists.descripcion?.lookupOptions.length) {
        options.data = { id: "x_descripcion", form: "fsco_parametroadd" };
    } else {
        options.ajax = { id: "x_descripcion", form: "fsco_parametroadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_parametroadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_parametroadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
