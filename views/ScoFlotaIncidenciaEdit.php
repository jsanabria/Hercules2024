...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fsco_flota_incidenciaedit" id="fsco_flota_incidenciaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { sco_flota_incidencia: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fsco_flota_incidenciaedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fsco_flota_incidenciaedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["Nflota_incidencia", [fields.Nflota_incidencia.visible && fields.Nflota_incidencia.required ? ew.Validators.required(fields.Nflota_incidencia.caption) : null, ew.Validators.integer], fields.Nflota_incidencia.isInvalid],
...
<?php if ($Page->Nflota_incidencia->Visible) { // Nflota_incidencia ?>
    <div id="r_Nflota_incidencia"<?= $Page->Nflota_incidencia->rowAttributes() ?>>
        <label id="elh_sco_flota_incidencia_Nflota_incidencia" for="x_Nflota_incidencia" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Nflota_incidencia->caption() ?><?= $Page->Nflota_incidencia->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->Nflota_incidencia->cellAttributes() ?>>
<span id="el_sco_flota_incidencia_Nflota_incidencia">
<span<?= $Page->Nflota_incidencia->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Nflota_incidencia->getDisplayValue($Page->Nflota_incidencia->EditValue))) ?>"></span>
<input type="hidden" data-table="sco_flota_incidencia" data-field="x_Nflota_incidencia" data-hidden="1" name="x_Nflota_incidencia" id="x_Nflota_incidencia" value="<?= HtmlEncode($Page->Nflota_incidencia->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="sco_flota_incidencia" data-field="x_fecha_registro" data-hidden="1" name="x_fecha_registro" id="x_fecha_registro" value="<?= HtmlEncode($Page->fecha_registro->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_sco_flota_incidencia_flota">
    <select
        ...
        data-select2-id="fsco_flota_incidenciaedit_x_flota"
...
<script>
loadjs.ready("fsco_flota_incidenciaedit", function() {
    var options = { name: "x_flota", selectId: "fsco_flota_incidenciaedit_x_flota" };
    if (fsco_flota_incidenciaedit.lists.flota?.lookupOptions.length) {
        options.data = { id: "x_flota", form: "fsco_flota_incidenciaedit" };
    } else {
        options.ajax = { id: "x_flota", form: "fsco_flota_incidenciaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_flota_incidencia_tipo">
    <select
        ...
        data-select2-id="fsco_flota_incidenciaedit_x_tipo"
        <?php } ?>
        data-table="sco_flota_incidencia"
        data-field="x_tipo"
...
<script>
loadjs.ready("fsco_flota_incidenciaedit", function() {
    var options = { name: "x_tipo", selectId: "fsco_flota_incidenciaedit_x_tipo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_flota_incidenciaedit.lists.tipo?.lookupOptions.length) {
        options.data = { id: "x_tipo", form: "fsco_flota_incidenciaedit" };
    } else {
        options.ajax = { id: "x_tipo", form: "fsco_flota_incidenciaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_sco_flota_incidencia_falla">
    <select
        ...
        data-select2-id="fsco_flota_incidenciaedit_x_falla"
        <?php } ?>
        data-table="sco_flota_incidencia"
        data-field="x_falla"
...
<script>
loadjs.ready("fsco_flota_incidenciaedit", function() {
    var options = { name: "x_falla", selectId: "fsco_flota_incidenciaedit_x_falla" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_flota_incidenciaedit.lists.falla?.lookupOptions.length) {
        options.data = { id: "x_falla", form: "fsco_flota_incidenciaedit" };
    } else {
        options.ajax = { id: "x_falla", form: "fsco_flota_incidenciaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_sco_flota_incidencia_reparacion">
    <select
        ...
        data-select2-id="fsco_flota_incidenciaedit_x_reparacion"
        <?php } ?>
        data-table="sco_flota_incidencia"
...
<script>
loadjs.ready("fsco_flota_incidenciaedit", function() {
    var options = { name: "x_reparacion", selectId: "fsco_flota_incidenciaedit_x_reparacion" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_flota_incidenciaedit.lists.reparacion?.lookupOptions.length) {
        options.data = { id: "x_reparacion", form: "fsco_flota_incidenciaedit" };
    } else {
        options.ajax = { id: "x_reparacion", form: "fsco_flota_incidenciaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_sco_flota_incidencia_cambio_aceite">
    <select
        ...
        data-select2-id="fsco_flota_incidenciaedit_x_cambio_aceite"
        <?php } ?>
        data-table="sco_flota_incidencia"
...
<script>
loadjs.ready("fsco_flota_incidenciaedit", function() {
    var options = { name: "x_cambio_aceite", selectId: "fsco_flota_incidenciaedit_x_cambio_aceite" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_flota_incidenciaedit.lists.cambio_aceite?.lookupOptions.length) {
        options.data = { id: "x_cambio_aceite", form: "fsco_flota_incidenciaedit" };
    } else {
        options.ajax = { id: "x_cambio_aceite", form: "fsco_flota_incidenciaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_sco_flota_incidencia_proveedor">
    <select
        ...
        data-select2-id="fsco_flota_incidenciaedit_x_proveedor"
        <?php } ?>
        data-table="sco_flota_incidencia"
...
<script>
loadjs.ready("fsco_flota_incidenciaedit", function() {
    var options = { name: "x_proveedor", selectId: "fsco_flota_incidenciaedit_x_proveedor" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsco_flota_incidenciaedit.lists.proveedor?.lookupOptions.length) {
        options.data = { id: "x_proveedor", form: "fsco_flota_incidenciaedit" };
    } else {
        options.ajax = { id: "x_proveedor", form: "fsco_flota_incidenciaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["fsco_flota_incidenciaedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fsco_flota_incidenciaedit", "x_fecha_reparacion", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php
    if (in_array("sco_flota_incidencia_detalle", explode(",", $Page->getCurrentDetailTable())) && $sco_flota_incidencia_detalle->DetailEdit) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsco_flota_incidenciaedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsco_flota_incidenciaedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
