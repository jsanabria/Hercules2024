...
<head>
    <meta charset="UTF-8">
    <title>Disponibilidad de Cremaciones</title>
...
<body>
    <div id="tt" class="easyui-tabs" style="width:100%;height:430px">
        <div title="Servicios de Cremaci&oacute;n del d&iacute;a: <?php echo $_fecha; ?>" style="padding:10px" data-options="iconCls:'icon-ok'">
            <div style="width:100%;height:100%;">
                <div class="right">
                    <table>
                        <tr>
                            <td class="title"><div style="width:50px"><b>Hora</b></div></td>
                            <td class="title"><div style="width:200px"><b>Nombre Difunto</b></div></td>
                            <td class="title"><div style="width:200px"><b>Apellido Difunto</b></div></td>
                            <td class="title"><div style="width:200px"><b>Capilla / Funeraria</b></div></td>
                            <td class="title"><div style="width:100px"><b>Estatus</b></div></td>
                        </tr>
                        <?php
                        $xx_cant = 0;
                        $sql_param = "SELECT valor2 AS cantidad FROM sco_parametro WHERE codigo = '019';";
                        ...
                        $sql_cremaciones = "SELECT
                                            ...
                                            (SELECT COUNT(*) FROM sco_nota WHERE expediente = exp.Nexpediente) AS notas, DATE_FORMAT(ord.hora_fin, '%h:%i:%p') AS hora_servicio
                                        FROM
                                            sco_orden AS ord
                        ...
                        if ($stmt = $link->prepare($sql_cremaciones)) {
                            ...
                            while ($row = $result->fetch_assoc()) {
                                ...
                                // Query for cremacion status within the loop - consider optimizing if possible
                                $my_est = "";
                                $sql_estatus = "SELECT estatus FROM sco_cremacion_estatus WHERE expediente = ? ORDER BY fecha_hora DESC LIMIT 1";
                                if ($stmt_estatus = $link->prepare($sql_estatus)) {
                                ...
                                if ($hora != (isset($bloque_horario[$row["pauta"]]) ? $bloque_horario[$row["pauta"]] : null)) {
                                    $blackboard_string .= '<td class="' . $drop . '"><div><b><i>' . (isset($bloque_horario[$row["pauta"]]) ? $bloque_horario[$row["pauta"]] : '') . '</i></b></div></td>';
                                ...
                                $blackboard_string .= '<td class="' . $drop . '"><div><b>' . htmlspecialchars($my_est) . '</b></div></td>';
                                $ids[] = $row["Nexpediente"];
                        ...
                        $count = 1;
                        $blackboard[$hora] = $blackboard_string; // Assign the last buffered string to the last hour block
                        $blackboard_string = ""; // Reset for next use if any
                        ...
                        ?>
                        <tr><td colspan="9"><b>Total Cremaciones Programadas: <?php echo $cremaciones; ?></b></td></tr>
                    </table>
                    <br />
                </div>
            </div>
        ...
        <?php foreach ($ids as $value) { ?>
            $('#state_<?php echo trim(htmlspecialchars($value)); ?>').combobox({
                onSelect: function() {
                    ...
                    if(confirm("Seguro que desea cambiar el estatus de CREMACION del Exp # <?php echo trim(htmlspecialchars($value)); ?>")) {
