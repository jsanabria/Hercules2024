...
$sql = "UPDATE sco_orden_salida SET estatus = 'P2', autoriza = ?, fecha_autoriza = NOW() WHERE Norden_salida = ?";
...
if ($stmt->execute()) {
    if ($stmt->affected_rows > 0) {
        echo '<div class="alert alert-success"><span class="glyphicon glyphicon-ok"></span> <strong>Procesado Autorizado</strong></div>';
    } else {
        echo '<div class="alert alert-warning"><span class="glyphicon glyphicon-info-sign"></span> <strong>Procesado. No se encontraron órdenes para autorizar.</strong></div>';
    }
} else {
