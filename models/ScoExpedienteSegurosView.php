 ...
 * Page class
 */
class ScoExpedienteSegurosView extends ScoExpedienteSeguros
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoExpedienteSegurosView";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoExpedienteSegurosView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("Nexpediente_seguros") ?? Route("Nexpediente_seguros")) !== null) {
            $this->RecKey["Nexpediente_seguros"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("Nexpediente_seguros") ?? Route("Nexpediente_seguros")) !== null) {
            ...
            $this->RecKey["Nexpediente_seguros"] = $this->Nexpediente_seguros->QueryStringValue;
        } elseif (Post("Nexpediente_seguros") !== null) {
            $this->Nexpediente_seguros->setFormValue(Post("Nexpediente_seguros"));
            $this->RecKey["Nexpediente_seguros"] = $this->Nexpediente_seguros->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->Nexpediente_seguros->setQueryStringValue($keyValue);
            $this->RecKey["Nexpediente_seguros"] = $this->Nexpediente_seguros->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "ScoExpedienteSegurosList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "ScoExpedienteSegurosList"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_sco_expediente_seguros_adjunto");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("sco_expediente_seguros_adjunto", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("ScoExpedienteSegurosAdjuntoList?" . Config("TABLE_SHOW_MASTER") . "=sco_expediente_seguros&" . GetForeignKeyUrl("fk_Nexpediente_seguros", $this->Nexpediente_seguros->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_expediente_seguros')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_expediente_seguros_adjunto"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_expediente_seguros')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_expediente_seguros_adjunto"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
