 ...
 * Page class
 */
class ViewOrdenProvList extends ViewOrdenProv
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewOrdenProvList";
    ...
    public $FormName = "fview_orden_provlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ViewOrdenProvList";
    ...
    public function setVisibility()
    {
        ...
        $this->horas_velacion->setVisibility();
        $this->llevar_a->setVisibility();
        $this->ajunto->setVisibility();
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'view_orden_prov';
        $this->TableName = 'view_orden_prov';
        ...
        // Table object (view_orden_prov)
        if (!isset($GLOBALS["view_orden_prov"]) || $GLOBALS["view_orden_prov"]::class == PROJECT_NAMESPACE . "view_orden_prov") {
            $GLOBALS["view_orden_prov"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ViewOrdenProvAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ViewOrdenProvDelete";
        $this->MultiUpdateUrl = "ViewOrdenProvUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'view_orden_prov');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ViewOrdenProvView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        $this->setupLookupOptions($this->servicio_atendido);

        // Update form name to avoid conflict
        if ($this->IsModal) {
            $this->FormName = "fview_orden_provgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->horas_velacion->AdvancedSearch->toJson(), ","); // Field horas_velacion
        $filterList = Concat($filterList, $this->llevar_a->AdvancedSearch->toJson(), ","); // Field llevar_a
        $filterList = Concat($filterList, $this->ajunto->AdvancedSearch->toJson(), ","); // Field ajunto
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_orden_provsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field horas_velacion
        $this->horas_velacion->AdvancedSearch->SearchValue = @$filter["x_horas_velacion"];
        $this->horas_velacion->AdvancedSearch->SearchOperator = @$filter["z_horas_velacion"];
        $this->horas_velacion->AdvancedSearch->SearchCondition = @$filter["v_horas_velacion"];
        $this->horas_velacion->AdvancedSearch->SearchValue2 = @$filter["y_horas_velacion"];
        $this->horas_velacion->AdvancedSearch->SearchOperator2 = @$filter["w_horas_velacion"];
        $this->horas_velacion->AdvancedSearch->save();
        ...
        // Field ajunto
        $this->ajunto->AdvancedSearch->SearchValue = @$filter["x_ajunto"];
        $this->ajunto->AdvancedSearch->SearchOperator = @$filter["z_ajunto"];
        $this->ajunto->AdvancedSearch->SearchCondition = @$filter["v_ajunto"];
        $this->ajunto->AdvancedSearch->SearchValue2 = @$filter["y_ajunto"];
        $this->ajunto->AdvancedSearch->SearchOperator2 = @$filter["w_ajunto"];
        $this->ajunto->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->horas_velacion, $default, false); // horas_velacion
        $this->buildSearchSql($where, $this->llevar_a, $default, false); // llevar_a
        $this->buildSearchSql($where, $this->ajunto, $default, false); // ajunto
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->horas_velacion->AdvancedSearch->save(); // horas_velacion
            $this->llevar_a->AdvancedSearch->save(); // llevar_a
            $this->ajunto->AdvancedSearch->save(); // ajunto
    ...
    public function showFilterList()
    {
        ...
        // Field horas_velacion
        $filter = $this->queryBuilderWhere("horas_velacion");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->horas_velacion, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->horas_velacion->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field llevar_a
        $filter = $this->queryBuilderWhere("llevar_a");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->llevar_a, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->llevar_a->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field ajunto
        $filter = $this->queryBuilderWhere("ajunto");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->ajunto, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->ajunto->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->horas_velacion;
        $searchFlds[] = &$this->llevar_a;
        $searchFlds[] = &$this->ajunto;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->horas_velacion->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->llevar_a->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->ajunto->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->horas_velacion->AdvancedSearch->unsetSession();
        $this->llevar_a->AdvancedSearch->unsetSession();
        $this->ajunto->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->horas_velacion->AdvancedSearch->load();
        $this->llevar_a->AdvancedSearch->load();
        $this->ajunto->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->expediente->Expression . " DESC" . ", " . $this->paso->Expression . " ASC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->horas_velacion); // horas_velacion
            $this->updateSort($this->llevar_a); // llevar_a
            $this->updateSort($this->ajunto); // ajunto
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_orden_provlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_orden_provlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "horas_velacion");
            $this->createColumnOption($option, "llevar_a");
            $this->createColumnOption($option, "ajunto");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_orden_provsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_orden_provsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_orden_provlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_orden_prov", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_orden_prov",
    ...
    protected function loadSearchValues()
    {
        ...
        // horas_velacion
        if ($this->horas_velacion->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->horas_velacion->AdvancedSearch->SearchValue != "" || $this->horas_velacion->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // ajunto
        if ($this->ajunto->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ajunto->AdvancedSearch->SearchValue != "" || $this->ajunto->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function loadRowValues($row = null)
    {
        ...
        $this->horas_velacion->setDbValue($row['horas_velacion']);
        $this->llevar_a->setDbValue($row['llevar_a']);
        $this->ajunto->setDbValue($row['ajunto']);
    ...
    protected function newRow()
    {
        ...
        $row['horas_velacion'] = $this->horas_velacion->DefaultValue;
        $row['llevar_a'] = $this->llevar_a->DefaultValue;
        $row['ajunto'] = $this->ajunto->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // horas_velacion
            $this->horas_velacion->ViewValue = $this->horas_velacion->CurrentValue;

            // llevar_a
            $this->llevar_a->ViewValue = $this->llevar_a->CurrentValue;

            // ajunto
            $this->ajunto->ViewValue = $this->ajunto->CurrentValue;
            ...
            // horas_velacion
            $this->horas_velacion->HrefValue = "";
            $this->horas_velacion->TooltipValue = "";
            ...
            // ajunto
            $this->ajunto->HrefValue = "";
            $this->ajunto->TooltipValue = "";
            ...
            if (!$this->tipo->Raw) {
                $this->tipo->AdvancedSearch->SearchValue = HtmlDecode($this->tipo->AdvancedSearch->SearchValue);
            }
            $this->tipo->EditValue = HtmlEncode($this->tipo->AdvancedSearch->SearchValue);
            ...
            // horas_velacion
            $this->horas_velacion->setupEditAttributes();
            if (!$this->horas_velacion->Raw) {
                $this->horas_velacion->AdvancedSearch->SearchValue = HtmlDecode($this->horas_velacion->AdvancedSearch->SearchValue);
            }
            $this->horas_velacion->EditValue = HtmlEncode($this->horas_velacion->AdvancedSearch->SearchValue);
            $this->horas_velacion->PlaceHolder = RemoveHtml($this->horas_velacion->caption());

            // llevar_a
            $this->llevar_a->setupEditAttributes();
            if (!$this->llevar_a->Raw) {
                $this->llevar_a->AdvancedSearch->SearchValue = HtmlDecode($this->llevar_a->AdvancedSearch->SearchValue);
            }
            $this->llevar_a->EditValue = HtmlEncode($this->llevar_a->AdvancedSearch->SearchValue);
            $this->llevar_a->PlaceHolder = RemoveHtml($this->llevar_a->caption());

            // ajunto
            $this->ajunto->setupEditAttributes();
            $this->ajunto->EditValue = HtmlEncode($this->ajunto->AdvancedSearch->SearchValue);
            $this->ajunto->PlaceHolder = RemoveHtml($this->ajunto->caption());
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fview_orden_provlist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_orden_provsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
