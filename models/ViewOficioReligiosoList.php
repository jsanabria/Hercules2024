 ...
 * Page class
 */
class ViewOficioReligiosoList extends ViewOficioReligioso
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewOficioReligiosoList";
    ...
    public $FormName = "fview_oficio_religiosolist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ViewOficioReligiosoList";
    ...
    public function __construct()
    {
        ...
        // Table object (view_oficio_religioso)
        if (!isset($GLOBALS["view_oficio_religioso"]) || $GLOBALS["view_oficio_religioso"]::class == PROJECT_NAMESPACE . "view_oficio_religioso") {
            $GLOBALS["view_oficio_religioso"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ViewOficioReligiosoAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ViewOficioReligiosoDelete";
        $this->MultiUpdateUrl = "ViewOficioReligiosoUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'view_oficio_religioso');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ViewOficioReligiosoView"); // If View page, no primary button
                } else { // List page
    ...
    protected function getRecordKeyValue($ar)
    {
        $key = "";
        if (is_array($ar)) {
            $key .= @$ar['Norden'] . Config("COMPOSITE_KEY_SEPARATOR");
            $key .= @$ar['Nexpediente'];
        }
        return $key;
    }
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_oficio_religiosogrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->servicio2->AdvancedSearch->toJson(), ","); // Field servicio2
        $filterList = Concat($filterList, $this->ministro->AdvancedSearch->toJson(), ","); // Field ministro
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_oficio_religiososrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field servicio2
        $this->servicio2->AdvancedSearch->SearchValue = @$filter["x_servicio2"];
        $this->servicio2->AdvancedSearch->SearchOperator = @$filter["z_servicio2"];
        $this->servicio2->AdvancedSearch->SearchCondition = @$filter["v_servicio2"];
        $this->servicio2->AdvancedSearch->SearchValue2 = @$filter["y_servicio2"];
        $this->servicio2->AdvancedSearch->SearchOperator2 = @$filter["w_servicio2"];
        $this->servicio2->AdvancedSearch->save();

        // Field ministro
        $this->ministro->AdvancedSearch->SearchValue = @$filter["x_ministro"];
        $this->ministro->AdvancedSearch->SearchOperator = @$filter["z_ministro"];
        $this->ministro->AdvancedSearch->SearchCondition = @$filter["v_ministro"];
        $this->ministro->AdvancedSearch->SearchValue2 = @$filter["y_ministro"];
        $this->ministro->AdvancedSearch->SearchOperator2 = @$filter["w_ministro"];
        $this->ministro->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->servicio2, $default, false); // servicio2
        $this->buildSearchSql($where, $this->ministro, $default, false); // ministro
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->servicio2->AdvancedSearch->save(); // servicio2
            $this->ministro->AdvancedSearch->save(); // ministro
    ...
    public function showFilterList()
    {
        ...
        // Field servicio2
        $filter = $this->queryBuilderWhere("servicio2");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->servicio2, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->servicio2->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field ministro
        $filter = $this->queryBuilderWhere("ministro");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->ministro, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->ministro->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->servicio2;
        $searchFlds[] = &$this->ministro;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->servicio2->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->ministro->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->servicio2->AdvancedSearch->unsetSession();
        $this->ministro->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->servicio2->AdvancedSearch->load();
        $this->ministro->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->fecha_servicio->Expression . " DESC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->servicio2); // servicio2
            $this->updateSort($this->ministro); // ministro
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->servicio2->setSort("");
                $this->ministro->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"view_oficio_religioso\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"view_oficio_religioso\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_oficio_religiosolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_oficio_religiosolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Norden->CurrentValue . Config("COMPOSITE_KEY_SEPARATOR") . $this->Nexpediente->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "servicio2");
            $this->createColumnOption($option, "ministro");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_oficio_religiososrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_oficio_religiososrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_oficio_religiosolist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_oficio_religioso", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_oficio_religioso",
    ...
    protected function loadSearchValues()
    {
        ...
        // servicio2
        if ($this->servicio2->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->servicio2->AdvancedSearch->SearchValue != "" || $this->servicio2->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // ministro
        if ($this->ministro->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ministro->AdvancedSearch->SearchValue != "" || $this->ministro->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function newRow()
    {
        ...
        $row['servicio2'] = $this->servicio2->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->servicio->Raw) {
                $this->servicio->AdvancedSearch->SearchValue = HtmlDecode($this->servicio->AdvancedSearch->SearchValue);
            }
            $this->servicio->EditValue = HtmlEncode($this->servicio->AdvancedSearch->SearchValue);
            $this->servicio->PlaceHolder = RemoveHtml($this->servicio->caption());

            // servicio2
            $this->servicio2->setupEditAttributes();
            if (!$this->servicio2->Raw) {
                $this->servicio2->AdvancedSearch->SearchValue = HtmlDecode($this->servicio2->AdvancedSearch->SearchValue);
            }
            $this->servicio2->EditValue = HtmlEncode($this->servicio2->AdvancedSearch->SearchValue);
            ...
            $this->fecha_servicio->setupEditAttributes();
            $this->fecha_servicio->EditValue2 = HtmlEncode(FormatDateTime(UnFormatDateTime($this->fecha_servicio->AdvancedSearch->SearchValue2, $this->fecha_servicio->formatPattern()), $this->fecha_servicio->formatPattern()));
            $this->fecha_servicio->PlaceHolder = RemoveHtml($this->fecha_servicio->caption());
        }
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckInteger($this->cedula->AdvancedSearch->SearchValue)) {
            $this->cedula->addErrorMessage($this->cedula->getErrorMessage(false));
        }
        if (!CheckDate($this->fecha_servicio->AdvancedSearch->SearchValue, $this->fecha_servicio->formatPattern())) {
            $this->fecha_servicio->addErrorMessage($this->fecha_servicio->getErrorMessage(false));
        }
        if (!CheckDate($this->fecha_servicio->AdvancedSearch->SearchValue2, $this->fecha_servicio->formatPattern())) {
            $this->fecha_servicio->addErrorMessage($this->fecha_servicio->getErrorMessage(false));
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" form=\"fview_oficio_religiosolist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToExcel") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcel", true)) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" form=\"fview_oficio_religiosolist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToWord") . "</button>";
            } else {
                return "<a href=\"$exportUrl\" class=\"btn btn-default ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWord", true)) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPdf", true)) . "\" form=\"fview_oficio_religiosolist\" data-url=\"$exportUrl\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" . $Language->phrase("ExportToPdf") . "</button>";
            } else {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fview_oficio_religiosolist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_oficio_religiososrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    {
        // Example:
        $opt = &$this->ListOptions->add("estatus");
        $opt->Header = "Estatus";
        $opt->OnLeft = false; 
        $opt->moveTo(9); 
    }
    ...
    {
        // Example:
    	$id = intval($this->Nexpediente->CurrentValue);
    	$sql = "SELECT 
    				(SELECT nombre FROM sco_estatus WHERE Nstatus = a.estatus) AS estatus 
    			FROM 
    				sco_expediente AS a 
    			WHERE 
    				Nexpediente = $id;";
    	if($row = ExecuteRow($sql)) $this->ListOptions["estatus"]->Body = $row["estatus"];
        else $this->ListOptions["estatus"]->Body = '';
    }
