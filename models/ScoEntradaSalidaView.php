 ...
 * Page class
 */
class ScoEntradaSalidaView extends ScoEntradaSalida
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoEntradaSalidaView";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoEntradaSalidaView";
    ...
    {
        $this->Nentrada_salida->setVisibility();
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("Nentrada_salida") ?? Route("Nentrada_salida")) !== null) {
            $this->RecKey["Nentrada_salida"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("Nentrada_salida") ?? Route("Nentrada_salida")) !== null) {
            ...
            $this->RecKey["Nentrada_salida"] = $this->Nentrada_salida->QueryStringValue;
        } elseif (Post("Nentrada_salida") !== null) {
            $this->Nentrada_salida->setFormValue(Post("Nentrada_salida"));
            $this->RecKey["Nentrada_salida"] = $this->Nentrada_salida->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->Nentrada_salida->setQueryStringValue($keyValue);
            $this->RecKey["Nentrada_salida"] = $this->Nentrada_salida->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "ScoEntradaSalidaList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ScoEntradaSalidaList"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->Nentrada_salida->CurrentValue, $this->CurrentRow['Nentrada_salida'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "ScoEntradaSalidaList"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_sco_entrada_salida_detalle");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("sco_entrada_salida_detalle", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("ScoEntradaSalidaDetalleList?" . Config("TABLE_SHOW_MASTER") . "=sco_entrada_salida&" . GetForeignKeyUrl("fk_Nentrada_salida", $this->Nentrada_salida->CurrentValue) . "&" . GetForeignKeyUrl("fk_tipo_doc", $this->tipo_doc->CurrentValue) . "&" . GetForeignKeyUrl("fk_proveedor", $this->proveedor->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_entrada_salida')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_entrada_salida_detalle"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_entrada_salida')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_entrada_salida_detalle"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
