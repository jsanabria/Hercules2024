...
/**
 * Table class for sco_mensaje_cliente
 */
class ScoMensajeCliente extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "sco_mensaje_cliente";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // Nmensaje_cliente
        $this->Nmensaje_cliente = new DbField(
            $this, // Table
            'x_Nmensaje_cliente', // Variable name
            'Nmensaje_cliente', // Name
            '`Nmensaje_cliente`', // Expression
            '`Nmensaje_cliente`', // Basic search expression
            20, // Type
            20, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Nmensaje_cliente`', // Virtual expression
            false, // Is virtual
        ...
        $this->Nmensaje_cliente->InputTextType = "text";
        $this->Nmensaje_cliente->Raw = true;
        $this->Nmensaje_cliente->IsAutoIncrement = true; // Autoincrement field
        $this->Nmensaje_cliente->IsPrimaryKey = true; // Primary key field
        $this->Nmensaje_cliente->Nullable = false; // NOT NULL field
        $this->Nmensaje_cliente->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Nmensaje_cliente->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['Nmensaje_cliente'] = &$this->Nmensaje_cliente;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "sco_mensaje_cliente";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->Nmensaje_cliente->setDbValue($conn->lastInsertId());
            $rs['Nmensaje_cliente'] = $this->Nmensaje_cliente->DbValue;
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['Nmensaje_cliente']) && !EmptyValue($this->Nmensaje_cliente->CurrentValue)) {
                $rs['Nmensaje_cliente'] = $this->Nmensaje_cliente->CurrentValue;
            }
        }
        if ($success && $this->AuditTrailOnEdit && $rsold) {
            $rsaudit = $rs;
            $fldname = 'Nmensaje_cliente';
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('Nmensaje_cliente', $rs)) {
                AddFilter($where, QuotedName('Nmensaje_cliente', $this->Dbid) . '=' . QuotedValue($rs['Nmensaje_cliente'], $this->Nmensaje_cliente->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->Nmensaje_cliente->DbValue = $row['Nmensaje_cliente'];
    ...
    {
        return "`Nmensaje_cliente` = @Nmensaje_cliente@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->Nmensaje_cliente->CurrentValue : $this->Nmensaje_cliente->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->Nmensaje_cliente->CurrentValue = $keys[0];
            } else {
                $this->Nmensaje_cliente->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('Nmensaje_cliente', $row) ? $row['Nmensaje_cliente'] : null;
        } else {
            $val = !EmptyValue($this->Nmensaje_cliente->OldValue) && !$current ? $this->Nmensaje_cliente->OldValue : $this->Nmensaje_cliente->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@Nmensaje_cliente@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ScoMensajeClienteList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ScoMensajeClienteView" => $Language->phrase("View"),
            "ScoMensajeClienteEdit" => $Language->phrase("Edit"),
            "ScoMensajeClienteAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ScoMensajeClienteView",
            Config("API_ADD_ACTION") => "ScoMensajeClienteAdd",
            Config("API_EDIT_ACTION") => "ScoMensajeClienteEdit",
            Config("API_DELETE_ACTION") => "ScoMensajeClienteDelete",
            Config("API_LIST_ACTION") => "ScoMensajeClienteList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ScoMensajeClienteView", $parm);
        } else {
            $url = $this->keyUrl("ScoMensajeClienteView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ScoMensajeClienteAdd?" . $parm;
        } else {
            $url = "ScoMensajeClienteAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoMensajeClienteEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoMensajeClienteList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoMensajeClienteAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoMensajeClienteList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ScoMensajeClienteDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"Nmensaje_cliente\":" . VarToJson($this->Nmensaje_cliente->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->Nmensaje_cliente->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->Nmensaje_cliente->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("Nmensaje_cliente") ?? Route("Nmensaje_cliente")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->Nmensaje_cliente->CurrentValue = $key;
            } else {
                $this->Nmensaje_cliente->OldValue = $key;
            }
    ...
    {
        global $Response;
        $listPage = "ScoMensajeClienteList";
    ...
    public function renderEditRow()
    {
        ...
        // Nmensaje_cliente
        $this->Nmensaje_cliente->setupEditAttributes();
        $this->Nmensaje_cliente->EditValue = $this->Nmensaje_cliente->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->Nmensaje_cliente);
        ...
        while (($row = $result->fetch()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->Nmensaje_cliente);
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'sco_mensaje_cliente');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'sco_mensaje_cliente', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'sco_mensaje_cliente', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'sco_mensaje_cliente', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowRendering() {
    	...
    	if ($this->PageID == "list" || $this->PageID == "view") { // List/View page only
    		...
    		$this->contacto->CellAttrs["style"] = $style;
    		$this->difunto->CellAttrs["style"] = $style;
    	}
    }
