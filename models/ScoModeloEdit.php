 ...
 * Page class
 */
class ScoModeloEdit extends ScoModelo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoModeloEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoModeloEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Nmodelo") ?? Key(0) ?? Route(2)) !== null) {
                $this->Nmodelo->setQueryStringValue($keyValue);
                $this->Nmodelo->setOldValue($this->Nmodelo->QueryStringValue);
            } elseif (Post("Nmodelo") !== null) {
                $this->Nmodelo->setFormValue(Post("Nmodelo"));
                $this->Nmodelo->setOldValue($this->Nmodelo->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("Nmodelo") ?? Route("Nmodelo")) !== null) {
                    $this->Nmodelo->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->Nmodelo->CurrentValue = null;
                }
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ScoModeloList"); // Return to list page
                        return;
                        ...
                        if ($this->Nmodelo->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->Nmodelo->CurrentValue, $this->CurrentRow['Nmodelo'])) {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ScoModeloList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "ScoModeloList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ScoModeloList") {
                            ...
                            $returnUrl = "ScoModeloList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Nmodelo' first before field var 'x_Nmodelo'
        $val = $CurrentForm->hasValue("Nmodelo") ? $CurrentForm->getValue("Nmodelo") : $CurrentForm->getValue("x_Nmodelo");
        if (!$this->Nmodelo->IsDetailKey) {
            $this->Nmodelo->setFormValue($val);
        }
    }
    ...
    {
        global $CurrentForm;
        $this->Nmodelo->CurrentValue = $this->Nmodelo->FormValue;
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ScoModeloList"), "", $this->TableVar, true);
        $pageId = "edit";
