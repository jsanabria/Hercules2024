 ...
 * Page class
 */
class AutogestionTicketEdit extends AutogestionTicket
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AutogestionTicketEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "AutogestionTicketEdit";
    ...
    public function setVisibility()
    {
        ...
        $this->fecha_contactado->setVisibility();
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Nticket") ?? Key(0) ?? Route(2)) !== null) {
                $this->Nticket->setQueryStringValue($keyValue);
                $this->Nticket->setOldValue($this->Nticket->QueryStringValue);
            } elseif (Post("Nticket") !== null) {
                $this->Nticket->setFormValue(Post("Nticket"));
                $this->Nticket->setOldValue($this->Nticket->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("Nticket") ?? Route("Nticket")) !== null) {
                    $this->Nticket->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->Nticket->CurrentValue = null;
                }
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("AutogestionTicketList"); // Return to list page
                        return;
                        ...
                        if ($this->Nticket->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->Nticket->CurrentValue, $this->CurrentRow['Nticket'])) {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("AutogestionTicketList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "AutogestionTicketList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "AutogestionTicketList") {
                            ...
                            $returnUrl = "AutogestionTicketList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Nticket' first before field var 'x_Nticket'
        $val = $CurrentForm->hasValue("Nticket") ? $CurrentForm->getValue("Nticket") : $CurrentForm->getValue("x_Nticket");
        if (!$this->Nticket->IsDetailKey) {
            $this->Nticket->setFormValue($val);
        }
        ...
        // Check field name 'apellido_fallecido' first before field var 'x_apellido_fallecido'
        $val = $CurrentForm->hasValue("apellido_fallecido") ? $CurrentForm->getValue("apellido_fallecido") : $CurrentForm->getValue("x_apellido_fallecido");
        if (!$this->apellido_fallecido->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->apellido_fallecido->Visible = false; // Disable update for API request
            } else {
                $this->apellido_fallecido->setFormValue($val);
            }
        }

        // Check field name 'contactado' first before field var 'x_contactado'
        $val = $CurrentForm->hasValue("contactado") ? $CurrentForm->getValue("contactado") : $CurrentForm->getValue("x_contactado");
        if (!$this->contactado->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->contactado->Visible = false; // Disable update for API request
            } else {
                $this->contactado->setFormValue($val);
            }
        }

        // Check field name 'fecha_contactado' first before field var 'x_fecha_contactado'
        $val = $CurrentForm->hasValue("fecha_contactado") ? $CurrentForm->getValue("fecha_contactado") : $CurrentForm->getValue("x_fecha_contactado");
        if (!$this->fecha_contactado->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->fecha_contactado->Visible = false; // Disable update for API request
            } else {
                $this->fecha_contactado->setFormValue($val);
            }
            $this->fecha_contactado->CurrentValue = UnFormatDateTime($this->fecha_contactado->CurrentValue, $this->fecha_contactado->formatPattern());
        }
    ...
    {
        global $CurrentForm;
        $this->Nticket->CurrentValue = $this->Nticket->FormValue;
        ...
        $this->apellido_fallecido->CurrentValue = $this->apellido_fallecido->FormValue;
        $this->contactado->CurrentValue = $this->contactado->FormValue;
        $this->fecha_contactado->CurrentValue = $this->fecha_contactado->FormValue;
        $this->fecha_contactado->CurrentValue = UnFormatDateTime($this->fecha_contactado->CurrentValue, $this->fecha_contactado->formatPattern());
    ...
    public function renderRow()
    {
        ...
        // apellido_fallecido
        $this->apellido_fallecido->RowCssClass = "row";

        // ubicacion
        $this->ubicacion->RowCssClass = "row";

        // email
        $this->_email->RowCssClass = "row";

        // recaudos
        $this->recaudos->RowCssClass = "row";

        // recaudos2
        $this->recaudos2->RowCssClass = "row";

        // hora_conctactar
        $this->hora_conctactar->RowCssClass = "row";

        // contactado
        $this->contactado->RowCssClass = "row";

        // fecha_contactado
        $this->fecha_contactado->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->Nticket->Visible && $this->Nticket->Required) {
                if (!$this->Nticket->IsDetailKey && EmptyValue($this->Nticket->FormValue)) {
                    $this->Nticket->addErrorMessage(str_replace("%s", $this->Nticket->caption(), $this->Nticket->RequiredErrorMessage));
                }
            }
            ...
            if ($this->apellido_fallecido->Visible && $this->apellido_fallecido->Required) {
                if (!$this->apellido_fallecido->IsDetailKey && EmptyValue($this->apellido_fallecido->FormValue)) {
                    $this->apellido_fallecido->addErrorMessage(str_replace("%s", $this->apellido_fallecido->caption(), $this->apellido_fallecido->RequiredErrorMessage));
                }
            }
            if ($this->contactado->Visible && $this->contactado->Required) {
                if (!$this->contactado->IsDetailKey && EmptyValue($this->contactado->FormValue)) {
                    $this->contactado->addErrorMessage(str_replace("%s", $this->contactado->caption(), $this->contactado->RequiredErrorMessage));
                }
            }
            if ($this->fecha_contactado->Visible && $this->fecha_contactado->Required) {
                if (!$this->fecha_contactado->IsDetailKey && EmptyValue($this->fecha_contactado->FormValue)) {
                    $this->fecha_contactado->addErrorMessage(str_replace("%s", $this->fecha_contactado->caption(), $this->fecha_contactado->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // contactado
        $this->contactado->setDbValueDef($rsnew, $this->contactado->CurrentValue, $this->contactado->ReadOnly);
    ...
    {
        if (isset($row['contactado'])) { // contactado
            $this->contactado->CurrentValue = $row['contactado'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("AutogestionTicketList"), "", $this->TableVar, true);
        $pageId = "edit";
