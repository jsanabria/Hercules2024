 ...
 * Page class
 */
class ScoGramaNotaAdd extends ScoGramaNota
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoGramaNotaAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoGramaNotaAdd";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("Nnota") ?? Route("Nnota")) !== null) {
                $this->Nnota->setQueryStringValue($keyValue);
            }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("ScoGramaNotaList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ScoGramaNotaList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ScoGramaNotaView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions && !$this->getCurrentMasterTable()) {
                        ...
                        if (GetPageName($returnUrl) != "ScoGramaNotaList") {
                            ...
                            $returnUrl = "ScoGramaNotaList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'grama' first before field var 'x_grama'
        $val = $CurrentForm->hasValue("grama") ? $CurrentForm->getValue("grama") : $CurrentForm->getValue("x_grama");
        if (!$this->grama->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->grama->Visible = false; // Disable update for API request
            } else {
                $this->grama->setFormValue($val, true, $validate);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->grama->CurrentValue = $this->grama->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->grama->Visible && $this->grama->Required) {
                if (!$this->grama->IsDetailKey && EmptyValue($this->grama->FormValue)) {
                    $this->grama->addErrorMessage(str_replace("%s", $this->grama->caption(), $this->grama->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->grama->FormValue)) {
                $this->grama->addErrorMessage($this->grama->getErrorMessage(false));
            }
    ...
    protected function addRow($rsold = null)
    {
        ...
        $detailKeys = [];
        $detailKeys["grama"] = $this->grama->CurrentValue;
    ...
    protected function getAddRow()
    {
        ...
        // grama
        $this->grama->setDbValueDef($rsnew, $this->grama->CurrentValue, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ScoGramaNotaList"), "", $this->TableVar, true);
