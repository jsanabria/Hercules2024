 ...
 * Page class
 */
class ScoOrdenGrid extends ScoOrden
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoOrdenGrid";
    ...
    public $FormName = "fsco_ordengrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoOrdenGrid";
    ...
    public function emptyRow()
    {
        ...
        if (
            $CurrentForm->hasValue("x_user_registra") &&
            $CurrentForm->hasValue("o_user_registra") &&
            $this->user_registra->CurrentValue != $this->user_registra->DefaultValue &&
            !($this->user_registra->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->user_registra->CurrentValue == $this->user_registra->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_ministro") &&
            $CurrentForm->hasValue("o_ministro") &&
            $this->ministro->CurrentValue != $this->ministro->DefaultValue &&
            !($this->ministro->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->ministro->CurrentValue == $this->ministro->getSessionValue())
        ) {
            return false;
        }
        return true;
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_user_registra")) {
            $this->user_registra->setOldValue($CurrentForm->getValue("o_user_registra"));
        }

        // Check field name 'ministro' first before field var 'x_ministro'
        $val = $CurrentForm->hasValue("ministro") ? $CurrentForm->getValue("ministro") : $CurrentForm->getValue("x_ministro");
        if (!$this->ministro->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->ministro->Visible = false; // Disable update for API request
            } else {
                $this->ministro->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_ministro")) {
            $this->ministro->setOldValue($CurrentForm->getValue("o_ministro"));
        }
        ...
        if (!$this->Norden->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
    ...
    public function restoreFormValues()
    {
        ...
        $this->ministro->CurrentValue = $this->ministro->FormValue;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->ministro->EditValue = HtmlEncode($this->ministro->CurrentValue);
            $curVal = strval($this->ministro->CurrentValue);
            if ($curVal != "") {
                $this->ministro->EditValue = $this->ministro->lookupCacheOption($curVal);
                if ($this->ministro->EditValue === null) { // Lookup from database
                    $filterWrk = SearchFilter($this->ministro->Lookup->getTable()->Fields["username"]->searchExpression(), "=", $curVal, $this->ministro->Lookup->getTable()->Fields["username"]->searchDataType(), "");
                    $sqlWrk = $this->ministro->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->ministro->Lookup->renderViewRow($rswrk[0]);
                        $this->ministro->EditValue = $this->ministro->displayValue($arwrk);
                    } else {
                        $this->ministro->EditValue = HtmlEncode($this->ministro->CurrentValue);
                    }
                }
            } else {
                $this->ministro->EditValue = null;
            }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->ministro->Visible && $this->ministro->Required) {
                if (!$this->ministro->IsDetailKey && EmptyValue($this->ministro->FormValue)) {
                    $this->ministro->addErrorMessage(str_replace("%s", $this->ministro->caption(), $this->ministro->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // ministro
        $this->ministro->setDbValueDef($rsnew, $this->ministro->CurrentValue, $this->ministro->ReadOnly);
        return $rsnew;
    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['ministro'])) { // ministro
            $this->ministro->CurrentValue = $row['ministro'];
        }
    }
    ...
    protected function getAddRow()
    {
        ...
        // ministro
        $this->ministro->setDbValueDef($rsnew, $this->ministro->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['ministro'])) { // ministro
            $this->ministro->setFormValue($row['ministro']);
        }
