...
/**
 * Table class for sco_cia
 */
class ScoCia extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Ncia
        $this->Ncia = new DbField(
            $this, // Table
            'x_Ncia', // Variable name
            'Ncia', // Name
            '`Ncia`', // Expression
            '`Ncia`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Ncia`', // Virtual expression
            false, // Is virtual
        ...
        $this->Ncia->InputTextType = "text";
        $this->Ncia->Raw = true;
        $this->Ncia->IsAutoIncrement = true; // Autoincrement field
        $this->Ncia->IsPrimaryKey = true; // Primary key field
        $this->Ncia->Nullable = false; // NOT NULL field
        $this->Ncia->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Ncia->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['Ncia'] = &$this->Ncia;
        ...
        $this->servicio->Lookup = new Lookup($this->servicio, 'sco_servicio_tipo', false, 'Nservicio_tipo', ["nombre","","",""], '', '', [], [], [], [], [], [], false, '`nombre`', '', "`nombre`");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "sco_cia";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->Ncia->setDbValue($conn->lastInsertId());
            $rs['Ncia'] = $this->Ncia->DbValue;
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['Ncia']) && !EmptyValue($this->Ncia->CurrentValue)) {
                $rs['Ncia'] = $this->Ncia->CurrentValue;
            }
        }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('Ncia', $rs)) {
                AddFilter($where, QuotedName('Ncia', $this->Dbid) . '=' . QuotedValue($rs['Ncia'], $this->Ncia->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        $keys = [];
        $val = $current ? $this->Ncia->CurrentValue : $this->Ncia->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->Ncia->CurrentValue = $keys[0];
            } else {
                $this->Ncia->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('Ncia', $row) ? $row['Ncia'] : null;
        } else {
            $val = !EmptyValue($this->Ncia->OldValue) && !$current ? $this->Ncia->OldValue : $this->Ncia->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@Ncia@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ScoCiaList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ScoCiaView" => $Language->phrase("View"),
            "ScoCiaEdit" => $Language->phrase("Edit"),
            "ScoCiaAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ScoCiaView",
            Config("API_ADD_ACTION") => "ScoCiaAdd",
            Config("API_EDIT_ACTION") => "ScoCiaEdit",
            Config("API_DELETE_ACTION") => "ScoCiaDelete",
            Config("API_LIST_ACTION") => "ScoCiaList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ScoCiaView", $parm);
        } else {
            $url = $this->keyUrl("ScoCiaView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoCiaEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoCiaList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoCiaAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoCiaList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ScoCiaDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"Ncia\":" . VarToJson($this->Ncia->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->Ncia->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->Ncia->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("Ncia") ?? Route("Ncia")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->Ncia->CurrentValue = $key;
            } else {
                $this->Ncia->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        // Ncia
        $this->Ncia->setupEditAttributes();
        $this->Ncia->EditValue = $this->Ncia->CurrentValue;
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'sco_cia');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'sco_cia', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'sco_cia', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'sco_cia', $fldname, $key, $oldvalue);
            }
        }
    }
