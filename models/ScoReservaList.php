 ...
 * Page class
 */
class ScoReservaList extends ScoReserva
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoReservaList";
    ...
    public $FormName = "fsco_reservalist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoReservaList";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'sco_reserva';
        $this->TableName = 'sco_reserva';
        ...
        // Table object (sco_reserva)
        if (!isset($GLOBALS["sco_reserva"]) || $GLOBALS["sco_reserva"]::class == PROJECT_NAMESPACE . "sco_reserva") {
            $GLOBALS["sco_reserva"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ScoReservaAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoReservaDelete";
        $this->MultiUpdateUrl = "ScoReservaUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'sco_reserva');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ScoReservaView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->capilla);
        $this->setupLookupOptions($this->user_registra);

        // Update form name to avoid conflict
        if ($this->IsModal) {
            $this->FormName = "fsco_reservagrid";
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_reservasrch", $filters);
    ...
    public function showFilterList()
    {
        ...
        // Field motivo
        $filter = $this->queryBuilderWhere("motivo");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->motivo, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->motivo->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_reservalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_reservalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fsco_reservalist\"" .
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_reservasrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_reservasrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_reservalist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_reserva", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_reserva",
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // capilla
            $curVal = strval($this->capilla->CurrentValue);
            if ($curVal != "") {
                $this->capilla->ViewValue = $this->capilla->lookupCacheOption($curVal);
                if ($this->capilla->ViewValue === null) { // Lookup from database
                    $filterWrk = SearchFilter($this->capilla->Lookup->getTable()->Fields["Nservicio"]->searchExpression(), "=", $curVal, $this->capilla->Lookup->getTable()->Fields["Nservicio"]->searchDataType(), "");
                    $sqlWrk = $this->capilla->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->capilla->Lookup->renderViewRow($rswrk[0]);
                        $this->capilla->ViewValue = $this->capilla->displayValue($arwrk);
                    } else {
                        $this->capilla->ViewValue = $this->capilla->CurrentValue;
                    }
                }
            } else {
                $this->capilla->ViewValue = null;
            }
            ...
            if ($this->capilla->AdvancedSearch->ViewValue !== null) { // Load from cache
                ...
                if ($this->capilla->AdvancedSearch->ViewValue == "") {
                    $this->capilla->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->capilla->Lookup->getTable()->Fields["Nservicio"]->searchExpression(), "=", $this->capilla->AdvancedSearch->SearchValue, $this->capilla->Lookup->getTable()->Fields["Nservicio"]->searchDataType(), "");
                }
                ...
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->capilla->Lookup->renderViewRow($rswrk[0]);
                    $this->capilla->AdvancedSearch->ViewValue = $this->capilla->displayValue($arwrk);
                } else {
                    $this->capilla->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
                $arwrk = $rswrk;
            ...
            if (!$this->localidad->Raw) {
                $this->localidad->AdvancedSearch->SearchValue = HtmlDecode($this->localidad->AdvancedSearch->SearchValue);
            }
            $this->localidad->EditValue = HtmlEncode($this->localidad->AdvancedSearch->SearchValue);
            ...
            $this->hora_fin->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->hora_fin->AdvancedSearch->SearchValue, $this->hora_fin->formatPattern()), $this->hora_fin->formatPattern()));
            $this->hora_fin->PlaceHolder = RemoveHtml($this->hora_fin->caption());

            // motivo
            $this->motivo->setupEditAttributes();
            if (!$this->motivo->Raw) {
                $this->motivo->AdvancedSearch->SearchValue = HtmlDecode($this->motivo->AdvancedSearch->SearchValue);
            }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_reservasrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    public function pageDataRendering(&$header) {
    	// Example:
    	//$header = "your header";
    	$header = '<a href="Disponibilidad?username=' . CurrentUserName() . '" class="btn btn-primary">Ver Disponibilidad</a>';
    }
