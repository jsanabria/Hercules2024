 ...
 * Page class
 */
class ScoEmbalajeEdit extends ScoEmbalaje
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoEmbalajeEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoEmbalajeEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Nembalaje") ?? Key(0) ?? Route(2)) !== null) {
                $this->Nembalaje->setQueryStringValue($keyValue);
                $this->Nembalaje->setOldValue($this->Nembalaje->QueryStringValue);
            } elseif (Post("Nembalaje") !== null) {
                $this->Nembalaje->setFormValue(Post("Nembalaje"));
                $this->Nembalaje->setOldValue($this->Nembalaje->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("Nembalaje") ?? Route("Nembalaje")) !== null) {
                    ...
                    $this->Nembalaje->CurrentValue = null;
                }
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ScoEmbalajeList"); // Return to list page
                        return;
                        ...
                        if ($this->Nembalaje->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->Nembalaje->CurrentValue, $this->CurrentRow['Nembalaje'])) {
    ...
    protected function getEditRow($rsold)
    {
        ...
        // nombre_familiar
        $this->nombre_familiar->setDbValueDef($rsnew, $this->nombre_familiar->CurrentValue, $this->nombre_familiar->ReadOnly);

        // cedula_familiar
        $this->cedula_familiar->setDbValueDef($rsnew, $this->cedula_familiar->CurrentValue, $this->cedula_familiar->ReadOnly);
        ...
        // cremacion_nro
        $this->cremacion_nro->setDbValueDef($rsnew, $this->cremacion_nro->CurrentValue, $this->cremacion_nro->ReadOnly);

        // registro_civil
        $this->registro_civil->setDbValueDef($rsnew, $this->registro_civil->CurrentValue, $this->registro_civil->ReadOnly);

        // dimension_cofre
        $this->dimension_cofre->setDbValueDef($rsnew, $this->dimension_cofre->CurrentValue, $this->dimension_cofre->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['nombre_familiar'])) { // nombre_familiar
            $this->nombre_familiar->CurrentValue = $row['nombre_familiar'];
        }
        if (isset($row['cedula_familiar'])) { // cedula_familiar
            $this->cedula_familiar->CurrentValue = $row['cedula_familiar'];
        }
        ...
        if (isset($row['cremacion_nro'])) { // cremacion_nro
            $this->cremacion_nro->CurrentValue = $row['cremacion_nro'];
        }
        if (isset($row['registro_civil'])) { // registro_civil
            $this->registro_civil->CurrentValue = $row['registro_civil'];
        }
        if (isset($row['dimension_cofre'])) { // dimension_cofre
            $this->dimension_cofre->CurrentValue = $row['dimension_cofre'];
        }
