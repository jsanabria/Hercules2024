 ...
 * Page class
 */
class ScoLapidasRegistroEdit extends ScoLapidasRegistro
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoLapidasRegistroEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoLapidasRegistroEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Nlapidas_registro") ?? Key(0) ?? Route(2)) !== null) {
                $this->Nlapidas_registro->setQueryStringValue($keyValue);
                $this->Nlapidas_registro->setOldValue($this->Nlapidas_registro->QueryStringValue);
            } elseif (Post("Nlapidas_registro") !== null) {
                $this->Nlapidas_registro->setFormValue(Post("Nlapidas_registro"));
                $this->Nlapidas_registro->setOldValue($this->Nlapidas_registro->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("Nlapidas_registro") ?? Route("Nlapidas_registro")) !== null) {
                    ...
                    $this->Nlapidas_registro->CurrentValue = null;
                }
            }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->Nlapidas_registro->IsDetailKey) {
            $this->Nlapidas_registro->setFormValue($val, true, $validate);
        }
    ...
    {
        global $CurrentForm;
        $this->Nlapidas_registro->CurrentValue = $this->Nlapidas_registro->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->Nlapidas_registro->Visible && $this->Nlapidas_registro->Required) {
                if (!$this->Nlapidas_registro->IsDetailKey && EmptyValue($this->Nlapidas_registro->FormValue)) {
                    $this->Nlapidas_registro->addErrorMessage(str_replace("%s", $this->Nlapidas_registro->caption(), $this->Nlapidas_registro->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->Nlapidas_registro->FormValue)) {
        ...
        if (in_array("sco_lapidas_notas", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("ScoLapidasAdjuntoGrid");
        if (in_array("sco_lapidas_adjunto", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function editRow()
    {
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("ScoLapidasNotasGrid");
            if (in_array("sco_lapidas_notas", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "sco_lapidas_notas"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("ScoLapidasAdjuntoGrid");
            if (in_array("sco_lapidas_adjunto", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
    ...
    protected function getEditRow($rsold)
    {
        ...
        // maqueteo
        $this->maqueteo->setDbValueDef($rsnew, $this->maqueteo->CurrentValue, $this->maqueteo->ReadOnly);
        return $rsnew;
    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['maqueteo'])) { // maqueteo
            $this->maqueteo->CurrentValue = $row['maqueteo'];
        }
    }
    ...
    public function pageDataRendered(&$footer) {
    	// Example:
    	//$footer = $this->maqueteo->CurrentValue . " - ";
    	$footer = '<div id="maqueta" class="container"></div>';
    	$footer .= "<script>
    		myMaqueteo('" . $this->maqueteo->CurrentValue . "');
    		</script>";
    }
