 ...
 * Page class
 */
class ScoAdjuntoList extends ScoAdjunto
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoAdjuntoList";
    ...
    public $FormName = "fsco_adjuntolist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoAdjuntoList";
    ...
    {
        $this->Nadjunto->Visible = false;
        $this->tipo->Visible = false;
        $this->archivo->setVisibility();
        $this->expediente->Visible = false;
        $this->servicio->Visible = false;
        $this->flota->Visible = false;
        $this->flota_incidencia->Visible = false;
    ...
    public function __construct()
    {
        ...
        // Table object (sco_adjunto)
        if (!isset($GLOBALS["sco_adjunto"]) || $GLOBALS["sco_adjunto"]::class == PROJECT_NAMESPACE . "sco_adjunto") {
            $GLOBALS["sco_adjunto"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ScoAdjuntoAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoAdjuntoDelete";
        $this->MultiUpdateUrl = "ScoAdjuntoUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'sco_adjunto');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ScoAdjuntoView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_adjuntogrid";
        }
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "view_capillas") {
            $masterTbl = Container("view_capillas");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("ViewCapillasList"); // Return to master page
                return;
            } else {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "sco_servicio") {
            $masterTbl = Container("sco_servicio");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("ScoServicioList"); // Return to master page
                return;
            } else {
                $masterTbl->loadListRowValues($rsmaster);
                $masterTbl->RowType = RowType::MASTER; // Master row
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "sco_flota") {
            $masterTbl = Container("sco_flota");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("ScoFlotaList"); // Return to master page
                return;
            } else {
                $masterTbl->loadListRowValues($rsmaster);
                $masterTbl->RowType = RowType::MASTER; // Master row
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "sco_flota_incidencia") {
            $masterTbl = Container("sco_flota_incidencia");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("ScoFlotaIncidenciaList"); // Return to master page
                return;
            } else {
                $masterTbl->loadListRowValues($rsmaster);
                $masterTbl->RowType = RowType::MASTER; // Master row
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->Nadjunto->CurrentValue;
    ...
    {
        global $CurrentForm;
        if (!EmptyValue($this->archivo->Upload->Value)) {
            return false;
        }
        if (
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Nadjunto->AdvancedSearch->toJson(), ","); // Field Nadjunto
        ...
        $filterList = Concat($filterList, $this->flota_incidencia->AdvancedSearch->toJson(), ","); // Field flota_incidencia
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_adjuntosrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Nadjunto
        $this->Nadjunto->AdvancedSearch->SearchValue = @$filter["x_Nadjunto"];
        $this->Nadjunto->AdvancedSearch->SearchOperator = @$filter["z_Nadjunto"];
        $this->Nadjunto->AdvancedSearch->SearchCondition = @$filter["v_Nadjunto"];
        $this->Nadjunto->AdvancedSearch->SearchValue2 = @$filter["y_Nadjunto"];
        $this->Nadjunto->AdvancedSearch->SearchOperator2 = @$filter["w_Nadjunto"];
        $this->Nadjunto->AdvancedSearch->save();
        ...
        // Field flota_incidencia
        $this->flota_incidencia->AdvancedSearch->SearchValue = @$filter["x_flota_incidencia"];
        $this->flota_incidencia->AdvancedSearch->SearchOperator = @$filter["z_flota_incidencia"];
        $this->flota_incidencia->AdvancedSearch->SearchCondition = @$filter["v_flota_incidencia"];
        $this->flota_incidencia->AdvancedSearch->SearchValue2 = @$filter["y_flota_incidencia"];
        $this->flota_incidencia->AdvancedSearch->SearchOperator2 = @$filter["w_flota_incidencia"];
        $this->flota_incidencia->AdvancedSearch->save();
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetall") {
                ...
                $this->DbMasterFilter = "";
                $this->DbDetailFilter = "";
                        $this->servicio->setSessionValue("");
                        $this->servicio->setSessionValue("");
                        $this->flota->setSessionValue("");
                        $this->flota_incidencia->setSessionValue("");
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_adjunto\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_adjunto\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_adjuntolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_adjuntolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Nadjunto->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_adjunto\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "archivo");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_adjuntosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_adjuntosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit() && !$this->isMultiEdit()) { // Not grid add/grid edit/multi edit mode
            ...
            foreach ($this->ListActions as $listAction) {
                if ($listAction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_adjuntolist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
            ...
            if ($this->isGridAdd()) {
                ...
                if (!($this->ModalGridAdd && !IsMobile())) {
                    ...
                    $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" form=\"fsco_adjuntolist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridInsertLink") . "</button>";
                    // Add grid cancel
            ...
            if ($this->isGridEdit()) {
                ...
                if (!($this->ModalGridEdit && !IsMobile())) {
                    ...
                    $option->UseDropDownButton = false;
                        ...
                        $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" form=\"fsco_adjuntolist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridSaveLink") . "</button>";
                        $item = &$option->add("gridcancel");
                        $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_adjunto", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_adjunto",
    ...
    {
        $this->expediente->DefaultValue = $this->expediente->getDefault(); // PHP
        $this->expediente->OldValue = $this->expediente->DefaultValue;
        $this->servicio->DefaultValue = $this->servicio->getDefault(); // PHP
        $this->servicio->OldValue = $this->servicio->DefaultValue;
        $this->flota->DefaultValue = $this->flota->getDefault(); // PHP
        $this->flota->OldValue = $this->flota->DefaultValue;
        $this->flota_incidencia->DefaultValue = $this->flota_incidencia->getDefault(); // PHP
        $this->flota_incidencia->OldValue = $this->flota_incidencia->DefaultValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Nadjunto' first before field var 'x_Nadjunto'
        $val = $CurrentForm->hasValue("Nadjunto") ? $CurrentForm->getValue("Nadjunto") : $CurrentForm->getValue("x_Nadjunto");
        if (!$this->Nadjunto->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->Nadjunto->setFormValue($val);
        }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->Nadjunto->CurrentValue = $this->Nadjunto->FormValue;
        }
    ...
    {
        $row = [];
        $row['Nadjunto'] = $this->Nadjunto->DefaultValue;
        ...
        $row['flota_incidencia'] = $this->flota_incidencia->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->archivo->UseColorbox) {
                ...
                $this->archivo->LinkAttrs["data-rel"] = "sco_adjunto_x" . $this->RowCount . "_archivo";
            ...
            if (is_numeric($this->RowIndex)) {
                RenderUploadField($this->archivo, $this->RowIndex);
            }
    ...
    public function getRowHash($row)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['nota']); // nota
        $hash .= GetFieldHash($row['activo']); // activo
        return md5($hash);
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->archivo->Visible && !$this->archivo->Upload->KeepFile) {
            if (!EmptyValue($this->archivo->Upload->FileName)) {
                $this->archivo->Upload->DbValue = null;
                FixUploadFileNames($this->archivo);
                $this->archivo->setDbValueDef($rsnew, $this->archivo->Upload->FileName, false);
            }
        }
        ...
        if ($insertRow) {
            ...
            if ($addRow) {
                if ($this->archivo->Visible && !$this->archivo->Upload->KeepFile) {
                    $this->archivo->Upload->DbValue = null;
                    if (!SaveUploadFiles($this->archivo, $rsnew['archivo'], false)) {
    ...
    protected function getAddRow()
    {
        ...
        if ($this->archivo->Visible && !$this->archivo->Upload->KeepFile) {
            if ($this->archivo->Upload->FileName == "") {
                ...
                FixUploadTempFileNames($this->archivo);
        ...
        if ($this->servicio->getSessionValue() != "") {
            $rsnew['servicio'] = $this->servicio->getSessionValue();
        }

        // flota
        if ($this->flota->getSessionValue() != "") {
            $rsnew['flota'] = $this->flota->getSessionValue();
        }

        // flota_incidencia
        if ($this->flota_incidencia->getSessionValue() != "") {
            $rsnew['flota_incidencia'] = $this->flota_incidencia->getSessionValue();
        }
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        if (isset($row['archivo'])) { // archivo
            $this->archivo->setFormValue($row['archivo']);
        }
        if (isset($row['nota'])) { // nota
        ...
        if (isset($row['flota'])) { // flota
            $this->flota->setFormValue($row['flota']);
        }
        if (isset($row['flota_incidencia'])) { // flota_incidencia
            $this->flota_incidencia->setFormValue($row['flota_incidencia']);
        }
    }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_adjuntosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "view_capillas") {
                $validMaster = true;
                $masterTbl = Container("view_capillas");
                if (($parm = Get("fk_Nservicio", Get("servicio"))) !== null) {
                    $masterTbl->Nservicio->setQueryStringValue($parm);
                    $this->servicio->QueryStringValue = $masterTbl->Nservicio->QueryStringValue; // DO NOT change, master/detail key data type can be different
                    $this->servicio->setSessionValue($this->servicio->QueryStringValue);
                    $foreignKeys["servicio"] = $this->servicio->QueryStringValue;
                } else {
                    $validMaster = false;
                }
            }
            if ($masterTblVar == "sco_servicio") {
                $validMaster = true;
                $masterTbl = Container("sco_servicio");
                if (($parm = Get("fk_Nservicio", Get("servicio"))) !== null) {
                    $masterTbl->Nservicio->setQueryStringValue($parm);
            ...
            if ($masterTblVar == "sco_flota") {
                $validMaster = true;
                $masterTbl = Container("sco_flota");
                if (($parm = Get("fk_Nflota", Get("flota"))) !== null) {
                    $masterTbl->Nflota->setQueryStringValue($parm);
                    $this->flota->QueryStringValue = $masterTbl->Nflota->QueryStringValue; // DO NOT change, master/detail key data type can be different
                    $this->flota->setSessionValue($this->flota->QueryStringValue);
                    $foreignKeys["flota"] = $this->flota->QueryStringValue;
                    if (!is_numeric($masterTbl->Nflota->QueryStringValue)) {
                        $validMaster = false;
                    }
                } else {
                    $validMaster = false;
            ...
            if ($masterTblVar == "sco_flota_incidencia") {
                $validMaster = true;
                $masterTbl = Container("sco_flota_incidencia");
                if (($parm = Get("fk_Nflota_incidencia", Get("flota_incidencia"))) !== null) {
                    $masterTbl->Nflota_incidencia->setQueryStringValue($parm);
                    $this->flota_incidencia->QueryStringValue = $masterTbl->Nflota_incidencia->QueryStringValue; // DO NOT change, master/detail key data type can be different
                    $this->flota_incidencia->setSessionValue($this->flota_incidencia->QueryStringValue);
                    $foreignKeys["flota_incidencia"] = $this->flota_incidencia->QueryStringValue;
                    if (!is_numeric($masterTbl->Nflota_incidencia->QueryStringValue)) {
                        $validMaster = false;
                    }
                } else {
                    $validMaster = false;
            ...
            if ($masterTblVar == "view_capillas") {
                $validMaster = true;
                $masterTbl = Container("view_capillas");
                if (($parm = Post("fk_Nservicio", Post("servicio"))) !== null) {
                    $masterTbl->Nservicio->setFormValue($parm);
                    $this->servicio->FormValue = $masterTbl->Nservicio->FormValue;
                    $this->servicio->setSessionValue($this->servicio->FormValue);
                    $foreignKeys["servicio"] = $this->servicio->FormValue;
                } else {
                    $validMaster = false;
                }
            }
            ...
            if ($masterTblVar == "sco_flota") {
                $validMaster = true;
                $masterTbl = Container("sco_flota");
                if (($parm = Post("fk_Nflota", Post("flota"))) !== null) {
                    $masterTbl->Nflota->setFormValue($parm);
                    $this->flota->FormValue = $masterTbl->Nflota->FormValue;
                    $this->flota->setSessionValue($this->flota->FormValue);
                    $foreignKeys["flota"] = $this->flota->FormValue;
                    if (!is_numeric($masterTbl->Nflota->FormValue)) {
                        $validMaster = false;
                    }
                } else {
                    $validMaster = false;
            ...
            if ($masterTblVar == "sco_flota_incidencia") {
                $validMaster = true;
                $masterTbl = Container("sco_flota_incidencia");
                if (($parm = Post("fk_Nflota_incidencia", Post("flota_incidencia"))) !== null) {
                    $masterTbl->Nflota_incidencia->setFormValue($parm);
                    $this->flota_incidencia->FormValue = $masterTbl->Nflota_incidencia->FormValue;
                    $this->flota_incidencia->setSessionValue($this->flota_incidencia->FormValue);
                    $foreignKeys["flota_incidencia"] = $this->flota_incidencia->FormValue;
                    if (!is_numeric($masterTbl->Nflota_incidencia->FormValue)) {
                        $validMaster = false;
                    }
                } else {
                    $validMaster = false;
        ...
        if ($validMaster) {
            ...
            if ($masterTblVar != "view_capillas") {
                if (!array_key_exists("servicio", $foreignKeys)) { // Not current foreign key
                    $this->servicio->setSessionValue("");
                }
            }
            if ($masterTblVar != "sco_servicio") {
            ...
            if ($masterTblVar != "sco_flota") {
                if (!array_key_exists("flota", $foreignKeys)) { // Not current foreign key
                    $this->flota->setSessionValue("");
                }
            }
            if ($masterTblVar != "sco_flota_incidencia") {
                if (!array_key_exists("flota_incidencia", $foreignKeys)) { // Not current foreign key
                    $this->flota_incidencia->setSessionValue("");
                }
            }
            if ($masterTblVar != "sco_expediente") {
    ...
    public function pageLoad() {
    	//echo "Page Load";
    	/*if(trim($_GET["fk_Nexpediente"])!="") {
    		$id = $_GET["fk_Nexpediente"];
    		$value = ExecuteScalar("SELECT count(*) FROM sco_adjunto WHERE expediente = '$id'");
    		if($value==0) { 
    			header("Location: sco_adjuntoadd.php?showmaster=sco_expediente&fk_Nexpediente=$id");
    			die();
    		}
    	}*/
    }
    ...
    public function pageDataRendering(&$header) {
    	// Example:
    	//$header = "your header";
    	/*if(CurrentPage()->getCurrentMasterTable()=="sco_expediente")
    		$header = subHeaderExp(CurrentTable()->expediente->CurrentValue);*/
    }
