 ...
 * Page class
 */
class ScoLapidasList extends ScoLapidas
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoLapidasList";
    ...
    public $FormName = "fsco_lapidaslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoLapidasList";
    ...
    public function setVisibility()
    {
        ...
        $this->instalacion->setVisibility();
    ...
    public function __construct()
    {
        ...
        // Table object (sco_lapidas)
        if (!isset($GLOBALS["sco_lapidas"]) || $GLOBALS["sco_lapidas"]::class == PROJECT_NAMESPACE . "sco_lapidas") {
            $GLOBALS["sco_lapidas"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ScoLapidasAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoLapidasDelete";
        $this->MultiUpdateUrl = "ScoLapidasUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'sco_lapidas');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ScoLapidasView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_lapidasgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Nlapidas->AdvancedSearch->toJson(), ","); // Field Nlapidas
        ...
        $filterList = Concat($filterList, $this->instalacion->AdvancedSearch->toJson(), ","); // Field instalacion
        $filterList = Concat($filterList, $this->nota1->AdvancedSearch->toJson(), ","); // Field nota1
        $filterList = Concat($filterList, $this->cliente->AdvancedSearch->toJson(), ","); // Field cliente
        $filterList = Concat($filterList, $this->nota2->AdvancedSearch->toJson(), ","); // Field nota2
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_lapidassrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Nlapidas
        $this->Nlapidas->AdvancedSearch->SearchValue = @$filter["x_Nlapidas"];
        $this->Nlapidas->AdvancedSearch->SearchOperator = @$filter["z_Nlapidas"];
        $this->Nlapidas->AdvancedSearch->SearchCondition = @$filter["v_Nlapidas"];
        $this->Nlapidas->AdvancedSearch->SearchValue2 = @$filter["y_Nlapidas"];
        $this->Nlapidas->AdvancedSearch->SearchOperator2 = @$filter["w_Nlapidas"];
        $this->Nlapidas->AdvancedSearch->save();
        ...
        // Field instalacion
        $this->instalacion->AdvancedSearch->SearchValue = @$filter["x_instalacion"];
        $this->instalacion->AdvancedSearch->SearchOperator = @$filter["z_instalacion"];
        $this->instalacion->AdvancedSearch->SearchCondition = @$filter["v_instalacion"];
        $this->instalacion->AdvancedSearch->SearchValue2 = @$filter["y_instalacion"];
        $this->instalacion->AdvancedSearch->SearchOperator2 = @$filter["w_instalacion"];
        $this->instalacion->AdvancedSearch->save();

        // Field nota1
        $this->nota1->AdvancedSearch->SearchValue = @$filter["x_nota1"];
        $this->nota1->AdvancedSearch->SearchOperator = @$filter["z_nota1"];
        $this->nota1->AdvancedSearch->SearchCondition = @$filter["v_nota1"];
        $this->nota1->AdvancedSearch->SearchValue2 = @$filter["y_nota1"];
        $this->nota1->AdvancedSearch->SearchOperator2 = @$filter["w_nota1"];
        $this->nota1->AdvancedSearch->save();
        ...
        // Field nota2
        $this->nota2->AdvancedSearch->SearchValue = @$filter["x_nota2"];
        $this->nota2->AdvancedSearch->SearchOperator = @$filter["z_nota2"];
        $this->nota2->AdvancedSearch->SearchCondition = @$filter["v_nota2"];
        $this->nota2->AdvancedSearch->SearchValue2 = @$filter["y_nota2"];
        $this->nota2->AdvancedSearch->SearchOperator2 = @$filter["w_nota2"];
        $this->nota2->AdvancedSearch->save();
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->instalacion;
        $searchFlds[] = &$this->nota1;
        $searchFlds[] = &$this->cliente;
        $searchFlds[] = &$this->nota2;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->instalacion); // instalacion
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_lapidas\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_lapidaslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_lapidaslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Nlapidas->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "instalacion");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_lapidassrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_lapidassrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_lapidaslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_lapidas", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_lapidas",
    ...
    {
        $row = [];
        $row['Nlapidas'] = $this->Nlapidas->DefaultValue;
        ...
        $row['instalacion'] = $this->instalacion->DefaultValue;
        $row['nota1'] = $this->nota1->DefaultValue;
        $row['cliente'] = $this->cliente->DefaultValue;
        $row['nota2'] = $this->nota2->DefaultValue;
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_lapidassrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
