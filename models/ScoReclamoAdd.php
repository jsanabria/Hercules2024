 ...
 * Page class
 */
class ScoReclamoAdd extends ScoReclamo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoReclamoAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoReclamoAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("ScoReclamoList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ScoReclamoList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ScoReclamoView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ScoReclamoList") {
                            ...
                            $returnUrl = "ScoReclamoList"; // Return list page content
                        }
                    }
    ...
    {
        $this->mensaje_cliente->DefaultValue = $this->mensaje_cliente->getDefault(); // PHP
        $this->mensaje_cliente->OldValue = $this->mensaje_cliente->DefaultValue;
    }
    ...
    public function renderRow()
    {
        ...
        // mensaje_cliente
        $this->mensaje_cliente->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->parentesco->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->parentesco->Lookup->getTable()->Fields["campo_descripcion"]->searchExpression(), "=", $this->parentesco->CurrentValue, $this->parentesco->Lookup->getTable()->Fields["campo_descripcion"]->searchDataType(), "");
                }
                $lookupFilter = $this->parentesco->getSelectFilter($this); // PHP
    ...
    protected function validateForm()
    {
        ...
        if (in_array("sco_reclamo_nota", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("ScoReclamoAdjuntoGrid");
        if (in_array("sco_reclamo_adjunto", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("ScoReclamoNotaGrid");
            if (in_array("sco_reclamo_nota", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->reclamo->setSessionValue($this->Nreclamo->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "sco_reclamo_nota"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->reclamo->setSessionValue(""); // Clear master key if insert failed
                }
            }
            $detailPage = Container("ScoReclamoAdjuntoGrid");
            if (in_array("sco_reclamo_adjunto", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->reclamo->setSessionValue($this->Nreclamo->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "sco_reclamo_adjunto"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->reclamo->setSessionValue(""); // Clear master key if insert failed
