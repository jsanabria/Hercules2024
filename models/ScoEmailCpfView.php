 ...
 * Page class
 */
class ScoEmailCpfView extends ScoEmailCpf
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoEmailCpfView";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoEmailCpfView";
    ...
    public function __construct()
    {
        ...
        // Set up record key
        if (($keyValue = Get("Nemail_cpf") ?? Route("Nemail_cpf")) !== null) {
            $this->RecKey["Nemail_cpf"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("Nemail_cpf") ?? Route("Nemail_cpf")) !== null) {
            $this->Nemail_cpf->setQueryStringValue($keyValue);
            $this->RecKey["Nemail_cpf"] = $this->Nemail_cpf->QueryStringValue;
        } elseif (Post("Nemail_cpf") !== null) {
            $this->Nemail_cpf->setFormValue(Post("Nemail_cpf"));
            $this->RecKey["Nemail_cpf"] = $this->Nemail_cpf->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->Nemail_cpf->setQueryStringValue($keyValue);
            $this->RecKey["Nemail_cpf"] = $this->Nemail_cpf->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "ScoEmailCpfList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ScoEmailCpfList"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->Nemail_cpf->CurrentValue, $this->CurrentRow['Nemail_cpf'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "ScoEmailCpfList"; // No matching record, return to list
                    } else {
    ...
    */
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ScoEmailCpfList"), "", $this->TableVar, true);
        $pageId = "view";
