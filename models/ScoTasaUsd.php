...
/**
 * Table class for sco_tasa_usd
 */
class ScoTasaUsd extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->moneda->addMethod("getSelectFilter", fn() => CurrentPageID() == "add" ? "`codigo` = '045' AND SUBSTRING(UPPER(LTRIM(valor1)), 1, 2) <> 'BS'" : "`codigo` = '045'");
        ...
        $this->moneda->Lookup = new Lookup($this->moneda, 'sco_parametro', false, 'valor1', ["valor1","","",""], '', '', [], [], [], [], [], [], false, '', '', "`valor1`");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "sco_tasa_usd";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ScoTasaUsdList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ScoTasaUsdView" => $Language->phrase("View"),
            "ScoTasaUsdEdit" => $Language->phrase("Edit"),
            "ScoTasaUsdAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ScoTasaUsdView",
            Config("API_ADD_ACTION") => "ScoTasaUsdAdd",
            Config("API_EDIT_ACTION") => "ScoTasaUsdEdit",
            Config("API_DELETE_ACTION") => "ScoTasaUsdDelete",
            Config("API_LIST_ACTION") => "ScoTasaUsdList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ScoTasaUsdView", $parm);
        } else {
            $url = $this->keyUrl("ScoTasaUsdView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoTasaUsdEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoTasaUsdList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoTasaUsdAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoTasaUsdList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ScoTasaUsdDelete", $parm);
        }
    }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'sco_tasa_usd');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'sco_tasa_usd', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'sco_tasa_usd', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'sco_tasa_usd', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowInserting($rsold, &$rsnew)
    {
        ...
        $rsnew["fecha"] = CurrentDate();
        $rsnew["hora"]  = CurrentTime();
