 ...
 * Page class
 */
class ScoTarifaList extends ScoTarifa
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoTarifaList";
    ...
    public $FormName = "fsco_tarifalist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoTarifaList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ScoTarifaAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoTarifaDelete";
        $this->MultiUpdateUrl = "ScoTarifaUpdate";
    ...
    public function emptyRow()
    {
        ...
        if (
            $CurrentForm->hasValue("x_horas") &&
            $CurrentForm->hasValue("o_horas") &&
            $this->horas->CurrentValue != $this->horas->DefaultValue &&
            !($this->horas->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->horas->CurrentValue == $this->horas->getSessionValue())
        ) {
            return false;
        }
        if (
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Ntarifa->AdvancedSearch->toJson(), ","); // Field Ntarifa
        ...
        $filterList = Concat($filterList, $this->crear_nuevo_precio->AdvancedSearch->toJson(), ","); // Field crear_nuevo_precio
        $filterList = Concat($filterList, $this->porc_recargo->AdvancedSearch->toJson(), ","); // Field porc_recargo
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_tarifasrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Ntarifa
        $this->Ntarifa->AdvancedSearch->SearchValue = @$filter["x_Ntarifa"];
        $this->Ntarifa->AdvancedSearch->SearchOperator = @$filter["z_Ntarifa"];
        $this->Ntarifa->AdvancedSearch->SearchCondition = @$filter["v_Ntarifa"];
        $this->Ntarifa->AdvancedSearch->SearchValue2 = @$filter["y_Ntarifa"];
        $this->Ntarifa->AdvancedSearch->SearchOperator2 = @$filter["w_Ntarifa"];
        $this->Ntarifa->AdvancedSearch->save();
        ...
        // Field crear_nuevo_precio
        $this->crear_nuevo_precio->AdvancedSearch->SearchValue = @$filter["x_crear_nuevo_precio"];
        $this->crear_nuevo_precio->AdvancedSearch->SearchOperator = @$filter["z_crear_nuevo_precio"];
        $this->crear_nuevo_precio->AdvancedSearch->SearchCondition = @$filter["v_crear_nuevo_precio"];
        $this->crear_nuevo_precio->AdvancedSearch->SearchValue2 = @$filter["y_crear_nuevo_precio"];
        $this->crear_nuevo_precio->AdvancedSearch->SearchOperator2 = @$filter["w_crear_nuevo_precio"];
        $this->crear_nuevo_precio->AdvancedSearch->save();

        // Field porc_recargo
        $this->porc_recargo->AdvancedSearch->SearchValue = @$filter["x_porc_recargo"];
        $this->porc_recargo->AdvancedSearch->SearchOperator = @$filter["z_porc_recargo"];
        $this->porc_recargo->AdvancedSearch->SearchCondition = @$filter["v_porc_recargo"];
        $this->porc_recargo->AdvancedSearch->SearchValue2 = @$filter["y_porc_recargo"];
        $this->porc_recargo->AdvancedSearch->SearchOperator2 = @$filter["w_porc_recargo"];
        $this->porc_recargo->AdvancedSearch->save();
    }
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Ntarifa, $default, false); // Ntarifa
        ...
        $this->buildSearchSql($where, $this->crear_nuevo_precio, $default, false); // crear_nuevo_precio
        $this->buildSearchSql($where, $this->porc_recargo, $default, false); // porc_recargo
        ...
        if (!$default && $this->Command == "search") {
            $this->Ntarifa->AdvancedSearch->save(); // Ntarifa
            ...
            $this->crear_nuevo_precio->AdvancedSearch->save(); // crear_nuevo_precio
            $this->porc_recargo->AdvancedSearch->save(); // porc_recargo
    ...
    {
        if ($this->Ntarifa->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->crear_nuevo_precio->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->porc_recargo->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    {
        $this->Ntarifa->AdvancedSearch->unsetSession();
        ...
        $this->crear_nuevo_precio->AdvancedSearch->unsetSession();
        $this->porc_recargo->AdvancedSearch->unsetSession();
    }
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->Ntarifa->AdvancedSearch->load();
        ...
        $this->crear_nuevo_precio->AdvancedSearch->load();
        $this->porc_recargo->AdvancedSearch->load();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->tipo->Expression . " ASC" . ", " . $this->servicio->Expression . " ASC"; // Set up default sort
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->fecha->setSort("");
                $this->activo->setSort("");
                $this->crear_nuevo_precio->setSort("");
                $this->porc_recargo->setSort("");
            }
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_tarifa\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_tarifa\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_tarifalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_tarifalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Ntarifa->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_tarifa\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_tarifasrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_tarifasrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit() && !$this->isMultiEdit()) { // Not grid add/grid edit/multi edit mode
            ...
            foreach ($this->ListActions as $listAction) {
                if ($listAction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_tarifalist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
            ...
            if ($this->isGridEdit()) {
                ...
                if (!($this->ModalGridEdit && !IsMobile())) {
                    ...
                    $option->UseDropDownButton = false;
                        ...
                        $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" form=\"fsco_tarifalist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridSaveLink") . "</button>";
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_tarifa", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_tarifa",
    ...
    protected function loadSearchValues()
    {
        ...
        // Ntarifa
        if ($this->Ntarifa->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Ntarifa->AdvancedSearch->SearchValue != "" || $this->Ntarifa->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // crear_nuevo_precio
        if ($this->crear_nuevo_precio->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->crear_nuevo_precio->AdvancedSearch->SearchValue != "" || $this->crear_nuevo_precio->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // porc_recargo
        if ($this->porc_recargo->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->porc_recargo->AdvancedSearch->SearchValue != "" || $this->porc_recargo->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->Ntarifa->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->servicio->AdvancedSearch->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->servicio->Lookup->getTable()->Fields["Nservicio"]->searchExpression(), "=", $this->servicio->AdvancedSearch->SearchValue, $this->servicio->Lookup->getTable()->Fields["Nservicio"]->searchDataType(), "");
                }
                $sqlWrk = $this->servicio->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
    ...
    public function getRowHash($row)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['horas']); // horas
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_tarifasrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->tipo->Visible || $this->servicio->Visible;
    }
