 ...
 * Page class
 */
class ScoProveedorView extends ScoProveedor
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoProveedorView";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoProveedorView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("Nproveedor") ?? Route("Nproveedor")) !== null) {
            $this->RecKey["Nproveedor"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("Nproveedor") ?? Route("Nproveedor")) !== null) {
            ...
            $this->RecKey["Nproveedor"] = $this->Nproveedor->QueryStringValue;
        } elseif (Post("Nproveedor") !== null) {
            $this->Nproveedor->setFormValue(Post("Nproveedor"));
            $this->RecKey["Nproveedor"] = $this->Nproveedor->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->Nproveedor->setQueryStringValue($keyValue);
            $this->RecKey["Nproveedor"] = $this->Nproveedor->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "ScoProveedorList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ScoProveedorList"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->Nproveedor->CurrentValue, $this->CurrentRow['Nproveedor'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "ScoProveedorList"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_sco_ofrece_servicio");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("sco_ofrece_servicio", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("ScoOfreceServicioList?" . Config("TABLE_SHOW_MASTER") . "=sco_proveedor&" . GetForeignKeyUrl("fk_Nproveedor", $this->Nproveedor->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_proveedor')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_ofrece_servicio"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_proveedor')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_ofrece_servicio"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
