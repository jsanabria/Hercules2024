 ...
 * Page class
 */
class ScoParcelaVentasEdit extends ScoParcelaVentas
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoParcelaVentasEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoParcelaVentasEdit";
    ...
    public function run()
    {
        ...
        $this->subseccion->Required = false;
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ScoParcelaVentasList"); // Return to list page
                        return;
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->Nparcela_ventas->IsDetailKey) {
            $this->Nparcela_ventas->setFormValue($val, true, $validate);
        }

        // Check field name 'fecha_compra' first before field var 'x_fecha_compra'
        $val = $CurrentForm->hasValue("fecha_compra") ? $CurrentForm->getValue("fecha_compra") : $CurrentForm->getValue("x_fecha_compra");
        if (!$this->fecha_compra->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->fecha_compra->Visible = false; // Disable update for API request
            } else {
                $this->fecha_compra->setFormValue($val, true, $validate);
            }
            $this->fecha_compra->CurrentValue = UnFormatDateTime($this->fecha_compra->CurrentValue, $this->fecha_compra->formatPattern());
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->fecha_compra->CurrentValue = $this->fecha_compra->FormValue;
        $this->fecha_compra->CurrentValue = UnFormatDateTime($this->fecha_compra->CurrentValue, $this->fecha_compra->formatPattern());
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->fecha_compra->EditValue = HtmlEncode(FormatDateTime($this->fecha_compra->CurrentValue, $this->fecha_compra->formatPattern()));
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if (!CheckInteger($this->Nparcela_ventas->FormValue)) {
                $this->Nparcela_ventas->addErrorMessage($this->Nparcela_ventas->getErrorMessage(false));
            }
            if ($this->fecha_compra->Visible && $this->fecha_compra->Required) {
                if (!$this->fecha_compra->IsDetailKey && EmptyValue($this->fecha_compra->FormValue)) {
                    $this->fecha_compra->addErrorMessage(str_replace("%s", $this->fecha_compra->caption(), $this->fecha_compra->RequiredErrorMessage));
                }
            }
            if (!CheckDate($this->fecha_compra->FormValue, $this->fecha_compra->formatPattern())) {
                $this->fecha_compra->addErrorMessage($this->fecha_compra->getErrorMessage(false));
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // fecha_compra
        $this->fecha_compra->setDbValueDef($rsnew, UnFormatDateTime($this->fecha_compra->CurrentValue, $this->fecha_compra->formatPattern()), $this->fecha_compra->ReadOnly);

        // ci_vendedor
        $this->ci_vendedor->setDbValueDef($rsnew, $this->ci_vendedor->CurrentValue, $this->ci_vendedor->ReadOnly);

        // vendedor
        $this->vendedor->setDbValueDef($rsnew, $this->vendedor->CurrentValue, $this->vendedor->ReadOnly);

        // valor_compra
        $this->valor_compra->setDbValueDef($rsnew, $this->valor_compra->CurrentValue, $this->valor_compra->ReadOnly);

        // moneda_compra
        $this->moneda_compra->setDbValueDef($rsnew, $this->moneda_compra->CurrentValue, $this->moneda_compra->ReadOnly);

        // tasa_compra
        $this->tasa_compra->setDbValueDef($rsnew, $this->tasa_compra->CurrentValue, $this->tasa_compra->ReadOnly);

        // nota
        $this->nota->setDbValueDef($rsnew, $this->nota->CurrentValue, $this->nota->ReadOnly);

        // numero_factura
        $this->numero_factura->setDbValueDef($rsnew, $this->numero_factura->CurrentValue, $this->numero_factura->ReadOnly);

        // orden_pago
        $this->orden_pago->setDbValueDef($rsnew, $this->orden_pago->CurrentValue, $this->orden_pago->ReadOnly);
        return $rsnew;
    }
    ...
    {
        if (isset($row['fecha_compra'])) { // fecha_compra
            $this->fecha_compra->CurrentValue = $row['fecha_compra'];
        }
        if (isset($row['ci_vendedor'])) { // ci_vendedor
            $this->ci_vendedor->CurrentValue = $row['ci_vendedor'];
        }
        if (isset($row['vendedor'])) { // vendedor
            $this->vendedor->CurrentValue = $row['vendedor'];
        }
        if (isset($row['valor_compra'])) { // valor_compra
            $this->valor_compra->CurrentValue = $row['valor_compra'];
        }
        if (isset($row['moneda_compra'])) { // moneda_compra
            $this->moneda_compra->CurrentValue = $row['moneda_compra'];
        }
        if (isset($row['tasa_compra'])) { // tasa_compra
            $this->tasa_compra->CurrentValue = $row['tasa_compra'];
        }
        if (isset($row['nota'])) { // nota
            $this->nota->CurrentValue = $row['nota'];
        }
        if (isset($row['numero_factura'])) { // numero_factura
            $this->numero_factura->CurrentValue = $row['numero_factura'];
        }
        if (isset($row['orden_pago'])) { // orden_pago
            $this->orden_pago->CurrentValue = $row['orden_pago'];
        }
    }
