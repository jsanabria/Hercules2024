 ...
 * Page class
 */
class ScoNotaOrdenCompraPreview extends ScoNotaOrdenCompra
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoNotaOrdenCompraPreview";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoNotaOrdenCompraPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "sco_orden_compra") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=sco_orden_compra&" . GetForeignKeyUrl("fk_Norden_compra", $this->orden_compra->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys($keys)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "sco_orden_compra") {
            $this->orden_compra->setQueryStringValue($keys[0] ?? "");
        }
    }
