 ...
 * Page class
 */
class ScoExpedienteEstatusEdit extends ScoExpedienteEstatus
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoExpedienteEstatusEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoExpedienteEstatusEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("expediente") ?? Key(0) ?? Route(2)) !== null) {
                $this->expediente->setQueryStringValue($keyValue);
                $this->expediente->setOldValue($this->expediente->QueryStringValue);
            } elseif (Post("expediente") !== null) {
                $this->expediente->setFormValue(Post("expediente"));
                $this->expediente->setOldValue($this->expediente->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
            if (($keyValue = Get("estatus") ?? Key(1) ?? Route(3)) !== null) {
                $this->estatus->setQueryStringValue($keyValue);
                $this->estatus->setOldValue($this->estatus->QueryStringValue);
            } elseif (Post("estatus") !== null) {
                $this->estatus->setFormValue(Post("estatus"));
                $this->estatus->setOldValue($this->estatus->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("expediente") ?? Route("expediente")) !== null) {
                    $this->expediente->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->expediente->CurrentValue = null;
                }
                if (($keyValue = Get("estatus") ?? Route("estatus")) !== null) {
                    $this->estatus->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ScoExpedienteEstatusList"); // Return to list page
                        return;
                        ...
                        if ($this->expediente->CurrentValue != null && $this->estatus->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->expediente->CurrentValue, $this->CurrentRow['expediente']) && SameString($this->estatus->CurrentValue, $this->CurrentRow['estatus'])) {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ScoExpedienteEstatusList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "ScoExpedienteEstatusList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions && !$this->getCurrentMasterTable()) {
                        ...
                        if (GetPageName($returnUrl) != "ScoExpedienteEstatusList") {
                            ...
                            $returnUrl = "ScoExpedienteEstatusList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->estatus->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->estatus->Lookup->getTable()->Fields["Nstatus"]->searchExpression(), "=", $this->estatus->CurrentValue, $this->estatus->Lookup->getTable()->Fields["Nstatus"]->searchDataType(), "");
                }
                $lookupFilter = $this->estatus->getSelectFilter($this); // PHP
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ScoExpedienteEstatusList"), "", $this->TableVar, true);
        $pageId = "edit";
