 ...
 * Page class
 */
class ScoEmailCpfEdit extends ScoEmailCpf
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoEmailCpfEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoEmailCpfEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Nemail_cpf") ?? Key(0) ?? Route(2)) !== null) {
                $this->Nemail_cpf->setQueryStringValue($keyValue);
                $this->Nemail_cpf->setOldValue($this->Nemail_cpf->QueryStringValue);
            } elseif (Post("Nemail_cpf") !== null) {
                $this->Nemail_cpf->setFormValue(Post("Nemail_cpf"));
                $this->Nemail_cpf->setOldValue($this->Nemail_cpf->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("Nemail_cpf") ?? Route("Nemail_cpf")) !== null) {
                    ...
                    $this->Nemail_cpf->CurrentValue = null;
                }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ScoEmailCpfList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "ScoEmailCpfList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ScoEmailCpfList") {
                            ...
                            $returnUrl = "ScoEmailCpfList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Nemail_cpf' first before field var 'x_Nemail_cpf'
        $val = $CurrentForm->hasValue("Nemail_cpf") ? $CurrentForm->getValue("Nemail_cpf") : $CurrentForm->getValue("x_Nemail_cpf");
        if (!$this->Nemail_cpf->IsDetailKey) {
            $this->Nemail_cpf->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->Nemail_cpf->CurrentValue = $this->Nemail_cpf->FormValue;
    ...
    public function renderRow()
    {
        ...
        // Nemail_cpf
        $this->Nemail_cpf->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->Nemail_cpf->Visible && $this->Nemail_cpf->Required) {
                if (!$this->Nemail_cpf->IsDetailKey && EmptyValue($this->Nemail_cpf->FormValue)) {
                    $this->Nemail_cpf->addErrorMessage(str_replace("%s", $this->Nemail_cpf->caption(), $this->Nemail_cpf->RequiredErrorMessage));
                }
            }
