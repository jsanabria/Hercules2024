 ...
 * Page class
 */
class ScoServicioAdd extends ScoServicio
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoServicioAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoServicioAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "ScoServicioView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->secuencia->DefaultValue = $this->secuencia->getDefault(); // PHP
        $this->secuencia->OldValue = $this->secuencia->DefaultValue;
        $this->sto_min->DefaultValue = $this->sto_min->getDefault(); // PHP
        $this->sto_min->OldValue = $this->sto_min->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->tipo->getSessionValue() != "") {
                $this->tipo->CurrentValue = GetForeignKeyValue($this->tipo->getSessionValue());
                $curVal = strval($this->tipo->CurrentValue);
                if ($curVal != "") {
                    $this->tipo->ViewValue = $this->tipo->lookupCacheOption($curVal);
                    if ($this->tipo->ViewValue === null) { // Lookup from database
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'sco_servicio'
        $validMasterRecord = true;
        $detailKeys = [];
        $detailKeys["tipo"] = $this->tipo->CurrentValue;
        ...
        if ($insertRow && $this->ValidateKey && strval($rsnew['Nservicio']) == "") {
        ...
        if ($addRow) {
            ...
            if (in_array("sco_adjunto", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->servicio->setSessionValue($this->Nservicio->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "sco_adjunto"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->servicio->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
    ...
    protected function getAddRow()
    {
        ...
        // sto_min
        $this->sto_min->setDbValueDef($rsnew, $this->sto_min->CurrentValue, strval($this->sto_min->CurrentValue) == "");

        // activo
        $this->activo->setDbValueDef($rsnew, $this->activo->CurrentValue, strval($this->activo->CurrentValue) == "");

        // venta
        $this->venta->setDbValueDef($rsnew, $this->venta->CurrentValue, strval($this->venta->CurrentValue) == "");
        return $rsnew;
    }
