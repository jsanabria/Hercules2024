 ...
 * Page class
 */
class ScoMensajeClienteList extends ScoMensajeCliente
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoMensajeClienteList";
    ...
    public $FormName = "fsco_mensaje_clientelist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoMensajeClienteList";
    ...
    {
        $this->Nmensaje_cliente->setVisibility();
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'sco_mensaje_cliente';
        $this->TableName = 'sco_mensaje_cliente';
        ...
        // Table object (sco_mensaje_cliente)
        if (!isset($GLOBALS["sco_mensaje_cliente"]) || $GLOBALS["sco_mensaje_cliente"]::class == PROJECT_NAMESPACE . "sco_mensaje_cliente") {
            $GLOBALS["sco_mensaje_cliente"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ScoMensajeClienteAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoMensajeClienteDelete";
        $this->MultiUpdateUrl = "ScoMensajeClienteUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'sco_mensaje_cliente');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ScoMensajeClienteView"); // If View page, no primary button
                } else { // List page
    ...
    protected function hideFieldsForAddEdit()
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->Nmensaje_cliente->Visible = false;
        }
    }
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_mensaje_clientegrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Nmensaje_cliente->AdvancedSearch->toJson(), ","); // Field Nmensaje_cliente
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_mensaje_clientesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Nmensaje_cliente
        $this->Nmensaje_cliente->AdvancedSearch->SearchValue = @$filter["x_Nmensaje_cliente"];
        $this->Nmensaje_cliente->AdvancedSearch->SearchOperator = @$filter["z_Nmensaje_cliente"];
        $this->Nmensaje_cliente->AdvancedSearch->SearchCondition = @$filter["v_Nmensaje_cliente"];
        $this->Nmensaje_cliente->AdvancedSearch->SearchValue2 = @$filter["y_Nmensaje_cliente"];
        $this->Nmensaje_cliente->AdvancedSearch->SearchOperator2 = @$filter["w_Nmensaje_cliente"];
        $this->Nmensaje_cliente->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->Nmensaje_cliente->Expression . " DESC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->Nmensaje_cliente); // Nmensaje_cliente
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_mensaje_cliente\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_mensaje_clientelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_mensaje_clientelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Nmensaje_cliente->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "Nmensaje_cliente");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_mensaje_clientesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_mensaje_clientesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_mensaje_clientelist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_mensaje_cliente", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_mensaje_cliente",
    ...
    public function loadRowValues($row = null)
    {
        ...
        $this->Nmensaje_cliente->setDbValue($row['Nmensaje_cliente']);
    ...
    {
        $row = [];
        $row['Nmensaje_cliente'] = $this->Nmensaje_cliente->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            // Nmensaje_cliente
            $this->Nmensaje_cliente->ViewValue = $this->Nmensaje_cliente->CurrentValue;
            ...
            // Nmensaje_cliente
            $this->Nmensaje_cliente->HrefValue = "";
            $this->Nmensaje_cliente->TooltipValue = "";
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_mensaje_clientesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    public function pageDataRendering(&$header) {
    	...
    	$header = '<table align="left" cellpadding="3" cellspacing="3" width="50%">
    					<tr height="30">
    					  	<td bgcolor="red" width="20%" align="center">
    						<b>RECEPCION</b></td>
    					  	<td bgcolor="yellow" width="20%" align="center">
    						<b>VERIFICACION</b></td>
    					  	<td bgcolor="blue" width="20%" align="center">
    						<b>PROCESANDO</b></td>
    					  	<td bgcolor="green" width="20%" align="center">
    						<b>RESUELTO</b></td>
    					  	<td bgcolor="gray" width="20%" align="center">
    						<b>RECHAZADO</b></td>
    				    </tr>
    			</table><br /><br />';
    }
    ...
    public function pageDataRendered(&$footer) {
    	// Example:
    	$footer = '<table align="left" cellpadding="3" cellspacing="3" width="50%">
    					<tr height="30">
    					  	<td bgcolor="red" width="20%" align="center">
    						<b>RECEPCION</b></td>
    					  	<td bgcolor="yellow" width="20%" align="center">
    ...
    public function listOptionsRendered() {
    	// Example:
    	$sql = "SELECT estatus FROM sco_reclamo WHERE mensaje_cliente = '" . $this->Nmensaje_cliente->CurrentValue . "'";
    	$estatus = ExecuteScalar($sql);
    	$this->ListOptions->Items["new"]->Body = "<strong>$estatus</strong>";
    }
