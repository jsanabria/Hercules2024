 ...
 * Page class
 */
class ScoExpedienteSegurosList extends ScoExpedienteSeguros
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoExpedienteSegurosList";
    ...
    public $FormName = "fsco_expediente_seguroslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoExpedienteSegurosList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ScoExpedienteSegurosAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoExpedienteSegurosDelete";
        $this->MultiUpdateUrl = "ScoExpedienteSegurosUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_expediente_segurosgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Nexpediente_seguros->AdvancedSearch->toJson(), ","); // Field Nexpediente_seguros
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_expediente_segurossrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Nexpediente_seguros
        $this->Nexpediente_seguros->AdvancedSearch->SearchValue = @$filter["x_Nexpediente_seguros"];
        $this->Nexpediente_seguros->AdvancedSearch->SearchOperator = @$filter["z_Nexpediente_seguros"];
        $this->Nexpediente_seguros->AdvancedSearch->SearchCondition = @$filter["v_Nexpediente_seguros"];
        $this->Nexpediente_seguros->AdvancedSearch->SearchValue2 = @$filter["y_Nexpediente_seguros"];
        $this->Nexpediente_seguros->AdvancedSearch->SearchOperator2 = @$filter["w_Nexpediente_seguros"];
        $this->Nexpediente_seguros->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Nexpediente_seguros, $default, false); // Nexpediente_seguros
        ...
        if (!$default && $this->Command == "search") {
            $this->Nexpediente_seguros->AdvancedSearch->save(); // Nexpediente_seguros
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->Nexpediente_seguros->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->Nexpediente_seguros->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->Nexpediente_seguros->AdvancedSearch->load();
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->Nexpediente_seguros->setSort("");
                $this->seguro->setSort("");
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_sco_expediente_seguros_adjunto"
        $item = &$this->ListOptions->add("detail_sco_expediente_seguros_adjunto");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'sco_expediente_seguros_adjunto');
        $item->OnLeft = true;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("sco_expediente_seguros_adjunto");
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_expediente_seguros\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_expediente_seguros\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_expediente_seguroslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_expediente_seguroslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_sco_expediente_seguros_adjunto"
        $opt = $this->ListOptions["detail_sco_expediente_seguros_adjunto"];
        if ($Security->allowList(CurrentProjectID() . 'sco_expediente_seguros_adjunto')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("sco_expediente_seguros_adjunto", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("ScoExpedienteSegurosAdjuntoList?" . Config("TABLE_SHOW_MASTER") . "=sco_expediente_seguros&" . GetForeignKeyUrl("fk_Nexpediente_seguros", $this->Nexpediente_seguros->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("ScoExpedienteSegurosAdjuntoGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_expediente_seguros')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_expediente_seguros_adjunto");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "sco_expediente_seguros_adjunto";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_expediente_seguros')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_expediente_seguros_adjunto");
                ...
                $detailEditTblVar .= "sco_expediente_seguros_adjunto";
            }
            if ($links != "") {
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Nexpediente_seguros->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function renderListOptionsExt()
    {
        ...
        $detailFilters = [];
        $masterKeys = []; // Reset
        $masterKeys["Nexpediente_seguros"] = strval($this->Nexpediente_seguros->DbValue);

        // Column "detail_sco_expediente_seguros_adjunto"
        if ($this->DetailPages?->getItem("sco_expediente_seguros_adjunto")?->Visible && $Security->allowList(CurrentProjectID() . 'sco_expediente_seguros_adjunto')) {
            $link = "";
            $option = $this->ListOptions["detail_sco_expediente_seguros_adjunto"];
            $detailTbl = Container("sco_expediente_seguros_adjunto");
            $detailFilter = $detailTbl->getDetailFilter($this);
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "ScoExpedienteSegurosAdjuntoPreview?t=sco_expediente_seguros&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"sco_expediente_seguros_adjunto\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'sco_expediente_seguros')) {
                $label = $Language->tablePhrase("sco_expediente_seguros_adjunto", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"sco_expediente_seguros_adjunto\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("ScoExpedienteSegurosAdjuntoList?" . Config("TABLE_SHOW_MASTER") . "=sco_expediente_seguros");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $Language->tablePhrase("sco_expediente_seguros_adjunto", "TblCaption");
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_expediente_seguros')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_expediente_seguros_adjunto"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($viewurl) . "\">" . $caption . "</a>";
            }
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_expediente_seguros')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_expediente_seguros_adjunto"));
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_expediente_seguros\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $detailTableLink = "";
        $item = &$option->add("detailadd_sco_expediente_seguros_adjunto");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=sco_expediente_seguros_adjunto");
        $detailPage = Container("ScoExpedienteSegurosAdjuntoGrid");
        $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'sco_expediente_seguros') && $Security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "sco_expediente_seguros_adjunto";
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_expediente_segurossrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_expediente_segurossrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_expediente_seguroslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_expediente_seguros", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_expediente_seguros",
    ...
    protected function loadSearchValues()
    {
        ...
        // Nexpediente_seguros
        if ($this->Nexpediente_seguros->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Nexpediente_seguros->AdvancedSearch->SearchValue != "" || $this->Nexpediente_seguros->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // seguro
            $curVal = trim(strval($this->seguro->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->seguro->AdvancedSearch->ViewValue = $this->seguro->lookupCacheOption($curVal);
            } else {
                $this->seguro->AdvancedSearch->ViewValue = $this->seguro->Lookup !== null && is_array($this->seguro->lookupOptions()) && count($this->seguro->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->seguro->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->seguro->EditValue = array_values($this->seguro->lookupOptions());
                if ($this->seguro->AdvancedSearch->ViewValue == "") {
                    $this->seguro->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->seguro->Lookup->getTable()->Fields["Nseguro"]->searchExpression(), "=", $this->seguro->AdvancedSearch->SearchValue, $this->seguro->Lookup->getTable()->Fields["Nseguro"]->searchDataType(), "");
                }
                ...
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->seguro->Lookup->renderViewRow($rswrk[0]);
                    $this->seguro->AdvancedSearch->ViewValue = $this->seguro->displayValue($arwrk);
                } else {
                    $this->seguro->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
                $arwrk = $rswrk;
            ...
            // user_registra
            $curVal = trim(strval($this->user_registra->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->user_registra->AdvancedSearch->ViewValue = $this->user_registra->lookupCacheOption($curVal);
            } else {
                $this->user_registra->AdvancedSearch->ViewValue = $this->user_registra->Lookup !== null && is_array($this->user_registra->lookupOptions()) && count($this->user_registra->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->user_registra->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->user_registra->EditValue = array_values($this->user_registra->lookupOptions());
                if ($this->user_registra->AdvancedSearch->ViewValue == "") {
                    $this->user_registra->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->user_registra->Lookup->getTable()->Fields["username"]->searchExpression(), "=", $this->user_registra->AdvancedSearch->SearchValue, $this->user_registra->Lookup->getTable()->Fields["username"]->searchDataType(), "");
                }
                $sqlWrk = $this->user_registra->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCache($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                ...
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->user_registra->Lookup->renderViewRow($rswrk[0]);
                    $this->user_registra->AdvancedSearch->ViewValue = $this->user_registra->displayValue($arwrk);
                } else {
                    $this->user_registra->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
                $arwrk = $rswrk;
                $this->user_registra->EditValue = $arwrk;
            }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_expediente_segurossrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
