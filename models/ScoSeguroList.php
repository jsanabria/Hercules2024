 ...
 * Page class
 */
class ScoSeguroList extends ScoSeguro
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoSeguroList";
    ...
    public $FormName = "fsco_segurolist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoSeguroList";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'sco_seguro';
        $this->TableName = 'sco_seguro';
        ...
        // Table object (sco_seguro)
        if (!isset($GLOBALS["sco_seguro"]) || $GLOBALS["sco_seguro"]::class == PROJECT_NAMESPACE . "sco_seguro") {
            $GLOBALS["sco_seguro"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ScoSeguroAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoSeguroDelete";
        $this->MultiUpdateUrl = "ScoSeguroUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'sco_seguro');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ScoSeguroView"); // If View page, no primary button
                } else { // List page
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Nseguro->AdvancedSearch->toJson(), ","); // Field Nseguro
        ...
        $filterList = Concat($filterList, $this->email3->AdvancedSearch->toJson(), ","); // Field email3
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_segurosrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Nseguro
        $this->Nseguro->AdvancedSearch->SearchValue = @$filter["x_Nseguro"];
        $this->Nseguro->AdvancedSearch->SearchOperator = @$filter["z_Nseguro"];
        $this->Nseguro->AdvancedSearch->SearchCondition = @$filter["v_Nseguro"];
        $this->Nseguro->AdvancedSearch->SearchValue2 = @$filter["y_Nseguro"];
        $this->Nseguro->AdvancedSearch->SearchOperator2 = @$filter["w_Nseguro"];
        $this->Nseguro->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Nseguro, $default, false); // Nseguro
        ...
        $this->buildSearchSql($where, $this->email3, $default, false); // email3
        ...
        if (!$default && $this->Command == "search") {
            $this->Nseguro->AdvancedSearch->save(); // Nseguro
    ...
    public function showFilterList()
    {
        ...
        // Field tipo_contratacion
        $filter = $this->queryBuilderWhere("tipo_contratacion");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->tipo_contratacion, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->tipo_contratacion->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->Nseguro->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->Nseguro->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->Nseguro->AdvancedSearch->load();
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_seguro\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_seguro\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_segurolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_segurolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Nseguro->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_seguro\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_segurosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_segurosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_segurolist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_seguro", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_seguro",
    ...
    protected function loadSearchValues()
    {
        ...
        // Nseguro
        if ($this->Nseguro->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Nseguro->AdvancedSearch->SearchValue != "" || $this->Nseguro->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // email3
        if ($this->email3->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->email3->AdvancedSearch->SearchValue != "" || $this->email3->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function loadRowValues($row = null)
    {
        ...
        $this->Nseguro->setDbValue($row['Nseguro']);
    ...
    {
        $row = [];
        $row['Nseguro'] = $this->Nseguro->DefaultValue;
        ...
        $row['email3'] = $this->email3->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            // Nseguro
            $this->Nseguro->ViewValue = $this->Nseguro->CurrentValue;

            // tipo_contratacion
            if (strval($this->tipo_contratacion->CurrentValue) != "") {
                $this->tipo_contratacion->ViewValue = $this->tipo_contratacion->optionCaption($this->tipo_contratacion->CurrentValue);
            } else {
            ...
            $this->tipo_contratacion->EditValue = $this->tipo_contratacion->options(true);
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_segurosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
