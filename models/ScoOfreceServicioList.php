 ...
 * Page class
 */
class ScoOfreceServicioList extends ScoOfreceServicio
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoOfreceServicioList";
    ...
    public $FormName = "fsco_ofrece_serviciolist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoOfreceServicioList";
    ...
    {
        $this->Nfrece_servicio->Visible = false;
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'sco_ofrece_servicio';
        $this->TableName = 'sco_ofrece_servicio';
        ...
        // Table object (sco_ofrece_servicio)
        if (!isset($GLOBALS["sco_ofrece_servicio"]) || $GLOBALS["sco_ofrece_servicio"]::class == PROJECT_NAMESPACE . "sco_ofrece_servicio") {
            $GLOBALS["sco_ofrece_servicio"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ScoOfreceServicioAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoOfreceServicioDelete";
        $this->MultiUpdateUrl = "ScoOfreceServicioUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'sco_ofrece_servicio');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ScoOfreceServicioView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_ofrece_serviciogrid";
        }
        ...
        if (IsPost() && ($this->isGridUpdate() || $this->isMultiUpdate() || $this->isGridOverwrite()) && (Session(SESSION_INLINE_MODE) == "gridedit" || Session(SESSION_INLINE_MODE) == "multiedit")) {
            ...
            if ($gridUpdate) {
                ...
                if ($this->IsModal && !$this->UseAjaxActions) {
                    $this->terminate("ScoOfreceServicioList");
                    return;
                }
            } else {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "sco_proveedor") {
            $masterTbl = Container("sco_proveedor");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("ScoProveedorList"); // Return to master page
                return;
            } else {
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->Nfrece_servicio->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if (
            $CurrentForm->hasValue("x_servicio") &&
            $CurrentForm->hasValue("o_servicio") &&
            $this->servicio->CurrentValue != $this->servicio->DefaultValue &&
            !($this->servicio->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->servicio->CurrentValue == $this->servicio->getSessionValue())
        ) {
            return false;
        }
        return true;
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Nfrece_servicio->AdvancedSearch->toJson(), ","); // Field Nfrece_servicio
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_ofrece_serviciosrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Nfrece_servicio
        $this->Nfrece_servicio->AdvancedSearch->SearchValue = @$filter["x_Nfrece_servicio"];
        $this->Nfrece_servicio->AdvancedSearch->SearchOperator = @$filter["z_Nfrece_servicio"];
        $this->Nfrece_servicio->AdvancedSearch->SearchCondition = @$filter["v_Nfrece_servicio"];
        $this->Nfrece_servicio->AdvancedSearch->SearchValue2 = @$filter["y_Nfrece_servicio"];
        $this->Nfrece_servicio->AdvancedSearch->SearchOperator2 = @$filter["w_Nfrece_servicio"];
        $this->Nfrece_servicio->AdvancedSearch->save();
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_ofrece_servicio\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_ofrece_serviciolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_ofrece_serviciolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Nfrece_servicio->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_ofrece_servicio\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_ofrece_serviciosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_ofrece_serviciosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit() && !$this->isMultiEdit()) { // Not grid add/grid edit/multi edit mode
            ...
            foreach ($this->ListActions as $listAction) {
                if ($listAction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_ofrece_serviciolist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
            ...
            if ($this->isGridAdd()) {
                ...
                if (!($this->ModalGridAdd && !IsMobile())) {
                    ...
                    $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" form=\"fsco_ofrece_serviciolist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridInsertLink") . "</button>";
                    // Add grid cancel
            ...
            if ($this->isGridEdit()) {
                ...
                if (!($this->ModalGridEdit && !IsMobile())) {
                    ...
                    $option->UseDropDownButton = false;
                        ...
                        $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" form=\"fsco_ofrece_serviciolist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridSaveLink") . "</button>";
                        $item = &$option->add("gridcancel");
                        $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_ofrece_servicio", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_ofrece_servicio",
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_servicio")) {
            $this->servicio->setOldValue($CurrentForm->getValue("o_servicio"));
        }

        // Check field name 'Nfrece_servicio' first before field var 'x_Nfrece_servicio'
        $val = $CurrentForm->hasValue("Nfrece_servicio") ? $CurrentForm->getValue("Nfrece_servicio") : $CurrentForm->getValue("x_Nfrece_servicio");
        if (!$this->Nfrece_servicio->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->Nfrece_servicio->setFormValue($val);
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->Nfrece_servicio->CurrentValue = $this->Nfrece_servicio->FormValue;
        }
    ...
    public function loadRowValues($row = null)
    {
        ...
        $this->Nfrece_servicio->setDbValue($row['Nfrece_servicio']);
    ...
    {
        $row = [];
        $row['Nfrece_servicio'] = $this->Nfrece_servicio->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            // Nfrece_servicio
            $this->Nfrece_servicio->ViewValue = $this->Nfrece_servicio->CurrentValue;
            ...
            if ($this->tipo->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->tipo->Lookup->getTable()->Fields["Nservicio_tipo"]->searchExpression(), "=", $this->tipo->CurrentValue, $this->tipo->Lookup->getTable()->Fields["Nservicio_tipo"]->searchDataType(), "");
                }
    ...
    protected function editRow()
    {
        ...
        // Check referential integrity for master table 'sco_proveedor'
        $detailKeys = [];
        $keyValue = $rsnew['proveedor'] ?? $rsold['proveedor'];
        $detailKeys['proveedor'] = $keyValue;
        $masterTable = Container("sco_proveedor");
        ...
        if (!$validMasterRecord) {
            $relatedRecordMsg = str_replace("%t", "sco_proveedor", $Language->phrase("RelatedRecordRequired"));
    ...
    public function getRowHash($row)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['tipo']); // tipo
        $hash .= GetFieldHash($row['servicio']); // servicio
        return md5($hash);
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'sco_ofrece_servicio'
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_ofrece_serviciosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "sco_proveedor") {
                $validMaster = true;
                $masterTbl = Container("sco_proveedor");
                if (($parm = Get("fk_Nproveedor", Get("proveedor"))) !== null) {
                    $masterTbl->Nproveedor->setQueryStringValue($parm);
                    $this->proveedor->QueryStringValue = $masterTbl->Nproveedor->QueryStringValue; // DO NOT change, master/detail key data type can be different
                    $this->proveedor->setSessionValue($this->proveedor->QueryStringValue);
                    $foreignKeys["proveedor"] = $this->proveedor->QueryStringValue;
                    if (!is_numeric($masterTbl->Nproveedor->QueryStringValue)) {
                        $validMaster = false;
                    }
                } else {
                    $validMaster = false;
            ...
            if ($masterTblVar == "sco_proveedor") {
                $validMaster = true;
                $masterTbl = Container("sco_proveedor");
                if (($parm = Post("fk_Nproveedor", Post("proveedor"))) !== null) {
                    $masterTbl->Nproveedor->setFormValue($parm);
                    $this->proveedor->FormValue = $masterTbl->Nproveedor->FormValue;
                    $this->proveedor->setSessionValue($this->proveedor->FormValue);
                    $foreignKeys["proveedor"] = $this->proveedor->FormValue;
                    if (!is_numeric($masterTbl->Nproveedor->FormValue)) {
                        $validMaster = false;
                    }
                } else {
                    $validMaster = false;
