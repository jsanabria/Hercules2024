 ...
 * Page class
 */
class ScoUserEdit extends ScoUser
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoUserEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoUserEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Nuser") ?? Key(0) ?? Route(2)) !== null) {
                $this->Nuser->setQueryStringValue($keyValue);
                $this->Nuser->setOldValue($this->Nuser->QueryStringValue);
            } elseif (Post("Nuser") !== null) {
                $this->Nuser->setFormValue(Post("Nuser"));
                $this->Nuser->setOldValue($this->Nuser->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->Nuser->IsDetailKey) {
            $this->Nuser->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->Nuser->CurrentValue = $this->Nuser->FormValue;
    ...
    protected function validateForm()
    {
        ...
        if (in_array("sco_user_nota", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("ScoUserAdjuntoGrid");
        if (in_array("sco_user_adjunto", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function editRow()
    {
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("ScoUserNotaGrid");
            if (in_array("sco_user_nota", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "sco_user_nota"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("ScoUserAdjuntoGrid");
            if (in_array("sco_user_adjunto", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
    ...
    protected function getEditRow($rsold)
    {
        ...
        // fecha_ingreso_cia
        $this->fecha_ingreso_cia->setDbValueDef($rsnew, UnFormatDateTime($this->fecha_ingreso_cia->CurrentValue, $this->fecha_ingreso_cia->formatPattern()), $this->fecha_ingreso_cia->ReadOnly);

        // fecha_egreso_cia
        $this->fecha_egreso_cia->setDbValueDef($rsnew, UnFormatDateTime($this->fecha_egreso_cia->CurrentValue, $this->fecha_egreso_cia->formatPattern()), $this->fecha_egreso_cia->ReadOnly);

        // motivo_egreso
        $this->motivo_egreso->setDbValueDef($rsnew, $this->motivo_egreso->CurrentValue, $this->motivo_egreso->ReadOnly);
        ...
        // celular_1
        $this->celular_1->setDbValueDef($rsnew, $this->celular_1->CurrentValue, $this->celular_1->ReadOnly);

        // celular_2
        $this->celular_2->setDbValueDef($rsnew, $this->celular_2->CurrentValue, $this->celular_2->ReadOnly);

        // telefono_1
        $this->telefono_1->setDbValueDef($rsnew, $this->telefono_1->CurrentValue, $this->telefono_1->ReadOnly);

        // email
        $this->_email->setDbValueDef($rsnew, $this->_email->CurrentValue, $this->_email->ReadOnly);

        // hora_entrada
        $this->hora_entrada->setDbValueDef($rsnew, $this->hora_entrada->CurrentValue, $this->hora_entrada->ReadOnly);

        // hora_salida
        $this->hora_salida->setDbValueDef($rsnew, $this->hora_salida->CurrentValue, $this->hora_salida->ReadOnly);
        ...
        // evaluacion
        $this->evaluacion->setDbValueDef($rsnew, $this->evaluacion->CurrentValue, $this->evaluacion->ReadOnly);
        return $rsnew;
    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['fecha_ingreso_cia'])) { // fecha_ingreso_cia
            $this->fecha_ingreso_cia->CurrentValue = $row['fecha_ingreso_cia'];
        }
        if (isset($row['fecha_egreso_cia'])) { // fecha_egreso_cia
            $this->fecha_egreso_cia->CurrentValue = $row['fecha_egreso_cia'];
        }
        if (isset($row['motivo_egreso'])) { // motivo_egreso
            $this->motivo_egreso->CurrentValue = $row['motivo_egreso'];
        }
        ...
        if (isset($row['celular_1'])) { // celular_1
            $this->celular_1->CurrentValue = $row['celular_1'];
        }
        if (isset($row['celular_2'])) { // celular_2
            $this->celular_2->CurrentValue = $row['celular_2'];
        }
        if (isset($row['telefono_1'])) { // telefono_1
            $this->telefono_1->CurrentValue = $row['telefono_1'];
        }
        if (isset($row['email'])) { // email
            $this->_email->CurrentValue = $row['email'];
        }
        if (isset($row['hora_entrada'])) { // hora_entrada
            $this->hora_entrada->CurrentValue = $row['hora_entrada'];
        }
        if (isset($row['hora_salida'])) { // hora_salida
            $this->hora_salida->CurrentValue = $row['hora_salida'];
        }
        ...
        if (isset($row['evaluacion'])) { // evaluacion
            $this->evaluacion->CurrentValue = $row['evaluacion'];
        }
    }
