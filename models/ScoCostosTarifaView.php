 ...
 * Page class
 */
class ScoCostosTarifaView extends ScoCostosTarifa
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoCostosTarifaView";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoCostosTarifaView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("Ncostos_tarifa") ?? Route("Ncostos_tarifa")) !== null) {
            $this->RecKey["Ncostos_tarifa"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("Ncostos_tarifa") ?? Route("Ncostos_tarifa")) !== null) {
            ...
            $this->RecKey["Ncostos_tarifa"] = $this->Ncostos_tarifa->QueryStringValue;
        } elseif (Post("Ncostos_tarifa") !== null) {
            $this->Ncostos_tarifa->setFormValue(Post("Ncostos_tarifa"));
            $this->RecKey["Ncostos_tarifa"] = $this->Ncostos_tarifa->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->Ncostos_tarifa->setQueryStringValue($keyValue);
            $this->RecKey["Ncostos_tarifa"] = $this->Ncostos_tarifa->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "ScoCostosTarifaList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "ScoCostosTarifaList"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_sco_costos_tarifa_detalle");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("sco_costos_tarifa_detalle", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("ScoCostosTarifaDetalleList?" . Config("TABLE_SHOW_MASTER") . "=sco_costos_tarifa&" . GetForeignKeyUrl("fk_Ncostos_tarifa", $this->Ncostos_tarifa->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_costos_tarifa')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_costos_tarifa_detalle"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_costos_tarifa')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_costos_tarifa_detalle"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
    ...
    public function pageDataRendering(&$header) {
    	// Example:
    	$header = '<button type="button" class="btn btn-primary" id="tarifario" name="tarifario">Mostrar Tarifario</button>';
    }
