 ...
 * Page class
 */
class ScoExpedienteEstatusGrid extends ScoExpedienteEstatus
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoExpedienteEstatusGrid";
    ...
    public $FormName = "fsco_expediente_estatusgrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoExpedienteEstatusGrid";
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->expediente->CurrentValue;
                    if ($key != "") {
                        $key .= Config("COMPOSITE_KEY_SEPARATOR");
                    }
                    $key .= $this->estatus->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if (
            $CurrentForm->hasValue("x_halcon") &&
            $CurrentForm->hasValue("o_halcon") &&
            $this->halcon->CurrentValue != $this->halcon->DefaultValue &&
            !($this->halcon->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->halcon->CurrentValue == $this->halcon->getSessionValue())
        ) {
            return false;
        }
        return true;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'halcon' first before field var 'x_halcon'
        $val = $CurrentForm->hasValue("halcon") ? $CurrentForm->getValue("halcon") : $CurrentForm->getValue("x_halcon");
        if (!$this->halcon->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->halcon->Visible = false; // Disable update for API request
            } else {
                $this->halcon->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_halcon")) {
            $this->halcon->setOldValue($CurrentForm->getValue("o_halcon"));
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->halcon->CurrentValue = $this->halcon->FormValue;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $curVal = trim(strval($this->halcon->CurrentValue));
            if ($curVal != "") {
                $this->halcon->ViewValue = $this->halcon->lookupCacheOption($curVal);
            } else {
                $this->halcon->ViewValue = $this->halcon->Lookup !== null && is_array($this->halcon->lookupOptions()) && count($this->halcon->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->halcon->ViewValue !== null) { // Load from cache
                $this->halcon->EditValue = array_values($this->halcon->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->halcon->Lookup->getTable()->Fields["username"]->searchExpression(), "=", $this->halcon->CurrentValue, $this->halcon->Lookup->getTable()->Fields["username"]->searchDataType(), "");
                }
                $lookupFilter = $this->halcon->getSelectFilter($this); // PHP
                $sqlWrk = $this->halcon->Lookup->getSql(true, $filterWrk, $lookupFilter, $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCache($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->halcon->Visible && $this->halcon->Required) {
                if (!$this->halcon->IsDetailKey && EmptyValue($this->halcon->FormValue)) {
                    $this->halcon->addErrorMessage(str_replace("%s", $this->halcon->caption(), $this->halcon->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // halcon
        $this->halcon->setDbValueDef($rsnew, $this->halcon->CurrentValue, $this->halcon->ReadOnly);
        return $rsnew;
    }
    ...
    {
        if (isset($row['halcon'])) { // halcon
            $this->halcon->CurrentValue = $row['halcon'];
        }
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($insertRow && $this->ValidateKey && strval($rsnew['expediente']) == "") {
            $this->setFailureMessage($Language->phrase("InvalidKeyValue"));
            $insertRow = false;
        }

        // Check if key value entered
        if ($insertRow && $this->ValidateKey && strval($rsnew['estatus']) == "") {
            $this->setFailureMessage($Language->phrase("InvalidKeyValue"));
            $insertRow = false;
        }
    ...
    protected function getAddRow()
    {
        ...
        // halcon
        $this->halcon->setDbValueDef($rsnew, $this->halcon->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['halcon'])) { // halcon
            $this->halcon->setFormValue($row['halcon']);
        }
