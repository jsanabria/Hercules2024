...
/**
 * Table class for sco_email_cpf
 */
class ScoEmailCpf extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "sco_email_cpf";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // Nemail_cpf
        $this->Nemail_cpf = new DbField(
            $this, // Table
            'x_Nemail_cpf', // Variable name
            'Nemail_cpf', // Name
            '`Nemail_cpf`', // Expression
            '`Nemail_cpf`', // Basic search expression
            20, // Type
            20, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Nemail_cpf`', // Virtual expression
            false, // Is virtual
        ...
        $this->Nemail_cpf->InputTextType = "text";
        $this->Nemail_cpf->Raw = true;
        $this->Nemail_cpf->IsAutoIncrement = true; // Autoincrement field
        $this->Nemail_cpf->IsPrimaryKey = true; // Primary key field
        $this->Nemail_cpf->Nullable = false; // NOT NULL field
        $this->Nemail_cpf->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Nemail_cpf->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['Nemail_cpf'] = &$this->Nemail_cpf;
        ...
        $this->expediente->Lookup = new Lookup($this->expediente, 'sco_expediente', false, 'Nexpediente', ["nombre_contacto","email","",""], '', '', [], [], [], [], [], [], false, '', '', "CONCAT(COALESCE(`nombre_contacto`, ''),'" . ValueSeparator(1, $this->expediente) . "',COALESCE(`email`,''))");
        ...
        $this->estatus->Lookup = new Lookup($this->estatus, 'sco_email_cpf', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "sco_email_cpf";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->Nemail_cpf->setDbValue($conn->lastInsertId());
            $rs['Nemail_cpf'] = $this->Nemail_cpf->DbValue;
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['Nemail_cpf']) && !EmptyValue($this->Nemail_cpf->CurrentValue)) {
                $rs['Nemail_cpf'] = $this->Nemail_cpf->CurrentValue;
            }
        }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('Nemail_cpf', $rs)) {
                AddFilter($where, QuotedName('Nemail_cpf', $this->Dbid) . '=' . QuotedValue($rs['Nemail_cpf'], $this->Nemail_cpf->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->Nemail_cpf->DbValue = $row['Nemail_cpf'];
    ...
    {
        $keys = [];
        $val = $current ? $this->Nemail_cpf->CurrentValue : $this->Nemail_cpf->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->Nemail_cpf->CurrentValue = $keys[0];
            } else {
                $this->Nemail_cpf->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('Nemail_cpf', $row) ? $row['Nemail_cpf'] : null;
        } else {
            $val = !EmptyValue($this->Nemail_cpf->OldValue) && !$current ? $this->Nemail_cpf->OldValue : $this->Nemail_cpf->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@Nemail_cpf@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ScoEmailCpfList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ScoEmailCpfView" => $Language->phrase("View"),
            "ScoEmailCpfEdit" => $Language->phrase("Edit"),
            "ScoEmailCpfAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ScoEmailCpfView",
            Config("API_ADD_ACTION") => "ScoEmailCpfAdd",
            Config("API_EDIT_ACTION") => "ScoEmailCpfEdit",
            Config("API_DELETE_ACTION") => "ScoEmailCpfDelete",
            Config("API_LIST_ACTION") => "ScoEmailCpfList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ScoEmailCpfView", $parm);
        } else {
            $url = $this->keyUrl("ScoEmailCpfView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoEmailCpfEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoEmailCpfList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoEmailCpfAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoEmailCpfList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ScoEmailCpfDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"Nemail_cpf\":" . VarToJson($this->Nemail_cpf->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->Nemail_cpf->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->Nemail_cpf->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("Nemail_cpf") ?? Route("Nemail_cpf")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->Nemail_cpf->CurrentValue = $key;
            } else {
                $this->Nemail_cpf->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        $this->Nemail_cpf->EditValue = $this->Nemail_cpf->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->Nemail_cpf);
        ...
        while (($row = $result->fetch()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->Nemail_cpf);
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'sco_email_cpf');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'sco_email_cpf', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'sco_email_cpf', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'sco_email_cpf', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowRendering() {
    	...
    	if ($this->PageID == "list" || $this->PageID == "view") { // List/View page only
    		if($this->estatus->CurrentValue == "PROCESADO") $color = '#51aa51';
    		else $color = '#FFFF66';
