 ...
 * Page class
 */
class ScoOrdenSalidaList extends ScoOrdenSalida
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoOrdenSalidaList";
    ...
    public $FormName = "fsco_orden_salidalist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoOrdenSalidaList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ScoOrdenSalidaAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoOrdenSalidaDelete";
        $this->MultiUpdateUrl = "ScoOrdenSalidaUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_orden_salidagrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Norden_salida->AdvancedSearch->toJson(), ","); // Field Norden_salida
        ...
        $filterList = Concat($filterList, $this->acompanantes->AdvancedSearch->toJson(), ","); // Field acompanantes
        ...
        $filterList = Concat($filterList, $this->autoriza->AdvancedSearch->toJson(), ","); // Field autoriza
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_orden_salidasrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Norden_salida
        $this->Norden_salida->AdvancedSearch->SearchValue = @$filter["x_Norden_salida"];
        $this->Norden_salida->AdvancedSearch->SearchOperator = @$filter["z_Norden_salida"];
        $this->Norden_salida->AdvancedSearch->SearchCondition = @$filter["v_Norden_salida"];
        $this->Norden_salida->AdvancedSearch->SearchValue2 = @$filter["y_Norden_salida"];
        $this->Norden_salida->AdvancedSearch->SearchOperator2 = @$filter["w_Norden_salida"];
        $this->Norden_salida->AdvancedSearch->save();
        ...
        // Field acompanantes
        $this->acompanantes->AdvancedSearch->SearchValue = @$filter["x_acompanantes"];
        $this->acompanantes->AdvancedSearch->SearchOperator = @$filter["z_acompanantes"];
        $this->acompanantes->AdvancedSearch->SearchCondition = @$filter["v_acompanantes"];
        $this->acompanantes->AdvancedSearch->SearchValue2 = @$filter["y_acompanantes"];
        $this->acompanantes->AdvancedSearch->SearchOperator2 = @$filter["w_acompanantes"];
        $this->acompanantes->AdvancedSearch->save();
        ...
        // Field autoriza
        $this->autoriza->AdvancedSearch->SearchValue = @$filter["x_autoriza"];
        $this->autoriza->AdvancedSearch->SearchOperator = @$filter["z_autoriza"];
        $this->autoriza->AdvancedSearch->SearchCondition = @$filter["v_autoriza"];
        $this->autoriza->AdvancedSearch->SearchValue2 = @$filter["y_autoriza"];
        $this->autoriza->AdvancedSearch->SearchOperator2 = @$filter["w_autoriza"];
        $this->autoriza->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Norden_salida, $default, false); // Norden_salida
        ...
        $this->buildSearchSql($where, $this->acompanantes, $default, false); // acompanantes
        $this->buildSearchSql($where, $this->placa, $default, false); // placa
        $this->buildSearchSql($where, $this->motivo, $default, false); // motivo
        $this->buildSearchSql($where, $this->observaciones, $default, false); // observaciones
        $this->buildSearchSql($where, $this->autoriza, $default, false); // autoriza
        ...
        if (!$default && $this->Command == "search") {
            $this->Norden_salida->AdvancedSearch->save(); // Norden_salida
            ...
            $this->acompanantes->AdvancedSearch->save(); // acompanantes
            ...
            $this->autoriza->AdvancedSearch->save(); // autoriza
    ...
    public function showFilterList()
    {
        ...
        // Field Norden_salida
        $filter = $this->queryBuilderWhere("Norden_salida");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->Norden_salida, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->Norden_salida->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field autoriza
        $filter = $this->queryBuilderWhere("autoriza");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->autoriza, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->autoriza->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->Norden_salida->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->acompanantes->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->autoriza->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->Norden_salida->AdvancedSearch->unsetSession();
        ...
        $this->acompanantes->AdvancedSearch->unsetSession();
        ...
        $this->autoriza->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->Norden_salida->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->Norden_salida->Expression . " DESC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->Norden_salida); // Norden_salida
            ...
            $this->updateSort($this->autoriza); // autoriza
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_orden_salida\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_orden_salida\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_orden_salidalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_orden_salidalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Norden_salida->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_orden_salida\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "Norden_salida");
            ...
            $this->createColumnOption($option, "autoriza");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_orden_salidasrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_orden_salidasrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_orden_salidalist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_orden_salida", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_orden_salida",
    ...
    protected function loadSearchValues()
    {
        ...
        // Norden_salida
        if ($this->Norden_salida->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Norden_salida->AdvancedSearch->SearchValue != "" || $this->Norden_salida->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // acompanantes
        if ($this->acompanantes->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->acompanantes->AdvancedSearch->SearchValue != "" || $this->acompanantes->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // observaciones
        if ($this->observaciones->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->observaciones->AdvancedSearch->SearchValue != "" || $this->observaciones->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // autoriza
        if ($this->autoriza->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->autoriza->AdvancedSearch->SearchValue != "" || $this->autoriza->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->Norden_salida->EditValue = $this->Norden_salida->AdvancedSearch->SearchValue;
            $this->Norden_salida->PlaceHolder = RemoveHtml($this->Norden_salida->caption());
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_orden_salidasrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    public function pageDataRendering(&$header) {
        // 1. Definir la consulta para obtener TODOS los parámetros necesarios en UNA SOLA VEZ
        $sql = "SELECT valor1, valor2, valor4 FROM sco_parametro 
                WHERE codigo = '031' ORDER BY valor3 ASC";

        // 2. Ejecutar la consulta para obtener todas las filas.
        // **FUNCIÓN CORRECTA para PHPMaker 2017: ExecuteRows()**
        $rows = ExecuteRows($sql);

        // 3. Verificar si se obtuvieron resultados
        // ExecuteRows() devuelve un array si es exitoso o FALSE si hay un error.
        if ($rows !== FALSE && count($rows) > 0) {
            // 4. Iterar sobre el array de resultados para generar los botones
            foreach ($rows as $row) {
                $valor1_safe = HtmlEncode($row["valor1"]);
                $valor2_safe = HtmlEncode($row["valor2"]);
                $valor4_safe = HtmlEncode($row["valor4"]);
                $header .= '<a href="?myst=' . $valor1_safe . '" class="btn" style="' . $valor4_safe . '">' . $valor2_safe . '</a> ';
            }
        } 

        // 5. Añadir el botón de Reset
        $header .= '<a href="?cmd=reset" class="btn btn-default btn-lg"><span class="icon-reset-search ewIcon"></span></a>';

        // 6. Añadir el botón de Exportar
        $header .= ' <a href="dashboard/ListadoMaster?id=orden_salida" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-export"></span> Exportar a Excel</a> '; 

        // 7. Ocultar la variable de estatus
