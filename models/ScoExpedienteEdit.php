 ...
 * Page class
 */
class ScoExpedienteEdit extends ScoExpediente
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoExpedienteEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoExpedienteEdit";
    ...
    public function run()
    {
        ...
        $this->setVisibility();
        $this->edad_fallecido->Required = false;
        $this->costos->Required = false;
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->edad_fallecido->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->edad_fallecido->setFormValue($val);
            }
        }
        ...
        if (!$this->unir_con_expediente->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->unir_con_expediente->setFormValue($val);
            }
        }
        ...
        if (!$this->fecha_autoriza->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->fecha_autoriza->setFormValue($val);
            }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $curVal = trim(strval($this->unir_con_expediente->CurrentValue));
            if ($curVal != "") {
                $this->unir_con_expediente->ViewValue = $this->unir_con_expediente->lookupCacheOption($curVal);
            } else {
                $this->unir_con_expediente->ViewValue = $this->unir_con_expediente->Lookup !== null && is_array($this->unir_con_expediente->lookupOptions()) && count($this->unir_con_expediente->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->unir_con_expediente->ViewValue !== null) { // Load from cache
                $this->unir_con_expediente->EditValue = array_values($this->unir_con_expediente->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->unir_con_expediente->Lookup->getTable()->Fields["Nexpediente"]->searchExpression(), "=", $this->unir_con_expediente->CurrentValue, $this->unir_con_expediente->Lookup->getTable()->Fields["Nexpediente"]->searchDataType(), "");
                }
                $lookupFilter = $this->unir_con_expediente->getSelectFilter($this); // PHP
                $sqlWrk = $this->unir_con_expediente->Lookup->getSql(true, $filterWrk, $lookupFilter, $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCache($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->unir_con_expediente->EditValue = $arwrk;
            }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->autoriza_cremar->Visible && $this->autoriza_cremar->Required) {
                if (!$this->autoriza_cremar->IsDetailKey && EmptyValue($this->autoriza_cremar->FormValue)) {
        ...
        if (in_array("sco_orden", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("ScoSeguimientoGrid");
        if (in_array("sco_seguimiento", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("ScoExpedienteEstatusGrid");
        if (in_array("sco_expediente_estatus", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        ...
        if (in_array("sco_cremacion_estatus", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("ScoParcelaGrid");
        if (in_array("sco_parcela", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("ScoExpedienteEncuestaCalidadGrid");
        if (in_array("sco_expediente_encuesta_calidad", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("ScoExpedienteCiaGrid");
        if (in_array("sco_expediente_cia", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("ScoEmbalajeGrid");
        if (in_array("sco_embalaje", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("ScoReembolsoGrid");
        if (in_array("sco_reembolso", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("ScoNotaGrid");
        if (in_array("sco_nota", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function editRow()
    {
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("ScoOrdenGrid");
            if (in_array("sco_orden", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "sco_orden"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("ScoSeguimientoGrid");
            if (in_array("sco_seguimiento", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "sco_seguimiento"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("ScoExpedienteEstatusGrid");
            if (in_array("sco_expediente_estatus", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
