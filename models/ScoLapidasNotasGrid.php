 ...
 * Page class
 */
class ScoLapidasNotasGrid extends ScoLapidasNotas
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoLapidasNotasGrid";
    ...
    public $FormName = "fsco_lapidas_notasgrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoLapidasNotasGrid";
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->Nlapidas_notas->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Nlapidas_notas' first before field var 'x_Nlapidas_notas'
        $val = $CurrentForm->hasValue("Nlapidas_notas") ? $CurrentForm->getValue("Nlapidas_notas") : $CurrentForm->getValue("x_Nlapidas_notas");
        if (!$this->Nlapidas_notas->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->Nlapidas_notas->setFormValue($val);
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->Nlapidas_notas->CurrentValue = $this->Nlapidas_notas->FormValue;
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $curVal = strval($this->usuario->CurrentValue);
            if ($curVal != "") {
                $this->usuario->EditValue = $this->usuario->lookupCacheOption($curVal);
                if ($this->usuario->EditValue === null) { // Lookup from database
                    $filterWrk = SearchFilter($this->usuario->Lookup->getTable()->Fields["username"]->searchExpression(), "=", $curVal, $this->usuario->Lookup->getTable()->Fields["username"]->searchDataType(), "");
                    $sqlWrk = $this->usuario->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->usuario->Lookup->renderViewRow($rswrk[0]);
                        $this->usuario->EditValue = $this->usuario->displayValue($arwrk);
                    } else {
                        $this->usuario->EditValue = HtmlEncode($this->usuario->CurrentValue);
                    }
                }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'sco_lapidas_notas'
