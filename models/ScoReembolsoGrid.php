 ...
 * Page class
 */
class ScoReembolsoGrid extends ScoReembolso
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoReembolsoGrid";
    ...
    public $FormName = "fsco_reembolsogrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoReembolsoGrid";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_reembolsogrid";
        }
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->Nreembolso->CurrentValue;
    ...
    public function emptyRow()
    {
        ...
        if (
            $CurrentForm->hasValue("x_nro_ref") &&
            $CurrentForm->hasValue("o_nro_ref") &&
            $this->nro_ref->CurrentValue != $this->nro_ref->DefaultValue &&
            !($this->nro_ref->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->nro_ref->CurrentValue == $this->nro_ref->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_motivo") &&
            $CurrentForm->hasValue("o_motivo") &&
            $this->motivo->CurrentValue != $this->motivo->DefaultValue &&
            !($this->motivo->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->motivo->CurrentValue == $this->motivo->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_estatus") &&
            $CurrentForm->hasValue("o_estatus") &&
            $this->estatus->CurrentValue != $this->estatus->DefaultValue &&
            !($this->estatus->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->estatus->CurrentValue == $this->estatus->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_coordinador") &&
            $CurrentForm->hasValue("o_coordinador") &&
            $this->coordinador->CurrentValue != $this->coordinador->DefaultValue &&
            !($this->coordinador->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->coordinador->CurrentValue == $this->coordinador->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_fecha_pago") &&
            $CurrentForm->hasValue("o_fecha_pago") &&
            $this->fecha_pago->CurrentValue != $this->fecha_pago->DefaultValue &&
            !($this->fecha_pago->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->fecha_pago->CurrentValue == $this->fecha_pago->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_email_enviado") &&
            $CurrentForm->hasValue("o_email_enviado") &&
            $this->email_enviado->CurrentValue != $this->email_enviado->DefaultValue &&
            !($this->email_enviado->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->email_enviado->CurrentValue == $this->email_enviado->getSessionValue())
        ) {
            return false;
        }
        return true;
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_reembolso\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_reembolso\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->CurrentMode == "view") { // Check view mode
            ...
            if ($this->ModalAdd && !IsMobile()) {
                $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_reembolso\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
            } else {
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_reembolso", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_reembolso",
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'nro_ref' first before field var 'x_nro_ref'
        $val = $CurrentForm->hasValue("nro_ref") ? $CurrentForm->getValue("nro_ref") : $CurrentForm->getValue("x_nro_ref");
        if (!$this->nro_ref->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->nro_ref->Visible = false; // Disable update for API request
            } else {
                $this->nro_ref->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_nro_ref")) {
            $this->nro_ref->setOldValue($CurrentForm->getValue("o_nro_ref"));
        }
        ...
        if ($CurrentForm->hasValue("o_motivo")) {
            $this->motivo->setOldValue($CurrentForm->getValue("o_motivo"));
        }
        ...
        if ($CurrentForm->hasValue("o_estatus")) {
            $this->estatus->setOldValue($CurrentForm->getValue("o_estatus"));
        }

        // Check field name 'coordinador' first before field var 'x_coordinador'
        $val = $CurrentForm->hasValue("coordinador") ? $CurrentForm->getValue("coordinador") : $CurrentForm->getValue("x_coordinador");
        if (!$this->coordinador->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->coordinador->Visible = false; // Disable update for API request
            } else {
                $this->coordinador->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_coordinador")) {
            $this->coordinador->setOldValue($CurrentForm->getValue("o_coordinador"));
        }
        ...
        if (!$this->fecha_pago->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->fecha_pago->setFormValue($val, true, $validate);
            }
            $this->fecha_pago->CurrentValue = UnFormatDateTime($this->fecha_pago->CurrentValue, $this->fecha_pago->formatPattern());
        }
        if ($CurrentForm->hasValue("o_fecha_pago")) {
            $this->fecha_pago->setOldValue($CurrentForm->getValue("o_fecha_pago"));
        }

        // Check field name 'email_enviado' first before field var 'x_email_enviado'
        $val = $CurrentForm->hasValue("email_enviado") ? $CurrentForm->getValue("email_enviado") : $CurrentForm->getValue("x_email_enviado");
        if (!$this->email_enviado->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->email_enviado->Visible = false; // Disable update for API request
            } else {
                $this->email_enviado->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_email_enviado")) {
            $this->email_enviado->setOldValue($CurrentForm->getValue("o_email_enviado"));
        }
        ...
        if (!$this->Nreembolso->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->Nreembolso->setFormValue($val);
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->Nreembolso->CurrentValue = $this->Nreembolso->FormValue;
        }
        ...
        $this->nro_ref->CurrentValue = $this->nro_ref->FormValue;
        $this->motivo->CurrentValue = $this->motivo->FormValue;
        $this->estatus->CurrentValue = $this->estatus->FormValue;
        $this->coordinador->CurrentValue = $this->coordinador->FormValue;
        $this->fecha_pago->CurrentValue = $this->fecha_pago->FormValue;
        $this->fecha_pago->CurrentValue = UnFormatDateTime($this->fecha_pago->CurrentValue, $this->fecha_pago->formatPattern());
        $this->email_enviado->CurrentValue = $this->email_enviado->FormValue;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->nro_ref->EditValue = HtmlEncode($this->nro_ref->CurrentValue);
            ...
            $this->coordinador->EditValue = HtmlEncode($this->coordinador->CurrentValue);
            $curVal = strval($this->coordinador->CurrentValue);
            if ($curVal != "") {
                $this->coordinador->EditValue = $this->coordinador->lookupCacheOption($curVal);
                if ($this->coordinador->EditValue === null) { // Lookup from database
                    $filterWrk = SearchFilter($this->coordinador->Lookup->getTable()->Fields["username"]->searchExpression(), "=", $curVal, $this->coordinador->Lookup->getTable()->Fields["username"]->searchDataType(), "");
                    $sqlWrk = $this->coordinador->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->coordinador->Lookup->renderViewRow($rswrk[0]);
                        $this->coordinador->EditValue = $this->coordinador->displayValue($arwrk);
                    } else {
                        $this->coordinador->EditValue = HtmlEncode($this->coordinador->CurrentValue);
                    }
                }
            } else {
                $this->coordinador->EditValue = null;
            }
            $this->coordinador->PlaceHolder = RemoveHtml($this->coordinador->caption());

            // fecha_pago
            $this->fecha_pago->setupEditAttributes();
            $this->fecha_pago->EditValue = HtmlEncode(FormatDateTime($this->fecha_pago->CurrentValue, $this->fecha_pago->formatPattern()));
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->nro_ref->Visible && $this->nro_ref->Required) {
                if (!$this->nro_ref->IsDetailKey && EmptyValue($this->nro_ref->FormValue)) {
                    $this->nro_ref->addErrorMessage(str_replace("%s", $this->nro_ref->caption(), $this->nro_ref->RequiredErrorMessage));
                }
            }
            ...
            if ($this->coordinador->Visible && $this->coordinador->Required) {
                if (!$this->coordinador->IsDetailKey && EmptyValue($this->coordinador->FormValue)) {
                    $this->coordinador->addErrorMessage(str_replace("%s", $this->coordinador->caption(), $this->coordinador->RequiredErrorMessage));
                }
            }
            if ($this->fecha_pago->Visible && $this->fecha_pago->Required) {
                if (!$this->fecha_pago->IsDetailKey && EmptyValue($this->fecha_pago->FormValue)) {
                    $this->fecha_pago->addErrorMessage(str_replace("%s", $this->fecha_pago->caption(), $this->fecha_pago->RequiredErrorMessage));
                }
            }
            if (!CheckDate($this->fecha_pago->FormValue, $this->fecha_pago->formatPattern())) {
                $this->fecha_pago->addErrorMessage($this->fecha_pago->getErrorMessage(false));
            }
            if ($this->email_enviado->Visible && $this->email_enviado->Required) {
                if ($this->email_enviado->FormValue == "") {
                    $this->email_enviado->addErrorMessage(str_replace("%s", $this->email_enviado->caption(), $this->email_enviado->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // nro_ref
        $this->nro_ref->setDbValueDef($rsnew, $this->nro_ref->CurrentValue, $this->nro_ref->ReadOnly);
        ...
        // coordinador
        $this->coordinador->setDbValueDef($rsnew, $this->coordinador->CurrentValue, $this->coordinador->ReadOnly);

        // fecha_pago
        $this->fecha_pago->setDbValueDef($rsnew, UnFormatDateTime($this->fecha_pago->CurrentValue, $this->fecha_pago->formatPattern()), $this->fecha_pago->ReadOnly);

        // email_enviado
        $this->email_enviado->setDbValueDef($rsnew, $this->email_enviado->CurrentValue, $this->email_enviado->ReadOnly);
        return $rsnew;
    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['nro_ref'])) { // nro_ref
            $this->nro_ref->CurrentValue = $row['nro_ref'];
        }
        ...
        if (isset($row['coordinador'])) { // coordinador
            $this->coordinador->CurrentValue = $row['coordinador'];
        }
        if (isset($row['fecha_pago'])) { // fecha_pago
            $this->fecha_pago->CurrentValue = $row['fecha_pago'];
        }
        if (isset($row['email_enviado'])) { // email_enviado
            $this->email_enviado->CurrentValue = $row['email_enviado'];
        }
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "sco_expediente") {
            $this->expediente->Visible = true; // Need to insert foreign key
            $this->expediente->CurrentValue = $this->expediente->getSessionValue();
        }
    ...
    protected function getAddRow()
    {
        ...
        // nro_ref
        $this->nro_ref->setDbValueDef($rsnew, $this->nro_ref->CurrentValue, false);
        ...
        // coordinador
        $this->coordinador->setDbValueDef($rsnew, $this->coordinador->CurrentValue, false);

        // fecha_pago
        $this->fecha_pago->setDbValueDef($rsnew, UnFormatDateTime($this->fecha_pago->CurrentValue, $this->fecha_pago->formatPattern()), false);

        // email_enviado
        $this->email_enviado->setDbValueDef($rsnew, $this->email_enviado->CurrentValue, strval($this->email_enviado->CurrentValue) == "");
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['nro_ref'])) { // nro_ref
            $this->nro_ref->setFormValue($row['nro_ref']);
        }
        if (isset($row['motivo'])) { // motivo
        ...
        if (isset($row['coordinador'])) { // coordinador
            $this->coordinador->setFormValue($row['coordinador']);
        }
        if (isset($row['fecha_pago'])) { // fecha_pago
            $this->fecha_pago->setFormValue($row['fecha_pago']);
        }
        if (isset($row['email_enviado'])) { // email_enviado
            $this->email_enviado->setFormValue($row['email_enviado']);
        }
    ...
    public function pageDataRendering(&$header) {
    	// Example:
    	//$header = "your header";
    	$header = ' <a href="dashboard/listado_master.php?id=compras" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-export"></span> Exportar a Excel</a> '; 
    	$header = '<table align="left" cellpadding="3" cellspacing="3" width="50%">
    					<tr height="30">
    					  	<td bgcolor="red" width="25%" align="center">
    						<b>NUEVO</b></td>
    					  	<td bgcolor="yellow" width="25%" align="center">
    						<b>ANALISIS</b></td>
    					  	<td bgcolor="green" width="25%" align="center">
    						<b>PAGADO</b></td>
    					  	<td bgcolor="black" width="25%" align="center">
    						<b>ANULADO</b></td>
    				    </tr>
    			</table><br><br>';
    }
