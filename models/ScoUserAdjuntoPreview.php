 ...
 * Page class
 */
class ScoUserAdjuntoPreview extends ScoUserAdjunto
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoUserAdjuntoPreview";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoUserAdjuntoPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "sco_user") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=sco_user&" . GetForeignKeyUrl("fk_Nuser", $this->user->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys($keys)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "sco_user") {
            $this->user->setQueryStringValue($keys[0] ?? "");
        }
    }
