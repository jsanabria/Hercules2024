 ...
 * Page class
 */
class ScoParcelaView extends ScoParcela
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoParcelaView";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoParcelaView";
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("Nparcela") ?? Route("Nparcela")) !== null) {
            ...
            $this->RecKey["Nparcela"] = $this->Nparcela->QueryStringValue;
        } elseif (Post("Nparcela") !== null) {
            $this->Nparcela->setFormValue(Post("Nparcela"));
            $this->RecKey["Nparcela"] = $this->Nparcela->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->Nparcela->setQueryStringValue($keyValue);
            $this->RecKey["Nparcela"] = $this->Nparcela->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "ScoParcelaList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "ScoParcelaList"; // No matching record, return to list
                    } else {
