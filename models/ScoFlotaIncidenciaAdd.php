 ...
 * Page class
 */
class ScoFlotaIncidenciaAdd extends ScoFlotaIncidencia
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoFlotaIncidenciaAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoFlotaIncidenciaAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "ScoFlotaIncidenciaView") {
    ...
    {
        $this->fecha_registro->DefaultValue = $this->fecha_registro->getDefault(); // PHP
        $this->fecha_registro->OldValue = $this->fecha_registro->DefaultValue;
        $this->cambio_aceite->DefaultValue = $this->cambio_aceite->getDefault(); // PHP
        $this->cambio_aceite->OldValue = $this->cambio_aceite->DefaultValue;
    }
    ...
    protected function validateForm()
    {
        ...
        if (in_array("sco_flota_incidencia_detalle", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("ScoFlotaIncidenciaDetalleGrid");
            if (in_array("sco_flota_incidencia_detalle", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->flota_incidencia->setSessionValue($this->Nflota_incidencia->CurrentValue); // Set master key
                ...
                if (!$addRow) {
                $detailPage->flota_incidencia->setSessionValue(""); // Clear master key if insert failed
                }
            }
            $detailPage = Container("ScoAdjuntoGrid");
    ...
    protected function getAddRow()
    {
        ...
        // flota
        $this->flota->setDbValueDef($rsnew, $this->flota->CurrentValue, false);

        // tipo
        $this->tipo->setDbValueDef($rsnew, $this->tipo->CurrentValue, false);

        // falla
        $this->falla->setDbValueDef($rsnew, $this->falla->CurrentValue, false);
        ...
        // reparacion
        $this->reparacion->setDbValueDef($rsnew, $this->reparacion->CurrentValue, false);

        // cambio_aceite
        $this->cambio_aceite->setDbValueDef($rsnew, $this->cambio_aceite->CurrentValue, strval($this->cambio_aceite->CurrentValue) == "");

        // kilometraje
        $this->kilometraje->setDbValueDef($rsnew, $this->kilometraje->CurrentValue, false);

        // proveedor
        $this->proveedor->setDbValueDef($rsnew, $this->proveedor->CurrentValue, false);

        // fecha_reparacion
        $this->fecha_reparacion->setDbValueDef($rsnew, UnFormatDateTime($this->fecha_reparacion->CurrentValue, $this->fecha_reparacion->formatPattern()), false);
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['falla'])) { // falla
            $this->falla->setFormValue($row['falla']);
        }
