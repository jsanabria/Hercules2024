 ...
 * Page class
 */
class ScoSeguroEdit extends ScoSeguro
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoSeguroEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoSeguroEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Nseguro") ?? Key(0) ?? Route(2)) !== null) {
                $this->Nseguro->setQueryStringValue($keyValue);
                $this->Nseguro->setOldValue($this->Nseguro->QueryStringValue);
            } elseif (Post("Nseguro") !== null) {
                $this->Nseguro->setFormValue(Post("Nseguro"));
                $this->Nseguro->setOldValue($this->Nseguro->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("Nseguro") ?? Route("Nseguro")) !== null) {
                    $this->Nseguro->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->Nseguro->CurrentValue = null;
                }
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ScoSeguroList"); // Return to list page
                        return;
                        ...
                        if ($this->Nseguro->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->Nseguro->CurrentValue, $this->CurrentRow['Nseguro'])) {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ScoSeguroList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "ScoSeguroList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ScoSeguroList") {
                            ...
                            $returnUrl = "ScoSeguroList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'email3' first before field var 'x_email3'
        $val = $CurrentForm->hasValue("email3") ? $CurrentForm->getValue("email3") : $CurrentForm->getValue("x_email3");
        if (!$this->email3->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->email3->Visible = false; // Disable update for API request
            } else {
                $this->email3->setFormValue($val, true, $validate);
            }
        }
        ...
        // Check field name 'Nseguro' first before field var 'x_Nseguro'
        $val = $CurrentForm->hasValue("Nseguro") ? $CurrentForm->getValue("Nseguro") : $CurrentForm->getValue("x_Nseguro");
        if (!$this->Nseguro->IsDetailKey) {
            $this->Nseguro->setFormValue($val);
        }
    }
    ...
    {
        global $CurrentForm;
        $this->Nseguro->CurrentValue = $this->Nseguro->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->email3->EditValue = HtmlEncode($this->email3->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->email3->Visible && $this->email3->Required) {
                if (!$this->email3->IsDetailKey && EmptyValue($this->email3->FormValue)) {
                    $this->email3->addErrorMessage(str_replace("%s", $this->email3->caption(), $this->email3->RequiredErrorMessage));
                }
            }
            if (!CheckEmail($this->email3->FormValue)) {
                $this->email3->addErrorMessage($this->email3->getErrorMessage(false));
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // email3
        $this->email3->setDbValueDef($rsnew, $this->email3->CurrentValue, $this->email3->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['email3'])) { // email3
            $this->email3->CurrentValue = $row['email3'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ScoSeguroList"), "", $this->TableVar, true);
        $pageId = "edit";
