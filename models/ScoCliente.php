...
/**
 * Table class for sco_cliente
 */
class ScoCliente extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Ncliente
        $this->Ncliente = new DbField(
            $this, // Table
            'x_Ncliente', // Variable name
            'Ncliente', // Name
            '`Ncliente`', // Expression
            '`Ncliente`', // Basic search expression
            19, // Type
            10, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Ncliente`', // Virtual expression
            false, // Is virtual
        ...
        $this->Ncliente->InputTextType = "text";
        $this->Ncliente->Raw = true;
        $this->Ncliente->IsAutoIncrement = true; // Autoincrement field
        $this->Ncliente->IsPrimaryKey = true; // Primary key field
        $this->Ncliente->Nullable = false; // NOT NULL field
        $this->Ncliente->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Ncliente->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['Ncliente'] = &$this->Ncliente;

        // rif_ci
        $this->rif_ci = new DbField(
            $this, // Table
            'x_rif_ci', // Variable name
            'rif_ci', // Name
            '`rif_ci`', // Expression
            '`rif_ci`', // Basic search expression
            200, // Type
            15, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`rif_ci`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->rif_ci->addMethod("getViewCustomAttributes", fn() =>  'class="convertuppercase" onchange="func_convert_uppercase(this)" autocomplete="off" ');
        $this->rif_ci->InputTextType = "text";
        $this->rif_ci->Required = true; // Required field
        $this->rif_ci->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['rif_ci'] = &$this->rif_ci;
        ...
        $this->activo->Lookup = new Lookup($this->activo, 'sco_cliente', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "sco_cliente";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->Ncliente->setDbValue($conn->lastInsertId());
            $rs['Ncliente'] = $this->Ncliente->DbValue;
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['Ncliente']) && !EmptyValue($this->Ncliente->CurrentValue)) {
                $rs['Ncliente'] = $this->Ncliente->CurrentValue;
            }
        }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('Ncliente', $rs)) {
                AddFilter($where, QuotedName('Ncliente', $this->Dbid) . '=' . QuotedValue($rs['Ncliente'], $this->Ncliente->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->Ncliente->DbValue = $row['Ncliente'];
        $this->rif_ci->DbValue = $row['rif_ci'];
    ...
    {
        $keys = [];
        $val = $current ? $this->Ncliente->CurrentValue : $this->Ncliente->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->Ncliente->CurrentValue = $keys[0];
            } else {
                $this->Ncliente->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('Ncliente', $row) ? $row['Ncliente'] : null;
        } else {
            $val = !EmptyValue($this->Ncliente->OldValue) && !$current ? $this->Ncliente->OldValue : $this->Ncliente->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@Ncliente@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ScoClienteList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ScoClienteView" => $Language->phrase("View"),
            "ScoClienteEdit" => $Language->phrase("Edit"),
            "ScoClienteAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ScoClienteView",
            Config("API_ADD_ACTION") => "ScoClienteAdd",
            Config("API_EDIT_ACTION") => "ScoClienteEdit",
            Config("API_DELETE_ACTION") => "ScoClienteDelete",
            Config("API_LIST_ACTION") => "ScoClienteList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ScoClienteView", $parm);
        } else {
            $url = $this->keyUrl("ScoClienteView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoClienteEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoClienteList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoClienteAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoClienteList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ScoClienteDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"Ncliente\":" . VarToJson($this->Ncliente->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->Ncliente->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->Ncliente->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("Ncliente") ?? Route("Ncliente")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->Ncliente->CurrentValue = $key;
            } else {
                $this->Ncliente->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        $this->Ncliente->EditValue = $this->Ncliente->CurrentValue;

        // rif_ci
        $this->rif_ci->setupEditAttributes();
        if (!$this->rif_ci->Raw) {
            $this->rif_ci->CurrentValue = HtmlDecode($this->rif_ci->CurrentValue);
        }
        $this->rif_ci->EditValue = $this->rif_ci->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->Ncliente);
                    $doc->exportCaption($this->rif_ci);
        ...
        while (($row = $result->fetch()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->Ncliente);
                        $doc->exportField($this->rif_ci);
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'sco_cliente');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'sco_cliente', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'sco_cliente', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'sco_cliente', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowInserting($rsold, &$rsnew) {
    	...
    	$rsnew["rif_ci"] = strtoupper($rsnew["rif_ci"]);
    	return TRUE;
    }
    ...
    public function rowDeleting(&$rs) {
    	// Enter your code here
    	// To cancel, set return value to False
    	$sql = "select count(*) as total from sco_contrato where cliente = '".$rs["Ncliente"]."';";
    	$total = ExecuteScalar($sql);
    	if($total>0)
    	{
    		$this->CancelMessage = "Hay contratos asociados a este cliente, no se puede eliminar.";
    		return FALSE;
    	}
    	return TRUE;
    }
