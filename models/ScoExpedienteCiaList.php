 ...
 * Page class
 */
class ScoExpedienteCiaList extends ScoExpedienteCia
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoExpedienteCiaList";
    ...
    public $FormName = "fsco_expediente_cialist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoExpedienteCiaList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ScoExpedienteCiaAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoExpedienteCiaDelete";
        $this->MultiUpdateUrl = "ScoExpedienteCiaUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_expediente_ciagrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Nexpediente_cia->AdvancedSearch->toJson(), ","); // Field Nexpediente_cia
        $filterList = Concat($filterList, $this->cia->AdvancedSearch->toJson(), ","); // Field cia
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_expediente_ciasrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Nexpediente_cia
        $this->Nexpediente_cia->AdvancedSearch->SearchValue = @$filter["x_Nexpediente_cia"];
        $this->Nexpediente_cia->AdvancedSearch->SearchOperator = @$filter["z_Nexpediente_cia"];
        $this->Nexpediente_cia->AdvancedSearch->SearchCondition = @$filter["v_Nexpediente_cia"];
        $this->Nexpediente_cia->AdvancedSearch->SearchValue2 = @$filter["y_Nexpediente_cia"];
        $this->Nexpediente_cia->AdvancedSearch->SearchOperator2 = @$filter["w_Nexpediente_cia"];
        $this->Nexpediente_cia->AdvancedSearch->save();

        // Field cia
        $this->cia->AdvancedSearch->SearchValue = @$filter["x_cia"];
        $this->cia->AdvancedSearch->SearchOperator = @$filter["z_cia"];
        $this->cia->AdvancedSearch->SearchCondition = @$filter["v_cia"];
        $this->cia->AdvancedSearch->SearchValue2 = @$filter["y_cia"];
        $this->cia->AdvancedSearch->SearchOperator2 = @$filter["w_cia"];
        $this->cia->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Nexpediente_cia, $default, false); // Nexpediente_cia
        $this->buildSearchSql($where, $this->cia, $default, false); // cia
        ...
        if (!$default && $this->Command == "search") {
            $this->Nexpediente_cia->AdvancedSearch->save(); // Nexpediente_cia
            $this->cia->AdvancedSearch->save(); // cia
    ...
    public function showFilterList()
    {
        ...
        // Field cia
        $filter = $this->queryBuilderWhere("cia");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->cia, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->cia->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->Nexpediente_cia->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->cia->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->Nexpediente_cia->AdvancedSearch->unsetSession();
        $this->cia->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->Nexpediente_cia->AdvancedSearch->load();
        $this->cia->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->expediente->Expression . " DESC" . ", " . $this->cia->Expression . " ASC"; // Set up default sort
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_expediente_cia\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_expediente_cia\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_expediente_cialist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_expediente_cialist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Nexpediente_cia->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_expediente_cia\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "cia");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_expediente_ciasrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_expediente_ciasrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_expediente_cialist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_expediente_cia", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_expediente_cia",
    ...
    protected function loadSearchValues()
    {
        ...
        // Nexpediente_cia
        if ($this->Nexpediente_cia->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Nexpediente_cia->AdvancedSearch->SearchValue != "" || $this->Nexpediente_cia->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // cia
        if ($this->cia->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->cia->AdvancedSearch->SearchValue != "" || $this->cia->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $curVal = trim(strval($this->cia->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->cia->AdvancedSearch->ViewValue = $this->cia->lookupCacheOption($curVal);
            } else {
                $this->cia->AdvancedSearch->ViewValue = $this->cia->Lookup !== null && is_array($this->cia->lookupOptions()) && count($this->cia->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->cia->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->cia->EditValue = array_values($this->cia->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->cia->Lookup->getTable()->Fields["Ncia"]->searchExpression(), "=", $this->cia->AdvancedSearch->SearchValue, $this->cia->Lookup->getTable()->Fields["Ncia"]->searchDataType(), "");
                }
            ...
            $curVal = strval($this->expediente->AdvancedSearch->SearchValue);
            if ($curVal != "") {
                ...
                if ($this->expediente->EditValue === null) { // Lookup from database
                    ...
                    if ($ari > 0) { // Lookup values found
                        ...
                        $this->expediente->EditValue = HtmlEncode($this->expediente->AdvancedSearch->SearchValue);
                    }
                }
            } else {
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckInteger($this->expediente->AdvancedSearch->SearchValue)) {
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fsco_expediente_cialist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_expediente_ciasrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($doc)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "sco_expediente") {
            $sco_expediente = new ScoExpedienteList();
            $rsmaster = $sco_expediente->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->setTable($sco_expediente);
                    $sco_expediente->exportDocument($doc, $rsmaster);
                    $doc->exportEmptyRow();
                    $doc->setTable($this);
                }
