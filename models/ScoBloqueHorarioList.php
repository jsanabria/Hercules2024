 ...
 * Page class
 */
class ScoBloqueHorarioList extends ScoBloqueHorario
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoBloqueHorarioList";
    ...
    public $FormName = "fsco_bloque_horariolist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoBloqueHorarioList";
    ...
    {
        $this->Nbloque_horario->Visible = false;
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'sco_bloque_horario';
        $this->TableName = 'sco_bloque_horario';
        ...
        // Table object (sco_bloque_horario)
        if (!isset($GLOBALS["sco_bloque_horario"]) || $GLOBALS["sco_bloque_horario"]::class == PROJECT_NAMESPACE . "sco_bloque_horario") {
            $GLOBALS["sco_bloque_horario"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ScoBloqueHorarioAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoBloqueHorarioDelete";
        $this->MultiUpdateUrl = "ScoBloqueHorarioUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'sco_bloque_horario');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ScoBloqueHorarioView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        $this->setupLookupOptions($this->bloque);
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_bloque_horariogrid";
        }
        ...
        if (IsPost() && ($this->isGridUpdate() || $this->isMultiUpdate() || $this->isGridOverwrite()) && (Session(SESSION_INLINE_MODE) == "gridedit" || Session(SESSION_INLINE_MODE) == "multiedit")) {
            ...
            if ($gridUpdate) {
                ...
                if ($this->IsModal && !$this->UseAjaxActions) {
                    $this->terminate("ScoBloqueHorarioList");
                    return;
                }
            } else {
    ...
    protected function inlineEditMode()
    {
        ...
        $inlineEdit = true;
        if (($keyValue = Get("Nbloque_horario") ?? Route("Nbloque_horario")) !== null) {
            $this->Nbloque_horario->setQueryStringValue($keyValue);
    ...
    {
        if (!SameString($this->Nbloque_horario->OldValue, $this->Nbloque_horario->CurrentValue)) {
            return false;
        }
        return true;
    }
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->Nbloque_horario->CurrentValue;
    ...
    public function emptyRow()
    {
        global $CurrentForm;
        if (
            $CurrentForm->hasValue("x_servicio_tipo") &&
            $CurrentForm->hasValue("o_servicio_tipo") &&
            $this->servicio_tipo->CurrentValue != $this->servicio_tipo->DefaultValue &&
            !($this->servicio_tipo->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->servicio_tipo->CurrentValue == $this->servicio_tipo->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_hora") &&
            $CurrentForm->hasValue("o_hora") &&
            $this->hora->CurrentValue != $this->hora->DefaultValue &&
            !($this->hora->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->hora->CurrentValue == $this->hora->getSessionValue())
        ) {
            return false;
        }
        if (
            $CurrentForm->hasValue("x_bloque") &&
            $CurrentForm->hasValue("o_bloque") &&
            $this->bloque->CurrentValue != $this->bloque->DefaultValue &&
            !($this->bloque->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->bloque->CurrentValue == $this->bloque->getSessionValue())
        ) {
            return false;
        }
        return true;
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Nbloque_horario->AdvancedSearch->toJson(), ","); // Field Nbloque_horario
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_bloque_horariosrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Nbloque_horario
        $this->Nbloque_horario->AdvancedSearch->SearchValue = @$filter["x_Nbloque_horario"];
        $this->Nbloque_horario->AdvancedSearch->SearchOperator = @$filter["z_Nbloque_horario"];
        $this->Nbloque_horario->AdvancedSearch->SearchCondition = @$filter["v_Nbloque_horario"];
        $this->Nbloque_horario->AdvancedSearch->SearchValue2 = @$filter["y_Nbloque_horario"];
        $this->Nbloque_horario->AdvancedSearch->SearchOperator2 = @$filter["w_Nbloque_horario"];
        $this->Nbloque_horario->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Nbloque_horario, $default, false); // Nbloque_horario
        ...
        $this->buildSearchSql($where, $this->bloque, $default, false); // bloque
        ...
        if (!$default && $this->Command == "search") {
            $this->Nbloque_horario->AdvancedSearch->save(); // Nbloque_horario
            $this->servicio_tipo->AdvancedSearch->save(); // servicio_tipo
            $this->hora->AdvancedSearch->save(); // hora
            $this->bloque->AdvancedSearch->save(); // bloque
    ...
    public function showFilterList()
    {
        ...
        // Field bloque
        $filter = $this->queryBuilderWhere("bloque");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->bloque, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->bloque->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        if ($this->BasicSearch->Keyword != "") {
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->hora;
        $searchFlds[] = &$this->bloque;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->Nbloque_horario->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->bloque->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    {
        $this->Nbloque_horario->AdvancedSearch->unsetSession();
        $this->servicio_tipo->AdvancedSearch->unsetSession();
        $this->hora->AdvancedSearch->unsetSession();
        $this->bloque->AdvancedSearch->unsetSession();
    }
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->Nbloque_horario->AdvancedSearch->load();
        $this->servicio_tipo->AdvancedSearch->load();
        $this->hora->AdvancedSearch->load();
        $this->bloque->AdvancedSearch->load();
    }
    ...
    public function renderListOptions()
    {
        ...
        if ($this->isInlineAddRow() || $this->isInlineCopyRow()) { // Inline Add/Copy
            ...
            if ($this->UseAjaxActions) {
                ...
                $opt->Body = <<<INLINEADD
                    <div{$divClass}>
                        <button class="ew-grid-link ew-inline-insert" title="{$insertTitle}" data-caption="{$insertTitle}" form="fsco_bloque_horariolist" formaction="{$inlineInsertUrl}">{$insertCaption}</button>
        ...
        if ($this->isInlineEditRow()) { // Inline-Edit
            ...
            $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
                ...
                if ($this->UseAjaxActions) {
                    ...
                    $opt->Body = <<<INLINEEDIT
                    <div{$divClass}>
                        <button class="ew-grid-link ew-inline-update" title="{$updateTitle}" data-caption="{$updateTitle}" form="fsco_bloque_horariolist" formaction="{$inlineUpdateUrl}">{$updateCaption}</button>
            ...
            $opt->Body .= "<input type=\"hidden\" name=\"" . $this->OldKeyName . "\" id=\"" . $this->OldKeyName . "\" value=\"" . HtmlEncode($this->Nbloque_horario->CurrentValue) . "\">";
            return;
        }
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_bloque_horario\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_bloque_horario\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_bloque_horariolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_bloque_horariolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Nbloque_horario->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_bloque_horario\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "bloque");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_bloque_horariosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_bloque_horariosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit() && !$this->isMultiEdit()) { // Not grid add/grid edit/multi edit mode
            ...
            foreach ($this->ListActions as $listAction) {
                if ($listAction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_bloque_horariolist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
            ...
            if ($this->isGridAdd()) {
                ...
                if (!($this->ModalGridAdd && !IsMobile())) {
                    ...
                    $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" form=\"fsco_bloque_horariolist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridInsertLink") . "</button>";
                    // Add grid cancel
            ...
            if ($this->isGridEdit()) {
                ...
                if (!($this->ModalGridEdit && !IsMobile())) {
                    ...
                    $option->UseDropDownButton = false;
                        ...
                        $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" form=\"fsco_bloque_horariolist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridSaveLink") . "</button>";
                        $item = &$option->add("gridcancel");
                        $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_bloque_horario", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_bloque_horario",
    ...
    protected function loadSearchValues()
    {
        ...
        // Nbloque_horario
        if ($this->Nbloque_horario->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Nbloque_horario->AdvancedSearch->SearchValue != "" || $this->Nbloque_horario->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // bloque
        if ($this->bloque->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->bloque->AdvancedSearch->SearchValue != "" || $this->bloque->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_servicio_tipo")) {
            $this->servicio_tipo->setOldValue($CurrentForm->getValue("o_servicio_tipo"));
        }
        ...
        if ($CurrentForm->hasValue("o_bloque")) {
            $this->bloque->setOldValue($CurrentForm->getValue("o_bloque"));
        }

        // Check field name 'Nbloque_horario' first before field var 'x_Nbloque_horario'
        $val = $CurrentForm->hasValue("Nbloque_horario") ? $CurrentForm->getValue("Nbloque_horario") : $CurrentForm->getValue("x_Nbloque_horario");
        if (!$this->Nbloque_horario->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->Nbloque_horario->setFormValue($val);
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->Nbloque_horario->CurrentValue = $this->Nbloque_horario->FormValue;
        }
    ...
    public function loadRowValues($row = null)
    {
        ...
        $this->Nbloque_horario->setDbValue($row['Nbloque_horario']);
    ...
    {
        $row = [];
        $row['Nbloque_horario'] = $this->Nbloque_horario->DefaultValue;
        $row['servicio_tipo'] = $this->servicio_tipo->DefaultValue;
        $row['hora'] = $this->hora->DefaultValue;
        $row['bloque'] = $this->bloque->DefaultValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            // Nbloque_horario
            $this->Nbloque_horario->ViewValue = $this->Nbloque_horario->CurrentValue;
            ...
            // hora
            if (strval($this->hora->CurrentValue) != "") {
                $this->hora->ViewValue = $this->hora->optionCaption($this->hora->CurrentValue);
            } else {
                $this->hora->ViewValue = null;
            }

            // bloque
            if (strval($this->bloque->CurrentValue) != "") {
                $this->bloque->ViewValue = $this->bloque->optionCaption($this->bloque->CurrentValue);
            } else {
                $this->bloque->ViewValue = null;
            }
            ...
            $this->hora->EditValue = $this->hora->options(true);
            $this->hora->PlaceHolder = RemoveHtml($this->hora->caption());

            // bloque
            $this->bloque->setupEditAttributes();
            $this->bloque->EditValue = $this->bloque->options(true);
            ...
            // servicio_tipo
            $this->servicio_tipo->setupEditAttributes();
            $curVal = trim(strval($this->servicio_tipo->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->servicio_tipo->AdvancedSearch->ViewValue = $this->servicio_tipo->lookupCacheOption($curVal);
            } else {
                $this->servicio_tipo->AdvancedSearch->ViewValue = $this->servicio_tipo->Lookup !== null && is_array($this->servicio_tipo->lookupOptions()) && count($this->servicio_tipo->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->servicio_tipo->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->servicio_tipo->EditValue = array_values($this->servicio_tipo->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->servicio_tipo->Lookup->getTable()->Fields["Nservicio_tipo"]->searchExpression(), "=", $this->servicio_tipo->AdvancedSearch->SearchValue, $this->servicio_tipo->Lookup->getTable()->Fields["Nservicio_tipo"]->searchDataType(), "");
                }
                $lookupFilter = $this->servicio_tipo->getSelectFilter($this); // PHP
                $sqlWrk = $this->servicio_tipo->Lookup->getSql(true, $filterWrk, $lookupFilter, $this, false, true);
                $conn = Conn();
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->bloque->Visible && $this->bloque->Required) {
                if (!$this->bloque->IsDetailKey && EmptyValue($this->bloque->FormValue)) {
                    $this->bloque->addErrorMessage(str_replace("%s", $this->bloque->caption(), $this->bloque->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // hora
        $this->hora->setDbValueDef($rsnew, $this->hora->CurrentValue, $this->hora->ReadOnly);

        // bloque
        $this->bloque->setDbValueDef($rsnew, $this->bloque->CurrentValue, $this->bloque->ReadOnly);
        return $rsnew;
    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['bloque'])) { // bloque
            $this->bloque->CurrentValue = $row['bloque'];
        }
    }
    ...
    public function getRowHash($row)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['servicio_tipo']); // servicio_tipo
        $hash .= GetFieldHash($row['hora']); // hora
        $hash .= GetFieldHash($row['bloque']); // bloque
        return md5($hash);
    }
    ...
    protected function getAddRow()
    {
        ...
        // hora
        $this->hora->setDbValueDef($rsnew, $this->hora->CurrentValue, false);

        // bloque
        $this->bloque->setDbValueDef($rsnew, $this->bloque->CurrentValue, false);
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['hora'])) { // hora
            $this->hora->setFormValue($row['hora']);
        }
        if (isset($row['bloque'])) { // bloque
            $this->bloque->setFormValue($row['bloque']);
        }
    }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_bloque_horariosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
