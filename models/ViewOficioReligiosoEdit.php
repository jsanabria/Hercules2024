 ...
 * Page class
 */
class ViewOficioReligiosoEdit extends ViewOficioReligioso
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewOficioReligiosoEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ViewOficioReligiosoEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("Nexpediente") ?? Key(1) ?? Route(3)) !== null) {
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ViewOficioReligiosoList"); // Return to list page
                        return;
                        ...
                        if ($this->Norden->CurrentValue != null && $this->Nexpediente->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->Norden->CurrentValue, $this->CurrentRow['Norden']) && SameString($this->Nexpediente->CurrentValue, $this->CurrentRow['Nexpediente'])) {
            ...
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ViewOficioReligiosoList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                $returnUrl = "ViewOficioReligiosoList";
                if (GetPageName($returnUrl) == "ViewOficioReligiosoList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ViewOficioReligiosoList") {
                            ...
                            $returnUrl = "ViewOficioReligiosoList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    public function renderRow()
    {
        ...
        // servicio2
        $this->servicio2->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // ministro
            $curVal = trim(strval($this->ministro->CurrentValue));
            if ($curVal != "") {
                $this->ministro->ViewValue = $this->ministro->lookupCacheOption($curVal);
            } else {
                $this->ministro->ViewValue = $this->ministro->Lookup !== null && is_array($this->ministro->lookupOptions()) && count($this->ministro->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->ministro->ViewValue !== null) { // Load from cache
                $this->ministro->EditValue = array_values($this->ministro->lookupOptions());
                if ($this->ministro->ViewValue == "") {
                    $this->ministro->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->ministro->Lookup->getTable()->Fields["username"]->searchExpression(), "=", $this->ministro->CurrentValue, $this->ministro->Lookup->getTable()->Fields["username"]->searchDataType(), "");
                }
                $lookupFilter = $this->ministro->getSelectFilter($this); // PHP
                $sqlWrk = $this->ministro->Lookup->getSql(true, $filterWrk, $lookupFilter, $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCache($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
    ...
    protected function getEditRow($rsold)
    {
        ...
        // servicio_atendido
        $this->servicio_atendido->setDbValueDef($rsnew, $this->servicio_atendido->CurrentValue, $this->servicio_atendido->ReadOnly);
        return $rsnew;
    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['servicio_atendido'])) { // servicio_atendido
            $this->servicio_atendido->CurrentValue = $row['servicio_atendido'];
        }
    }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ViewOficioReligiosoList"), "", $this->TableVar, true);
        $pageId = "edit";
    ...
    public function pageDataRendering(&$header) {
    	...
    	$sql = "SELECT 
    				a.cedula_fallecido, a.nombre_fallecido, a.apellidos_fallecido, 
    	...
    	$header = "<h2><strong>Difunto: $nombre_fallecido $apellidos_fallecido</strong></h2>";
    }
