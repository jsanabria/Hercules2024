 ...
 * Page class
 */
class ScoReclamoEdit extends ScoReclamo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoReclamoEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoReclamoEdit";
    ...
    protected function validateForm()
    {
        ...
        if (in_array("sco_reclamo_nota", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("ScoReclamoAdjuntoGrid");
        if (in_array("sco_reclamo_adjunto", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function editRow()
    {
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("ScoReclamoNotaGrid");
            if (in_array("sco_reclamo_nota", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "sco_reclamo_nota"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("ScoReclamoAdjuntoGrid");
            if (in_array("sco_reclamo_adjunto", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
