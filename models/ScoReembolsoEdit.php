 ...
 * Page class
 */
class ScoReembolsoEdit extends ScoReembolso
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoReembolsoEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoReembolsoEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Nreembolso") ?? Key(0) ?? Route(2)) !== null) {
                $this->Nreembolso->setQueryStringValue($keyValue);
                $this->Nreembolso->setOldValue($this->Nreembolso->QueryStringValue);
            } elseif (Post("Nreembolso") !== null) {
                $this->Nreembolso->setFormValue(Post("Nreembolso"));
                $this->Nreembolso->setOldValue($this->Nreembolso->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("Nreembolso") ?? Route("Nreembolso")) !== null) {
                    ...
                    $this->Nreembolso->CurrentValue = null;
                }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'fecha_tasa' first before field var 'x_fecha_tasa'
        $val = $CurrentForm->hasValue("fecha_tasa") ? $CurrentForm->getValue("fecha_tasa") : $CurrentForm->getValue("x_fecha_tasa");
        if (!$this->fecha_tasa->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->fecha_tasa->Visible = false; // Disable update for API request
            } else {
                $this->fecha_tasa->setFormValue($val, true, $validate);
            }
            $this->fecha_tasa->CurrentValue = UnFormatDateTime($this->fecha_tasa->CurrentValue, $this->fecha_tasa->formatPattern());
        }
        ...
        if (!$this->Nreembolso->IsDetailKey) {
    ...
    public function restoreFormValues()
    {
        ...
        $this->fecha_tasa->CurrentValue = $this->fecha_tasa->FormValue;
        $this->fecha_tasa->CurrentValue = UnFormatDateTime($this->fecha_tasa->CurrentValue, $this->fecha_tasa->formatPattern());
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->fecha_tasa->EditValue = HtmlEncode(FormatDateTime($this->fecha_tasa->CurrentValue, $this->fecha_tasa->formatPattern()));
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->fecha_tasa->Visible && $this->fecha_tasa->Required) {
                if (!$this->fecha_tasa->IsDetailKey && EmptyValue($this->fecha_tasa->FormValue)) {
                    $this->fecha_tasa->addErrorMessage(str_replace("%s", $this->fecha_tasa->caption(), $this->fecha_tasa->RequiredErrorMessage));
                }
            }
            if (!CheckDate($this->fecha_tasa->FormValue, $this->fecha_tasa->formatPattern())) {
                $this->fecha_tasa->addErrorMessage($this->fecha_tasa->getErrorMessage(false));
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // fecha_tasa
        $this->fecha_tasa->setDbValueDef($rsnew, UnFormatDateTime($this->fecha_tasa->CurrentValue, $this->fecha_tasa->formatPattern()), $this->fecha_tasa->ReadOnly);
        ...
        // nro_cta
        $this->nro_cta->setDbValueDef($rsnew, $this->nro_cta->CurrentValue, $this->nro_cta->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['fecha_tasa'])) { // fecha_tasa
            $this->fecha_tasa->CurrentValue = $row['fecha_tasa'];
        }
        ...
        if (isset($row['nro_cta'])) { // nro_cta
            $this->nro_cta->CurrentValue = $row['nro_cta'];
        }
