 ...
 * Page class
 */
class ScoServicioEdit extends ScoServicio
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoServicioEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoServicioEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Nservicio") ?? Key(0) ?? Route(2)) !== null) {
                $this->Nservicio->setQueryStringValue($keyValue);
                $this->Nservicio->setOldValue($this->Nservicio->QueryStringValue);
            } elseif (Post("Nservicio") !== null) {
                $this->Nservicio->setFormValue(Post("Nservicio"));
                $this->Nservicio->setOldValue($this->Nservicio->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("Nservicio") ?? Route("Nservicio")) !== null) {
                    $this->Nservicio->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->Nservicio->CurrentValue = null;
                }
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ScoServicioList"); // Return to list page
                        return;
                        ...
                        if ($this->Nservicio->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->Nservicio->CurrentValue, $this->CurrentRow['Nservicio'])) {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ScoServicioList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
            ...
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "ScoServicioList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions && !$this->getCurrentMasterTable()) {
                        ...
                        if (GetPageName($returnUrl) != "ScoServicioList") {
                            ...
                            $returnUrl = "ScoServicioList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Nservicio' first before field var 'x_Nservicio'
        $val = $CurrentForm->hasValue("Nservicio") ? $CurrentForm->getValue("Nservicio") : $CurrentForm->getValue("x_Nservicio");
        if (!$this->Nservicio->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->Nservicio->Visible = false; // Disable update for API request
            } else {
                $this->Nservicio->setFormValue($val);
            }
        }
        ...
        // Check field name 'sto_min' first before field var 'x_sto_min'
        $val = $CurrentForm->hasValue("sto_min") ? $CurrentForm->getValue("sto_min") : $CurrentForm->getValue("x_sto_min");
        if (!$this->sto_min->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->sto_min->Visible = false; // Disable update for API request
            } else {
                $this->sto_min->setFormValue($val, true, $validate);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->Nservicio->CurrentValue = $this->Nservicio->FormValue;
        $this->nombre->CurrentValue = $this->nombre->FormValue;
        $this->articulo_inventario->CurrentValue = $this->articulo_inventario->FormValue;
        $this->sto_min->CurrentValue = $this->sto_min->FormValue;
    ...
    public function renderRow()
    {
        ...
        // Nservicio
        $this->Nservicio->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->Nservicio->Visible && $this->Nservicio->Required) {
                if (!$this->Nservicio->IsDetailKey && EmptyValue($this->Nservicio->FormValue)) {
                    $this->Nservicio->addErrorMessage(str_replace("%s", $this->Nservicio->caption(), $this->Nservicio->RequiredErrorMessage));
                }
            }
            ...
            if ($this->sto_min->Visible && $this->sto_min->Required) {
                if (!$this->sto_min->IsDetailKey && EmptyValue($this->sto_min->FormValue)) {
                    $this->sto_min->addErrorMessage(str_replace("%s", $this->sto_min->caption(), $this->sto_min->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->sto_min->FormValue)) {
                $this->sto_min->addErrorMessage($this->sto_min->getErrorMessage(false));
            }
    ...
    protected function editRow()
    {
        ...
        // Check field with unique index (Nservicio)
        if ($this->Nservicio->CurrentValue != "") {
            $filterChk = "(`Nservicio` = '" . AdjustSql($this->Nservicio->CurrentValue, $this->Dbid) . "')";
            ...
            if ($rsChk->fetch()) {
                $idxErrMsg = str_replace("%f", $this->Nservicio->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->Nservicio->CurrentValue, $idxErrMsg);
        ...
        // Check referential integrity for master table 'sco_servicio_tipo'
        $detailKeys = [];
        $keyValue = $rsnew['tipo'] ?? $rsold['tipo'];
        $detailKeys['tipo'] = $keyValue;
        ...
        if (!$validMasterRecord) {
            $relatedRecordMsg = str_replace("%t", "sco_servicio_tipo", $Language->phrase("RelatedRecordRequired"));
    ...
    protected function getEditRow($rsold)
    {
        ...
        // sto_min
        $this->sto_min->setDbValueDef($rsnew, $this->sto_min->CurrentValue, $this->sto_min->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['sto_min'])) { // sto_min
            $this->sto_min->CurrentValue = $row['sto_min'];
        }
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("sco_adjunto", $detailTblVar)) {
                ...
                if ($detailPageObj->DetailEdit) {
                    ...
                    $detailPageObj->servicio->IsDetailKey = true;
                    $detailPageObj->servicio->CurrentValue = $this->Nservicio->CurrentValue;
                    $detailPageObj->servicio->setSessionValue($detailPageObj->servicio->CurrentValue);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ScoServicioList"), "", $this->TableVar, true);
        $pageId = "edit";
