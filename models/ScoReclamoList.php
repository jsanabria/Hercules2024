 ...
 * Page class
 */
class ScoReclamoList extends ScoReclamo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoReclamoList";
    ...
    public $FormName = "fsco_reclamolist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoReclamoList";
    ...
    public function setVisibility()
    {
        ...
        $this->mensaje_cliente->Visible = false;
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ScoReclamoAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoReclamoDelete";
        $this->MultiUpdateUrl = "ScoReclamoUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_reclamogrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Nreclamo->AdvancedSearch->toJson(), ","); // Field Nreclamo
        ...
        $filterList = Concat($filterList, $this->mensaje_cliente->AdvancedSearch->toJson(), ","); // Field mensaje_cliente
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_reclamosrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Nreclamo
        $this->Nreclamo->AdvancedSearch->SearchValue = @$filter["x_Nreclamo"];
        $this->Nreclamo->AdvancedSearch->SearchOperator = @$filter["z_Nreclamo"];
        $this->Nreclamo->AdvancedSearch->SearchCondition = @$filter["v_Nreclamo"];
        $this->Nreclamo->AdvancedSearch->SearchValue2 = @$filter["y_Nreclamo"];
        $this->Nreclamo->AdvancedSearch->SearchOperator2 = @$filter["w_Nreclamo"];
        $this->Nreclamo->AdvancedSearch->save();
        ...
        $this->mensaje_cliente->AdvancedSearch->SearchValue = @$filter["x_mensaje_cliente"];
        $this->mensaje_cliente->AdvancedSearch->SearchOperator = @$filter["z_mensaje_cliente"];
        $this->mensaje_cliente->AdvancedSearch->SearchCondition = @$filter["v_mensaje_cliente"];
        $this->mensaje_cliente->AdvancedSearch->SearchValue2 = @$filter["y_mensaje_cliente"];
        $this->mensaje_cliente->AdvancedSearch->SearchOperator2 = @$filter["w_mensaje_cliente"];
        $this->mensaje_cliente->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Nreclamo, $default, false); // Nreclamo
        ...
        $this->buildSearchSql($where, $this->mensaje_cliente, $default, false); // mensaje_cliente
        ...
        if (!$default && $this->Command == "search") {
            $this->Nreclamo->AdvancedSearch->save(); // Nreclamo
            ...
            $this->mensaje_cliente->AdvancedSearch->save(); // mensaje_cliente
    ...
    public function showFilterList()
    {
        ...
        // Field Nreclamo
        $filter = $this->queryBuilderWhere("Nreclamo");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->Nreclamo, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->Nreclamo->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->Nreclamo->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->mensaje_cliente->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->Nreclamo->AdvancedSearch->unsetSession();
        ...
        $this->mensaje_cliente->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->Nreclamo->AdvancedSearch->load();
        ...
        $this->mensaje_cliente->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->Nreclamo->Expression . " DESC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->Nreclamo); // Nreclamo
    ...
    protected function setupListOptions()
    {
        ...
        $item = &$this->ListOptions->add("detail_sco_reclamo_nota");
        $item->CssClass = "text-nowrap";
        ...
        $item = &$this->ListOptions->add("detail_sco_reclamo_adjunto");
        $item->CssClass = "text-nowrap";
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("sco_reclamo_nota");
        $pages->add("sco_reclamo_adjunto");
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_reclamo\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_reclamo\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_reclamolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_reclamolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_sco_reclamo_nota"
        $opt = $this->ListOptions["detail_sco_reclamo_nota"];
        if ($Security->allowList(CurrentProjectID() . 'sco_reclamo_nota')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("sco_reclamo_nota", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("ScoReclamoNotaList?" . Config("TABLE_SHOW_MASTER") . "=sco_reclamo&" . GetForeignKeyUrl("fk_Nreclamo", $this->Nreclamo->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("ScoReclamoNotaGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_reclamo')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_reclamo_nota");
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_reclamo')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_reclamo_nota");
        ...
        // "detail_sco_reclamo_adjunto"
        $opt = $this->ListOptions["detail_sco_reclamo_adjunto"];
        if ($Security->allowList(CurrentProjectID() . 'sco_reclamo_adjunto')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("sco_reclamo_adjunto", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("ScoReclamoAdjuntoList?" . Config("TABLE_SHOW_MASTER") . "=sco_reclamo&" . GetForeignKeyUrl("fk_Nreclamo", $this->Nreclamo->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("ScoReclamoAdjuntoGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_reclamo')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_reclamo_adjunto");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_reclamo')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_reclamo_adjunto");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Nreclamo->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function renderListOptionsExt()
    {
        ...
        $detailFilters = [];
        $masterKeys = []; // Reset
        $masterKeys["Nreclamo"] = strval($this->Nreclamo->DbValue);

        // Column "detail_sco_reclamo_nota"
        if ($this->DetailPages?->getItem("sco_reclamo_nota")?->Visible && $Security->allowList(CurrentProjectID() . 'sco_reclamo_nota')) {
            $link = "";
            $option = $this->ListOptions["detail_sco_reclamo_nota"];
            $detailTbl = Container("sco_reclamo_nota");
            $detailFilter = $detailTbl->getDetailFilter($this);
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "ScoReclamoNotaPreview?t=sco_reclamo&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"sco_reclamo_nota\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'sco_reclamo')) {
                $label = $Language->tablePhrase("sco_reclamo_nota", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"sco_reclamo_nota\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("ScoReclamoNotaList?" . Config("TABLE_SHOW_MASTER") . "=sco_reclamo");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $Language->tablePhrase("sco_reclamo_nota", "TblCaption");
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_reclamo')) {
                ...
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_reclamo_nota"));
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_reclamo')) {
                ...
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_reclamo_nota"));
        ...
        // Column "detail_sco_reclamo_adjunto"
        if ($this->DetailPages?->getItem("sco_reclamo_adjunto")?->Visible && $Security->allowList(CurrentProjectID() . 'sco_reclamo_adjunto')) {
            $link = "";
            $option = $this->ListOptions["detail_sco_reclamo_adjunto"];
            $detailTbl = Container("sco_reclamo_adjunto");
            $detailFilter = $detailTbl->getDetailFilter($this);
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "ScoReclamoAdjuntoPreview?t=sco_reclamo&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"sco_reclamo_adjunto\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'sco_reclamo')) {
                $label = $Language->tablePhrase("sco_reclamo_adjunto", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"sco_reclamo_adjunto\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("ScoReclamoAdjuntoList?" . Config("TABLE_SHOW_MASTER") . "=sco_reclamo");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $Language->tablePhrase("sco_reclamo_adjunto", "TblCaption");
                $caption = $Language->phrase("MasterDetailListLink");
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . $title . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($detaillnk) . "\">" . $caption . "</a>";
            }
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_reclamo')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_reclamo_adjunto"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($viewurl) . "\">" . $caption . "</a>";
            }
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_reclamo')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_reclamo_adjunto"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($editurl) . "\">" . $caption . "</a>";
            }
            $btngrp .= "</div>";
            if ($link != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_reclamo\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $detailTableLink = "";
        $item = &$option->add("detailadd_sco_reclamo_nota");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=sco_reclamo_nota");
        $detailPage = Container("ScoReclamoNotaGrid");
        $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'sco_reclamo') && $Security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
        ...
        $item = &$option->add("detailadd_sco_reclamo_adjunto");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=sco_reclamo_adjunto");
        $detailPage = Container("ScoReclamoAdjuntoGrid");
        $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'sco_reclamo') && $Security->canAdd());
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "Nreclamo");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_reclamosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_reclamosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_reclamolist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_reclamo", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_reclamo",
    ...
    protected function loadSearchValues()
    {
        ...
        // Nreclamo
        if ($this->Nreclamo->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Nreclamo->AdvancedSearch->SearchValue != "" || $this->Nreclamo->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // mensaje_cliente
        if ($this->mensaje_cliente->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->mensaje_cliente->AdvancedSearch->SearchValue != "" || $this->mensaje_cliente->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->Nreclamo->EditValue = $this->Nreclamo->AdvancedSearch->SearchValue;
            $this->Nreclamo->PlaceHolder = RemoveHtml($this->Nreclamo->caption());
            ...
            if ($this->registra->AdvancedSearch->ViewValue !== null) { // Load from cache
                ...
                if ($this->registra->AdvancedSearch->ViewValue == "") {
                    $this->registra->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
                ...
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->registra->Lookup->renderViewRow($rswrk[0]);
                    $this->registra->AdvancedSearch->ViewValue = $this->registra->displayValue($arwrk);
                } else {
                    $this->registra->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
                $arwrk = $rswrk;
            ...
            if ($this->modifica->AdvancedSearch->ViewValue !== null) { // Load from cache
                ...
                if ($this->modifica->AdvancedSearch->ViewValue == "") {
                    $this->modifica->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                ...
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->modifica->Lookup->renderViewRow($rswrk[0]);
                    $this->modifica->AdvancedSearch->ViewValue = $this->modifica->displayValue($arwrk);
                } else {
                    $this->modifica->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
                $arwrk = $rswrk;
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckInteger($this->Nreclamo->AdvancedSearch->SearchValue)) {
            $this->Nreclamo->addErrorMessage($this->Nreclamo->getErrorMessage(false));
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_reclamosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    ...
    public function pageDataRendering(&$header) {
        // 1. Definición de Estatus (Array centralizado para facilitar cambios)
        $estatusList = [
            ['label' => 'RECEPCIÓN',    'color' => 'bg-danger'],
            ['label' => 'VERIFICACIÓN', 'color' => 'bg-warning text-dark'],
            ['label' => 'PROCESANDO',   'color' => 'bg-primary'],
            ['label' => 'RESUELTO',      'color' => 'bg-success'],
            ['label' => 'RECHAZADO',     'color' => 'bg-secondary']
        ];

        // 2. Construcción del Header Unificado (Botón + Leyenda)
        // d-flex: activa el modo flexible | align-items-center: centra verticalmente | gap-3: espacio entre el botón y la leyenda
        $html = '<div class="d-flex align-items-center gap-3 mb-4">';

        // Botón de Exportación (Elemento a la izquierda)
        // flex-shrink-0: evita que el botón se reduzca si hay poco espacio
        $html .= '<a href="dashboard/ListadoMaster?id=reclamos" class="btn btn-primary btn-sm flex-shrink-0 shadow-sm">
                    <i class="fa-solid fa-file-excel me-1"></i> Exportar a Excel
                  </a>';

        // Contenedor de Badges (Elemento a la derecha)
        // flex-grow-1: permite que la leyenda use el resto del ancho disponible
        $html .= '<div class="d-flex flex-wrap gap-1 flex-grow-1">';
        foreach ($estatusList as $item) {
            $html .= sprintf(
                '<span class="badge %s p-2 shadow-sm text-center" style="min-width: 100px; font-size: 0.75rem; flex: 1;">%s</span>',
                $item['color'],
                $item['label']
            );
        }
        ...
        // Asignación final al header
        $header = $html;

        // 3. Ocultar campos
        $camposAOcultar = ["seccion", "modulo", "sub_seccion", "parcela", "boveda"];
        foreach ($camposAOcultar as $campo) {
            if (isset($this->$campo)) {
    ...
    public function pageDataRendered(&$footer) {
        // 1. Definición de los estados y sus colores correspondientes
        $estatusList = [
            ['label' => 'RECEPCION',    'class' => 'bg-danger text-white'],
            ['label' => 'VERIFICACION', 'class' => 'bg-warning text-dark'],
            ['label' => 'PROCESANDO',   'class' => 'bg-primary text-white'],
            ['label' => 'RESUELTO',      'class' => 'bg-success text-white'],
            ['label' => 'RECHAZADO',     'class' => 'bg-secondary text-white']
        ];

        // 2. Construcción de la leyenda usando Flexbox y Badges (Píldoras)
        $html = '<div class="d-flex flex-wrap gap-2 mt-3 mb-2">';
        $html .= '<span class="small text-muted w-100 mb-1"><i class="fa-solid fa-circle-info"></i> Leyenda de estatus:</span>';
        foreach ($estatusList as $item) {
            $html .= sprintf(
                '<div class="badge %s rounded-pill shadow-sm" style="min-width: 110px; padding: 8px 12px; font-weight: 600; letter-spacing: 0.5px;">%s</div>',
                $item['class'],
                $item['label']
            );
        }
    ...
    {
        // 1. Verificamos que la fecha de registro sea válida
        if (!EmptyValue($this->registro->CurrentValue)) {
            $fechaDesde = $this->registro->CurrentValue;
            $estatus = $this->estatus->CurrentValue;

            // Determinamos si el proceso terminó para fijar la fecha de corte
            $finalizado = in_array($estatus, ["RESUELTO", "RECHAZADO"]);

            // Si está finalizado usamos 'modificacion', de lo contrario la fecha actual
            $fechaHasta = ($finalizado && !EmptyValue($this->modificacion->CurrentValue)) 
                ? $this->modificacion->CurrentValue 
                : CurrentDate();

            // Acceso directo al ítem "new"
            if (isset($this->ListOptions->Items["new"])) {
                $this->ListOptions->Items["new"]->Body = tiempoTranscurridoFechas($fechaDesde, $fechaHasta);
            }
        }

        // 2. Validación de mensaje al cliente
        if (isset($this->ListOptions->Items["new2"])) {
            $tieneMensaje = trim($this->mensaje_cliente->CurrentValue ?? 0) !== 0;
            $this->ListOptions->Items["new2"]->Body = $tieneMensaje ? "SI" : "NO";
        }

        // 3. Conteo de adjuntos con seguridad SQL y Font Awesome 6
        if (isset($this->ListOptions->Items["new3"])) {
            $nReclamo = $this->Nreclamo->CurrentValue;

            // ExecuteScalar con AdjustSql para mayor seguridad
            $sql = "SELECT COUNT(*) FROM sco_reclamo_adjunto WHERE reclamo = '" . AdjustSql($nReclamo) . "'";
            $cnt = ExecuteScalar($sql);

            // Cambiamos a etiqueta <i> y clase de Font Awesome 6
            $this->ListOptions->Items["new3"]->Body = ($cnt > 0) 
                ? '<i class="fa-solid fa-paperclip text-primary" title="Tiene adjuntos"></i>' 
                : '';
        }
    }
