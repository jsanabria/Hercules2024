...
/**
 * Table class for view_orden_prov
 */
class ViewOrdenProv extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "view_orden_prov";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // horas_velacion
        $this->horas_velacion = new DbField(
            $this, // Table
            'x_horas_velacion', // Variable name
            'horas_velacion', // Name
            '`horas_velacion`', // Expression
            '`horas_velacion`', // Basic search expression
            200, // Type
            4, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`horas_velacion`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->horas_velacion->InputTextType = "text";
        $this->horas_velacion->Nullable = false; // NOT NULL field
        $this->horas_velacion->Required = true; // Required field
        $this->horas_velacion->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['horas_velacion'] = &$this->horas_velacion;
        ...
        $this->llevar_a->Nullable = false; // NOT NULL field
        $this->llevar_a->Required = true; // Required field
        $this->llevar_a->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['llevar_a'] = &$this->llevar_a;

        // ajunto
        $this->ajunto = new DbField(
            $this, // Table
            'x_ajunto', // Variable name
            'ajunto', // Name
            '`ajunto`', // Expression
            '`ajunto`', // Basic search expression
            200, // Type
            3072, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`ajunto`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->ajunto->InputTextType = "text";
        $this->ajunto->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['ajunto'] = &$this->ajunto;
        ...
        $this->servicio_atendido->Lookup = new Lookup($this->servicio_atendido, 'view_orden_prov', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "view_orden_prov";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->horas_velacion->DbValue = $row['horas_velacion'];
        $this->llevar_a->DbValue = $row['llevar_a'];
        $this->ajunto->DbValue = $row['ajunto'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ViewOrdenProvList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ViewOrdenProvView" => $Language->phrase("View"),
            "ViewOrdenProvEdit" => $Language->phrase("Edit"),
            "ViewOrdenProvAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ViewOrdenProvView",
            Config("API_ADD_ACTION") => "ViewOrdenProvAdd",
            Config("API_EDIT_ACTION") => "ViewOrdenProvEdit",
            Config("API_DELETE_ACTION") => "ViewOrdenProvDelete",
            Config("API_LIST_ACTION") => "ViewOrdenProvList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ViewOrdenProvView", $parm);
        } else {
            $url = $this->keyUrl("ViewOrdenProvView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ViewOrdenProvAdd?" . $parm;
        } else {
            $url = "ViewOrdenProvAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewOrdenProvEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewOrdenProvList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewOrdenProvAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewOrdenProvList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ViewOrdenProvDelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        if (!$this->horas_velacion->Raw) {
            $this->horas_velacion->CurrentValue = HtmlDecode($this->horas_velacion->CurrentValue);
        }
        $this->horas_velacion->EditValue = $this->horas_velacion->CurrentValue;
        ...
        $this->ajunto->EditValue = $this->ajunto->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->horas_velacion);
                    $doc->exportCaption($this->llevar_a);
                    $doc->exportCaption($this->ajunto);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->horas_velacion);
                        $doc->exportField($this->llevar_a);
                        $doc->exportField($this->ajunto);
