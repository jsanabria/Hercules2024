 ...
 * Page class
 */
class ScoLapidasAdjuntoGrid extends ScoLapidasAdjunto
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoLapidasAdjuntoGrid";
    ...
    public $FormName = "fsco_lapidas_adjuntogrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoLapidasAdjuntoGrid";
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->Nlapidas_adjunto->CurrentValue;
    ...
    {
        $this->lapida_registro->DefaultValue = $this->lapida_registro->getDefault(); // PHP
        $this->lapida_registro->OldValue = $this->lapida_registro->DefaultValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Nlapidas_adjunto' first before field var 'x_Nlapidas_adjunto'
        $val = $CurrentForm->hasValue("Nlapidas_adjunto") ? $CurrentForm->getValue("Nlapidas_adjunto") : $CurrentForm->getValue("x_Nlapidas_adjunto");
        if (!$this->Nlapidas_adjunto->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->Nlapidas_adjunto->setFormValue($val);
        }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->Nlapidas_adjunto->CurrentValue = $this->Nlapidas_adjunto->FormValue;
        }
    ...
    protected function editRow()
    {
        ...
        // Check referential integrity for master table 'sco_lapidas_registro'
        $detailKeys = [];
        $keyValue = $rsnew['lapida_registro'] ?? $rsold['lapida_registro'];
        $detailKeys['lapida_registro'] = $keyValue;
        ...
        if (!$validMasterRecord) {
            $relatedRecordMsg = str_replace("%t", "sco_lapidas_registro", $Language->phrase("RelatedRecordRequired"));
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "sco_lapidas_registro") {
            $this->lapida_registro->Visible = true; // Need to insert foreign key
            $this->lapida_registro->CurrentValue = $this->lapida_registro->getSessionValue();
        }
        ...
        // Check referential integrity for master table 'sco_lapidas_adjunto'
        $validMasterRecord = true;
        $detailKeys = [];
        $detailKeys["lapida_registro"] = $this->lapida_registro->getSessionValue();
        $masterTable = Container("sco_lapidas_registro");
        $masterFilter = $this->getMasterFilter($masterTable, $detailKeys);
        if (!EmptyValue($masterFilter)) {
            $rsmaster = $masterTable->loadRs($masterFilter)->fetch();
    ...
    protected function getAddRow()
    {
        ...
        if ($this->lapida_registro->getSessionValue() != "") {
            $rsnew['lapida_registro'] = $this->lapida_registro->getSessionValue();
        }
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['lapida_registro'])) { // lapida_registro
            $this->lapida_registro->setFormValue($row['lapida_registro']);
        }
    }
