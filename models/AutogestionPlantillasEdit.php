 ...
 * Page class
 */
class AutogestionPlantillasEdit extends AutogestionPlantillas
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AutogestionPlantillasEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "AutogestionPlantillasEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Nplantilla") ?? Key(0) ?? Route(2)) !== null) {
                $this->Nplantilla->setQueryStringValue($keyValue);
                $this->Nplantilla->setOldValue($this->Nplantilla->QueryStringValue);
            } elseif (Post("Nplantilla") !== null) {
                $this->Nplantilla->setFormValue(Post("Nplantilla"));
                $this->Nplantilla->setOldValue($this->Nplantilla->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("Nplantilla") ?? Route("Nplantilla")) !== null) {
                    $this->Nplantilla->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->Nplantilla->CurrentValue = null;
                }
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("AutogestionPlantillasList"); // Return to list page
                        return;
                        ...
                        if ($this->Nplantilla->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->Nplantilla->CurrentValue, $this->CurrentRow['Nplantilla'])) {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("AutogestionPlantillasList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "AutogestionPlantillasList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "AutogestionPlantillasList") {
                            ...
                            $returnUrl = "AutogestionPlantillasList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Nplantilla' first before field var 'x_Nplantilla'
        $val = $CurrentForm->hasValue("Nplantilla") ? $CurrentForm->getValue("Nplantilla") : $CurrentForm->getValue("x_Nplantilla");
        if (!$this->Nplantilla->IsDetailKey) {
            $this->Nplantilla->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->Nplantilla->CurrentValue = $this->Nplantilla->FormValue;
    ...
    public function renderRow()
    {
        ...
        // Nplantilla
        $this->Nplantilla->RowCssClass = "row";
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("AutogestionPlantillasList"), "", $this->TableVar, true);
        $pageId = "edit";
