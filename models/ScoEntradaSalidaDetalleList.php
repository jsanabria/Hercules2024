 ...
 * Page class
 */
class ScoEntradaSalidaDetalleList extends ScoEntradaSalidaDetalle
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoEntradaSalidaDetalleList";
    ...
    public $FormName = "fsco_entrada_salida_detallelist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoEntradaSalidaDetalleList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ScoEntradaSalidaDetalleAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoEntradaSalidaDetalleDelete";
        $this->MultiUpdateUrl = "ScoEntradaSalidaDetalleUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_entrada_salida_detallegrid";
        }
        ...
        if (IsPost() && ($this->isGridUpdate() || $this->isMultiUpdate() || $this->isGridOverwrite()) && (Session(SESSION_INLINE_MODE) == "gridedit" || Session(SESSION_INLINE_MODE) == "multiedit")) {
            ...
            if ($gridUpdate) {
                ...
                if ($this->IsModal && !$this->UseAjaxActions) {
                    $this->terminate("ScoEntradaSalidaDetalleList");
                    return;
                }
            } else {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "sco_entrada_salida") {
            $masterTbl = Container("sco_entrada_salida");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("ScoEntradaSalidaList"); // Return to master page
                return;
            } else {
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->Nentrada_salida_detalle->CurrentValue;
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Nentrada_salida_detalle->AdvancedSearch->toJson(), ","); // Field Nentrada_salida_detalle
        $filterList = Concat($filterList, $this->entrada_salida->AdvancedSearch->toJson(), ","); // Field entrada_salida
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_entrada_salida_detallesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Nentrada_salida_detalle
        $this->Nentrada_salida_detalle->AdvancedSearch->SearchValue = @$filter["x_Nentrada_salida_detalle"];
        $this->Nentrada_salida_detalle->AdvancedSearch->SearchOperator = @$filter["z_Nentrada_salida_detalle"];
        $this->Nentrada_salida_detalle->AdvancedSearch->SearchCondition = @$filter["v_Nentrada_salida_detalle"];
        $this->Nentrada_salida_detalle->AdvancedSearch->SearchValue2 = @$filter["y_Nentrada_salida_detalle"];
        $this->Nentrada_salida_detalle->AdvancedSearch->SearchOperator2 = @$filter["w_Nentrada_salida_detalle"];
        $this->Nentrada_salida_detalle->AdvancedSearch->save();

        // Field entrada_salida
        $this->entrada_salida->AdvancedSearch->SearchValue = @$filter["x_entrada_salida"];
        $this->entrada_salida->AdvancedSearch->SearchOperator = @$filter["z_entrada_salida"];
        $this->entrada_salida->AdvancedSearch->SearchCondition = @$filter["v_entrada_salida"];
        $this->entrada_salida->AdvancedSearch->SearchValue2 = @$filter["y_entrada_salida"];
        $this->entrada_salida->AdvancedSearch->SearchOperator2 = @$filter["w_entrada_salida"];
        $this->entrada_salida->AdvancedSearch->save();
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->Nentrada_salida_detalle->setSort("");
                $this->entrada_salida->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_entrada_salida_detalle\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_entrada_salida_detalle\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_entrada_salida_detallelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_entrada_salida_detallelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Nentrada_salida_detalle->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_entrada_salida_detalle\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_entrada_salida_detallesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_entrada_salida_detallesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit() && !$this->isMultiEdit()) { // Not grid add/grid edit/multi edit mode
            ...
            foreach ($this->ListActions as $listAction) {
                if ($listAction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_entrada_salida_detallelist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
            ...
            if ($this->isGridAdd()) {
                ...
                if (!($this->ModalGridAdd && !IsMobile())) {
                    ...
                    $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" form=\"fsco_entrada_salida_detallelist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridInsertLink") . "</button>";
                    // Add grid cancel
            ...
            if ($this->isGridEdit()) {
                ...
                if (!($this->ModalGridEdit && !IsMobile())) {
                    ...
                    $option->UseDropDownButton = false;
                        ...
                        $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" form=\"fsco_entrada_salida_detallelist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridSaveLink") . "</button>";
                        $item = &$option->add("gridcancel");
                        $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_entrada_salida_detalle", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_entrada_salida_detalle",
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->Nentrada_salida_detalle->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
    ...
    protected function deleteRows()
    {
        ...
        foreach ($rsold as $row) {
            ...
            $thisKey .= $row['Nentrada_salida_detalle'];
    ...
    public function getRowHash($row)
    {
        ...
        $hash .= GetFieldHash($row['cantidad']); // cantidad
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_entrada_salida_detallesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    public function listOptionsRendered() {
    	// Example:
    	$articulo = CurrentTable()->articulo->CurrentValue;
    	$sql = "select Ncostos_articulo from sco_costos_articulos where articulo_hercules = '$articulo';";
    	if(!$row = ExecuteRow($sql)) {
    		$this->ListOptions->Items["new"]->Body = '<div class="alert alert-warning" role="alert"><a onClick="js:alert(\'Alerta!!! ' . CurrentTable()->articulo->ViewValue . ' articulo de Hercules no esta asociado a codigo de articulo AS400!.\');" class="alert-link"><span class="glyphicon glyphicon-bell"></span></a></div>';
    	}
    	else {
    		...
    		if($mayos_costo_compra >= $precio_venta)
    			$this->ListOptions->Items["new"]->Body = '<div class="alert alert-danger" role="alert"><a onClick="js:alert(\'Alerta!!! Ultimo costo de compra por: Bs.' . number_format($mayos_costo_compra,2,",",".") . ' es superior al precio de venta actual por: Bs.' . number_format($precio_venta,2,",",".") . '; ajuste la estructura de costo actual o genere una nueva.\');" class="alert-link"><span class="glyphicon glyphicon-bell"></span></a></div>';
    		else
    			$this->ListOptions->Items["new"]->Body = '<div class="alert alert-success" role="alert"><a onClick="js:alert(\'Ultimo costo de compra por: Bs.' . number_format($mayos_costo_compra,2,",",".") . ' es adecuado a la estructura de costo actual para un precio de venta por: Bs.' . number_format($precio_venta,2,",",".") . '\');" class="alert-link"><span class="glyphicon glyphicon-ok"></span></a></div>';
    	}
    }
