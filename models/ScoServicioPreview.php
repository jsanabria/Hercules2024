 ...
 * Page class
 */
class ScoServicioPreview extends ScoServicio
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoServicioPreview";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoServicioPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "sco_servicio_tipo") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=sco_servicio_tipo&" . GetForeignKeyUrl("fk_Nservicio_tipo", $this->tipo->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys($keys)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "sco_servicio_tipo") {
            $this->tipo->setQueryStringValue($keys[0] ?? "");
        }
    }
