...
/**
 * Table class for view_parcela_ventas
 */
class ViewParcelaVentas extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "view_parcela_ventas";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->ci_comprador->Required = true; // Required field
        ...
        $this->usuario_vende->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        ...
        $this->fecha_venta->Required = true; // Required field
        ...
        $this->valor_venta->Required = true; // Required field
        ...
        $this->moneda_venta->Required = true; // Required field
        ...
        $this->tasa_venta->Required = true; // Required field
        ...
        $this->estatus->addMethod("getDefault", fn() => VENTA);
    ...
    public function getDetailUrl()
    {
        ...
        if ($detailUrl == "") {
            $detailUrl = "ViewParcelaVentasList";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "view_parcela_ventas";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ViewParcelaVentasList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ViewParcelaVentasView" => $Language->phrase("View"),
            "ViewParcelaVentasEdit" => $Language->phrase("Edit"),
            "ViewParcelaVentasAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ViewParcelaVentasView",
            Config("API_ADD_ACTION") => "ViewParcelaVentasAdd",
            Config("API_EDIT_ACTION") => "ViewParcelaVentasEdit",
            Config("API_DELETE_ACTION") => "ViewParcelaVentasDelete",
            Config("API_LIST_ACTION") => "ViewParcelaVentasList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ViewParcelaVentasView", $parm);
        } else {
            $url = $this->keyUrl("ViewParcelaVentasView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ViewParcelaVentasAdd?" . $parm;
        } else {
            $url = "ViewParcelaVentasAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ViewParcelaVentasEdit", $parm);
        } else {
            $url = $this->keyUrl("ViewParcelaVentasEdit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewParcelaVentasList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ViewParcelaVentasAdd", $parm);
        } else {
            $url = $this->keyUrl("ViewParcelaVentasAdd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewParcelaVentasList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ViewParcelaVentasDelete", $parm);
        }
    }
    ...
    {
        global $Response;
        $listPage = "ViewParcelaVentasList";
    ...
    public function renderEditRow()
    {
        ...
        if (!$this->terraza->Raw) {
            $this->terraza->CurrentValue = HtmlDecode($this->terraza->CurrentValue);
        }
        $this->terraza->EditValue = $this->terraza->CurrentValue;
        $this->terraza->PlaceHolder = RemoveHtml($this->terraza->caption());
        ...
        if (!$this->usuario_vende->Raw) {
            $this->usuario_vende->CurrentValue = HtmlDecode($this->usuario_vende->CurrentValue);
        }
        $this->usuario_vende->EditValue = $this->usuario_vende->CurrentValue;
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'view_parcela_ventas');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'view_parcela_ventas', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'view_parcela_ventas', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'view_parcela_ventas', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowUpdating($rsold, &$rsnew) {
        ...
        // --- CASO 1: PROCESO DE ANULACIÓN ---
        if ($rsnew["estatus"] == "ANULADO" && $rsold["estatus"] != "ANULADO") {
            $canAnular = ExecuteScalar("SELECT b.codigo FROM sco_grupo_funciones AS a 
                                       JOIN sco_funciones AS b ON b.Nfuncion = a.funcion 
                                       WHERE a.grupo = '$userLevel' AND b.codigo = '200'");
            if ($canAnular == "200") {
                $rsnew["nota"] = ($rsold["nota"] ?? "") . "\n[LOG]: Usuario $userName ANULÓ la venta el " . CurrentDateTime() . ".";
            } else {
                $this->setFailureMessage("No tiene nivel de seguridad (Cód: 200) para ANULAR.");
                return FALSE;
            }
        }

        // --- CASO 2: REVERSO A COMPRA (REVERTIR) ---
        if ($rsnew["estatus"] == "COMPRA" && $rsold["estatus"] == "VENTA") {
            $canRevertir = ExecuteScalar("SELECT b.codigo FROM sco_grupo_funciones AS a 
                                         JOIN sco_funciones AS b ON b.Nfuncion = a.funcion 
                                         WHERE a.grupo = '$userLevel' AND b.codigo = '100'");
            if ($canRevertir == "100") {
                $rsnew["usuario_vende"] = "";
                $rsnew["fecha_venta"] = "1990-01-01";
                $rsnew["moneda_venta"] = "";
                $rsnew["valor_venta"] = 0.00;
                $rsnew["tasa_venta"] = 0.00;
                $rsnew["ci_comprador"] = "";
                $rsnew["comprador"] = "";
                $rsnew["nota"] = ($rsold["nota"] ?? "") . "\n[LOG]: Usuario $userName REVIRTIÓ a COMPRA el " . CurrentDateTime() . ".";
            } else {
                $this->setFailureMessage("No está autorizado (Cód: 100) para REVERTIR esta parcela.");
                return FALSE;
            }
        }

        // Si es una venta normal (de estatus vacío o compra a venta)
        if ($rsold["estatus"] == "COMPRA" && $rsnew["estatus"] == "VENTA") {
            $rsnew["fecha_venta"] = $hoy;
            $rsnew["usuario_vende"] = $userName;
            $rsnew["estatus"] = "VENTA";
        }
    ...
    public function rowRendering() {
        // 1. Solo ejecutar en páginas de visualización
        if ($this->PageID == "list" || $this->PageID == "view") {
            $estatus = trim($this->estatus->CurrentValue ?? "");
            $color = "";
            $texto = "white"; // Color de letra por defecto para fondos oscuros

            // 2. Definición de colores (Usamos variables CSS o nombres de color web)
            switch($estatus) {
                case "VENTA":
                    $color = "#f8d7da"; // Rojo muy claro (Bootstrap Danger soft)
                    $texto = "#721c24"; // Texto rojo oscuro
                    break;
                case "ANULADO":
                    ...
                    break;
                case "COMPRA":
                    $color = "#d4edda"; // Verde claro (Opcional, para resaltar activos)
                    $texto = "#155724";
                    break;
            }

            // 3. Aplicar el estilo si hay un color definido
            if ($color !== "") {
                $style = "background-color: $color !important; color: $texto !important; font-weight: bold;";

                // Lista de campos a los que queremos aplicar el estilo
                $campos = ["seccion", "modulo", "subseccion", "parcela", "estatus"];
