 ...
 * Page class
 */
class ScoUserNotaEdit extends ScoUserNota
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoUserNotaEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoUserNotaEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Nuser_nota") ?? Key(0) ?? Route(2)) !== null) {
                $this->Nuser_nota->setQueryStringValue($keyValue);
                $this->Nuser_nota->setOldValue($this->Nuser_nota->QueryStringValue);
            } elseif (Post("Nuser_nota") !== null) {
                $this->Nuser_nota->setFormValue(Post("Nuser_nota"));
                $this->Nuser_nota->setOldValue($this->Nuser_nota->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("Nuser_nota") ?? Route("Nuser_nota")) !== null) {
                    $this->Nuser_nota->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->Nuser_nota->CurrentValue = null;
                }
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ScoUserNotaList"); // Return to list page
                        return;
                        ...
                        if ($this->Nuser_nota->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->Nuser_nota->CurrentValue, $this->CurrentRow['Nuser_nota'])) {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ScoUserNotaList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "ScoUserNotaList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions && !$this->getCurrentMasterTable()) {
                        ...
                        if (GetPageName($returnUrl) != "ScoUserNotaList") {
                            ...
                            $returnUrl = "ScoUserNotaList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->Nuser_nota->IsDetailKey) {
    ...
    public function renderRow()
    {
        ...
        // Nuser_nota
        $this->Nuser_nota->RowCssClass = "row";
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ScoUserNotaList"), "", $this->TableVar, true);
        $pageId = "edit";
