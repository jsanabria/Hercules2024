 ...
 * Page class
 */
class ScoClienteEdit extends ScoCliente
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoClienteEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoClienteEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Ncliente") ?? Key(0) ?? Route(2)) !== null) {
                $this->Ncliente->setQueryStringValue($keyValue);
                $this->Ncliente->setOldValue($this->Ncliente->QueryStringValue);
            } elseif (Post("Ncliente") !== null) {
                $this->Ncliente->setFormValue(Post("Ncliente"));
                $this->Ncliente->setOldValue($this->Ncliente->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("Ncliente") ?? Route("Ncliente")) !== null) {
                    $this->Ncliente->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->Ncliente->CurrentValue = null;
                }
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ScoClienteList"); // Return to list page
                        return;
                        ...
                        if ($this->Ncliente->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->Ncliente->CurrentValue, $this->CurrentRow['Ncliente'])) {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ScoClienteList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "ScoClienteList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ScoClienteList") {
                            ...
                            $returnUrl = "ScoClienteList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Ncliente' first before field var 'x_Ncliente'
        $val = $CurrentForm->hasValue("Ncliente") ? $CurrentForm->getValue("Ncliente") : $CurrentForm->getValue("x_Ncliente");
        if (!$this->Ncliente->IsDetailKey) {
            $this->Ncliente->setFormValue($val, true, $validate);
        }

        // Check field name 'rif_ci' first before field var 'x_rif_ci'
        $val = $CurrentForm->hasValue("rif_ci") ? $CurrentForm->getValue("rif_ci") : $CurrentForm->getValue("x_rif_ci");
        if (!$this->rif_ci->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->rif_ci->Visible = false; // Disable update for API request
            } else {
                $this->rif_ci->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->Ncliente->CurrentValue = $this->Ncliente->FormValue;
        $this->rif_ci->CurrentValue = $this->rif_ci->FormValue;
    ...
    public function renderRow()
    {
        ...
        // Ncliente
        $this->Ncliente->RowCssClass = "row";

        // rif_ci
        $this->rif_ci->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->rif_ci->EditValue = HtmlEncode($this->rif_ci->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->Ncliente->Visible && $this->Ncliente->Required) {
                if (!$this->Ncliente->IsDetailKey && EmptyValue($this->Ncliente->FormValue)) {
                    $this->Ncliente->addErrorMessage(str_replace("%s", $this->Ncliente->caption(), $this->Ncliente->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->Ncliente->FormValue)) {
                $this->Ncliente->addErrorMessage($this->Ncliente->getErrorMessage(false));
            }
            if ($this->rif_ci->Visible && $this->rif_ci->Required) {
                if (!$this->rif_ci->IsDetailKey && EmptyValue($this->rif_ci->FormValue)) {
                    $this->rif_ci->addErrorMessage(str_replace("%s", $this->rif_ci->caption(), $this->rif_ci->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // rif_ci
        $this->rif_ci->setDbValueDef($rsnew, $this->rif_ci->CurrentValue, $this->rif_ci->ReadOnly);
    ...
    {
        if (isset($row['rif_ci'])) { // rif_ci
            $this->rif_ci->CurrentValue = $row['rif_ci'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ScoClienteList"), "", $this->TableVar, true);
        $pageId = "edit";
