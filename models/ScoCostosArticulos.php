...
/**
 * Table class for sco_costos_articulos
 */
class ScoCostosArticulos extends DbTable
{
    protected $SqlFrom = "";
    ...
    // Fields
    public $tipo;
    public $Ncostos_articulo;
    public $descripcion;
    public $precio;
    public $variacion;
    public $tipo_hercules;
    public $articulo_hercules;
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "sco_costos_articulos";
        $this->Dbid = 'DB';
        ...
        $this->tipo->Lookup = new Lookup($this->tipo, 'sco_costos_tipos', false, 'Ncostos_tipos', ["descripcion","","",""], '', '', [], [], [], [], [], [], false, '`Ncostos_tipos`', '', "`descripcion`");
        $this->tipo->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        $this->Fields['tipo'] = &$this->tipo;

        // Ncostos_articulo
        $this->Ncostos_articulo = new DbField(
            $this, // Table
            'x_Ncostos_articulo', // Variable name
            'Ncostos_articulo', // Name
            '`Ncostos_articulo`', // Expression
            '`Ncostos_articulo`', // Basic search expression
            200, // Type
            10, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Ncostos_articulo`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->Ncostos_articulo->InputTextType = "text";
        $this->Ncostos_articulo->Raw = true;
        $this->Ncostos_articulo->IsPrimaryKey = true; // Primary key field
        $this->Ncostos_articulo->Nullable = false; // NOT NULL field
        $this->Ncostos_articulo->Required = true; // Required field
        $this->Ncostos_articulo->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['Ncostos_articulo'] = &$this->Ncostos_articulo;
        ...
        // tipo_hercules
        $this->tipo_hercules = new DbField(
            $this, // Table
            'x_tipo_hercules', // Variable name
            'tipo_hercules', // Name
            '`tipo_hercules`', // Expression
            '`tipo_hercules`', // Basic search expression
            129, // Type
            4, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`tipo_hercules`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->tipo_hercules->InputTextType = "text";
        $this->tipo_hercules->setSelectMultiple(false); // Select one
        $this->tipo_hercules->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->tipo_hercules->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        $this->tipo_hercules->Lookup = new Lookup($this->tipo_hercules, 'sco_servicio_tipo', false, 'Nservicio_tipo', ["nombre","","",""], '', '', [], ["x_articulo_hercules"], [], [], [], [], false, '`nombre` ASC', '', "`nombre`");
        $this->tipo_hercules->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        $this->Fields['tipo_hercules'] = &$this->tipo_hercules;

        // articulo_hercules
        $this->articulo_hercules = new DbField(
            $this, // Table
            'x_articulo_hercules', // Variable name
            'articulo_hercules', // Name
            '`articulo_hercules`', // Expression
            '`articulo_hercules`', // Basic search expression
            200, // Type
            10, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`articulo_hercules`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->articulo_hercules->InputTextType = "text";
        $this->articulo_hercules->setSelectMultiple(false); // Select one
        $this->articulo_hercules->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->articulo_hercules->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        $this->articulo_hercules->Lookup = new Lookup($this->articulo_hercules, 'sco_servicio', false, 'Nservicio', ["nombre","","",""], '', '', ["x_tipo_hercules"], [], ["tipo"], ["x_tipo"], [], [], false, '`nombre` DESC', '', "`nombre`");
        $this->articulo_hercules->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        $this->Fields['articulo_hercules'] = &$this->articulo_hercules;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "sco_costos_articulos";
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('Ncostos_articulo', $rs)) {
                AddFilter($where, QuotedName('Ncostos_articulo', $this->Dbid) . '=' . QuotedValue($rs['Ncostos_articulo'], $this->Ncostos_articulo->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->Ncostos_articulo->DbValue = $row['Ncostos_articulo'];
        $this->descripcion->DbValue = $row['descripcion'];
        $this->precio->DbValue = $row['precio'];
        $this->variacion->DbValue = $row['variacion'];
        $this->tipo_hercules->DbValue = $row['tipo_hercules'];
        $this->articulo_hercules->DbValue = $row['articulo_hercules'];
    }
    ...
    {
        return "`Ncostos_articulo` = '@Ncostos_articulo@'";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->Ncostos_articulo->CurrentValue : $this->Ncostos_articulo->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->Ncostos_articulo->CurrentValue = $keys[0];
            } else {
                $this->Ncostos_articulo->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('Ncostos_articulo', $row) ? $row['Ncostos_articulo'] : null;
        } else {
            $val = !EmptyValue($this->Ncostos_articulo->OldValue) && !$current ? $this->Ncostos_articulo->OldValue : $this->Ncostos_articulo->CurrentValue;
        }
        if ($val === null) {
            return "0=1"; // Invalid key
        } else {
            $keyFilter = str_replace("@Ncostos_articulo@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ScoCostosArticulosList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ScoCostosArticulosView" => $Language->phrase("View"),
            "ScoCostosArticulosEdit" => $Language->phrase("Edit"),
            "ScoCostosArticulosAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ScoCostosArticulosView",
            Config("API_ADD_ACTION") => "ScoCostosArticulosAdd",
            Config("API_EDIT_ACTION") => "ScoCostosArticulosEdit",
            Config("API_DELETE_ACTION") => "ScoCostosArticulosDelete",
            Config("API_LIST_ACTION") => "ScoCostosArticulosList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ScoCostosArticulosView", $parm);
        } else {
            $url = $this->keyUrl("ScoCostosArticulosView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ScoCostosArticulosAdd?" . $parm;
        } else {
            $url = "ScoCostosArticulosAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoCostosArticulosEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoCostosArticulosList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoCostosArticulosAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoCostosArticulosList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ScoCostosArticulosDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"Ncostos_articulo\":" . VarToJson($this->Ncostos_articulo->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->Ncostos_articulo->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->Ncostos_articulo->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("Ncostos_articulo") ?? Route("Ncostos_articulo")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->Ncostos_articulo->CurrentValue = $key;
            } else {
                $this->Ncostos_articulo->OldValue = $key;
            }
    ...
    {
        global $Response;
        $listPage = "ScoCostosArticulosList";
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->Ncostos_articulo);
                    $doc->exportCaption($this->descripcion);
                    $doc->exportCaption($this->precio);
                    $doc->exportCaption($this->variacion);
                    $doc->exportCaption($this->tipo_hercules);
                    $doc->exportCaption($this->articulo_hercules);
                } else {
                    $doc->exportCaption($this->tipo);
                    $doc->exportCaption($this->Ncostos_articulo);
                    $doc->exportCaption($this->descripcion);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->Ncostos_articulo);
                        $doc->exportField($this->descripcion);
                        $doc->exportField($this->precio);
                        $doc->exportField($this->variacion);
                        $doc->exportField($this->tipo_hercules);
                        $doc->exportField($this->articulo_hercules);
                    } else {
                        $doc->exportField($this->tipo);
                        $doc->exportField($this->Ncostos_articulo);
                        $doc->exportField($this->descripcion);
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'sco_costos_articulos');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'sco_costos_articulos', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'sco_costos_articulos', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'sco_costos_articulos', $fldname, $key, $oldvalue);
            }
        }
    }
