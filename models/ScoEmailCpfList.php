 ...
 * Page class
 */
class ScoEmailCpfList extends ScoEmailCpf
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoEmailCpfList";
    ...
    public $FormName = "fsco_email_cpflist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoEmailCpfList";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'sco_email_cpf';
        $this->TableName = 'sco_email_cpf';
        ...
        // Table object (sco_email_cpf)
        if (!isset($GLOBALS["sco_email_cpf"]) || $GLOBALS["sco_email_cpf"]::class == PROJECT_NAMESPACE . "sco_email_cpf") {
            $GLOBALS["sco_email_cpf"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ScoEmailCpfAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoEmailCpfDelete";
        $this->MultiUpdateUrl = "ScoEmailCpfUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'sco_email_cpf');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ScoEmailCpfView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_email_cpfgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Nemail_cpf->AdvancedSearch->toJson(), ","); // Field Nemail_cpf
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_email_cpfsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Nemail_cpf
        $this->Nemail_cpf->AdvancedSearch->SearchValue = @$filter["x_Nemail_cpf"];
        $this->Nemail_cpf->AdvancedSearch->SearchOperator = @$filter["z_Nemail_cpf"];
        $this->Nemail_cpf->AdvancedSearch->SearchCondition = @$filter["v_Nemail_cpf"];
        $this->Nemail_cpf->AdvancedSearch->SearchValue2 = @$filter["y_Nemail_cpf"];
        $this->Nemail_cpf->AdvancedSearch->SearchOperator2 = @$filter["w_Nemail_cpf"];
        $this->Nemail_cpf->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Nemail_cpf, $default, false); // Nemail_cpf
        $this->buildSearchSql($where, $this->expediente, $default, false); // expediente
        $this->buildSearchSql($where, $this->fecha_hora, $default, false); // fecha_hora
        ...
        if (!$default && $this->Command == "search") {
            $this->Nemail_cpf->AdvancedSearch->save(); // Nemail_cpf
    ...
    public function showFilterList()
    {
        ...
        // Field Nemail_cpf
        $filter = $this->queryBuilderWhere("Nemail_cpf");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->Nemail_cpf, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->Nemail_cpf->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field fecha_hora
        $filter = $this->queryBuilderWhere("fecha_hora");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->fecha_hora, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->fecha_hora->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->Nemail_cpf->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->Nemail_cpf->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->Nemail_cpf->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->Nemail_cpf->Expression . " DESC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->Nemail_cpf); // Nemail_cpf
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_email_cpf\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_email_cpf\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_email_cpflist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_email_cpflist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Nemail_cpf->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "Nemail_cpf");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_email_cpfsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_email_cpfsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_email_cpflist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_email_cpf", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_email_cpf",
    ...
    protected function loadSearchValues()
    {
        ...
        // Nemail_cpf
        if ($this->Nemail_cpf->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Nemail_cpf->AdvancedSearch->SearchValue != "" || $this->Nemail_cpf->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // fecha_hora
        if ($this->fecha_hora->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->fecha_hora->AdvancedSearch->SearchValue != "" || $this->fecha_hora->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function loadRowValues($row = null)
    {
        ...
        $this->Nemail_cpf->setDbValue($row['Nemail_cpf']);
    ...
    {
        $row = [];
        $row['Nemail_cpf'] = $this->Nemail_cpf->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            // Nemail_cpf
            $this->Nemail_cpf->ViewValue = $this->Nemail_cpf->CurrentValue;
            ...
            // Nemail_cpf
            $this->Nemail_cpf->HrefValue = "";
            $this->Nemail_cpf->TooltipValue = "";
            ...
            // Nemail_cpf
            $this->Nemail_cpf->setupEditAttributes();
            $this->Nemail_cpf->EditValue = $this->Nemail_cpf->AdvancedSearch->SearchValue;
            $this->Nemail_cpf->PlaceHolder = RemoveHtml($this->Nemail_cpf->caption());
            ...
            $this->fecha_hora->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->fecha_hora->AdvancedSearch->SearchValue, $this->fecha_hora->formatPattern()), $this->fecha_hora->formatPattern()));
            $this->fecha_hora->PlaceHolder = RemoveHtml($this->fecha_hora->caption());
            $this->fecha_hora->setupEditAttributes();
            $this->fecha_hora->EditValue2 = HtmlEncode(FormatDateTime(UnFormatDateTime($this->fecha_hora->AdvancedSearch->SearchValue2, $this->fecha_hora->formatPattern()), $this->fecha_hora->formatPattern()));
            $this->fecha_hora->PlaceHolder = RemoveHtml($this->fecha_hora->caption());
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckDate($this->fecha_hora->AdvancedSearch->SearchValue, $this->fecha_hora->formatPattern())) {
            $this->fecha_hora->addErrorMessage($this->fecha_hora->getErrorMessage(false));
        }
        if (!CheckDate($this->fecha_hora->AdvancedSearch->SearchValue2, $this->fecha_hora->formatPattern())) {
            $this->fecha_hora->addErrorMessage($this->fecha_hora->getErrorMessage(false));
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_email_cpfsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
