 ...
 * Page class
 */
class AutogestionPlantillasList extends AutogestionPlantillas
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AutogestionPlantillasList";
    ...
    public $FormName = "fautogestion_plantillaslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "AutogestionPlantillasList";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'autogestion_plantillas';
        $this->TableName = 'autogestion_plantillas';
        ...
        // Table object (autogestion_plantillas)
        if (!isset($GLOBALS["autogestion_plantillas"]) || $GLOBALS["autogestion_plantillas"]::class == PROJECT_NAMESPACE . "autogestion_plantillas") {
            $GLOBALS["autogestion_plantillas"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "AutogestionPlantillasAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "AutogestionPlantillasDelete";
        $this->MultiUpdateUrl = "AutogestionPlantillasUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'autogestion_plantillas');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "AutogestionPlantillasView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fautogestion_plantillasgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Nplantilla->AdvancedSearch->toJson(), ","); // Field Nplantilla
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fautogestion_plantillassrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Nplantilla
        $this->Nplantilla->AdvancedSearch->SearchValue = @$filter["x_Nplantilla"];
        $this->Nplantilla->AdvancedSearch->SearchOperator = @$filter["z_Nplantilla"];
        $this->Nplantilla->AdvancedSearch->SearchCondition = @$filter["v_Nplantilla"];
        $this->Nplantilla->AdvancedSearch->SearchValue2 = @$filter["y_Nplantilla"];
        $this->Nplantilla->AdvancedSearch->SearchOperator2 = @$filter["w_Nplantilla"];
        $this->Nplantilla->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Nplantilla, $default, false); // Nplantilla
        ...
        $this->buildSearchSql($where, $this->nivel, $default, false); // nivel
        $this->buildSearchSql($where, $this->orden, $default, false); // orden
    ...
    public function showFilterList()
    {
        ...
        // Field nivel
        $filter = $this->queryBuilderWhere("nivel");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->nivel, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->nivel->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field orden
        $filter = $this->queryBuilderWhere("orden");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->orden, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->orden->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->Nplantilla->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->nivel->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->servicio_tipo->Expression . " ASC" . ", " . $this->nivel->Expression . " ASC" . ", " . $this->codigo->Expression . " ASC"; // Set up default sort
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"autogestion_plantillas\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"autogestion_plantillas\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fautogestion_plantillaslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fautogestion_plantillaslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Nplantilla->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"autogestion_plantillas\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fautogestion_plantillassrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fautogestion_plantillassrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fautogestion_plantillaslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_autogestion_plantillas", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_autogestion_plantillas",
    ...
    protected function loadSearchValues()
    {
        ...
        // Nplantilla
        if ($this->Nplantilla->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Nplantilla->AdvancedSearch->SearchValue != "" || $this->Nplantilla->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // nivel
        if ($this->nivel->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->nivel->AdvancedSearch->SearchValue != "" || $this->nivel->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // orden
        if ($this->orden->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->orden->AdvancedSearch->SearchValue != "" || $this->orden->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function loadRowValues($row = null)
    {
        ...
        $this->Nplantilla->setDbValue($row['Nplantilla']);
    ...
    {
        $row = [];
        $row['Nplantilla'] = $this->Nplantilla->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            // Nplantilla
            $this->Nplantilla->ViewValue = $this->Nplantilla->CurrentValue;
            ...
            if (strval($this->nivel->CurrentValue) != "") {
                $this->nivel->ViewValue = $this->nivel->optionCaption($this->nivel->CurrentValue);
            } else {
            ...
            $this->nivel->EditValue = $this->nivel->options(true);
            $this->nivel->PlaceHolder = RemoveHtml($this->nivel->caption());

            // orden
            $this->orden->setupEditAttributes();
            $this->orden->EditValue = $this->orden->AdvancedSearch->SearchValue;
            ...
            // mostrar
            $this->mostrar->EditValue = $this->mostrar->options(false);
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fautogestion_plantillassrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
