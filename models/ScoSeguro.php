...
/**
 * Table class for sco_seguro
 */
class ScoSeguro extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Nseguro
        $this->Nseguro = new DbField(
            $this, // Table
            'x_Nseguro', // Variable name
            'Nseguro', // Name
            '`Nseguro`', // Expression
            '`Nseguro`', // Basic search expression
            16, // Type
            4, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Nseguro`', // Virtual expression
            false, // Is virtual
        ...
        $this->Nseguro->InputTextType = "text";
        $this->Nseguro->Raw = true;
        $this->Nseguro->IsAutoIncrement = true; // Autoincrement field
        $this->Nseguro->IsPrimaryKey = true; // Primary key field
        $this->Nseguro->Nullable = false; // NOT NULL field
        $this->Nseguro->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Nseguro->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['Nseguro'] = &$this->Nseguro;
        ...
        $this->tipo_contratacion->Lookup = new Lookup($this->tipo_contratacion, 'sco_seguro', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        $this->tipo_contratacion->OptionCount = 2;
        ...
        $this->ci_rif->Nullable = false; // NOT NULL field
        $this->ci_rif->Required = true; // Required field
        $this->ci_rif->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        ...
        // email3
        $this->email3 = new DbField(
            $this, // Table
            'x_email3', // Variable name
            'email3', // Name
            '`email3`', // Expression
            '`email3`', // Basic search expression
            200, // Type
            100, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`email3`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->email3->InputTextType = "text";
        $this->email3->DefaultErrorMessage = $Language->phrase("IncorrectEmail");
        $this->email3->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        ...
        $this->activo->Lookup = new Lookup($this->activo, 'sco_seguro', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "sco_seguro";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->Nseguro->setDbValue($conn->lastInsertId());
            $rs['Nseguro'] = $this->Nseguro->DbValue;
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['Nseguro']) && !EmptyValue($this->Nseguro->CurrentValue)) {
                $rs['Nseguro'] = $this->Nseguro->CurrentValue;
            }
        }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('Nseguro', $rs)) {
                AddFilter($where, QuotedName('Nseguro', $this->Dbid) . '=' . QuotedValue($rs['Nseguro'], $this->Nseguro->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->Nseguro->DbValue = $row['Nseguro'];
    ...
    {
        $keys = [];
        $val = $current ? $this->Nseguro->CurrentValue : $this->Nseguro->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->Nseguro->CurrentValue = $keys[0];
            } else {
                $this->Nseguro->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('Nseguro', $row) ? $row['Nseguro'] : null;
        } else {
            $val = !EmptyValue($this->Nseguro->OldValue) && !$current ? $this->Nseguro->OldValue : $this->Nseguro->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@Nseguro@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ScoSeguroList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ScoSeguroView" => $Language->phrase("View"),
            "ScoSeguroEdit" => $Language->phrase("Edit"),
            "ScoSeguroAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ScoSeguroView",
            Config("API_ADD_ACTION") => "ScoSeguroAdd",
            Config("API_EDIT_ACTION") => "ScoSeguroEdit",
            Config("API_DELETE_ACTION") => "ScoSeguroDelete",
            Config("API_LIST_ACTION") => "ScoSeguroList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ScoSeguroView", $parm);
        } else {
            $url = $this->keyUrl("ScoSeguroView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoSeguroEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoSeguroList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoSeguroAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoSeguroList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ScoSeguroDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"Nseguro\":" . VarToJson($this->Nseguro->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->Nseguro->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->Nseguro->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("Nseguro") ?? Route("Nseguro")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->Nseguro->CurrentValue = $key;
            } else {
                $this->Nseguro->OldValue = $key;
            }
    ...
    public function renderListRow()
    {
        ...
        // Nseguro
        $this->Nseguro->HrefValue = "";
        $this->Nseguro->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // Nseguro
        $this->Nseguro->setupEditAttributes();
        $this->Nseguro->EditValue = $this->Nseguro->CurrentValue;
        ...
        // email3
        $this->email3->setupEditAttributes();
        if (!$this->email3->Raw) {
            $this->email3->CurrentValue = HtmlDecode($this->email3->CurrentValue);
        }
        $this->email3->EditValue = $this->email3->CurrentValue;
        $this->email3->PlaceHolder = RemoveHtml($this->email3->caption());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->Nseguro);
                    $doc->exportCaption($this->tipo_contratacion);
                    $doc->exportCaption($this->ci_rif);
                    $doc->exportCaption($this->nombre);
                    $doc->exportCaption($this->contacto);
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'sco_seguro');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'sco_seguro', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'sco_seguro', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'sco_seguro', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowInserting($rsold, &$rsnew) {
    	...
    	$sql = "select count(*) as total from sco_seguro where ci_rif = '".$rsnew["ci_rif"]."';";
    	$total = ExecuteScalar($sql);
    	if($total>0)
    	{
    		$this->CancelMessage = "El RIF: ".$rsnew["ci_rif"]." ya se encuentra registrado, no se puede agregar el nuevo registro.";
    		return FALSE;
    	}
    	return TRUE;
    }
    ...
    public function rowDeleting(&$rs) {
    	// Enter your code here
    	// To cancel, set return value to False
    	$sql = "select count(*) as total from sco_expediente where seguro = '".$rs["Nseguro"]."';";
    	$total = ExecuteScalar($sql);
    	if($total>0)
    	{
    		$this->CancelMessage = "Hay expedientes asociadas a este seguro, no se puede eliminar.";
    		return FALSE;
    	}
    	return TRUE;
    }
