...
/**
 * Table class for view_exp_factura
 */
class ViewExpFactura extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "view_exp_factura";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->cia->Lookup = new Lookup($this->cia, 'sco_cia', false, 'Ncia', ["nombre","","",""], '', '', [], [], [], [], [], [], false, '', '', "`nombre`");
        $this->cia->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->cia->SearchOperators = ["=", "<>", "IS NULL", "IS NOT NULL"];
        ...
        $this->servicio_tipo->Lookup = new Lookup($this->servicio_tipo, 'sco_servicio_tipo', false, 'Nservicio_tipo', ["nombre","","",""], '', '', [], [], [], [], [], [], false, '', '', "`nombre`");
        ...
        $this->Nexpediente->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        ...
        $this->estatus->Lookup = new Lookup($this->estatus, 'view_exp_factura', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "view_exp_factura";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ViewExpFacturaList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ViewExpFacturaView" => $Language->phrase("View"),
            "ViewExpFacturaEdit" => $Language->phrase("Edit"),
            "ViewExpFacturaAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ViewExpFacturaView",
            Config("API_ADD_ACTION") => "ViewExpFacturaAdd",
            Config("API_EDIT_ACTION") => "ViewExpFacturaEdit",
            Config("API_DELETE_ACTION") => "ViewExpFacturaDelete",
            Config("API_LIST_ACTION") => "ViewExpFacturaList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ViewExpFacturaView", $parm);
        } else {
            $url = $this->keyUrl("ViewExpFacturaView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ViewExpFacturaAdd?" . $parm;
        } else {
            $url = "ViewExpFacturaAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewExpFacturaEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewExpFacturaList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewExpFacturaAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ViewExpFacturaList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ViewExpFacturaDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"Nexpediente\":" . VarToJson($this->Nexpediente->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function renderEditRow()
    {
        ...
        if (!$this->Nexpediente->Raw) {
            $this->Nexpediente->CurrentValue = HtmlDecode($this->Nexpediente->CurrentValue);
        }
    ...
    public function rowRendering()
    {
        ...
        if ($this->PageID == "list" || $this->PageID == "view") { // List/View page only
            ...
            $sql = "SELECT estatus FROM sco_expediente WHERE Nexpediente = '" . $this->Nexpediente->CurrentValue . "'";
            if(trim(ExecuteScalar($sql) ?? '') == "7") {
                $this->cia->CellAttrs["style"] = $style;
                $this->nombre_fallecido->CellAttrs["style"] = $style;
                $this->apellidos_fallecido->CellAttrs["style"] = $style;
                $this->factura->CellAttrs["style"] = $style;
