 ...
 * Page class
 */
class ViewParcelaVentasView extends ViewParcelaVentas
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewParcelaVentasView";
    ...
    // CSS class/style
    public $CurrentPageName = "ViewParcelaVentasView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("Nparcela_ventas") ?? Route("Nparcela_ventas")) !== null) {
            $this->RecKey["Nparcela_ventas"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("Nparcela_ventas") ?? Route("Nparcela_ventas")) !== null) {
            ...
            $this->RecKey["Nparcela_ventas"] = $this->Nparcela_ventas->QueryStringValue;
        } elseif (Post("Nparcela_ventas") !== null) {
            $this->Nparcela_ventas->setFormValue(Post("Nparcela_ventas"));
            $this->RecKey["Nparcela_ventas"] = $this->Nparcela_ventas->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->Nparcela_ventas->setQueryStringValue($keyValue);
            $this->RecKey["Nparcela_ventas"] = $this->Nparcela_ventas->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "ViewParcelaVentasList"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "ViewParcelaVentasList"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $item = &$option->add("detail_sco_parcela_ventas_nota");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("sco_parcela_ventas_nota", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("ScoParcelaVentasNotaList?" . Config("TABLE_SHOW_MASTER") . "=view_parcela_ventas&" . GetForeignKeyUrl("fk_Nparcela_ventas", $this->Nparcela_ventas->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'view_parcela_ventas')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_parcela_ventas_nota"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'view_parcela_ventas')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_parcela_ventas_nota"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
    ...
    public function pageRedirecting(&$url) {
    	// Example:
        if($this->estatus->CurrentValue == "COMPRA") {
    	    $id = $this->Nparcela_ventas->CurrentValue;
            $url = "ScoParcelaVentasView/$id?showdetail=";
        }
    }
