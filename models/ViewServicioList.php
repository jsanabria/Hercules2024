 ...
 * Page class
 */
class ViewServicioList extends ViewServicio
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewServicioList";
    ...
    public $FormName = "fview_serviciolist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ViewServicioList";
    ...
    public function setVisibility()
    {
        ...
        $this->fecha_sepelio->setVisibility();
        $this->hora_sepelio->setVisibility();
        $this->estatus->setVisibility();
        $this->fecha_ofirel->Visible = false;
        $this->hora_ofirel->Visible = false;
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'view_servicio';
        $this->TableName = 'view_servicio';
        ...
        // Table object (view_servicio)
        if (!isset($GLOBALS["view_servicio"]) || $GLOBALS["view_servicio"]::class == PROJECT_NAMESPACE . "view_servicio") {
            $GLOBALS["view_servicio"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ViewServicioAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ViewServicioDelete";
        $this->MultiUpdateUrl = "ViewServicioUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'view_servicio');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ViewServicioView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_serviciogrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->fecha_sepelio->AdvancedSearch->toJson(), ","); // Field fecha_sepelio
        $filterList = Concat($filterList, $this->hora_sepelio->AdvancedSearch->toJson(), ","); // Field hora_sepelio
        $filterList = Concat($filterList, $this->estatus->AdvancedSearch->toJson(), ","); // Field estatus
        $filterList = Concat($filterList, $this->fecha_ofirel->AdvancedSearch->toJson(), ","); // Field fecha_ofirel
        $filterList = Concat($filterList, $this->hora_ofirel->AdvancedSearch->toJson(), ","); // Field hora_ofirel
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_serviciosrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field fecha_sepelio
        $this->fecha_sepelio->AdvancedSearch->SearchValue = @$filter["x_fecha_sepelio"];
        $this->fecha_sepelio->AdvancedSearch->SearchOperator = @$filter["z_fecha_sepelio"];
        $this->fecha_sepelio->AdvancedSearch->SearchCondition = @$filter["v_fecha_sepelio"];
        $this->fecha_sepelio->AdvancedSearch->SearchValue2 = @$filter["y_fecha_sepelio"];
        $this->fecha_sepelio->AdvancedSearch->SearchOperator2 = @$filter["w_fecha_sepelio"];
        $this->fecha_sepelio->AdvancedSearch->save();

        // Field hora_sepelio
        $this->hora_sepelio->AdvancedSearch->SearchValue = @$filter["x_hora_sepelio"];
        $this->hora_sepelio->AdvancedSearch->SearchOperator = @$filter["z_hora_sepelio"];
        $this->hora_sepelio->AdvancedSearch->SearchCondition = @$filter["v_hora_sepelio"];
        $this->hora_sepelio->AdvancedSearch->SearchValue2 = @$filter["y_hora_sepelio"];
        $this->hora_sepelio->AdvancedSearch->SearchOperator2 = @$filter["w_hora_sepelio"];
        $this->hora_sepelio->AdvancedSearch->save();
        ...
        // Field fecha_ofirel
        $this->fecha_ofirel->AdvancedSearch->SearchValue = @$filter["x_fecha_ofirel"];
        $this->fecha_ofirel->AdvancedSearch->SearchOperator = @$filter["z_fecha_ofirel"];
        $this->fecha_ofirel->AdvancedSearch->SearchCondition = @$filter["v_fecha_ofirel"];
        $this->fecha_ofirel->AdvancedSearch->SearchValue2 = @$filter["y_fecha_ofirel"];
        $this->fecha_ofirel->AdvancedSearch->SearchOperator2 = @$filter["w_fecha_ofirel"];
        $this->fecha_ofirel->AdvancedSearch->save();

        // Field hora_ofirel
        $this->hora_ofirel->AdvancedSearch->SearchValue = @$filter["x_hora_ofirel"];
        $this->hora_ofirel->AdvancedSearch->SearchOperator = @$filter["z_hora_ofirel"];
        $this->hora_ofirel->AdvancedSearch->SearchCondition = @$filter["v_hora_ofirel"];
        $this->hora_ofirel->AdvancedSearch->SearchValue2 = @$filter["y_hora_ofirel"];
        $this->hora_ofirel->AdvancedSearch->SearchOperator2 = @$filter["w_hora_ofirel"];
        $this->hora_ofirel->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->tipo_servicio, $default, false); // tipo_servicio
        ...
        $this->buildSearchSql($where, $this->fecha_sepelio, $default, false); // fecha_sepelio
        $this->buildSearchSql($where, $this->hora_sepelio, $default, false); // hora_sepelio
        $this->buildSearchSql($where, $this->estatus, $default, false); // estatus
        $this->buildSearchSql($where, $this->fecha_ofirel, $default, false); // fecha_ofirel
        $this->buildSearchSql($where, $this->hora_ofirel, $default, false); // hora_ofirel
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->tipo_servicio->AdvancedSearch->save(); // tipo_servicio
            ...
            $this->fecha_sepelio->AdvancedSearch->save(); // fecha_sepelio
            $this->hora_sepelio->AdvancedSearch->save(); // hora_sepelio
            $this->estatus->AdvancedSearch->save(); // estatus
            $this->fecha_ofirel->AdvancedSearch->save(); // fecha_ofirel
            $this->hora_ofirel->AdvancedSearch->save(); // hora_ofirel
    ...
    public function showFilterList()
    {
        ...
        // Field tipo_servicio
        $filter = $this->queryBuilderWhere("tipo_servicio");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->tipo_servicio, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->tipo_servicio->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field fecha_sepelio
        $filter = $this->queryBuilderWhere("fecha_sepelio");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->fecha_sepelio, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->fecha_sepelio->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field hora_sepelio
        $filter = $this->queryBuilderWhere("hora_sepelio");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->hora_sepelio, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->hora_sepelio->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->tipo_servicio->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->fecha_sepelio->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->hora_sepelio->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->estatus->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->fecha_ofirel->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->hora_ofirel->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->fecha_sepelio->AdvancedSearch->loadDefault();
        return true;
    }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->tipo_servicio->AdvancedSearch->unsetSession();
        ...
        $this->fecha_sepelio->AdvancedSearch->unsetSession();
        $this->hora_sepelio->AdvancedSearch->unsetSession();
        $this->estatus->AdvancedSearch->unsetSession();
        $this->fecha_ofirel->AdvancedSearch->unsetSession();
        $this->hora_ofirel->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->tipo_servicio->AdvancedSearch->load();
        ...
        $this->fecha_sepelio->AdvancedSearch->load();
        $this->hora_sepelio->AdvancedSearch->load();
        $this->estatus->AdvancedSearch->load();
        $this->fecha_ofirel->AdvancedSearch->load();
        $this->hora_ofirel->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->fecha_inicio->Expression . " ASC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->tipo_servicio); // tipo_servicio
            ...
            $this->updateSort($this->fecha_sepelio); // fecha_sepelio
            $this->updateSort($this->hora_sepelio); // hora_sepelio
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->tipo_servicio->setSort("");
                $this->ataud->setSort("");
                $this->fecha_inicio->setSort("");
                $this->hora_inicio->setSort("");
                $this->fecha_fin->setSort("");
                $this->hora_fin->setSort("");
                $this->fecha_sepelio->setSort("");
                $this->hora_sepelio->setSort("");
                $this->estatus->setSort("");
                $this->fecha_ofirel->setSort("");
                $this->hora_ofirel->setSort("");
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_serviciolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_serviciolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "tipo_servicio");
            ...
            $this->createColumnOption($option, "fecha_sepelio");
            $this->createColumnOption($option, "hora_sepelio");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_serviciosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_serviciosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_serviciolist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_servicio", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_servicio",
    ...
    protected function loadSearchValues()
    {
        ...
        // tipo_servicio
        if ($this->tipo_servicio->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->tipo_servicio->AdvancedSearch->SearchValue != "" || $this->tipo_servicio->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // fecha_sepelio
        if ($this->fecha_sepelio->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->fecha_sepelio->AdvancedSearch->SearchValue != "" || $this->fecha_sepelio->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // hora_sepelio
        if ($this->hora_sepelio->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->hora_sepelio->AdvancedSearch->SearchValue != "" || $this->hora_sepelio->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // fecha_ofirel
        if ($this->fecha_ofirel->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->fecha_ofirel->AdvancedSearch->SearchValue != "" || $this->fecha_ofirel->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // hora_ofirel
        if ($this->hora_ofirel->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->hora_ofirel->AdvancedSearch->SearchValue != "" || $this->hora_ofirel->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function loadRowValues($row = null)
    {
        ...
        $this->fecha_sepelio->setDbValue($row['fecha_sepelio']);
        $this->hora_sepelio->setDbValue($row['hora_sepelio']);
        $this->estatus->setDbValue($row['estatus']);
        $this->fecha_ofirel->setDbValue($row['fecha_ofirel']);
        $this->hora_ofirel->setDbValue($row['hora_ofirel']);
    ...
    protected function newRow()
    {
        ...
        $row['fecha_sepelio'] = $this->fecha_sepelio->DefaultValue;
        $row['hora_sepelio'] = $this->hora_sepelio->DefaultValue;
        $row['estatus'] = $this->estatus->DefaultValue;
        $row['fecha_ofirel'] = $this->fecha_ofirel->DefaultValue;
        $row['hora_ofirel'] = $this->hora_ofirel->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // fecha_sepelio
            $this->fecha_sepelio->ViewValue = $this->fecha_sepelio->CurrentValue;
            $this->fecha_sepelio->ViewValue = FormatDateTime($this->fecha_sepelio->ViewValue, $this->fecha_sepelio->formatPattern());

            // hora_sepelio
            $this->hora_sepelio->ViewValue = $this->hora_sepelio->CurrentValue;

            // estatus
            $this->estatus->ViewValue = $this->estatus->CurrentValue;

            // fecha_ofirel
            $this->fecha_ofirel->ViewValue = $this->fecha_ofirel->CurrentValue;
            $this->fecha_ofirel->ViewValue = FormatDateTime($this->fecha_ofirel->ViewValue, $this->fecha_ofirel->formatPattern());

            // hora_ofirel
            $this->hora_ofirel->ViewValue = $this->hora_ofirel->CurrentValue;
            ...
            // fecha_sepelio
            $this->fecha_sepelio->HrefValue = "";
            $this->fecha_sepelio->TooltipValue = "";

            // hora_sepelio
            $this->hora_sepelio->HrefValue = "";
            $this->hora_sepelio->TooltipValue = "";
            ...
            if (!$this->capilla->Raw) {
                $this->capilla->AdvancedSearch->SearchValue = HtmlDecode($this->capilla->AdvancedSearch->SearchValue);
            }
            $this->capilla->EditValue = HtmlEncode($this->capilla->AdvancedSearch->SearchValue);
            $this->capilla->PlaceHolder = RemoveHtml($this->capilla->caption());

            // tipo_servicio
            $this->tipo_servicio->setupEditAttributes();
            if (!$this->tipo_servicio->Raw) {
                $this->tipo_servicio->AdvancedSearch->SearchValue = HtmlDecode($this->tipo_servicio->AdvancedSearch->SearchValue);
            }
            $this->tipo_servicio->EditValue = HtmlEncode($this->tipo_servicio->AdvancedSearch->SearchValue);
            ...
            if (!$this->hora_inicio->Raw) {
                $this->hora_inicio->AdvancedSearch->SearchValue = HtmlDecode($this->hora_inicio->AdvancedSearch->SearchValue);
            }
            $this->hora_inicio->EditValue = HtmlEncode($this->hora_inicio->AdvancedSearch->SearchValue);
            ...
            if (!$this->hora_fin->Raw) {
                $this->hora_fin->AdvancedSearch->SearchValue = HtmlDecode($this->hora_fin->AdvancedSearch->SearchValue);
            }
            $this->hora_fin->EditValue = HtmlEncode($this->hora_fin->AdvancedSearch->SearchValue);
            $this->hora_fin->PlaceHolder = RemoveHtml($this->hora_fin->caption());

            // fecha_sepelio
            $this->fecha_sepelio->setupEditAttributes();
            $this->fecha_sepelio->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->fecha_sepelio->AdvancedSearch->SearchValue, $this->fecha_sepelio->formatPattern()), $this->fecha_sepelio->formatPattern()));
            $this->fecha_sepelio->PlaceHolder = RemoveHtml($this->fecha_sepelio->caption());
            $this->fecha_sepelio->setupEditAttributes();
            $this->fecha_sepelio->EditValue2 = HtmlEncode(FormatDateTime(UnFormatDateTime($this->fecha_sepelio->AdvancedSearch->SearchValue2, $this->fecha_sepelio->formatPattern()), $this->fecha_sepelio->formatPattern()));
            $this->fecha_sepelio->PlaceHolder = RemoveHtml($this->fecha_sepelio->caption());

            // hora_sepelio
            $this->hora_sepelio->setupEditAttributes();
            if (!$this->hora_sepelio->Raw) {
                $this->hora_sepelio->AdvancedSearch->SearchValue = HtmlDecode($this->hora_sepelio->AdvancedSearch->SearchValue);
            }
            $this->hora_sepelio->EditValue = HtmlEncode($this->hora_sepelio->AdvancedSearch->SearchValue);
            $this->hora_sepelio->PlaceHolder = RemoveHtml($this->hora_sepelio->caption());

            // estatus
            $this->estatus->setupEditAttributes();
            $this->estatus->EditValue = $this->estatus->AdvancedSearch->SearchValue;
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckDate($this->fecha_sepelio->AdvancedSearch->SearchValue, $this->fecha_sepelio->formatPattern())) {
            $this->fecha_sepelio->addErrorMessage($this->fecha_sepelio->getErrorMessage(false));
        }
        if (!CheckDate($this->fecha_sepelio->AdvancedSearch->SearchValue2, $this->fecha_sepelio->formatPattern())) {
            $this->fecha_sepelio->addErrorMessage($this->fecha_sepelio->getErrorMessage(false));
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_serviciosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    public function pageDataRendering(&$header) {
        // El nombre del formulario ('fview_serviciolist') se pasa como argumento.
        $form_id = 'fview_serviciolist';

        // CAMBIOS CLAVE:
        // 1. Clase cambiada a "btn-primary" (azul).
        // 2. Estilo añadido al <a> para forzar el texto blanco (color: white;).
        $header = '<span data-phrase="PrintBtn"
                          class="btn btn-primary ewButton" 
                          data-caption="Imprimir Orden"
                          title="Imprimir Órdenes de Preparación seleccionadas">
                        <i class="glyphicon glyphicon-print ewIcon"></i>
                        <a href="javascript:void(0);" style="color: white; text-decoration: none;"
                           onclick="(() => { printprepa(\'' . $form_id . '\', 1); })();">
                            Imprimir Orden
                        </a>
                      </span>';
    }
    ...
    public function listOptionsLoad() {
    	...
    	$opt = &$this->ListOptions->Add("new");
    	$opt->Header = '<input type="checkbox" value="" id="selAll" name="selAll" />';
    	$opt->OnLeft = TRUE; // Link on left
    	$opt->MoveTo(0); // Move to first column
    }
    ...
    public function listOptionsRendered() {
    	...
    	$this->ListOptions->Items["new"]->Body = '<input type="checkbox" value="' . $this->Nexpediente->CurrentValue . '" id="prepa[]" name="prepa[]" />';
    }
