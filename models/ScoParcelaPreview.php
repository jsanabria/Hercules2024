 ...
 * Page class
 */
class ScoParcelaPreview extends ScoParcela
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoParcelaPreview";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoParcelaPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "sco_expediente") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=sco_expediente&" . GetForeignKeyUrl("fk_contrato_parcela", $this->contrato->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys($keys)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "sco_expediente") {
            $this->contrato->setQueryStringValue($keys[0] ?? "");
        }
    }
