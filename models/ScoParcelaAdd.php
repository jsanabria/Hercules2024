 ...
 * Page class
 */
class ScoParcelaAdd extends ScoParcela
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoParcelaAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoParcelaAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "ScoParcelaView") {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->Nparcela->IsDetailKey) {
            $this->Nparcela->setFormValue($val);
        }
    }
    ...
    protected function getAddRow()
    {
        ...
        // direc1
        $this->direc1->setDbValueDef($rsnew, $this->direc1->CurrentValue, false);

        // direc2
        $this->direc2->setDbValueDef($rsnew, $this->direc2->CurrentValue, false);

        // email
        $this->_email->setDbValueDef($rsnew, $this->_email->CurrentValue, false);

        // nac_ci_asociado
        $this->nac_ci_asociado->setDbValueDef($rsnew, $this->nac_ci_asociado->CurrentValue, false);

        // ci_asociado
        $this->ci_asociado->setDbValueDef($rsnew, $this->ci_asociado->CurrentValue, false);

        // nombre_asociado
        $this->nombre_asociado->setDbValueDef($rsnew, $this->nombre_asociado->CurrentValue, false);

        // nac_difunto
        $this->nac_difunto->setDbValueDef($rsnew, $this->nac_difunto->CurrentValue, false);

        // ci_difunto
        $this->ci_difunto->setDbValueDef($rsnew, $this->ci_difunto->CurrentValue, false);

        // edad
        $this->edad->setDbValueDef($rsnew, $this->edad->CurrentValue, false);

        // edo_civil
        $this->edo_civil->setDbValueDef($rsnew, $this->edo_civil->CurrentValue, false);

        // fecha_nacimiento
        $this->fecha_nacimiento->setDbValueDef($rsnew, $this->fecha_nacimiento->CurrentValue, false);

        // lugar
        $this->lugar->setDbValueDef($rsnew, $this->lugar->CurrentValue, false);

        // fecha_defuncion
        $this->fecha_defuncion->setDbValueDef($rsnew, $this->fecha_defuncion->CurrentValue, false);

        // causa
        $this->causa->setDbValueDef($rsnew, $this->causa->CurrentValue, false);

        // certificado
        $this->certificado->setDbValueDef($rsnew, $this->certificado->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['lugar'])) { // lugar
            $this->lugar->setFormValue($row['lugar']);
        }
        if (isset($row['fecha_defuncion'])) { // fecha_defuncion
            $this->fecha_defuncion->setFormValue($row['fecha_defuncion']);
        }
        if (isset($row['causa'])) { // causa
            $this->causa->setFormValue($row['causa']);
        }
        if (isset($row['certificado'])) { // certificado
            $this->certificado->setFormValue($row['certificado']);
        }
