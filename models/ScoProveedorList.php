 ...
 * Page class
 */
class ScoProveedorList extends ScoProveedor
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoProveedorList";
    ...
    public $FormName = "fsco_proveedorlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoProveedorList";
    ...
    public function setVisibility()
    {
        ...
        $this->telefono4->Visible = false;
        $this->fax->Visible = false;
        $this->correo->Visible = false;
        $this->correo_adicional->Visible = false;
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ScoProveedorAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoProveedorDelete";
        $this->MultiUpdateUrl = "ScoProveedorUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_proveedorgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Nproveedor->AdvancedSearch->toJson(), ","); // Field Nproveedor
        ...
        $filterList = Concat($filterList, $this->telefono4->AdvancedSearch->toJson(), ","); // Field telefono4
        ...
        $filterList = Concat($filterList, $this->correo_adicional->AdvancedSearch->toJson(), ","); // Field correo_adicional
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_proveedorsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Nproveedor
        $this->Nproveedor->AdvancedSearch->SearchValue = @$filter["x_Nproveedor"];
        $this->Nproveedor->AdvancedSearch->SearchOperator = @$filter["z_Nproveedor"];
        $this->Nproveedor->AdvancedSearch->SearchCondition = @$filter["v_Nproveedor"];
        $this->Nproveedor->AdvancedSearch->SearchValue2 = @$filter["y_Nproveedor"];
        $this->Nproveedor->AdvancedSearch->SearchOperator2 = @$filter["w_Nproveedor"];
        $this->Nproveedor->AdvancedSearch->save();
        ...
        // Field correo_adicional
        $this->correo_adicional->AdvancedSearch->SearchValue = @$filter["x_correo_adicional"];
        $this->correo_adicional->AdvancedSearch->SearchOperator = @$filter["z_correo_adicional"];
        $this->correo_adicional->AdvancedSearch->SearchCondition = @$filter["v_correo_adicional"];
        $this->correo_adicional->AdvancedSearch->SearchValue2 = @$filter["y_correo_adicional"];
        $this->correo_adicional->AdvancedSearch->SearchOperator2 = @$filter["w_correo_adicional"];
        $this->correo_adicional->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Nproveedor, $default, false); // Nproveedor
        $this->buildSearchSql($where, $this->rif, $default, false); // rif
        $this->buildSearchSql($where, $this->nombre, $default, false); // nombre
        $this->buildSearchSql($where, $this->sucursal, $default, false); // sucursal
        $this->buildSearchSql($where, $this->responsable, $default, false); // responsable
        ...
        $this->buildSearchSql($where, $this->telefono4, $default, false); // telefono4
        $this->buildSearchSql($where, $this->fax, $default, false); // fax
        $this->buildSearchSql($where, $this->correo, $default, false); // correo
        $this->buildSearchSql($where, $this->correo_adicional, $default, false); // correo_adicional
        $this->buildSearchSql($where, $this->estado, $default, false); // estado
        $this->buildSearchSql($where, $this->localidad, $default, false); // localidad
        ...
        $this->buildSearchSql($where, $this->tipo_proveedor, $default, false); // tipo_proveedor
        ...
        if (!$default && $this->Command == "search") {
            $this->Nproveedor->AdvancedSearch->save(); // Nproveedor
            $this->rif->AdvancedSearch->save(); // rif
            ...
            $this->correo_adicional->AdvancedSearch->save(); // correo_adicional
            ...
            $this->tipo_proveedor->AdvancedSearch->save(); // tipo_proveedor
    ...
    public function showFilterList()
    {
        ...
        // Field Nproveedor
        $filter = $this->queryBuilderWhere("Nproveedor");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->Nproveedor, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->Nproveedor->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field rif
        $filter = $this->queryBuilderWhere("rif");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->rif, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->rif->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field tipo_proveedor
        $filter = $this->queryBuilderWhere("tipo_proveedor");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->tipo_proveedor, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->tipo_proveedor->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->telefono4;
        $searchFlds[] = &$this->fax;
        $searchFlds[] = &$this->correo;
        $searchFlds[] = &$this->correo_adicional;
        $searchFlds[] = &$this->localidad;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->Nproveedor->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->rif->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->correo_adicional->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->tipo_proveedor->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->Nproveedor->AdvancedSearch->unsetSession();
        $this->rif->AdvancedSearch->unsetSession();
        ...
        $this->correo_adicional->AdvancedSearch->unsetSession();
        ...
        $this->tipo_proveedor->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->Nproveedor->AdvancedSearch->load();
        $this->rif->AdvancedSearch->load();
        ...
        $this->correo_adicional->AdvancedSearch->load();
        ...
        $this->tipo_proveedor->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->Nproveedor); // Nproveedor
            $this->updateSort($this->rif); // rif
            $this->updateSort($this->nombre); // nombre
            $this->updateSort($this->tipo_proveedor); // tipo_proveedor
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_sco_ofrece_servicio"
        $item = &$this->ListOptions->add("detail_sco_ofrece_servicio");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'sco_ofrece_servicio');
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_proveedor\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_proveedor\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_proveedorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_proveedorlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_sco_ofrece_servicio"
        $opt = $this->ListOptions["detail_sco_ofrece_servicio"];
        if ($Security->allowList(CurrentProjectID() . 'sco_ofrece_servicio')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("sco_ofrece_servicio", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("ScoOfreceServicioList?" . Config("TABLE_SHOW_MASTER") . "=sco_proveedor&" . GetForeignKeyUrl("fk_Nproveedor", $this->Nproveedor->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("ScoOfreceServicioGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_proveedor')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_ofrece_servicio");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "sco_ofrece_servicio";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_proveedor')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_ofrece_servicio");
                ...
                $detailEditTblVar .= "sco_ofrece_servicio";
            }
            if ($links != "") {
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Nproveedor->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function renderListOptionsExt()
    {
        ...
        $detailFilters = [];
        $masterKeys = []; // Reset
        $masterKeys["Nproveedor"] = strval($this->Nproveedor->DbValue);

        // Column "detail_sco_ofrece_servicio"
        if ($this->DetailPages?->getItem("sco_ofrece_servicio")?->Visible && $Security->allowList(CurrentProjectID() . 'sco_ofrece_servicio')) {
            $link = "";
            $option = $this->ListOptions["detail_sco_ofrece_servicio"];
            $detailTbl = Container("sco_ofrece_servicio");
            $detailFilter = $detailTbl->getDetailFilter($this);
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "ScoOfreceServicioPreview?t=sco_proveedor&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"sco_ofrece_servicio\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'sco_proveedor')) {
                $label = $Language->tablePhrase("sco_ofrece_servicio", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"sco_ofrece_servicio\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("ScoOfreceServicioList?" . Config("TABLE_SHOW_MASTER") . "=sco_proveedor");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $Language->tablePhrase("sco_ofrece_servicio", "TblCaption");
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_proveedor')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_ofrece_servicio"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($viewurl) . "\">" . $caption . "</a>";
            }
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_proveedor')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_ofrece_servicio"));
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_proveedor\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $detailTableLink = "";
        $item = &$option->add("detailadd_sco_ofrece_servicio");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=sco_ofrece_servicio");
        $detailPage = Container("ScoOfreceServicioGrid");
        $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'sco_proveedor') && $Security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "sco_ofrece_servicio";
        }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "Nproveedor");
            $this->createColumnOption($option, "rif");
            $this->createColumnOption($option, "nombre");
            $this->createColumnOption($option, "tipo_proveedor");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_proveedorsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_proveedorsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_proveedorlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_proveedor", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_proveedor",
    ...
    protected function loadSearchValues()
    {
        ...
        // Nproveedor
        if ($this->Nproveedor->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Nproveedor->AdvancedSearch->SearchValue != "" || $this->Nproveedor->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // rif
        if ($this->rif->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->rif->AdvancedSearch->SearchValue != "" || $this->rif->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // responsable
        if ($this->responsable->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->responsable->AdvancedSearch->SearchValue != "" || $this->responsable->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // telefono4
        if ($this->telefono4->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->telefono4->AdvancedSearch->SearchValue != "" || $this->telefono4->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // correo_adicional
        if ($this->correo_adicional->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->correo_adicional->AdvancedSearch->SearchValue != "" || $this->correo_adicional->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // localidad
        if ($this->localidad->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->localidad->AdvancedSearch->SearchValue != "" || $this->localidad->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // tipo_proveedor
        if ($this->tipo_proveedor->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->tipo_proveedor->AdvancedSearch->SearchValue != "" || $this->tipo_proveedor->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->Nproveedor->EditValue = $this->Nproveedor->AdvancedSearch->SearchValue;
            $this->Nproveedor->PlaceHolder = RemoveHtml($this->Nproveedor->caption());

            // rif
            $this->rif->setupEditAttributes();
            if (!$this->rif->Raw) {
                $this->rif->AdvancedSearch->SearchValue = HtmlDecode($this->rif->AdvancedSearch->SearchValue);
            }
            $this->rif->EditValue = HtmlEncode($this->rif->AdvancedSearch->SearchValue);
            ...
            $curVal = trim(strval($this->tipo_proveedor->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->tipo_proveedor->AdvancedSearch->ViewValue = $this->tipo_proveedor->lookupCacheOption($curVal);
            } else {
                $this->tipo_proveedor->AdvancedSearch->ViewValue = $this->tipo_proveedor->Lookup !== null && is_array($this->tipo_proveedor->lookupOptions()) && count($this->tipo_proveedor->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->tipo_proveedor->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->tipo_proveedor->EditValue = array_values($this->tipo_proveedor->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->tipo_proveedor->Lookup->getTable()->Fields["valor1"]->searchExpression(), "=", $this->tipo_proveedor->AdvancedSearch->SearchValue, $this->tipo_proveedor->Lookup->getTable()->Fields["valor1"]->searchDataType(), "");
                }
                $lookupFilter = $this->tipo_proveedor->getSelectFilter($this); // PHP
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_proveedorsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
