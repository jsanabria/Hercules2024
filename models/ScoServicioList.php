 ...
 * Page class
 */
class ScoServicioList extends ScoServicio
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoServicioList";
    ...
    public $FormName = "fsco_serviciolist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoServicioList";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'sco_servicio';
        $this->TableName = 'sco_servicio';
        ...
        // Table object (sco_servicio)
        if (!isset($GLOBALS["sco_servicio"]) || $GLOBALS["sco_servicio"]::class == PROJECT_NAMESPACE . "sco_servicio") {
            $GLOBALS["sco_servicio"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ScoServicioAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoServicioDelete";
        $this->MultiUpdateUrl = "ScoServicioUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'sco_servicio');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ScoServicioView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_serviciogrid";
        }
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "sco_servicio_tipo") {
            $masterTbl = Container("sco_servicio_tipo");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("ScoServicioTipoList"); // Return to master page
                return;
            } else {
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->Nservicio->AdvancedSearch->toJson(), ","); // Field Nservicio
        ...
        $filterList = Concat($filterList, $this->sto_min->AdvancedSearch->toJson(), ","); // Field sto_min
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_serviciosrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Nservicio
        $this->Nservicio->AdvancedSearch->SearchValue = @$filter["x_Nservicio"];
        $this->Nservicio->AdvancedSearch->SearchOperator = @$filter["z_Nservicio"];
        $this->Nservicio->AdvancedSearch->SearchCondition = @$filter["v_Nservicio"];
        $this->Nservicio->AdvancedSearch->SearchValue2 = @$filter["y_Nservicio"];
        $this->Nservicio->AdvancedSearch->SearchOperator2 = @$filter["w_Nservicio"];
        $this->Nservicio->AdvancedSearch->save();
        ...
        // Field sto_min
        $this->sto_min->AdvancedSearch->SearchValue = @$filter["x_sto_min"];
        $this->sto_min->AdvancedSearch->SearchOperator = @$filter["z_sto_min"];
        $this->sto_min->AdvancedSearch->SearchCondition = @$filter["v_sto_min"];
        $this->sto_min->AdvancedSearch->SearchValue2 = @$filter["y_sto_min"];
        $this->sto_min->AdvancedSearch->SearchOperator2 = @$filter["w_sto_min"];
        $this->sto_min->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Nservicio, $default, false); // Nservicio
        ...
        $this->buildSearchSql($where, $this->secuencia, $default, false); // secuencia
        $this->buildSearchSql($where, $this->sto_min, $default, false); // sto_min
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->Nservicio->AdvancedSearch->save(); // Nservicio
            $this->nombre->AdvancedSearch->save(); // nombre
            $this->articulo_inventario->AdvancedSearch->save(); // articulo_inventario
            $this->secuencia->AdvancedSearch->save(); // secuencia
            $this->sto_min->AdvancedSearch->save(); // sto_min
    ...
    public function showFilterList()
    {
        ...
        // Field Nservicio
        $filter = $this->queryBuilderWhere("Nservicio");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->Nservicio, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->Nservicio->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->Nservicio->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->secuencia->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->sto_min->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->Nservicio->AdvancedSearch->unsetSession();
        $this->nombre->AdvancedSearch->unsetSession();
        $this->articulo_inventario->AdvancedSearch->unsetSession();
        $this->secuencia->AdvancedSearch->unsetSession();
        $this->sto_min->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->Nservicio->AdvancedSearch->load();
        $this->nombre->AdvancedSearch->load();
        $this->articulo_inventario->AdvancedSearch->load();
        $this->secuencia->AdvancedSearch->load();
        $this->sto_min->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->Nservicio); // Nservicio
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_servicio\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_servicio\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_serviciolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_serviciolist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        if ($Security->allowList(CurrentProjectID() . 'sco_adjunto')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("ScoAdjuntoList?" . Config("TABLE_SHOW_MASTER") . "=sco_servicio&" . GetForeignKeyUrl("fk_Nservicio", $this->Nservicio->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("ScoAdjuntoGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_servicio')) {
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_servicio')) {
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Nservicio->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function renderListOptionsExt()
    {
        ...
        $detailFilters = [];
        $masterKeys = []; // Reset
        $masterKeys["Nservicio"] = strval($this->Nservicio->DbValue);
        ...
        if ($this->DetailPages?->getItem("sco_adjunto")?->Visible && $Security->allowList(CurrentProjectID() . 'sco_adjunto')) {
            ...
            $url = "ScoAdjuntoPreview?t=sco_servicio&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"sco_adjunto\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'sco_servicio')) {
                ...
                $detaillnk = GetUrl("ScoAdjuntoList?" . Config("TABLE_SHOW_MASTER") . "=sco_servicio");
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_servicio')) {
            ...
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_servicio')) {
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_servicio\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'sco_servicio') && $Security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "Nservicio");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_serviciosrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_serviciosrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_serviciolist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_servicio", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_servicio",
    ...
    protected function loadSearchValues()
    {
        ...
        // Nservicio
        if ($this->Nservicio->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Nservicio->AdvancedSearch->SearchValue != "" || $this->Nservicio->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // secuencia
        if ($this->secuencia->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->secuencia->AdvancedSearch->SearchValue != "" || $this->secuencia->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // sto_min
        if ($this->sto_min->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->sto_min->AdvancedSearch->SearchValue != "" || $this->sto_min->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function newRow()
    {
        ...
        $row['Nservicio'] = $this->Nservicio->DefaultValue;
        ...
        $row['sto_min'] = $this->sto_min->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // Nservicio
            $this->Nservicio->ViewValue = $this->Nservicio->CurrentValue;
            ...
            // sto_min
            $this->sto_min->ViewValue = $this->sto_min->CurrentValue;
            $this->sto_min->ViewValue = FormatNumber($this->sto_min->ViewValue, $this->sto_min->formatPattern());
            ...
            // Nservicio
            $this->Nservicio->HrefValue = "";
            $this->Nservicio->TooltipValue = "";
            ...
            if ($this->tipo->AdvancedSearch->ViewValue !== null) { // Load from cache
                ...
                if ($this->tipo->AdvancedSearch->ViewValue == "") {
                    $this->tipo->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->tipo->Lookup->getTable()->Fields["Nservicio_tipo"]->searchExpression(), "=", $this->tipo->AdvancedSearch->SearchValue, $this->tipo->Lookup->getTable()->Fields["Nservicio_tipo"]->searchDataType(), "");
                }
                $lookupFilter = $this->tipo->getSelectFilter($this); // PHP
                ...
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->tipo->Lookup->renderViewRow($rswrk[0]);
                    $this->tipo->AdvancedSearch->ViewValue = $this->tipo->displayValue($arwrk);
                } else {
                    $this->tipo->AdvancedSearch->ViewValue = $Language->phrase("PleaseSelect");
                }
                $arwrk = $rswrk;
            ...
            // Nservicio
            $this->Nservicio->setupEditAttributes();
            if (!$this->Nservicio->Raw) {
                $this->Nservicio->AdvancedSearch->SearchValue = HtmlDecode($this->Nservicio->AdvancedSearch->SearchValue);
            }
            $this->Nservicio->EditValue = HtmlEncode($this->Nservicio->AdvancedSearch->SearchValue);
            $this->Nservicio->PlaceHolder = RemoveHtml($this->Nservicio->caption());
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_serviciosrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "sco_servicio_tipo") {
                $validMaster = true;
                $masterTbl = Container("sco_servicio_tipo");
                if (($parm = Get("fk_Nservicio_tipo", Get("tipo"))) !== null) {
                    $masterTbl->Nservicio_tipo->setQueryStringValue($parm);
                    $this->tipo->QueryStringValue = $masterTbl->Nservicio_tipo->QueryStringValue; // DO NOT change, master/detail key data type can be different
                    $this->tipo->setSessionValue($this->tipo->QueryStringValue);
                    $foreignKeys["tipo"] = $this->tipo->QueryStringValue;
                } else {
                    $validMaster = false;
                }
            }
            ...
            if ($masterTblVar == "sco_servicio_tipo") {
                $validMaster = true;
                $masterTbl = Container("sco_servicio_tipo");
                if (($parm = Post("fk_Nservicio_tipo", Post("tipo"))) !== null) {
                    $masterTbl->Nservicio_tipo->setFormValue($parm);
                    $this->tipo->FormValue = $masterTbl->Nservicio_tipo->FormValue;
                    $this->tipo->setSessionValue($this->tipo->FormValue);
                    $foreignKeys["tipo"] = $this->tipo->FormValue;
                } else {
                    $validMaster = false;
                }
            }
        ...
        if ($validMaster) {
            ...
            if ($masterTblVar != "sco_servicio_tipo") {
                if (!array_key_exists("tipo", $foreignKeys)) { // Not current foreign key
                    $this->tipo->setSessionValue("");
                }
            }
        }
    ...
    public function listOptionsLoad() {
    	// Example:
    	$opt = &$this->ListOptions->Add("Precio");
    	$opt->Header = "Precio";
    	$opt->OnLeft = FALSE; // Link on left
    	$opt->MoveTo(0); // Move to first column
    	$opt = &$this->ListOptions->Add("Disponible");
    	$opt->Header = "Disponible";
    	$opt->OnLeft = FALSE; // Link on left
    	$opt->MoveTo(0); // Move to first column
    	$opt = &$this->ListOptions->Add("Salidas");
    	$opt->Header = "Salidas";
    	$opt->OnLeft = FALSE; // Link on left
    	$opt->MoveTo(0); // Move to first column
    	$opt = &$this->ListOptions->Add("Entradas");
    	$opt->Header = "Entradas";
    	$opt->OnLeft = FALSE; // Link on left
    	$opt->MoveTo(0); // Move to first column
    }
    ...
    public function listOptionsRendered() {
        $servicioId = $this->Nservicio->CurrentValue ?? '';
        $tipoServicio = $this->tipo->CurrentValue ?? '';

        // Si el ID del servicio está vacío, no se puede consultar, salimos.
        if (empty($servicioId)) {
            return;
        }

        // --- 1. Lógica de Precio ---
        $this->ListOptions->Items["Precio"]->Body = '—'; // Valor por defecto

        // Bloque para servicios tipo "CAP" (Probablemente Capacitación/Tarifas por Hora)
        if (substr($tipoServicio, 0, 3) === "CAP") {
            // CORRECCIÓN: Usar una sola consulta para obtener todas las tarifas y evitar el bucle ineficiente.
            $sql_tarifas = "
                SELECT 
                    horas, 
                    precio 
                FROM 
                    sco_tarifa
                WHERE 
                    servicio = '" . adjustSql($servicioId) . "' AND activo = 'S'
                ORDER BY horas ASC;
            ";
            $tarifas_rs = ExecuteRows($sql_tarifas);
            $precioBody = "";
            if (!empty($tarifas_rs)) {
                foreach ($tarifas_rs as $row) {
                    // Formato: "X Horas: Bs. YYY,YY / "
                    $precioBody .= $row["horas"] . " Horas: Bs. " . 
                                    number_format($row["precio"], 2, ",", ".") . " / ";
                }
                // Eliminar el último separador " / "
                $this->ListOptions->Items["Precio"]->Body = rtrim($precioBody, " / ");
            }
        } 
        // Bloque para otros servicios (asume una única tarifa)
        else {
            $sql_precio = "
                SELECT 
                    precio 
                ...
                WHERE 
                    servicio = '" . adjustSql($servicioId) . "' AND activo = 'S';
            ";
            $precio = ExecuteScalar($sql_precio);
            if ($precio !== false && $precio !== null) {
                $this->ListOptions->Items["Precio"]->Body = "Bs. " . number_format($precio, 2, ",", ".");
            }
        }

        // --- 2. Lógica de Inventario/Stock ---

        // Obtener todas las cantidades de una sola vez para evitar redundancia
        $sql_inventario = "
            SELECT 
                SUM(CASE WHEN cantidad >= 0 THEN cantidad ELSE 0 END) AS Entradas,
                SUM(CASE WHEN cantidad < 0 THEN cantidad ELSE 0 END) AS Salidas,
                SUM(cantidad) AS Disponible
            FROM 
                sco_entrada_salida_detalle 
            WHERE 
                articulo = '" . adjustSql($servicioId) . "';
        ";
        $inventario_row = ExecuteRow($sql_inventario);

        // Los valores de SUM() serán NULL si no hay registros, lo cual es manejado por number_format si convertimos a 0.
        $entradas = (float)($inventario_row["Entradas"] ?? 0);
        $salidas = (float)($inventario_row["Salidas"] ?? 0);
        $disponible = (float)($inventario_row["Disponible"] ?? 0);

        // Aplicar los resultados
        $this->ListOptions->Items["Disponible"]->Body = number_format($disponible, 0, "", ".");

        // Nota: Las salidas son números negativos en la base de datos (cantidad < 0), 
        // pero para la presentación deben ser mostrados como un valor positivo (e.g., 50, no -50).
        $this->ListOptions->Items["Entradas"]->Body = number_format($entradas, 0, "", ".");
        $this->ListOptions->Items["Salidas"]->Body = number_format(abs($salidas), 0, "", ".");
    }
