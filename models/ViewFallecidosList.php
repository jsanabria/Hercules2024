 ...
 * Page class
 */
class ViewFallecidosList extends ViewFallecidos
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewFallecidosList";
    ...
    public $FormName = "fview_fallecidoslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ViewFallecidosList";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'view_fallecidos';
        $this->TableName = 'view_fallecidos';
        ...
        // Table object (view_fallecidos)
        if (!isset($GLOBALS["view_fallecidos"]) || $GLOBALS["view_fallecidos"]::class == PROJECT_NAMESPACE . "view_fallecidos") {
            $GLOBALS["view_fallecidos"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ViewFallecidosAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ViewFallecidosDelete";
        $this->MultiUpdateUrl = "ViewFallecidosUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'view_fallecidos');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ViewFallecidosView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_fallecidosgrid";
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_fallecidossrch", $filters);
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_fallecidoslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_fallecidoslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_fallecidossrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_fallecidossrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_fallecidoslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_fallecidos", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_fallecidos",
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_fallecidossrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        // 1. Verificar si el usuario tiene permiso para LISTAR la tabla de servicios
        // Usamos el nombre de la tabla interna (usualmente la misma que el script)
        if (Security()->canList()) {
            // 2. Definir el botón con estilos de Bootstrap 5
            // Usamos btn-info o btn-secondary para diferenciarlo del botón de "Nueva Solicitud"
            $botonLista = '
            <div class="mb-4 d-inline-block me-2">
                <a href="ScoGramaList" class="btn btn-info shadow-sm d-inline-flex align-items-center text-white" style="padding: 10px 25px; border-radius: 8px; font-weight: 600;">
                    <i class="fas fa-list-ul me-2"></i> 
                    LISTAR SERVICIOS DE GRAMA
                </a>
            </div>';

            // 3. Inyectar el botón al header
            $header = $botonLista . $header;
        }
    }
    ...
    public function listOptionsLoad() {
    	// Example:
    	$opt = &$this->ListOptions->Add("grama");
    	$opt->Header = "";
    ...
    public function listOptionsRendered() {
        // 1. Obtener valores con valores por defecto
        $xParcela = $this->Nparcela->CurrentValue ?? 0;

        // 2. Construir el nombre de forma limpia (filtrando espacios vacíos)
        $partesNombre = [
            $this->apellido1->CurrentValue ?? "",
            $this->apellido2->CurrentValue ?? "",
            $this->nombre1->CurrentValue ?? "",
            $this->nombre2->CurrentValue ?? ""
        ];

        // Limpiamos nulos/vacíos y unimos con un solo espacio
        $xNombre = implode(" ", array_filter(array_map('trim', $partesNombre)));

        // 3. Escapar comillas simples para evitar errores en el onClick de JavaScript
        $xNombreJS = addslashes($xNombre);

        // 4. Usar un template string (heredoc o comillas dobles) para mayor claridad
        $this->ListOptions->Items["grama"]->Body = sprintf(
            '<button class="btn btn-primary btn-sm shadow-sm d-inline-flex align-items-center" 
                    type="button" 
                    onClick="CrearGrama(%d, \'%s\');" 
                    title="Crear Nuevo Expediente de Grama">
                <i class="fas fa-file-medical me-2"></i> 
                <span class="fw-bold">Crear Expediente</span>
            </button>',
            $xParcela,
            $xNombreJS
        );
