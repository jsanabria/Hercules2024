 ...
 * Page class
 */
class ScoCostosTarifaList extends ScoCostosTarifa
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoCostosTarifaList";
    ...
    public $FormName = "fsco_costos_tarifalist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoCostosTarifaList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ScoCostosTarifaAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoCostosTarifaDelete";
        $this->MultiUpdateUrl = "ScoCostosTarifaUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_costos_tarifagrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Ncostos_tarifa->AdvancedSearch->toJson(), ","); // Field Ncostos_tarifa
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_costos_tarifasrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Ncostos_tarifa
        $this->Ncostos_tarifa->AdvancedSearch->SearchValue = @$filter["x_Ncostos_tarifa"];
        $this->Ncostos_tarifa->AdvancedSearch->SearchOperator = @$filter["z_Ncostos_tarifa"];
        $this->Ncostos_tarifa->AdvancedSearch->SearchCondition = @$filter["v_Ncostos_tarifa"];
        $this->Ncostos_tarifa->AdvancedSearch->SearchValue2 = @$filter["y_Ncostos_tarifa"];
        $this->Ncostos_tarifa->AdvancedSearch->SearchOperator2 = @$filter["w_Ncostos_tarifa"];
        $this->Ncostos_tarifa->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Ncostos_tarifa, $default, false); // Ncostos_tarifa
        ...
        if (!$default && $this->Command == "search") {
            $this->Ncostos_tarifa->AdvancedSearch->save(); // Ncostos_tarifa
    ...
    public function showFilterList()
    {
        ...
        // Field localidad
        $filter = $this->queryBuilderWhere("localidad");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->localidad, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->localidad->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->Ncostos_tarifa->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->Ncostos_tarifa->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->Ncostos_tarifa->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->localidad->Expression . " ASC" . ", " . $this->tipo_servicio->Expression . " ASC" . ", " . $this->horas->Expression . " ASC"; // Set up default sort
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_sco_costos_tarifa_detalle"
        $item = &$this->ListOptions->add("detail_sco_costos_tarifa_detalle");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'sco_costos_tarifa_detalle');
        $item->OnLeft = true;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("sco_costos_tarifa_detalle");
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_costos_tarifa\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_costos_tarifa\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_costos_tarifalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_costos_tarifalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_sco_costos_tarifa_detalle"
        $opt = $this->ListOptions["detail_sco_costos_tarifa_detalle"];
        if ($Security->allowList(CurrentProjectID() . 'sco_costos_tarifa_detalle')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("sco_costos_tarifa_detalle", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("ScoCostosTarifaDetalleList?" . Config("TABLE_SHOW_MASTER") . "=sco_costos_tarifa&" . GetForeignKeyUrl("fk_Ncostos_tarifa", $this->Ncostos_tarifa->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("ScoCostosTarifaDetalleGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_costos_tarifa')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_costos_tarifa_detalle");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "sco_costos_tarifa_detalle";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_costos_tarifa')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_costos_tarifa_detalle");
                ...
                $detailEditTblVar .= "sco_costos_tarifa_detalle";
            }
            if ($links != "") {
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Ncostos_tarifa->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function renderListOptionsExt()
    {
        ...
        $detailFilters = [];
        $masterKeys = []; // Reset
        $masterKeys["Ncostos_tarifa"] = strval($this->Ncostos_tarifa->DbValue);

        // Column "detail_sco_costos_tarifa_detalle"
        if ($this->DetailPages?->getItem("sco_costos_tarifa_detalle")?->Visible && $Security->allowList(CurrentProjectID() . 'sco_costos_tarifa_detalle')) {
            $link = "";
            $option = $this->ListOptions["detail_sco_costos_tarifa_detalle"];
            $detailTbl = Container("sco_costos_tarifa_detalle");
            $detailFilter = $detailTbl->getDetailFilter($this);
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "ScoCostosTarifaDetallePreview?t=sco_costos_tarifa&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"sco_costos_tarifa_detalle\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'sco_costos_tarifa')) {
                $label = $Language->tablePhrase("sco_costos_tarifa_detalle", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"sco_costos_tarifa_detalle\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("ScoCostosTarifaDetalleList?" . Config("TABLE_SHOW_MASTER") . "=sco_costos_tarifa");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $Language->tablePhrase("sco_costos_tarifa_detalle", "TblCaption");
            ...
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_costos_tarifa')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_costos_tarifa_detalle"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($viewurl) . "\">" . $caption . "</a>";
            }
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_costos_tarifa')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_costos_tarifa_detalle"));
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_costos_tarifa\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $detailTableLink = "";
        $item = &$option->add("detailadd_sco_costos_tarifa_detalle");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=sco_costos_tarifa_detalle");
        $detailPage = Container("ScoCostosTarifaDetalleGrid");
        $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'sco_costos_tarifa') && $Security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "sco_costos_tarifa_detalle";
        }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "localidad");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_costos_tarifasrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_costos_tarifasrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_costos_tarifalist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_costos_tarifa", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_costos_tarifa",
    ...
    protected function loadSearchValues()
    {
        ...
        // Ncostos_tarifa
        if ($this->Ncostos_tarifa->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Ncostos_tarifa->AdvancedSearch->SearchValue != "" || $this->Ncostos_tarifa->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_costos_tarifasrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
