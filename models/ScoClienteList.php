 ...
 * Page class
 */
class ScoClienteList extends ScoCliente
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoClienteList";
    ...
    public $FormName = "fsco_clientelist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoClienteList";
    ...
    {
        $this->Ncliente->setVisibility();
        $this->rif_ci->setVisibility();
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'sco_cliente';
        $this->TableName = 'sco_cliente';
        ...
        // Table object (sco_cliente)
        if (!isset($GLOBALS["sco_cliente"]) || $GLOBALS["sco_cliente"]::class == PROJECT_NAMESPACE . "sco_cliente") {
            $GLOBALS["sco_cliente"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ScoClienteAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoClienteDelete";
        $this->MultiUpdateUrl = "ScoClienteUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'sco_cliente');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ScoClienteView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_clientegrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Ncliente->AdvancedSearch->toJson(), ","); // Field Ncliente
        $filterList = Concat($filterList, $this->rif_ci->AdvancedSearch->toJson(), ","); // Field rif_ci
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_clientesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Ncliente
        $this->Ncliente->AdvancedSearch->SearchValue = @$filter["x_Ncliente"];
        $this->Ncliente->AdvancedSearch->SearchOperator = @$filter["z_Ncliente"];
        $this->Ncliente->AdvancedSearch->SearchCondition = @$filter["v_Ncliente"];
        $this->Ncliente->AdvancedSearch->SearchValue2 = @$filter["y_Ncliente"];
        $this->Ncliente->AdvancedSearch->SearchOperator2 = @$filter["w_Ncliente"];
        $this->Ncliente->AdvancedSearch->save();

        // Field rif_ci
        $this->rif_ci->AdvancedSearch->SearchValue = @$filter["x_rif_ci"];
        $this->rif_ci->AdvancedSearch->SearchOperator = @$filter["z_rif_ci"];
        $this->rif_ci->AdvancedSearch->SearchCondition = @$filter["v_rif_ci"];
        $this->rif_ci->AdvancedSearch->SearchValue2 = @$filter["y_rif_ci"];
        $this->rif_ci->AdvancedSearch->SearchOperator2 = @$filter["w_rif_ci"];
        $this->rif_ci->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Ncliente, $default, false); // Ncliente
        $this->buildSearchSql($where, $this->rif_ci, $default, false); // rif_ci
        ...
        if (!$default && $this->Command == "search") {
            $this->Ncliente->AdvancedSearch->save(); // Ncliente
            $this->rif_ci->AdvancedSearch->save(); // rif_ci
    ...
    public function showFilterList()
    {
        ...
        // Field Ncliente
        $filter = $this->queryBuilderWhere("Ncliente");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->Ncliente, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->Ncliente->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field rif_ci
        $filter = $this->queryBuilderWhere("rif_ci");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->rif_ci, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->rif_ci->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->Ncliente->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->rif_ci->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->Ncliente->AdvancedSearch->unsetSession();
        $this->rif_ci->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->Ncliente->AdvancedSearch->load();
        $this->rif_ci->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->Ncliente); // Ncliente
            $this->updateSort($this->rif_ci); // rif_ci
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_cliente\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_cliente\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_clientelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_clientelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Ncliente->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_cliente\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "Ncliente");
            $this->createColumnOption($option, "rif_ci");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_clientesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_clientesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_clientelist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_cliente", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_cliente",
    ...
    protected function loadSearchValues()
    {
        ...
        // Ncliente
        if ($this->Ncliente->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Ncliente->AdvancedSearch->SearchValue != "" || $this->Ncliente->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // rif_ci
        if ($this->rif_ci->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->rif_ci->AdvancedSearch->SearchValue != "" || $this->rif_ci->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function loadRowValues($row = null)
    {
        ...
        $this->Ncliente->setDbValue($row['Ncliente']);
        $this->rif_ci->setDbValue($row['rif_ci']);
    ...
    {
        $row = [];
        $row['Ncliente'] = $this->Ncliente->DefaultValue;
        $row['rif_ci'] = $this->rif_ci->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            // Ncliente
            $this->Ncliente->ViewValue = $this->Ncliente->CurrentValue;

            // rif_ci
            $this->rif_ci->ViewValue = $this->rif_ci->CurrentValue;
            $this->rif_ci->ViewCustomAttributes = $this->rif_ci->getViewCustomAttributes(); // PHP
            ...
            // Ncliente
            $this->Ncliente->HrefValue = "";
            $this->Ncliente->TooltipValue = "";

            // rif_ci
            $this->rif_ci->HrefValue = "";
            $this->rif_ci->TooltipValue = "";
            ...
            // Ncliente
            $this->Ncliente->setupEditAttributes();
            $this->Ncliente->EditValue = $this->Ncliente->AdvancedSearch->SearchValue;
            $this->Ncliente->PlaceHolder = RemoveHtml($this->Ncliente->caption());

            // rif_ci
            $this->rif_ci->setupEditAttributes();
            if (!$this->rif_ci->Raw) {
                $this->rif_ci->AdvancedSearch->SearchValue = HtmlDecode($this->rif_ci->AdvancedSearch->SearchValue);
            }
            $this->rif_ci->EditValue = HtmlEncode($this->rif_ci->AdvancedSearch->SearchValue);
            $this->rif_ci->PlaceHolder = RemoveHtml($this->rif_ci->caption());
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_clientesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
