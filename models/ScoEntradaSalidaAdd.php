 ...
 * Page class
 */
class ScoEntradaSalidaAdd extends ScoEntradaSalida
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoEntradaSalidaAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoEntradaSalidaAdd";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("Nentrada_salida") ?? Route("Nentrada_salida")) !== null) {
                $this->Nentrada_salida->setQueryStringValue($keyValue);
            }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("ScoEntradaSalidaList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ScoEntradaSalidaList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ScoEntradaSalidaView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ScoEntradaSalidaList") {
                            ...
                            $returnUrl = "ScoEntradaSalidaList"; // Return list page content
                        }
                    }
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->registro->DefaultValue = $this->registro->getDefault(); // PHP
        $this->registro->OldValue = $this->registro->DefaultValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->registro->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->registro->setFormValue($val, true, $validate);
            }
            $this->registro->CurrentValue = UnFormatDateTime($this->registro->CurrentValue, $this->registro->formatPattern());
        }

        // Check field name 'Nentrada_salida' first before field var 'x_Nentrada_salida'
        $val = $CurrentForm->hasValue("Nentrada_salida") ? $CurrentForm->getValue("Nentrada_salida") : $CurrentForm->getValue("x_Nentrada_salida");
    }
    ...
    public function renderRow()
    {
        ...
        // Nentrada_salida
        $this->Nentrada_salida->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->registro->EditValue = HtmlEncode(FormatDateTime($this->registro->CurrentValue, $this->registro->formatPattern()));
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->tipo_doc->Visible && $this->tipo_doc->Required) {
                if (!$this->tipo_doc->IsDetailKey && EmptyValue($this->tipo_doc->FormValue)) {
                    $this->tipo_doc->addErrorMessage(str_replace("%s", $this->tipo_doc->caption(), $this->tipo_doc->RequiredErrorMessage));
                }
            }
            ...
            if (!CheckDate($this->registro->FormValue, $this->registro->formatPattern())) {
        ...
        if (in_array("sco_entrada_salida_detalle", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("ScoEntradaSalidaDetalleGrid");
            if (in_array("sco_entrada_salida_detalle", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->entrada_salida->setSessionValue($this->Nentrada_salida->CurrentValue); // Set master key
                $detailPage->tipo_doc->setSessionValue($this->tipo_doc->CurrentValue); // Set master key
                $detailPage->proveedor->setSessionValue($this->proveedor->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "sco_entrada_salida_detalle"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->entrada_salida->setSessionValue(""); // Clear master key if insert failed
                $detailPage->tipo_doc->setSessionValue(""); // Clear master key if insert failed
    ...
    protected function getAddRow()
    {
        ...
        // tipo_doc
        $this->tipo_doc->setDbValueDef($rsnew, $this->tipo_doc->CurrentValue, false);

        // proveedor
        $this->proveedor->setDbValueDef($rsnew, $this->proveedor->CurrentValue, false);

        // clasificacion
        $this->clasificacion->setDbValueDef($rsnew, $this->clasificacion->CurrentValue, false);
        ...
        // registro
        $this->registro->setDbValueDef($rsnew, UnFormatDateTime($this->registro->CurrentValue, $this->registro->formatPattern()), false);
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['clasificacion'])) { // clasificacion
            $this->clasificacion->setFormValue($row['clasificacion']);
        }
        ...
        if (isset($row['registro'])) { // registro
            $this->registro->setFormValue($row['registro']);
        }
    }
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("sco_entrada_salida_detalle", $detailTblVar)) {
                ...
                if ($detailPageObj->DetailAdd) {
                    ...
                    $detailPageObj->entrada_salida->IsDetailKey = true;
                    $detailPageObj->entrada_salida->CurrentValue = $this->Nentrada_salida->CurrentValue;
                    $detailPageObj->entrada_salida->setSessionValue($detailPageObj->entrada_salida->CurrentValue);
                    $detailPageObj->tipo_doc->IsDetailKey = true;
                    $detailPageObj->tipo_doc->CurrentValue = $this->tipo_doc->CurrentValue;
                    $detailPageObj->tipo_doc->setSessionValue($detailPageObj->tipo_doc->CurrentValue);
                    $detailPageObj->proveedor->IsDetailKey = true;
                    $detailPageObj->proveedor->CurrentValue = $this->proveedor->CurrentValue;
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ScoEntradaSalidaList"), "", $this->TableVar, true);
