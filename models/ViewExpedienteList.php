 ...
 * Page class
 */
class ViewExpedienteList extends ViewExpediente
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ViewExpedienteList";
    ...
    public $FormName = "fview_expedientelist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ViewExpedienteList";
    ...
    public function setVisibility()
    {
        ...
        $this->fecha_serv->setVisibility();
        $this->hora_serv->setVisibility();
    ...
    public function __construct()
    {
        ...
        // Table object (view_expediente)
        if (!isset($GLOBALS["view_expediente"]) || $GLOBALS["view_expediente"]::class == PROJECT_NAMESPACE . "view_expediente") {
            $GLOBALS["view_expediente"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ViewExpedienteAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ViewExpedienteDelete";
        $this->MultiUpdateUrl = "ViewExpedienteUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'view_expediente');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ViewExpedienteView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fview_expedientegrid";
        }
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fview_expedientesrch", $filters);
    ...
    public function showFilterList()
    {
        ...
        // Field permiso
        $filter = $this->queryBuilderWhere("permiso");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->permiso, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->permiso->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field fecha_serv
        $filter = $this->queryBuilderWhere("fecha_serv");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->fecha_serv, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->fecha_serv->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field hora_serv
        $filter = $this->queryBuilderWhere("hora_serv");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->hora_serv, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->hora_serv->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        if ($this->BasicSearch->Keyword != "") {
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->hora_inicio;
        $searchFlds[] = &$this->fecha_fin;
        $searchFlds[] = &$this->hora_fin;
        $searchFlds[] = &$this->servicio;
        $searchFlds[] = &$this->fecha_serv;
        $searchFlds[] = &$this->hora_serv;
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->Nexpediente->Expression . " DESC"; // Set up default sort
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->fecha_serv); // fecha_serv
            $this->updateSort($this->hora_serv); // hora_serv
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_expedientelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fview_expedientelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "fecha_serv");
            $this->createColumnOption($option, "hora_serv");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fview_expedientesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fview_expedientesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fview_expedientelist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_view_expediente", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_view_expediente",
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->permiso->Raw) {
                $this->permiso->AdvancedSearch->SearchValue = HtmlDecode($this->permiso->AdvancedSearch->SearchValue);
            }
            $this->permiso->EditValue = HtmlEncode($this->permiso->AdvancedSearch->SearchValue);
            ...
            $this->fecha_serv->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->fecha_serv->AdvancedSearch->SearchValue, $this->fecha_serv->formatPattern()), $this->fecha_serv->formatPattern()));
            $this->fecha_serv->PlaceHolder = RemoveHtml($this->fecha_serv->caption());
            $this->fecha_serv->setupEditAttributes();
            $this->fecha_serv->EditValue2 = HtmlEncode(FormatDateTime(UnFormatDateTime($this->fecha_serv->AdvancedSearch->SearchValue2, $this->fecha_serv->formatPattern()), $this->fecha_serv->formatPattern()));
            $this->fecha_serv->PlaceHolder = RemoveHtml($this->fecha_serv->caption());

            // hora_serv
            $this->hora_serv->setupEditAttributes();
            if (!$this->hora_serv->Raw) {
                $this->hora_serv->AdvancedSearch->SearchValue = HtmlDecode($this->hora_serv->AdvancedSearch->SearchValue);
            }
            $this->hora_serv->EditValue = HtmlEncode($this->hora_serv->AdvancedSearch->SearchValue);
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckDate($this->fecha_serv->AdvancedSearch->SearchValue, $this->fecha_serv->formatPattern())) {
            $this->fecha_serv->addErrorMessage($this->fecha_serv->getErrorMessage(false));
        }
        if (!CheckDate($this->fecha_serv->AdvancedSearch->SearchValue2, $this->fecha_serv->formatPattern())) {
            $this->fecha_serv->addErrorMessage($this->fecha_serv->getErrorMessage(false));
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fview_expedientesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    public function pageDataRendering(&$header) {
        // El nombre del formulario ('fview_expedientelist') se pasa como argumento.
        $form_id = 'fview_expedientelist';

        // Se usa la clase 'btn btn-primary' para el estilo azul con letras blancas.
        // El '1' en printprepa es para el reporte de Identificación Capilla/Difunto (rptOrdenPreparacion.php)
        $header .= '<span data-phrase="IdCapillaBtn"
                          class="btn btn-primary ewButton" 
                          data-caption="Identificación"
                          title="Imprimir Identificación de Capilla y Difunto">
                        <i class="glyphicon glyphicon-tag ewIcon"></i>
                        <a href="javascript:void(0);" style="color: white; text-decoration: none;"
                           onclick="(() => { printprepa(\'' . $form_id . '\', 1); })();">
                            Identificación Capilla Difunto
                        </a>
                      </span>';
    }
