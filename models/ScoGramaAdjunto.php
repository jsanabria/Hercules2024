...
/**
 * Table class for sco_grama_adjunto
 */
class ScoGramaAdjunto extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "sco_grama_adjunto";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->grama->addMethod("getDefault", fn() => 0);
        $this->grama->InputTextType = "text";
        $this->grama->Raw = true;
        $this->grama->IsForeignKey = true; // Foreign key field
        $this->grama->Nullable = false; // NOT NULL field
        $this->grama->Required = true; // Required field
        $this->grama->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->grama->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        $this->activo->Lookup = new Lookup($this->activo, 'sco_grama_adjunto', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "sco_grama_adjunto";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ScoGramaAdjuntoList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ScoGramaAdjuntoView" => $Language->phrase("View"),
            "ScoGramaAdjuntoEdit" => $Language->phrase("Edit"),
            "ScoGramaAdjuntoAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ScoGramaAdjuntoView",
            Config("API_ADD_ACTION") => "ScoGramaAdjuntoAdd",
            Config("API_EDIT_ACTION") => "ScoGramaAdjuntoEdit",
            Config("API_DELETE_ACTION") => "ScoGramaAdjuntoDelete",
            Config("API_LIST_ACTION") => "ScoGramaAdjuntoList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ScoGramaAdjuntoView", $parm);
        } else {
            $url = $this->keyUrl("ScoGramaAdjuntoView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ScoGramaAdjuntoAdd?" . $parm;
        } else {
            $url = "ScoGramaAdjuntoAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoGramaAdjuntoEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoGramaAdjuntoList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoGramaAdjuntoAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoGramaAdjuntoList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ScoGramaAdjuntoDelete", $parm);
        }
    }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'sco_grama_adjunto');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'sco_grama_adjunto', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'sco_grama_adjunto', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'sco_grama_adjunto', $fldname, $key, $oldvalue);
            }
        }
    }
