 ...
 * Page class
 */
class ScoProveedorEdit extends ScoProveedor
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoProveedorEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoProveedorEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Nproveedor") ?? Key(0) ?? Route(2)) !== null) {
                $this->Nproveedor->setQueryStringValue($keyValue);
                $this->Nproveedor->setOldValue($this->Nproveedor->QueryStringValue);
            } elseif (Post("Nproveedor") !== null) {
                $this->Nproveedor->setFormValue(Post("Nproveedor"));
                $this->Nproveedor->setOldValue($this->Nproveedor->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("Nproveedor") ?? Route("Nproveedor")) !== null) {
                    ...
                    $this->Nproveedor->CurrentValue = null;
                }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->Nproveedor->IsDetailKey) {
            $this->Nproveedor->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->Nproveedor->CurrentValue = $this->Nproveedor->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->Nproveedor->Visible && $this->Nproveedor->Required) {
                if (!$this->Nproveedor->IsDetailKey && EmptyValue($this->Nproveedor->FormValue)) {
                    $this->Nproveedor->addErrorMessage(str_replace("%s", $this->Nproveedor->caption(), $this->Nproveedor->RequiredErrorMessage));
                }
            }
        ...
        if (in_array("sco_ofrece_servicio", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
    ...
    protected function editRow()
    {
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("ScoOfreceServicioGrid");
            if (in_array("sco_ofrece_servicio", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
    ...
    protected function getEditRow($rsold)
    {
        ...
        // telefono4
        $this->telefono4->setDbValueDef($rsnew, $this->telefono4->CurrentValue, $this->telefono4->ReadOnly);
        ...
        // correo_adicional
        $this->correo_adicional->setDbValueDef($rsnew, $this->correo_adicional->CurrentValue, $this->correo_adicional->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['responsable'])) { // responsable
            $this->responsable->CurrentValue = $row['responsable'];
        }
        ...
        if (isset($row['telefono4'])) { // telefono4
            $this->telefono4->CurrentValue = $row['telefono4'];
        }
        ...
        if (isset($row['correo_adicional'])) { // correo_adicional
            $this->correo_adicional->CurrentValue = $row['correo_adicional'];
        }
