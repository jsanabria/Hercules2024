 ...
 * Page class
 */
class ScoEntradaSalidaList extends ScoEntradaSalida
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoEntradaSalidaList";
    ...
    public $FormName = "fsco_entrada_salidalist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ScoEntradaSalidaList";
    ...
    {
        $this->Nentrada_salida->Visible = false;
    ...
    public function __construct()
    {
        ...
        // Table object (sco_entrada_salida)
        if (!isset($GLOBALS["sco_entrada_salida"]) || $GLOBALS["sco_entrada_salida"]::class == PROJECT_NAMESPACE . "sco_entrada_salida") {
            $GLOBALS["sco_entrada_salida"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ScoEntradaSalidaAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ScoEntradaSalidaDelete";
        $this->MultiUpdateUrl = "ScoEntradaSalidaUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'sco_entrada_salida');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ScoEntradaSalidaView"); // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fsco_entrada_salidagrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->Nentrada_salida->AdvancedSearch->toJson(), ","); // Field Nentrada_salida
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsco_entrada_salidasrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field Nentrada_salida
        $this->Nentrada_salida->AdvancedSearch->SearchValue = @$filter["x_Nentrada_salida"];
        $this->Nentrada_salida->AdvancedSearch->SearchOperator = @$filter["z_Nentrada_salida"];
        $this->Nentrada_salida->AdvancedSearch->SearchCondition = @$filter["v_Nentrada_salida"];
        $this->Nentrada_salida->AdvancedSearch->SearchValue2 = @$filter["y_Nentrada_salida"];
        $this->Nentrada_salida->AdvancedSearch->SearchOperator2 = @$filter["w_Nentrada_salida"];
        $this->Nentrada_salida->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->Nentrada_salida, $default, false); // Nentrada_salida
        $this->buildSearchSql($where, $this->tipo_doc, $default, false); // tipo_doc
        ...
        if (!$default && $this->Command == "search") {
            $this->Nentrada_salida->AdvancedSearch->save(); // Nentrada_salida
    ...
    public function showFilterList()
    {
        ...
        // Field tipo_doc
        $filter = $this->queryBuilderWhere("tipo_doc");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->tipo_doc, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->tipo_doc->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->Nentrada_salida;
        $searchFlds[] = &$this->tipo_doc;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->Nentrada_salida->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->Nentrada_salida->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->Nentrada_salida->AdvancedSearch->load();
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->Nentrada_salida->Expression . " DESC"; // Set up default sort
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_sco_entrada_salida_detalle"
        $item = &$this->ListOptions->add("detail_sco_entrada_salida_detalle");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'sco_entrada_salida_detalle');
        $item->OnLeft = true;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("sco_entrada_salida_detalle");
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"sco_entrada_salida\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"sco_entrada_salida\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_entrada_salidalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsco_entrada_salidalist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_sco_entrada_salida_detalle"
        $opt = $this->ListOptions["detail_sco_entrada_salida_detalle"];
        if ($Security->allowList(CurrentProjectID() . 'sco_entrada_salida_detalle')) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("sco_entrada_salida_detalle", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("ScoEntradaSalidaDetalleList?" . Config("TABLE_SHOW_MASTER") . "=sco_entrada_salida&" . GetForeignKeyUrl("fk_Nentrada_salida", $this->Nentrada_salida->CurrentValue) . "&" . GetForeignKeyUrl("fk_tipo_doc", $this->tipo_doc->CurrentValue) . "&" . GetForeignKeyUrl("fk_proveedor", $this->proveedor->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("ScoEntradaSalidaDetalleGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_entrada_salida')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_entrada_salida_detalle");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "sco_entrada_salida_detalle";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_entrada_salida')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_entrada_salida_detalle");
                ...
                $detailEditTblVar .= "sco_entrada_salida_detalle";
            }
            if ($links != "") {
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->Nentrada_salida->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function renderListOptionsExt()
    {
        ...
        $detailFilters = [];
        $masterKeys = []; // Reset
        $masterKeys["Nentrada_salida"] = strval($this->Nentrada_salida->DbValue);
        $masterKeys["tipo_doc"] = strval($this->tipo_doc->DbValue);
        $masterKeys["proveedor"] = strval($this->proveedor->DbValue);

        // Column "detail_sco_entrada_salida_detalle"
        if ($this->DetailPages?->getItem("sco_entrada_salida_detalle")?->Visible && $Security->allowList(CurrentProjectID() . 'sco_entrada_salida_detalle')) {
            $link = "";
            $option = $this->ListOptions["detail_sco_entrada_salida_detalle"];
            $detailTbl = Container("sco_entrada_salida_detalle");
            $detailFilter = $detailTbl->getDetailFilter($this);
            $detailTbl->setCurrentMasterTable($this->TableVar);
            $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
            $url = "ScoEntradaSalidaDetallePreview?t=sco_entrada_salida&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"sco_entrada_salida_detalle\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group d-none\">";
            if ($Security->allowList(CurrentProjectID() . 'sco_entrada_salida')) {
                $label = $Language->tablePhrase("sco_entrada_salida_detalle", "TblCaption");
                $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"sco_entrada_salida_detalle\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                $detaillnk = GetUrl("ScoEntradaSalidaDetalleList?" . Config("TABLE_SHOW_MASTER") . "=sco_entrada_salida");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $Language->tablePhrase("sco_entrada_salida_detalle", "TblCaption");
            ...
            $detailPageObj = Container("ScoEntradaSalidaDetalleGrid");
            if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'sco_entrada_salida')) {
                $caption = $Language->phrase("MasterDetailViewLink");
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=sco_entrada_salida_detalle"));
                $btngrp .= "<a href=\"#\" class=\"me-2\" title=\"" . HtmlTitle($caption) . "\" data-ew-action=\"redirect\" data-url=\"" . HtmlEncode($viewurl) . "\">" . $caption . "</a>";
            }
            if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'sco_entrada_salida')) {
                $caption = $Language->phrase("MasterDetailEditLink");
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=sco_entrada_salida_detalle"));
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"sco_entrada_salida\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $detailTableLink = "";
        $item = &$option->add("detailadd_sco_entrada_salida_detalle");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=sco_entrada_salida_detalle");
        $detailPage = Container("ScoEntradaSalidaDetalleGrid");
        $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'sco_entrada_salida') && $Security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "sco_entrada_salida_detalle";
        }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "tipo_doc");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsco_entrada_salidasrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsco_entrada_salidasrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsco_entrada_salidalist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_sco_entrada_salida", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_sco_entrada_salida",
    ...
    protected function loadSearchValues()
    {
        ...
        // Nentrada_salida
        if ($this->Nentrada_salida->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Nentrada_salida->AdvancedSearch->SearchValue != "" || $this->Nentrada_salida->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // tipo_doc
        if ($this->tipo_doc->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->tipo_doc->AdvancedSearch->SearchValue != "" || $this->tipo_doc->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    {
        $row = [];
        $row['Nentrada_salida'] = $this->Nentrada_salida->DefaultValue;
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsco_entrada_salidasrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    public function pageDataRendering(&$header) {
    	// Example:
    	//$header = "your header";
    	$header .= '<input type="button" class="btn btn-primary" value="Imprimir S&oacute;lo Relaci&oacute;n de Facturas" onClick="js:printprepa(\'fsco_entrada_salidalist\');" />&nbsp;&nbsp;';
    }
    ...
    {
        // 1. Botón de Imprimir
        // Usa el valor del campo como parámetro para el reporte FPDF.
        $print_link = htmlspecialchars('fpdf_report/rptES.php?ides=' . $this->Nentrada_salida->ViewValue);

        // Usamos las clases de Font Awesome 5 (fa-print)
        $this->ListOptions->Items["new"]->Body = <<<HTML
            <a href="{$print_link}" target="_blank" title="Imprimir Documento">
                <span data-phrase="PrintBtn"
    ...
    HTML;
        ...
        // 2. Checkbox de Selección (Sin cambios)
        $checkbox_value = htmlspecialchars($this->Nentrada_salida->CurrentValue ?? '');
        $this->ListOptions->Items["new2"]->Body = <<<HTML
            <input type="checkbox" 
                   value="{$checkbox_value}" 
                   id="prepa[]" 
                   name="prepa[]"
                   title="Seleccionar para preparar">
    HTML;
    }
