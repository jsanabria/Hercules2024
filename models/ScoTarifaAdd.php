 ...
 * Page class
 */
class ScoTarifaAdd extends ScoTarifa
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoTarifaAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoTarifaAdd";
    ...
    public function setVisibility()
    {
        ...
        $this->crear_nuevo_precio->Visible = false;
        $this->porc_recargo->Visible = false;
    }
    ...
    public function __construct()
    {
        ...
        // Table object (sco_tarifa)
        if (!isset($GLOBALS["sco_tarifa"]) || $GLOBALS["sco_tarifa"]::class == PROJECT_NAMESPACE . "sco_tarifa") {
            $GLOBALS["sco_tarifa"] = &$this;
        }

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'sco_tarifa');
        }
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (
                    ...
                    if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                        ...
                        $result["view"] = SameString($pageName, "ScoTarifaView"); // If View page, no primary button
                    } else { // List page
    ...
    public function run()
    {
        ...
        $this->setupLookupOptions($this->crear_nuevo_precio);
        ...
        if (IsApi()) {
            ...
            if (($keyValue = Get("Ntarifa") ?? Route("Ntarifa")) !== null) {
                $this->Ntarifa->setQueryStringValue($keyValue);
            }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("ScoTarifaList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ScoTarifaList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ScoTarifaView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ScoTarifaList") {
                            ...
                            $returnUrl = "ScoTarifaList"; // Return list page content
                        }
                    }
    ...
    {
        $this->horas->DefaultValue = $this->horas->getDefault(); // PHP
        $this->horas->OldValue = $this->horas->DefaultValue;
        $this->activo->DefaultValue = $this->activo->getDefault(); // PHP
        $this->activo->OldValue = $this->activo->DefaultValue;
        $this->crear_nuevo_precio->DefaultValue = $this->crear_nuevo_precio->getDefault(); // PHP
        $this->crear_nuevo_precio->OldValue = $this->crear_nuevo_precio->DefaultValue;
        $this->porc_recargo->DefaultValue = $this->porc_recargo->getDefault(); // PHP
        $this->porc_recargo->OldValue = $this->porc_recargo->DefaultValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Ntarifa' first before field var 'x_Ntarifa'
        $val = $CurrentForm->hasValue("Ntarifa") ? $CurrentForm->getValue("Ntarifa") : $CurrentForm->getValue("x_Ntarifa");
    }
    ...
    {
        $row = [];
        $row['Ntarifa'] = $this->Ntarifa->DefaultValue;
        ...
        $row['crear_nuevo_precio'] = $this->crear_nuevo_precio->DefaultValue;
        $row['porc_recargo'] = $this->porc_recargo->DefaultValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        // crear_nuevo_precio
        $this->crear_nuevo_precio->RowCssClass = "row";

        // porc_recargo
        $this->porc_recargo->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->horas->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->horas->Lookup->getTable()->Fields["secuencia"]->searchExpression(), "=", $this->horas->CurrentValue, $this->horas->Lookup->getTable()->Fields["secuencia"]->searchDataType(), "");
                }
                $lookupFilter = $this->horas->getSelectFilter($this); // PHP
    ...
    protected function getAddRow()
    {
        ...
        // horas
        $this->horas->setDbValueDef($rsnew, $this->horas->CurrentValue, strval($this->horas->CurrentValue) == "");
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ScoTarifaList"), "", $this->TableVar, true);
