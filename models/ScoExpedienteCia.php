...
/**
 * Table class for sco_expediente_cia
 */
class ScoExpedienteCia extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "sco_expediente_cia";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // Nexpediente_cia
        $this->Nexpediente_cia = new DbField(
            $this, // Table
            'x_Nexpediente_cia', // Variable name
            'Nexpediente_cia', // Name
            '`Nexpediente_cia`', // Expression
            '`Nexpediente_cia`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Nexpediente_cia`', // Virtual expression
            false, // Is virtual
        ...
        $this->Nexpediente_cia->InputTextType = "text";
        $this->Nexpediente_cia->Raw = true;
        $this->Nexpediente_cia->IsAutoIncrement = true; // Autoincrement field
        $this->Nexpediente_cia->IsPrimaryKey = true; // Primary key field
        $this->Nexpediente_cia->Nullable = false; // NOT NULL field
        $this->Nexpediente_cia->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Nexpediente_cia->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['Nexpediente_cia'] = &$this->Nexpediente_cia;

        // cia
        $this->cia = new DbField(
            $this, // Table
            'x_cia', // Variable name
            'cia', // Name
            '`cia`', // Expression
            '`cia`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`cia`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->cia->InputTextType = "text";
        $this->cia->Raw = true;
        $this->cia->Required = true; // Required field
        $this->cia->setSelectMultiple(false); // Select one
        $this->cia->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->cia->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        $this->cia->Lookup = new Lookup($this->cia, 'sco_cia', false, 'Ncia', ["nombre","","",""], '', '', [], [], [], [], [], [], false, '`servicio`', '', "`nombre`");
        $this->cia->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->cia->SearchOperators = ["=", "<>", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        $this->expediente->Lookup = new Lookup($this->expediente, 'sco_expediente', false, 'Nexpediente', ["Nexpediente","nombre_fallecido","apellidos_fallecido",""], '', '', [], [], [], [], [], [], false, '', '', "CONCAT(COALESCE(`Nexpediente`, ''),'" . ValueSeparator(1, $this->expediente) . "',COALESCE(`nombre_fallecido`,''),'" . ValueSeparator(2, $this->expediente) . "',COALESCE(`apellidos_fallecido`,''))");
        ...
        $this->estatus->Lookup = new Lookup($this->estatus, 'sco_expediente_cia', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "sco_expediente_cia";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->Nexpediente_cia->setDbValue($conn->lastInsertId());
            $rs['Nexpediente_cia'] = $this->Nexpediente_cia->DbValue;
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['Nexpediente_cia']) && !EmptyValue($this->Nexpediente_cia->CurrentValue)) {
                $rs['Nexpediente_cia'] = $this->Nexpediente_cia->CurrentValue;
            }
        }
        if ($success && $this->AuditTrailOnEdit && $rsold) {
            $rsaudit = $rs;
            $fldname = 'Nexpediente_cia';
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('Nexpediente_cia', $rs)) {
                AddFilter($where, QuotedName('Nexpediente_cia', $this->Dbid) . '=' . QuotedValue($rs['Nexpediente_cia'], $this->Nexpediente_cia->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->Nexpediente_cia->DbValue = $row['Nexpediente_cia'];
        $this->cia->DbValue = $row['cia'];
    ...
    {
        return "`Nexpediente_cia` = @Nexpediente_cia@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->Nexpediente_cia->CurrentValue : $this->Nexpediente_cia->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->Nexpediente_cia->CurrentValue = $keys[0];
            } else {
                $this->Nexpediente_cia->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('Nexpediente_cia', $row) ? $row['Nexpediente_cia'] : null;
        } else {
            $val = !EmptyValue($this->Nexpediente_cia->OldValue) && !$current ? $this->Nexpediente_cia->OldValue : $this->Nexpediente_cia->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@Nexpediente_cia@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ScoExpedienteCiaList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ScoExpedienteCiaView" => $Language->phrase("View"),
            "ScoExpedienteCiaEdit" => $Language->phrase("Edit"),
            "ScoExpedienteCiaAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ScoExpedienteCiaView",
            Config("API_ADD_ACTION") => "ScoExpedienteCiaAdd",
            Config("API_EDIT_ACTION") => "ScoExpedienteCiaEdit",
            Config("API_DELETE_ACTION") => "ScoExpedienteCiaDelete",
            Config("API_LIST_ACTION") => "ScoExpedienteCiaList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ScoExpedienteCiaView", $parm);
        } else {
            $url = $this->keyUrl("ScoExpedienteCiaView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ScoExpedienteCiaAdd?" . $parm;
        } else {
            $url = "ScoExpedienteCiaAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoExpedienteCiaEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoExpedienteCiaList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoExpedienteCiaAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoExpedienteCiaList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ScoExpedienteCiaDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"Nexpediente_cia\":" . VarToJson($this->Nexpediente_cia->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl($url, $parm = "")
    {
        if ($this->Nexpediente_cia->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->Nexpediente_cia->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("Nexpediente_cia") ?? Route("Nexpediente_cia")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->Nexpediente_cia->CurrentValue = $key;
            } else {
                $this->Nexpediente_cia->OldValue = $key;
            }
    ...
    public function renderListRow()
    {
        ...
        // Nexpediente_cia
        $this->Nexpediente_cia->HrefValue = "";
        $this->Nexpediente_cia->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // Nexpediente_cia
        $this->Nexpediente_cia->setupEditAttributes();
        $this->Nexpediente_cia->EditValue = $this->Nexpediente_cia->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->Nexpediente_cia);
                    $doc->exportCaption($this->cia);
                    $doc->exportCaption($this->expediente);
                    $doc->exportCaption($this->servicio_tipo);
                    $doc->exportCaption($this->factura);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->Nexpediente_cia);
                        $doc->exportField($this->cia);
                        $doc->exportField($this->expediente);
                        $doc->exportField($this->servicio_tipo);
                        $doc->exportField($this->factura);
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'sco_expediente_cia');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'sco_expediente_cia', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'sco_expediente_cia', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'sco_expediente_cia', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowInserting($rsold, &$rsnew) {
    	...
    	$sql = "SELECT servicio FROM sco_cia WHERE Ncia = " . intval($rsnew["cia"]);
    	$servicio = ExecuteScalar($sql);
    	$rsnew["servicio_tipo"] = "$servicio";
    ...
    public function rowInserted($rsold, $rsnew) {
    	/*if(strtoupper(substr($rsnew["servicio_tipo"], 0, 3)) == "CAP") {
    		$sql = "SELECT group_concat(factura SEPARATOR ',') AS factura, sum(monto) AS monto 
    				FROM sco_expediente_cia
    				WHERE expediente = " . intval($rsnew["expediente"]) . " AND servicio_tipo LIKE 'CAP%'";
    		$row = ExecuteRow($sql);
    		...
    		$sql = "UPDATE sco_expediente
    				SET
    					venta=$monto, 
    					factura='$factura' 
    				WHERE Nexpediente = " . intval($rsnew["expediente"]) . ";";
    		Execute($sql);
    	}*/
    		$sql = "SELECT group_concat(factura SEPARATOR ',') AS factura, sum(monto) AS monto 
    				FROM sco_expediente_cia
    				WHERE expediente = " . intval($rsnew["expediente"]) . "";
    		$row = ExecuteRow($sql);
    		$factura = trim($row["factura"]);
    		$monto = floatval($row["monto"]);
    		$sql = "UPDATE sco_expediente
    ...
    public function rowUpdated($rsold, $rsnew) {
    	/*if(strtoupper(substr($rsold["servicio_tipo"], 0, 3)) == "CAP") {
    		if($rsold["factura"] != $rsnew["factura"] or $rsold["monto"] != $rsnew["monto"]) {
    			$sql = "SELECT group_concat(factura SEPARATOR ',') AS factura, sum(monto) AS monto 
    					FROM sco_expediente_cia
    					WHERE expediente = " . intval($rsold["expediente"]) . " AND servicio_tipo LIKE 'CAP%'";
    			$row = ExecuteRow($sql);
    			$factura = trim($row["factura"]);
    			$monto = floatval($row["monto"]);
    			$sql = "UPDATE sco_expediente
    					SET
    						venta=$monto, 
    						factura='$factura' 
    					WHERE Nexpediente = " . intval($rsold["expediente"]) . ";";
    			Execute($sql);
    		}
    	}*/
    		if($rsold["factura"] != $rsnew["factura"] or $rsold["monto"] != $rsnew["monto"]) {
    			$sql = "SELECT group_concat(factura SEPARATOR ',') AS factura, sum(monto) AS monto 
    					FROM sco_expediente_cia
    					WHERE expediente = " . intval($rsold["expediente"]) . "";
    			$row = ExecuteRow($sql);
    			$factura = trim($row["factura"]);
    			$monto = floatval($row["monto"]);
    			$sql = "UPDATE sco_expediente
    ...
    public function rowDeleted($rs) {
    	//echo "Row Deleted";
    	if(strtoupper(substr($rs["servicio_tipo"], 0, 3)) == "CAP") {
    		$sql = "SELECT group_concat(factura SEPARATOR ',') AS factura, sum(monto) AS monto 
    				FROM sco_expediente_cia
    				WHERE expediente = " . intval($rs["expediente"]) . " AND servicio_tipo LIKE 'CAP%'";
    		$row = ExecuteRow($sql);
    		$factura = $row["factura"];
    		$monto = floatval($row["monto"]);
    		$sql = "UPDATE sco_expediente
    				SET
    					venta=$monto, 
    					factura='$factura' 
    				WHERE Nexpediente = " . intval($rs["expediente"]);
    		Execute($sql);
    	}
    }
    ...
    public function rowRendering() {
    	// Enter your code here	
    	$color = "color:#f2dede; background:#a94442;";
    	$style = $color; 	
    	if ($this->PageID == "list" || $this->PageID == "view") { // List/View page only
    		$color = "color:#f2dede; background:#bfbdbb;";
    		$style = $color; 	
    		if(trim($this->estatus->CurrentValue) == "1") {
    			$this->cia->CellAttrs["style"] = $style;
    			$this->nombre_fallecido->CellAttrs["style"] = $style;
    			$this->apellidos_fallecido->CellAttrs["style"] = $style;
    			$this->factura->CellAttrs["style"] = $style;
