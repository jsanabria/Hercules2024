 ...
 * Page class
 */
class ScoArticuloEdit extends ScoArticulo
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoArticuloEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoArticuloEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("Narticulo") ?? Key(0) ?? Route(2)) !== null) {
                $this->Narticulo->setQueryStringValue($keyValue);
                $this->Narticulo->setOldValue($this->Narticulo->QueryStringValue);
            } elseif (Post("Narticulo") !== null) {
                $this->Narticulo->setFormValue(Post("Narticulo"));
                $this->Narticulo->setOldValue($this->Narticulo->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("Narticulo") ?? Route("Narticulo")) !== null) {
                    $this->Narticulo->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->Narticulo->CurrentValue = null;
                }
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ScoArticuloList"); // Return to list page
                        return;
                        ...
                        if ($this->Narticulo->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->Narticulo->CurrentValue, $this->CurrentRow['Narticulo'])) {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ScoArticuloList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "ScoArticuloList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ScoArticuloList") {
                            ...
                            $returnUrl = "ScoArticuloList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'Narticulo' first before field var 'x_Narticulo'
        $val = $CurrentForm->hasValue("Narticulo") ? $CurrentForm->getValue("Narticulo") : $CurrentForm->getValue("x_Narticulo");
        if (!$this->Narticulo->IsDetailKey) {
            $this->Narticulo->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->Narticulo->CurrentValue = $this->Narticulo->FormValue;
    ...
    public function renderRow()
    {
        ...
        // Narticulo
        $this->Narticulo->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->Narticulo->Visible && $this->Narticulo->Required) {
                if (!$this->Narticulo->IsDetailKey && EmptyValue($this->Narticulo->FormValue)) {
                    $this->Narticulo->addErrorMessage(str_replace("%s", $this->Narticulo->caption(), $this->Narticulo->RequiredErrorMessage));
                }
            }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ScoArticuloList"), "", $this->TableVar, true);
        $pageId = "edit";
