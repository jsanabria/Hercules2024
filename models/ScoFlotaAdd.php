 ...
 * Page class
 */
class ScoFlotaAdd extends ScoFlota
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoFlotaAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoFlotaAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    $returnUrl = "ScoFlotaList";
                    if (GetPageName($returnUrl) == "ScoFlotaList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ScoFlotaView") {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->placa->CurrentValue != "") { // Check field with unique index
            $filter = "(`placa` = '" . AdjustSql($this->placa->CurrentValue, $this->Dbid) . "')";
        ...
        if ($addRow) {
            ...
            if (in_array("sco_adjunto", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->flota->setSessionValue($this->Nflota->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "sco_adjunto"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->flota->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
    ...
    protected function getAddRow()
    {
        ...
        // tipo_flota
        $this->tipo_flota->setDbValueDef($rsnew, $this->tipo_flota->CurrentValue, false);
        ...
        // anho
        $this->anho->setDbValueDef($rsnew, $this->anho->CurrentValue, false);

        // serial_carroceria
        $this->serial_carroceria->setDbValueDef($rsnew, $this->serial_carroceria->CurrentValue, false);

        // serial_motor
        $this->serial_motor->setDbValueDef($rsnew, $this->serial_motor->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        if (isset($row['tipo_flota'])) { // tipo_flota
            $this->tipo_flota->setFormValue($row['tipo_flota']);
        }
        if (isset($row['marca'])) { // marca
            $this->marca->setFormValue($row['marca']);
        }
        if (isset($row['modelo'])) { // modelo
            $this->modelo->setFormValue($row['modelo']);
        }
        ...
        if (isset($row['anho'])) { // anho
            $this->anho->setFormValue($row['anho']);
        }
        if (isset($row['serial_carroceria'])) { // serial_carroceria
            $this->serial_carroceria->setFormValue($row['serial_carroceria']);
        }
        if (isset($row['serial_motor'])) { // serial_motor
            $this->serial_motor->setFormValue($row['serial_motor']);
        }
