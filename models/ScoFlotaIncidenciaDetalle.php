...
/**
 * Table class for sco_flota_incidencia_detalle
 */
class ScoFlotaIncidenciaDetalle extends DbTable
{
    protected $SqlFrom = "";
    ...
    // Fields
    public $Nflota_incidencia_detalle;
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "sco_flota_incidencia_detalle";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->Nflota_incidencia_detalle = new DbField(
            $this, // Table
            'x_Nflota_incidencia_detalle', // Variable name
            'Nflota_incidencia_detalle', // Name
            '`Nflota_incidencia_detalle`', // Expression
            '`Nflota_incidencia_detalle`', // Basic search expression
            20, // Type
            20, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Nflota_incidencia_detalle`', // Virtual expression
            false, // Is virtual
        ...
        $this->Nflota_incidencia_detalle->InputTextType = "text";
        $this->Nflota_incidencia_detalle->Raw = true;
        $this->Nflota_incidencia_detalle->IsAutoIncrement = true; // Autoincrement field
        $this->Nflota_incidencia_detalle->IsPrimaryKey = true; // Primary key field
        $this->Nflota_incidencia_detalle->Nullable = false; // NOT NULL field
        $this->Nflota_incidencia_detalle->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Nflota_incidencia_detalle->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['Nflota_incidencia_detalle'] = &$this->Nflota_incidencia_detalle;
        ...
        $this->tipo->Lookup = new Lookup($this->tipo, 'sco_flota_incidencia_detalle', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "sco_flota_incidencia_detalle";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->Nflota_incidencia_detalle->setDbValue($conn->lastInsertId());
            $rs['Nflota_incidencia_detalle'] = $this->Nflota_incidencia_detalle->DbValue;
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['Nflota_incidencia_detalle']) && !EmptyValue($this->Nflota_incidencia_detalle->CurrentValue)) {
                $rs['Nflota_incidencia_detalle'] = $this->Nflota_incidencia_detalle->CurrentValue;
            }
        }
        if ($success && $this->AuditTrailOnEdit && $rsold) {
            $rsaudit = $rs;
            $fldname = 'Nflota_incidencia_detalle';
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('Nflota_incidencia_detalle', $rs)) {
                AddFilter($where, QuotedName('Nflota_incidencia_detalle', $this->Dbid) . '=' . QuotedValue($rs['Nflota_incidencia_detalle'], $this->Nflota_incidencia_detalle->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->Nflota_incidencia_detalle->DbValue = $row['Nflota_incidencia_detalle'];
    ...
    {
        return "`Nflota_incidencia_detalle` = @Nflota_incidencia_detalle@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->Nflota_incidencia_detalle->CurrentValue : $this->Nflota_incidencia_detalle->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->Nflota_incidencia_detalle->CurrentValue = $keys[0];
            } else {
                $this->Nflota_incidencia_detalle->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('Nflota_incidencia_detalle', $row) ? $row['Nflota_incidencia_detalle'] : null;
        } else {
            $val = !EmptyValue($this->Nflota_incidencia_detalle->OldValue) && !$current ? $this->Nflota_incidencia_detalle->OldValue : $this->Nflota_incidencia_detalle->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@Nflota_incidencia_detalle@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ScoFlotaIncidenciaDetalleList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ScoFlotaIncidenciaDetalleView" => $Language->phrase("View"),
            "ScoFlotaIncidenciaDetalleEdit" => $Language->phrase("Edit"),
            "ScoFlotaIncidenciaDetalleAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    {
        return "ScoFlotaIncidenciaDetalleList";
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ScoFlotaIncidenciaDetalleView",
            Config("API_ADD_ACTION") => "ScoFlotaIncidenciaDetalleAdd",
            Config("API_EDIT_ACTION") => "ScoFlotaIncidenciaDetalleEdit",
            Config("API_DELETE_ACTION") => "ScoFlotaIncidenciaDetalleDelete",
            Config("API_LIST_ACTION") => "ScoFlotaIncidenciaDetalleList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ScoFlotaIncidenciaDetalleView", $parm);
        } else {
            $url = $this->keyUrl("ScoFlotaIncidenciaDetalleView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ScoFlotaIncidenciaDetalleAdd?" . $parm;
        } else {
            $url = "ScoFlotaIncidenciaDetalleAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoFlotaIncidenciaDetalleEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoFlotaIncidenciaDetalleList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoFlotaIncidenciaDetalleAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ScoFlotaIncidenciaDetalleList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ScoFlotaIncidenciaDetalleDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"Nflota_incidencia_detalle\":" . VarToJson($this->Nflota_incidencia_detalle->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl($url, $parm = "")
    {
        if ($this->Nflota_incidencia_detalle->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->Nflota_incidencia_detalle->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("Nflota_incidencia_detalle") ?? Route("Nflota_incidencia_detalle")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->Nflota_incidencia_detalle->CurrentValue = $key;
            } else {
                $this->Nflota_incidencia_detalle->OldValue = $key;
            }
    ...
    {
        global $Response;
        $listPage = "ScoFlotaIncidenciaDetalleList";
    ...
    public function renderListRow()
    {
        ...
        // Nflota_incidencia_detalle
        if (!EmptyValue($this->archivo->Upload->DbValue)) {
            $this->Nflota_incidencia_detalle->HrefValue = GetFileUploadUrl($this->archivo, $this->archivo->htmlDecode($this->archivo->Upload->DbValue)); // Add prefix/suffix
            $this->Nflota_incidencia_detalle->LinkAttrs["target"] = "_blank"; // Add target
            if ($this->isExport()) {
                $this->Nflota_incidencia_detalle->HrefValue = FullUrl($this->Nflota_incidencia_detalle->HrefValue, "href");
            }
        } else {
            $this->Nflota_incidencia_detalle->HrefValue = "";
        }
        $this->Nflota_incidencia_detalle->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // Nflota_incidencia_detalle
        $this->Nflota_incidencia_detalle->setupEditAttributes();
        $this->Nflota_incidencia_detalle->EditValue = $this->Nflota_incidencia_detalle->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->Nflota_incidencia_detalle);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->Nflota_incidencia_detalle);
    ...
    public function getFileData($fldparm, $key, $resize, $width = 0, $height = 0, $plugins = [])
    {
        ...
        if (count($ar) == 1) {
            $this->Nflota_incidencia_detalle->CurrentValue = $ar[0];
        } else {
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'sco_flota_incidencia_detalle');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        $key .= $rs['Nflota_incidencia_detalle'];
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'sco_flota_incidencia_detalle', $fldname, $key, "", $newvalue);
            }
        }
    }
    ...
    public function writeAuditTrailOnEdit(&$rsold, &$rsnew)
    {
        ...
        $key .= $rsold['Nflota_incidencia_detalle'];
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'sco_flota_incidencia_detalle', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'sco_flota_incidencia_detalle', $fldname, $key, $oldvalue);
            }
        }
    }
    ...
    public function rowInserted($rsold, $rsnew) {
    	//echo "Row Inserted"
    	actualizar_monto_incidencia($rsnew["flota_incidencia"]);
    }
    ...
    public function rowUpdated($rsold, $rsnew) {
    	//echo "Row Updated";
    	actualizar_monto_incidencia($rsold["flota_incidencia"]);
    }
    ...
    public function rowDeleted($rs) {
    	//echo "Row Deleted";
    	actualizar_monto_incidencia($rs["flota_incidencia"]);
    }
