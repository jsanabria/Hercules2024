 ...
 * Page class
 */
class ScoMascotaAdd extends ScoMascota
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ScoMascotaAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "ScoMascotaAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "ScoMascotaView") {
    ...
    protected function validateForm()
    {
        ...
        if (in_array("sco_nota_mascota", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("ScoMascotaEstatusGrid");
        if (in_array("sco_mascota_estatus", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("ScoNotaMascotaGrid");
            if (in_array("sco_nota_mascota", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->mascota->setSessionValue($this->Nmascota->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "sco_nota_mascota"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->mascota->setSessionValue(""); // Clear master key if insert failed
                }
            }
            $detailPage = Container("ScoMascotaEstatusGrid");
            if (in_array("sco_mascota_estatus", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->mascota->setSessionValue($this->Nmascota->CurrentValue); // Set master key
    ...
    protected function getAddRow()
    {
        ...
        // nombre_contratante
        $this->nombre_contratante->setDbValueDef($rsnew, $this->nombre_contratante->CurrentValue, false);

        // cedula_contratante
        $this->cedula_contratante->setDbValueDef($rsnew, $this->cedula_contratante->CurrentValue, false);

        // direccion_contratante
        $this->direccion_contratante->setDbValueDef($rsnew, $this->direccion_contratante->CurrentValue, false);
        ...
        // nombre_mascota
        $this->nombre_mascota->setDbValueDef($rsnew, $this->nombre_mascota->CurrentValue, false);

        // peso
        $this->peso->setDbValueDef($rsnew, $this->peso->CurrentValue, false);

        // raza
        $this->raza->setDbValueDef($rsnew, $this->raza->CurrentValue, false);

        // tipo
        $this->tipo->setDbValueDef($rsnew, $this->tipo->CurrentValue, false);

        // tipo_otro
        $this->tipo_otro->setDbValueDef($rsnew, $this->tipo_otro->CurrentValue, false);

        // color
        $this->color->setDbValueDef($rsnew, $this->color->CurrentValue, false);

        // procedencia
        $this->procedencia->setDbValueDef($rsnew, $this->procedencia->CurrentValue, false);
        ...
        // fecha_cremacion
        $this->fecha_cremacion->setDbValueDef($rsnew, UnFormatDateTime($this->fecha_cremacion->CurrentValue, $this->fecha_cremacion->formatPattern()), false);

        // hora_cremacion
        $this->hora_cremacion->setDbValueDef($rsnew, UnFormatDateTime($this->hora_cremacion->CurrentValue, $this->hora_cremacion->formatPattern()), false);
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        if (isset($row['nombre_contratante'])) { // nombre_contratante
            $this->nombre_contratante->setFormValue($row['nombre_contratante']);
        }
        if (isset($row['cedula_contratante'])) { // cedula_contratante
            $this->cedula_contratante->setFormValue($row['cedula_contratante']);
        }
        if (isset($row['direccion_contratante'])) { // direccion_contratante
            $this->direccion_contratante->setFormValue($row['direccion_contratante']);
        }
        ...
        if (isset($row['fecha_cremacion'])) { // fecha_cremacion
            $this->fecha_cremacion->setFormValue($row['fecha_cremacion']);
        }
        if (isset($row['hora_cremacion'])) { // hora_cremacion
            $this->hora_cremacion->setFormValue($row['hora_cremacion']);
        }
    }
