...
$sql = "SELECT 
            ...
            a.sexo, 
            a.nombre_fallecido, 
            a.apellidos_fallecido, 
            a.nombre_contacto, 
            a.email, a.telefono_contacto1, a.telefono_contacto2 
        FROM 
            sco_expediente AS a
        WHERE 
...
$sql = " SELECT 
            ...
            date_format(a.hora_inicio, '%h:%i%p') as hora_inicio, 
            date_format(a.fecha_fin, '%d/%m/%Y') as fecha_fin, 
            date_format(a.hora_fin, '%h:%i%p') as hora_fin, 
            a.horas 
        FROM 
            sco_orden AS a 
            JOIN sco_servicio AS b ON b.Nservicio = a.servicio AND paso = 1 
        WHERE 
            a.expediente = $Nexpediente;";
$rs = mysql_query($sql); 
$row = mysql_fetch_array($rs);
$capilla = $row["capilla"]; 
...
$sql = "SELECT  
            ...
            if(a.espera_cenizas='S', 'Espera Cenizas', 'No Espera Cenizas') AS espera_cenizas 
        FROM 
            sco_orden AS a 
            JOIN sco_servicio AS b ON b.Nservicio = a.servicio AND paso = 2 
        WHERE 
            a.expediente = $Nexpediente;";
$rs = mysql_query($sql); 
$row = mysql_fetch_array($rs);
...
$pdf->ln();
$pdf->Cell(20,5,utf8_decode('Objeto: El objeto del contrato se refiere únicamente al espacio de la capilla, pérgola o iglesia seleccionada.'),0,0,'L');
$pdf->ln();
$pdf->ln();
...
$sql = "SELECT CONCAT(IFNULL(valor1,''), ' ', IFNULL(valor2,''), ' ', IFNULL(valor3,''), ' ', IFNULL(valor4,'')) as texto FROM sco_parametro WHERE codigo = '056';"; 
$rs = mysql_query($sql); 
$row = mysql_fetch_array($rs);
$texto = $row["texto"];
$text = trim($texto);
if(trim($text) != "") {
	$pdf->SetFont('Arial','BU',8);
	$pdf->Cell(20,4,utf8_decode('Honores Musicales:'),0,0,'L');
	$pdf->ln();
	$pdf->SetFont('Arial','',8);
	$pdf->MultiCell(200,4,utf8_decode($text),0,'L');
	$pdf->ln();
