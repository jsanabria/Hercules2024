...
if($estatus == "registro") $where = "AND a.nota LIKE 'Cambio de estatus de%'";
else $where = "AND a.nota = '$estatus'";

$sql = "SELECT DISTINCT a.nota, replace(a.nota, 'Cambio de estatus de ', '') AS estatus 
		FROM sco_lapidas_notas AS a 
		WHERE 
...
while($row = mysql_fetch_array($resultado))
{
	...
	$sql = "SELECT 
				b.Nlapidas_registro, b.solicitante, 
				d.campo_descripcion AS parentesco, b.nombre_difunto, 
				date_format(a.fecha, '%d/%m/%Y') AS modificacion, c.nombre AS modifica, 
				REPLACE(a.nota, 'Cambio de estatus de ', '') AS estatus 
			FROM 
				sco_lapidas_notas AS a 
				JOIN sco_lapidas_registro AS b ON b.Nlapidas_registro = a.lapida_registro 
				LEFT OUTER JOIN sco_user AS c ON c.username = a.usuario 
				LEFT OUTER JOIN sco_tabla AS d ON d.campo_codigo = b.parentesco 
			WHERE 
				a.fecha BETWEEN '$fecha_desde' AND '$fecha_hasta' AND d.tabla = 'PARENTESCOS' 
				AND a.nota = '" . $row["nota"] . "';"; 
